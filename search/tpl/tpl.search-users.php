...
<table class="search-user">
	...
	<tr>
        ...
        <td class="search-user-info">
           <div class="b-layout__txt b-layout__txt_float_right b-page__desktop">
            <?php if (!get_uid(false) || is_emp()) {
    ?><a href="/new-personal-order/<?=$result['login']?>/" class="b-button b-button_flat b-button_flat_green b-button_margbot_10" >Предложить заказ</a><?php 
} ?>
            ...
            <div class="user-info">
                ...
                <?php if ($result['name_prof']) {
    ?><span class="spec">Специализация: <?= ($result['name_prof'] ? $name_prof : 'Нет специализации')?></span><?php 
}//if?>
                <?php if ($result['additional_spec']) {
    ?><span class="spec">Дополнительные специализации: <?=$result['additional_spec']?></span><?php 
}//if?>
                <span class="country_name"><?=$country_name ? $country_name : ''?><?=$country_name && $city_name ? ', ' : ''?><?= $city_name ? $city_name : ''?></span>
                <span class="user-opyt">Опыт работы: <?= view_exp($result['exp'])?><?php if ($result['in_office'] == 't') {
    ...
    ?>
            ...
            <?php if (!get_uid(false) || is_emp()) {
    ?><a href="/new-personal-order/<?=$result['login']?>/" class="b-button b-button_flat b-button_flat_green b-button_float_right b-page__iphone b-page__ipad" >Предложить заказ</a><?php 
}//if?>
