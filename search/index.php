...
if (!$user_limit_array[$userLimit]) {
    $set_usr_limit = current($user_limit_array);
} else {
    $set_usr_limit = $user_limit_array[$userLimit];
}
...
switch ($type) {
    ...
    case 'users':
        ...
        if ($filter['prof'][1] && is_array($filter['prof'][1])) {
            ...
            $_SESSION['string_professions'] = implode(' ', $a_professions);
        }
