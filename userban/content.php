...
<br><br>
<b><input type="radio" name="alltime" id="alltime1" value="1" checked> <label for="alltime1">Забанить</label><?php if ($bIsTime): ?> <input type="radio" name="alltime" id="alltime2" value="0"> <label for="alltime2">или на</label> <input type="text" size="3" name="time" maxlength="2" value="7"> дней</b><?php endif;
    ?><br><br>
<b>Комментарий:</b><br>
...
<ul><?php foreach ($sbrs as $s) {
    ?>
	<li style="margin-bottom:20px;"><a href="/norisk2/?id=<?=$s->id?>&access=A&<?=$s->isEmp() ? 'E' : 'F'?>=<?=$s->login?>" class="blue"><?=$s->name?></a><br/>
	<?php if ($s->reserved_id) {
    ?>Деньги зарезервированы <?=date('j '.strtolower($MONTHA[date('n', strtotime($s->reserved_time))]).' Y года в H:i', strtotime($s->reserved_time))?>
	<?php 
} else {
    ?>Деньги не зарезервированы<?php 
}
    ?><br/>
	Срок выполнения проекта: <?=$s->work_days?> <?=ending(abs($s->work_days), 'день', 'дня', 'дней')?>.<br/>
	<?php if (is_emp($usr->role)) {
    ?>
