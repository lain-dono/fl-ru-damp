...
class seo
{
     ...
     * Конструктор класса.
     *
     * @param string|bool $subdomain Определяем на какой странице работаем, по умолчанию false
     */
     ...
     */
    public function getFriendlyURL($subdomain, $direction, $dir, $folder = '')
    {
        global $host;
     ...
     * @param int     $direct_id    ID направления
     * @param int     $parent_id    ID раздела
     * @param int     $section_id   ID элемента
     * @param interer $subdomain_id ID поддомена
     *
     * @return bool true - ссылка есть, false - ссылки нет
     */
     ...
     *
     * @param int $section_id ИД раздела (подраздела)
     *
     ...
     * Берем данные только определенного раздела (без подразделов) по его ИД.
     *
     * @param int $section_id название раздела (подраздела)
     *
     * @return array Данные выборки
     */
    ...
    public function parseResultSections($result, $limit = null)
    {
        ...
        foreach ($result as $key => $value) {
            if ($value['parent'] == 0) {
                ...
                if (!isset($section[$value['parent']])) {
                    continue;
                }
                if ($limit != null) {
                    if (count($section[$value['parent']]['subsection']) >= $limit) {
                        continue;
                    }
                }
     ...
     */
    public function getSubdomainsByDirectID($direct_id)
    {
        global $DB;
     ...
     *
     * @param bool $active Если true, то берем только те к которым прикреплены какие-либо статьи  
     *
     ...
     * @param int   $subdomain ИД поддомена (региона)
     * @param int   $parent    ИД раздела
     */
     ...
     *
     * @param int $section   ИД раздела
     * @param int $subdomain ИД подддомена
     *
     ...
     *
     * @param int $bind      ИД Связи
     * @param int $section   ИД Подраздела
     * @param int $subdomain ИД поддомена
     * @param int $is_draft  Флаг устанавливающий подраздел как в черновик @todo наверное лучше кинуть флаг в таблицу seo_sections
     *
     * @return bool
     */
     ...
     * @param int   $subdomain ИД поддомена (региона) который редактируем
     *
     * @return bool
     */
     ...
     * @param string|int $city Город выбора (Может быть как ИД города, так и название его)
     *
     * @return array|bool Результат выборки, false если никого не выбрало
     */
