...
class externalServer_XML extends externalBase
{
     ...
     * @return object
     */
    public static function getInst($req)
    {
        if (1 == (int) $req['protocol-version']) {
            return new self($req['data']);
        }
    }
     ...
     */
    public function checkClientVersion($cv)
    {
        if (!$cv) {
            return;
        }
        ...
        if (strtolower($cv[0]) == 'freetray') {
            ...
            } elseif (stripos($cv[2], 'mac') !== false) {
                $lcv = FLTRAY_MAC_VERSION;
            } elseif (stripos($cv[2], 'linux') !== false) {
     ...
     */
    public function varXml2Php($vnode)
    {
        $pv = null;
        if ($vnode->childNodes) {
     ...
     */
    public function varPhp2Xml($pv, $vtype = self::XML_VAR_ANY_TAG, $key = null)
    {
