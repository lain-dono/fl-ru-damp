...
abstract class externalBase
{
     ...
     * @var array
     */
    protected static $_aError = array(
    ...
    private static $errHandler;
    private static $warnHandler;
    private static $errContext;

    private static $_isRunned;
     ...
     */
    final public static function run($req)
    {
        if (!self::$_isRunned) {
            self::$_isRunned = true;
    ...
    {
        if ($this->eHandler) {
            self::$errHandler = array($this, $this->eHandler);
        }
    }
     ...
     */
    public function regWarnHandler()
    {
        if ($this->wHandler) {
            self::$warnHandler = array($this, $this->wHandler);
        }
    }
     ...
     */
    public function errorSetContext($context)
    {
        self::$errContext = $context;
    }
    ...
    protected function error($code, $debug_info = null)
    {
        if (self::$errHandler) {
            ...
            call_user_func(self::$errHandler, $err);
        } else {
            die($code);
        }
    }
    ...
    protected function warning($code, $debug_info = null)
    {
        if (self::$warnHandler) {
            $err = $this->_createErr($code, $debug_info);
            call_user_func(self::$warnHandler, $err);
        }
    }
    ...
    {
        $err = array('code' => $code, 'message' => self::$errcodes[$code]);
        if ($debug_info) {
            $err['debug'] = $debug_info;
        }
        if (self::$errContext) {
            $err['context'] = self::$errContext;
        }
     ...
     */
    public function pg2exRow($fields, $data)
    {
        $row = array();
     ...
     */
    public function ex2pg($val, $dt = EXTERNAL_DT_STRING)
    {
        switch ($dt) {
            case EXTERNAL_DT_BOOL :
