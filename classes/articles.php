 ...
 *
	 ...
	 */
	public function GetMsgs($msg_cntr, $page, $num_msgs, &$error)
	{
	    global $DB;
 ...
 *
	 ...
	 */
	public function Add($title, $short, $msg, $sign, $file, $link)
	{
	    global $DB;
	    if ($file->tmp_name) {
 ...
 *
	 * @return char				сообщение об ошибке
	 */
	public function Del($msg, $admin = 0)
	{
	    global $DB;
	    if ($admin) {
	        $sql = 'DELETE FROM articles WHERE (id=?i) RETURNING logo';
	    } else {
	        return 0;
	    }
	    $ret = $DB->val($sql, $msg);
 ...
 *
     ...
     * @param CFile  $file
     */
    public function AddArticle($user, $title, $short, $msg, $fileid)
    {
        global $DB;
        validate_code_style($msg);
     ...
     */
    public function updateArticle($user, $article_id, $title, $short, $msg, $fileid)
    {
        global $DB;
        validate_code_style($msg);
     ...
     * @return <type>
     */
    public function getArticles($page, $num_msgs, $user_id, $tag, $approved = true, $author = null, $order = null, $declined = false)
    {
        global $DB;
     ...
     * @return integer количество статей
     */
    public function ArticlesCount($approved = true, $user = null, $tag = 0, $declined = false)
    {
        global $DB;
        $inner_tags = '';
        if (intval($tag) > 0) {
     ...
     */
    public function setRating($user_id, $article_id, $rtype)
    {
        global $DB;
        $article = self::getArticle($article_id, $user_id);
     ...
     * @return bool
     */
    public function bookmarkArticle($user_id, $article_id, $star)
    {
        global $DB;
        $article = self::getArticle($article_id, $user_id);
     ...
     * @return array
     */
    public function getBookmarks($user_id, $order = 'time')
    {
        global $DB;
        switch ($order) {
            case 'time':
     ...
     * @return boolean
     */
    public function setArticleLVT($user_id, $article, $hidden = null)
    {
        global $DB;
     ...
     */
    public function bookmarkDel($user_id, $article_id)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал
     */
    public function bookmarkEdit($user_id, $article_id, $title, $type)
    {
        global $DB;
        $titleq = '';
     ...
     * @return bool true - успех, false - провал
     */
    public function setApproved($article_id, $uid)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал
     */
    public function setDecline($article_id, $uid)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал
     */
    public function setUnDecline($article_id, $uid)
    {
        global $DB;
     ...
     * @return array
     */
    public function getNavigation($post_time, $approved = true)
    {
        global $DB;
        $field = 'post_time';
     ...
     * @return boolean
     */
    public function clearArticleTags($article_id)
    {
        global $DB;
     ...
     * @return integer
     */
    public function isApprovedArticles($uid)
    {
        global $DB;
     ...
     * */
    public function addWysiwygFile($cfile)
    {
     ...
     * */
    public function removeWysiwygTrash()
    {
        global $DB;
