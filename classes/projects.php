...
class projects
{
    ...
    */
     ...
     */
    public function setPageSize($set)
    {
        $this->page_size = $set;
    }
     ...
     */
    public function isProjectOfficePostedAfterNewSBR($prj)
    {
     ...
     */
      ...
      {
          if (!$sp = new_projects::getSpecs($id)) {
              return 'Прочее';
          }
    ...
    public static function _getSpecsStr($sp, $in_clue = '/', $group_clue = '&nbsp;&nbsp;', $lnk = false)
    {
        ...
        foreach ($sp as $item) {
            ...
            if ($item['subcategory_id']) {
                $name = professions::GetProfNameWP($item['subcategory_id'], $in_clue, 'Все разделы', $lnk);
            } elseif ($item['category_id']) {
                $name = professions::GetGroupNameWP($item['category_id']);
            } else {
                continue;
            }
    ...
    {
        if ($first_only) {
            return self::getPrimarySpec ($pid);
        }
        global $DB;
    ...
    */
    public function GetAlertsPrjTopDays()
    {
        global $DB;
 ...
 *
     ...
     */
    public function GetCurPrjs($fid, $closed = '', $is_owner = false, $is_moder = false, $kind = null, $trash = null, $page = 0)
    {
        global $DB;
        ...
        switch ($kind) {
            ...
            default:
                ...
                if (!empty($where_kind)) {
                    $where_kind = $DB->parse($where_kind, $uid, $uid);
                }
    ...
    */
    public function IsHaveComplain($project_id, $user_id, &$complain = null)
    {
        global $DB;
        if ($complain != null) {
     ...
     * @return type
     */
    public function IsHaveComplainType($project_id, $user_id, $type)
    {
        global $DB;
     ...
     */
    public function AddComplain($project_id, $user_id, $type, $msg, $files, $exported = false)
    {
        global $DB;
        $error = '';
     ...
     */
    public function SwitchStatusPrj($fid, $prj_id, $st = null)
    {
        global $DB;
        $status = self::GetStatusPrj($prj_id);
        if ($status[1] == 7) {
     ...
     */
    public function GetStatusPrj($prj_id)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function GetPrj($fid, $prj_id, $force = 0)
    {
        global $DB;
 ...
 *
	 ...
	 */
	public function GetProject($fid)
	{
	    global $DB;
	    $sql = '
			SELECT
     ...
     * @return type
     */
    public function getProjectByOfferId($offer_id, $emp_id)
    {
        global $DB;
    ...
    /**
      ...
      * @return array
      */   
     public function getProjectWithFeedback($project_id)
     {
         global $DB;
      ...
      * @return array
      */   
     public function getEmpPrjFeedback($datefrom, $dateto = null, $page = 1, $offset = 1000)
     {
         global $DB;
     ...
     * @return array
     */
    public function getFrlExec($datefrom, $dateto = null, $page = 1, $offset = 1000)
    {
        global $DB;
    ...
    */
     ...
     * @return array
     */
    public function getFrlOffer($datefrom, $dateto = null, $page = 1, $offset = 1000)
    {
        global $DB;
	 ...
	 * @return array               Данные выборки
	 */        
    public function GetNewProjectsPreviousDay(&$error, $get_specs = false, $limit = 0, $order_by_cost = false)
    {
        global $DB;
	 ...
	 * @return array Данные выборки
	 */
    public function GetNewProjects(&$error, $get_specs = false, $timeOffset = 0, $limit = 0)
    {
        global $DB;
        if ($timeOffset) {
	 ...
	 */
    public function SearchDB($prj_kind, $page = 1, $from_cache = 0, $filter = NULL, $is_ajax = false, $new = false)
    {
        ...
        if (!$out_HTML) {
            ...
            if ($prjs) {
                foreach ($prjs as $p) {
                    ...
                    if (strtotime($p['top_to']) >= time() && !$p['strong_top'] && isset($_COOKIE['hidetopprjlenta']) && $_COOKIE['hidetopprjlenta'] == 1 && $_COOKIE['hidetopprjlenta_time'] < strtotime($p['create_date'])) {
                        $_SESSION['hidetopprjlenta_more'] = 1;
                    }
                }
            }
	 ...
	 */
    public function DeleteWinner($uid, $blog_id)
    {
        global $DB;
	 ...
	 * @param integer $blog_id Ид блога
	 */
    public function SetWinner($uid, $blog_id)
    {
        global $DB;
        $sql = 'SELECT id_gr FROM blogs_msgs LEFT JOIN blogs_themes_old as blogs_themes ON blogs_msgs.thread_id=blogs_themes.thread_id LEFT JOIN blogs_msgs t ON (t.thread_id=blogs_msgs.thread_id AND t.reply_to IS NULL) WHERE t.fromuser_id=?i AND blogs_msgs.id=?i';
        $proj_id = $DB->val($sql, $uid, $blog_id);
 ...
 *
	 ...
	 */
    public function CheckWinner($prj_id)
    {
        global $DB;
        $prj_id = (int) $prj_id;
 ...
 *
	 ...
	 */
    public function CheckProOnly($thread)
    {
        global $DB;
 ...
 *
	 * @return array
	 */
    public function getAllAttach($prj_id)
    {
        global $DB;
        $ret = NULL;
        $sql = 
 ...
 *
	 ...
	 */
    public function DeletePublicProject($id, $user_id, $force = false)
    {
        global $DB;
     ...
     */
    public function isProjectsLimit($uid = null, $limit = 2)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function CountProject($post_date, $kind, $payed = 0, $project_id = null)
    {
        global $DB;
        if ($kind == 2 || $kind == 7) {
     ...
     */
    public function CountProjectNew($post_date, $kind = null, $top_from = null, $top_to = null, $strong_top = null)
    {
        global $DB;
     ...
     * @param type $project_id
     */    
    public function CountProjectByID($project_id)
    {
        global $DB;
        if (!$project_id) {
            return false;
        ...
        return self::CountProjectNew($prj['post_date'], $prj['kind'], $prj['top_from'], $prj['top_to'], $prj['strong_top']);
    }
     ...
     * @param integer $uid Ид пользователя
     */
    public function CloseAllUserPrj($uid)
    {
 ...
 *
	 * @return array
	 */
    public function GetPrjCust($prj_id)
    {
        global $DB;
     ...
     */
    public function changePrjFavState($uid, $pid)
    {
        global $DB;
 ...
 *
	 * @return array
	 */
    public function GetPrjHistory($prj_id)
    {
        global $DB;
        $sql = 'SELECT p.*, city.city_name, country.country_name 
     ...
     * @return boolean
     */
    public function checkShowTop(&$prj)
    {
   ...
   * @return string					сообщение об ошибке
   */
    public function SetExecutor($prj_id, $user_id, $emp_id)
    {
        global $DB;
   ...
   * @return string					сообщение об ошибке
   */
    public function ClearExecutor($prj_id, $emp_id, $force = false)
    {
        global $DB;
        if ($force) {
     ...
     * @param type $uid
     */
    public static function SetReadAll($uid)
    {
        global $DB;
     ...
     */
    public function SetRead($prj, $uid, $status = true)
    {
        global $DB;
     ...
     * @param int $project_id
     */
    public function incrementViews($project_id)
    {
        $dbstat = new DB('stat');
     ...
     * @return array строка.
     */
    public function getProjectWatch($prj_id, $cache = 180)
    {
        $db = new DB('stat');
        if ($cache >= 0) {
            $db->cache($cache);
     ...
     * @return array строка.
     */
    public function getProjectViews($prj_id)
    {
     ...
     */
    public function makeRamblerJobXML($projects)
    {
        $xml = '<document-list>';
        $xml .= '<date>'.date().'</date>';
 ...
 *
	 * @return array Данные
	 */
    public function GetFrlMenuProjects($fid, $folder, $offset = 0, $limit = 'ALL', &$count = -1)
    {
        global $DB;
        $ret = NULL;
     ...
     */
    public function Blocked($project_id, $reason, $reason_id = null, $uid = 0, $from_stream = false)
    {
        global $DB;
     ...
     * @return string Сообщение об ошибке
     */
    public function UnBlocked($project_id)
    {
        global $DB;
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
     ...
     */
    public function getBlockedReasonText($reasonId)
    {
        global $DB;
     ...
     * @return arrary
     */
    public function getBlockedReasonFull($reasonId)
    {
        global $DB;
     ...
     */
    public function addBlockedReason($name = '', $reason = '')
    {
        global $DB;
     ...
     * @return bool true при успехе и false при провале
     */
    public function updateBlockedReason($id = 0, $name = '', $reason = '')
    {
        global $DB;
        $data = compact('name', 'reason');
     ...
     * @return bool true при успехе и false при провале
     */
    public function deleteBlockedReason($id = 0)
    {
        global $DB;
     ...
     */
    public function GetBlockedPrjs(&$nums, &$error, $page = 1, $sort = '', $search = '', $admin = 0)
    {
        global $DB;
    ...
    */
    public function GetPrjComplains($prj_id)
    {
        global $DB;
        $prj_id = (int) $prj_id;
    ...
    * @return null
    */
    public function DeleteComplain($complain_id, $bMemDel = true)
    {
    ...
    * @return   null
    */
    public function DeleteComplains($prj_id)
    {
        global $DB;
    ...
    */
     ...
     * @return array [[массив с проектами]]
     */
    public function GetComplainPrjs(&$nums, &$error, $page = 1, $sort = '', $search = '', $admin = 0, $limit = 20, $group = 'new')
    {
        global $DB;
        $limit = intval($limit);
        $offset = $limit * ($page - 1);
     ...
     * @return array
     */
    public function getProjectComplaints($nPrjId = 0, $group = 'new')
    {
        $group_qry = '';
        switch ($group) {
        	case 'new':
     ...
     * @return int
     */
    public function GetComplainPrjsCount($group = 'new')
    {
        $group_qry = '';
        switch ($group) {
        	case 'new':
     ...
     * @return integer
     */    
    public function NumsBlockedProjects()
    {
        global $DB;
     ...
     * @return boolean
     */ 
    public function CheckBlocked($prj_id)
    {
        global $DB;
    ...
    * @return array|mixed  
    */
   public function GetExecProjects($ids, $connect = NULL)
   {
       global $DB;
       if (!$ids) {
           return NULL;
    ...
    * @return array|mixed
    */
   public function getProjects4Sending($ids, $connect = NULL)
   {
       global $DB;
       if (!$ids) {
           return NULL;
	 ...
	 */
   public function getRemindTopProjects()
   {
     ...
     */
    public function updateBudget($prj_id, $prj_cost, $prj_currency, $prj_costby, $prj_agreement)
    {
        global $DB;
        if ($prj_agreement) {
            $prj_cost = 0;
...
<tenders>'.implode("\n", $nodes).'</tenders>';
     ...
     * @return array Данные выборки
     */
    public function GetNewProjectsWithBudjet(&$error, $timeOffset = 0)
    {
        global $DB;
        if ($timeOffset) {
            $lOffset = $DB->parse("- interval '?i seconds'", $timeOffset);
     ...
     * @return string сообщение об ошибке
     */
    public function switchTrashProject($uid, $project_id, $st = null)
    {
        global $DB;
        if ($st === null) {
...
class new_projects extends projects
{
     ...
     */
    public function addPrj(&$prj, $attach, $categories = false)
    {
        global $DB;
        ...
        if ($prj['kind'] == 2) {
            return 2;
        }  // старые конкурсы добавлять нельзя
		if ($prj['kind'] == 7) {
     ...
     * @return resource
     */
    public function saveSpecs($id, $data)
    {
        if (!$data) {
            return false;
        }
     ...
     * @return array
     */
    public function getSpecs($id = array())
    {
 ...
 *
	 ...
	 */
    public function editPrj($prj, $newattach, $categories = false)
    {
        global $DB;
        if ($prj['kind'] == 7) {
            preg_match("/^([0-9]{1,2})\-([0-9]{1,2})\-([0-9]{4})$/", $prj['end_date'], $o1);
     ...
     */
    public function createFilterSql($filter, $kind = NULL)
    {
        ...
        if ($filter['cost_from'] || $filter['cost_to']) {
            ...
            $cex = array(2,3,4,1); 
            if (($cost_from = (float) $filter['cost_from']) < 0) {
                $cost_from = 0;
            }
            if (($cost_to = (float) $filter['cost_to']) < 0) {
                $cost_to = 0;
            }
 ...
 *
     ...
     */
    public function getProjects(&$num_prjs, $kind = -1, $page = 1, $comments = false, $filter = NULL, $is_blocked = true, $is_ajax = false, $prj_id = NULL,
                         $is_closed = false, $to_date = NULL, $withouttop = false)
    {
        ...
        $filterSql = self::createFilterSql($filter, $kind);
        ...
        */
        ...
        if (($ret || $tops) && !$tops_only) {
            ...
            if ($num_prjs != 'nenado') {
                if (!$filterSql) {
                    $num_prjs = (int) self::getProjectsCount($kind, $page, $is_emp ? $uid : null, $is_moder);
                } else {
     ...
     */
    public function yandexGenerateRss($filename, $kind)
    {
        global $DB;
     ...
     */
    public function adWords($filename)
    {
        global $DB;
        $profs = array();
        $groups = array();
     ...
     * @return text $filename полный путь к файлу куда webdav должен сохранить получившийся xml
     */
    public function jobradioGenerateRss($filename)
    {
        global $DB;
        $profs = array();
        $groups = array();
     ...
     * @return text $filename полный путь к файлу куда webdav должен сохранить получившийся xml
     */
    public function careerjetGenerateRss($filename)
    {
        global $DB;
        $profs = array();
        $groups = array();
    ...
    public function getProjectsForXml($interval = '1 month')
    {
        global $DB;
    ...
    public function trovitGenerateRss($filename, $prjs)
    {
        $HTTP_PREFIX = 'https://';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/project_exrates.php';
        $project_exRates = project_exrates::GetAll();
        ...
        foreach ($prjs  as $row) {
            ...
            if ($city != '') {
                $xml .= '<city><![CDATA['.iconv('CP1251', 'UTF-8', htmlspecialchars($city, ENT_QUOTES)).']]></city>'."\n";
            }; 
    ...
    public function indeedGenerateRss($filename, $prjs)
    {
        $HTTP_PREFIX = 'https://';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/project_exrates.php';
        $project_exRates = project_exrates::GetAll();
     ...
     * @return text $filename полный путь к файлу куда webdav должен сохранить получившийся xml
     */
    public function joobleGenerateRss($filename, $prjs)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/project_exrates.php';
        $project_exRates = project_exrates::GetAll();
        $exch = array(1 => 'FM', 'USD','Euro','Руб');
     ...
     */
    public function getLastProjects($kind = -1, $filter = NULL, $limit = 'ALL', $is_tray = false)
    {
        global $DB;
     ...
     */
    public function getTopProjects($check_blocked = true, $limit = 'ALL')
    {
        global $DB;
        if ($check_blocked) {
            $join_blocked = "
 ...
 *
	 ...
	 */
    public function checkRss($kind, $category = 0, $service = 1, $subcategory = 0)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function regRss($kind, $category = 0, $service = 1, $sub_category = 0)
    {
        global $DB;
        if (!$kind) {
            $kind = 1;
 ...
 *
	 * @return string
	 */   
    public function showUserOffers($pay_answers, $answers)
    {
	 ...
	 */
	public function SavePayedInfo($items, $prj_id, $bill_id, $topDays)
	{
	    global $DB;
	    $sql = '';
	    ...
	    if ($sql) {
	        $DB->query('INSERT INTO projects_payments(project_id, opid, pay_type, ammount, trs_sum) VALUES'.substr($sql, 1));
	    }
	}
     ...
     */
    public function UpPublicProject($id, $uid, $tr_id, &$error)
    {
        global $DB;
        $error = NULL;
     ...
     * @return boolean если есть ошибка возвращает false, иначе true
     */
    public function FreeUpPublicProject($id)
    {
        global $DB;
        if (!hasPermissions('projects')) {
            return false;
     ...
     */
    public function topPublicProject($id, $uid, &$error)
    {
        global $DB;
     ...
     */
    public function isKonkurs($kind)
    {
     ...
     */
    public function getKonkursPrice($isPRO = null)
    {
        if ($isPRO === null) {
            $isPRO = is_pro();
        }
     ...
     * @return float стоимость.
     */
    public function getPriceByCode($op_code)
    {
     ...
     * @return integer
     */
    public function getProjectsCount($kind = 0, $page = 1, $uid = null, $is_moder = null)
    {
        //$_uid = intval($uid);
        $_where = '';
    ...
    public static function getContestOpCodes()
    {
        ...
        return array_merge(self::$contestTaxesCodes, array(self::OPCODE_KON, self::OPCODE_PAYED_KON, self::OPCODE_KON_NOPRO));
    }
...
#public:
     ...
     */
    public function init($step, $prj_id = NULL)
    {
        ...
        if ($prj_id) {
            $this->destroy();
            if (!$this->_initFromDB($prj_id)) {
                return $this->destroy();
            }
            if ($this->CheckBlocked($prj_id) && !(hasPermissions('projects'))) {
                return $this->destroy();
            }
            $this->_isEdit = true;
     ...
     * @return array копия временной таблицы.
     */
    public function setProjectField($field, $value)
    {
        switch ($field) {
            case 'contacts':
     ...
     */
    public function getRemainingTopPeriod(&$days, &$hours, &$minutes, &$verb = NULL)
    {
        $time = time();
     ...
     * @return integer
     */
    public function getAddedTopDays()
    {
     ...
     */
    public function setAddedTopDays($num)
    {
     ...
     * @return array
     */
    public function getAttach($attach_id = NULL)
    {
        if ($attach_id === NULL) {
            return $this->_attach;
        }
     ...
     */
    public function addAttach($files, $formatted = false)
    {
        if (!$files) {
            return 0;
        }
     ...
     */
    public function addAttachedFiles($files, $from_draft = false)
    {
        global $DB;
     ...
     */
    public function delAttach($attach_id, $fix = false)
    {
     ...
     * @param type $file
     */
    public function initLogo($file, $link = null)
    {
        if ($link !== null) {
            $this->_project['link'] = $link;
        }
        $this->_logo = array('id' => $file->id, 'path' => $file->path, 'name' => $file->name, 'size' => $file->size, 'ftype' => $file->getext());
    }
     ...
     * @return string|integer Сообщение об ошибке. либо 0
     */
    public function setLogoNew(CFile $file)
    {
        $this->delLogo();
        $this->_tmpFiles[] = $file->name;
        $file_name = get_unanimated_gif($_SESSION['login'], $file->name, $file->path);
     ...
     * @param boolean $fix зафиксирвать в кэше или нет.
     */
    public function delLogo($fix = false)
    {
        if ($f = $this->getLogo()) {
            if ($f['path'] == $this->_tmpAbsDir) {
                $cfile = new CFile();
     ...
     * @return boolean
     */
    public function addSimpleProject($data = array())
    {
        if (empty($data)) {
            return false;
        }
     ...
     */
    public function saveProject($buyer_id = NULL, &$proj, $promo_codes = array())
    {
        $price = 0;
     ...
     */    
    public function isActiveKonkurs()
    {
     ...
     */
    public function getPrice(&$items = NULL, &$PROprice = 0, $pricesAsArray = false)
    {
     ...
     */
    public static function clearTmpAll($login)
    {
        if (!$login) {
            return;
        }
     ...
     * @return string путь
     */
    public static function getTmpPath($login)
    {
     ...
     * @return string имя
     */
    public static function getMemGrKey($login)
    {
...
#private:
    ...
    public function initFromDraft($draft_id, $uid)
    {
