...
class attachedfiles
{
     ...
     * @return object
     */
    public function __construct($session = '', $clear = false)
    {
        if (!$clear) {
            if (is_array($session)) {
     ...
     * @param int   $status Статус файла
     * @param int   $skey   Ключ сессии в какую добавляем файл, по умолчанию в нулевую
     */
     ...
     * @return bool
     */
    public function clearBySession()
    {
        global $DB;
