...
class contacts
{
     ...
     * Удаление контакта.
     *
     * @param int $contact_id идентификатор контакта
     */
     ...
     */
    public function getListContacts($filter = false, $page = 0)
    {
        global $DB;
        if ($page) {
     ...
     * @return array список контактов
     */
    public function getContacts($group_id = 0)
    {
        global $DB;
        if ($group_id) {
     ...
     */
    public function AddMail($subject, $message, $attaches, $contact_ids)
    {
        global $DB;
        $files = '';
        $fs = array();
