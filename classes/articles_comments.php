...
class articles_comments extends blogs_proto
{
     ...
     *
     * @param int    $fid     UID
     * @param int    $reply   Идентификатор сообщения ответом на которое является данное сообщение
     * @param int    $thread  Тема
     ...
     */
    public function Add($fid, $reply, $thread, $msg, $yt_link, $files, $ip, &$error, $small)
    {
        global $DB;
     ...
     * @return int ID нового сообщения
     */
    public function Update($id, $fid, $msg, $yt_link, $files, $files_cnt = 0, &$error, $small)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал
     */
    public function RestoreComment($id, $uid)
    {
        global $DB;
     ...
     *
     * @param int    $item_id ИД треда
     * @param string $error   Возвращает сообщения об ошибке
     */
    public function GetThreads($item_id, &$error)
    {
        global $DB;
        $curname = get_class($this);
     ...
     */
    public function getAttaches($ids, $autoindex = false)
    {
        global $DB;
        if (!count($ids)) {
            return false;
     ...
     *
     * @param int   $comment_id ID комментария, из которого удаляются вложения
     * @param array $attaches   Массив с идентификаторами файлов, которые нужно удалить
     */
    public function removeAttaches($comment_id, $attaches)
    {
        $comment_attaches = $this->getAttaches($comment_id);

        $file = new CFile();
        foreach ($attaches as $attach) {
            if (!isset($comment_attaches[$attach])) {
                continue;
            }
            $file->Delete($attach);
        }
...
//        $sql = "SELECT * FROM articles_comments WHERE id = $id";
     ...
     * @param <type> $files
     */
    public function reorderFiles($id, $files)
    {
        global $DB;
