...
class hh
{
     ...
     * @var array
     */
    public static $hh_currency2ex = array(   
      'RUR' => freelancer::RUR,
      'USD' => freelancer::USD,
      'EUR' => freelancer::EUR,
    );
     ...
     */
    public function addHHSpecProf($hh_field, $hh_spec_id, $prof_id)
    {
        global $DB;
        if (!$hh_field || !$prof_id) {
            return false;
     ...
     * @return boolean успешно?
     */
    public function delProf($hh_field, $prof_id, $hh_spec_id)
    {
        global $DB;
     ...
     * @return boolean успешно?
     */
    public function delHHSpec($hh_spec_id)
    {
        global $DB;
     ...
     * @return boolean успешно?
     */
	public function importHHCatalog()
	{
	    global $DB;
     ...
     * @return boolean успешно?
     */
	public function importHHRegions()
	{
	    global $DB;
	    libxml_disable_entity_loader();
        // Регионы
     ...
     * @return boolean успешно?
     */
	public function importHHCurrency()
	{
	    global $DB;
	    libxml_disable_entity_loader();
	    ...
	    $i = 0;
	    foreach ($sxml->children() as $item) {
	        $sql .= ($i++ ? ' UNION ALL ' : '')."SELECT '".(string) $item->{'code'}."', '".pg_escape_string(iconv('utf-8', 'windows-1251//TRANSLIT', $item->{'name'}))."', ".$item->{'rate'}.', '.(int) self::$hh_currency2ex[(string) $item->{'code'}];
	    }
	    $sql .= ') as x';
     ...
     */
    public function getCCByHHRegions($ids)
    {
        if (!is_array($ids)) {
     ...
     */
    public function getCountryByHHRegions($ids, &$fids = NULL)
    {
        global $DB;
        if (!is_array($ids)) {
            $ids = preg_split('/\D+/', $ids);
     ...
     */
    public function getCityByHHRegions($ids)
    {
        global $DB;
        if (!is_array($ids)) {
            $ids = preg_split('/\D+/', $ids);
     ...
     */
    public function getProfessionsByHHFields($ids)
    {
        global $DB;
        if (!is_array($ids)) {
            $ids = preg_split('/\D+/', $ids);
     ...
     * @return array массив с индексами-идентификаторами найденных профессий
     */
    public function getProfessionsByHHSpecs($ids)
    {
        global $DB;
        if (!is_array($ids)) {
            $ids = preg_split('/\D+/', $ids);
     ...
     * @return array
     */
    public function getHHCurrency($code)
    {
        global $DB;
     ...
     * @return string
     */
    public function packFilter($filter)
    {
     ...
     * @return array
     */
    public function unpackFilter($filter)
    {
     ...
     */
    public function getFilterByLink($flink)
    {
        global $DB;
    ...
    public function delOldFilters()
    {
        ...
        return !!$DB->query($sql, self::FILTER_LIFE);
    }
     ...
     */
    public function _encodeFilterLink($id, $last_used)
    {
     ...
     */
    public function _decodeFilterLink($flink, &$id, &$last_used)
    {
