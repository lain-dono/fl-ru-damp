 ...
 *
	 ...
	 */
    public function viewattachLeft($ulogin, $filename, $dir, &$file, $maxh = 1000, $maxw = 450, $maxpw = 307200, $show_ico = 0, $is_tn = 0, $show_download_ico = 0)
    {
     ...
     */
    public function viewattachExternal($ulogin, $filename, $dir, $url, $bigtext = 0)
    {
     ...
     * @return string
     */
    public function pda_pager($page, $pages, $count = PAGINATOR_PAGES_COUNT, $href = false)
    {
        if ($pages == 1) {
            return '';
        }
        ...
        if ($page > 1) {
            $html .= sprintf($href, '<a href=', $page - 1, '>Предыдущая</a>&nbsp;'); if (!($page < $pages)) {
     $html .= '<br />';
 }
        }
        if ($page < $pages) {
            $html .= sprintf($href, '&nbsp;<a href=', $page + 1, '>Следующая</a><br />');
        }

        for ($i = $start;$i <= $end;++$i) {
            if ($i == $start && $start > 1) {
                $html .= sprintf($href, '<a href="', 1, '">1</a>&nbsp;');  if ($i == 3) {
      $html .= sprintf($href, '<a href="', 2, '">2</a>&nbsp;');
  } elseif ($i != 2) {
      $html .= '&nbsp;..&nbsp;&nbsp;';
  }
            }
            $html .= ($page == $i ? '<span>'.$i.'</span>&nbsp;' : sprintf($href, '<a href=', $i, '>'.$i.'</a>&nbsp;'));
            if ($i == $end && $page < $pages - 1 && $pages > $end && $pages <= 50) {
                if ($pages - $end - 1 == 1) {
                    $html .= sprintf($href, '<a href="', $pages - 1, '">'.($pages - 1).'</a>&nbsp;');
                } elseif ($pages - $end - 1 > 1) {
                    $html .= '..&nbsp;';
                }
                $html .= sprintf($href, '<a href="', $pages, '">'.$pages.'</a>&nbsp;');
            } elseif ($i == $end && $page < $pages - 1 && $pages > $end && $pages > 50) {
                $html .= '&nbsp;...';
            }
        }
     ...
     */
    public function pda_info_user($user, $is_pro, $is_emp, $is_online, $addopt = false, $with_link = true, $is_view_online = true)
    {
        if ($user['photo'] == '') {
        ...
        if ($with_link) {
            $pda_html = '<a href="/users/'.$user['login'].'/"> '.$pda_html.'</a>';
        }
        ...
        if ($user['is_team'] == 't') {
            $pda_html .= '<img class="m-team" src="/pda/images/team.gif" alt="team" /> ';
        }

        if ($user['is_team'] != 't' && $is_pro && $is_emp) {
            $pda_html .= '<img class="m-pro" src="/pda/images/e_pro.png" alt="pro" width="21" height="9" /> ';
        } elseif ($user['is_team'] != 't' && $is_pro) {
            $pda_html .= '<img class="m-pro" src="/pda/images/f_pro.png" alt="pro" width="21" height="9" /> ';
        } 
        if ($is_view_online) {
            if ($is_online) {
                $pda_html .= '<img class="m-dot" src="/pda/images/dot_a.png" alt="На сайте" width="9" height="9" /> ';
            } else {
                $pda_html .= '<img class="m-dot" src="/pda/images/dot_ia.png" alt="Нет на сайте" width="9" height="9" /> ';
            }
        }
     ...
     * @return string
     */
    public function pda_external_link($href, $href_str = '', $size = 40, $type = 1, $title = '')
    {
