...
class walletAlpha extends Wallet
{
    ...
    public function payment($sum)
    {
        ...
        if ($result['orderId'] != '') {
            self::updateOrder($this->orderNumber, array('order_id' => $result['orderId'], 'state' => self::STATE_NEW));
...
class API_AlphaBank
{
    ...
    {
        return (is_release() ? self::API_URI : self::API_BETA_URI);
    }
    ...
    {
        return (is_release() ? self::RETURN_URL : self::RETURN_BETA_URL);
    }
    ...
    public function getAccessData($type = null)
    {
        switch ($type) {
            case 'bind':
                $this->userName = self::LOGIN_BINDING;
                $this->password = is_release() ? self::PASSWD_BINDING : self::PASSWD_TEST_BINDING;

                break;
            case 'autopay':
                $this->userName = self::LOGIN_AUTOPAY;
                $this->password = is_release() ? self::PASSWD_AUTOPAY : self::PASSWD_TEST_AUTOPAY;

                break;
            default:
                $this->userName = self::LOGIN;
                $this->password = is_release() ? self::PASSWD : self::PASSWD_TEST;
    ...
    {
        $this->error = false;
        $data = $this->register(self::REGISTER_SUM, $orderId, $accountId, 'Привязка пластиковой карты');
        if ($data['errorCode'] > 0) {
            $this->error = true;
    ...
    {
        $uri = self::getAPIUri().'register.do';
        $post = array(
    ...
    {
        $uri = self::getAPIUri().'getBindings.do';
        $post = array(
            'userName' => $this->userName,
            'password' => $this->password,
            'clientId' => $accountId,
    ...
    {
        $uri = self::getAPIUri().'getOrderStatus.do';
        $post = array(
            'userName' => $this->userName,
            'password' => $this->password,
            'orderId' => $orderId,
    ...
    {
        $uri = self::getAPIUri().'paymentOrderBinding.do';
        $post = array(
            'userName' => $this->userName,
            'password' => $this->password,
            'mdOrder' => $mdOrder,
