...
class API_Yandex extends API_OAuth
{
    ...
    public function isOAuth($uri)
    {
        ...
        return ($result['host'] == self::OAUTH_URI);
    }
     ...
     * @return string
     */
    public static function getAuthorizeUri($scope = null)
    {
        if (empty($scope)) {
            $scope = 'account-info payment.to-pattern("'.self::PATTERN_ID.'").limit(1,15000)';
        }
        ...
        $link = self::AUTH_URI.'/oauth/authorize?'.http_build_query($query);
    ...
    {
        $uri = self::API_URI.'/api/request-payment';
        $post = array(
            'pattern_id' => self::PATTERN_ID,
            'ammount' => round($sum, 2),
            'Sum' => round($sum, 2),
            'scid' => self::PATTERN_ID,
            'ShopID' => self::SHOP_ID,
        ...
        if ($this->isDebug()) {
            ...
            $post['test_result'] = self::STATUS_SUCCESS;
        }
    ...
    {
        $uri = self::API_URI.'/api/process-payment';
        $post = array(
    ...
    {
        $uri = self::API_URI.'/api/account-info';
        $resp = $this->request($uri, array());
