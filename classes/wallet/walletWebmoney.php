...
class API_Webmoney extends API_OAuth
{
    ...
    public static function getAuthorizeUri($scope = null)
    {
        ...
        $jws = new JWS(self::JWS_ALG, time());
        $jws->setPayload($query);
        $jws->sign(self::getPrivatekey());
        $req['request'] = $jws->getTokenString();

        $link = self::AUTH_URI.'/direct/security/auth?'.http_build_query($req);
    ...
    {
        $uri = self::AUTH_URI.'/direct/security/token';
        $post = array(
    ...
    {
        $uri = self::API_URI.'/direct/payment/init';
...
//        }
    ...
    {
        $uri = self::API_URI.'/direct/payment/complete';
        $post = array(
            'access_token' => $this->getAccessToken(),
            'merchant_id' => $this->getClientID(),//$this->pm->merchants[pmpay::MERCHANT_BILL],
    ...
    {
        $uri = self::API_URI.'/direct/payment/refund';
        $post = array(
            'access_token' => $this->getAccessToken(),
            'merchant_id' => $this->getClientID(),//$this->pm->merchants[pmpay::MERCHANT_BILL],
            'merchant_transaction_id' => $transaction_id,
