...
class opinions
{
     ...
     */
    public function GetMsgs($touser_id, $msg_cntr, $page, $num_msgs, $error, $users_table = 'users', $sort = false, $period = 0, $author = 0)
    {
     ...
     * @return mixed массив отзывов или сообщение об ошибке в случае неуспеха
     */
    public function GetMyMsgs($fromuser_id, $msg_cntr, $page, $num_msgs, $error, $sort = false, $period = 0)
    {
     ...
     */
    public function GetMsgInfo($msg_id, $error)
    {
        global $DB;
     ...
     */
    public function NewMsg($fromuser_id, $touser_id, $msgtext, $rating, $from_ip, &$new_id)
    {
        if ($fromuser_id == 0) {
            return;
        }
     ...
     */
    public function GetLastMessage($fromuser_id, $touser_id)
    {
        $sql =
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function DeleteMsg($fromuser_id, $msg, $admin = 0)
    {
        global $DB;
     ...
     * @return mixed
     */
    public function HideOpin($fromuser_login)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function Edit($fromuser_id, $msgid, $msg, $rating, $from_ip, $admin = 0, $modified_id = null)
    {
        global $DB;
     ...
     *               'frl' - от фрилансеров
     */    
    public static function GetCounts($user_id, $fromType = array())
    {
        global $DB;
     ...
     * @return array массив данных
     */
    public function GetCounts_OLD($user_id, $fromType)
    {
        global $DB;
     ...
     */
    public function getFilterCounts($userID, $isEmp, $rating = false, $author = false, $period = false)
    {
     ...
     */
    public function CheckUserCanPost($from_id, $to_id)
    {
        if (!$from_id) {
            return 1;
        }
     ...
     * @return array массив данных
     */
    public function getOpinionsData($date, $edate, $rating = NULL, $login = NULL)
    {
        $date = date('c', $date);
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function newCommentOpinion($comment, $user_id, $opinion_id)
    {
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function newCommentFeedback($comment, $user_id, $feedback_id)
    {
        /*if (self::isComOpinion($opinion_id) > 0)
            return false;*/
     ...
     */
    public function isComOpinion($id)
    {
        global $DB;
     ...
     */
    public function getCommentOpinion($uid)
    {
        global $DB;
     ...
     * @return mixed массив с комментариями или текст ошибки в случае неуспеха
     */
    public function getCommentOpinionById($ids)
    {
        global $DB;
     ...
     * @return mixed массив с комментариями или текст ошибки в случае неуспеха
     */
    public function getCommentFeedbackById($ids)
    {
        global $DB;
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function deleteComment($id, $uid, $admin = 0, $isFeedback = false)
    {
        global $DB;
     ...
     */
    public function GetMsgComInfo($msg_id)
    {
        global $DB;
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function editCommentOpinion($msg, $uid, $id_edit)
    {
        global $DB;
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function editCommentFeedback($msg, $uid, $id_edit, $feedbackAuthorID)
    {
        global $DB;
    ...
    {
        $opcount = self::GetCounts($to_id, array('emp', 'norisk', 'frl', 'all', 'total'));
    ...
    public static function printTheme($theme, $from, $counter, $with_container = true)
    {
        ...
        $opcomm = self::getCommentOpinionById(array($theme['id']));
...
<div class="ops-answer" id="ops_answer_'.$theme['id'].'">
    ...
    {
        if ($isFeedback) {
            $opcomms = self::getCommentFeedbackById(array($op_id));
        } else {
            $opcomms = self::getCommentOpinionById(array($op_id));
        }
    ...
    public static function printCommentOpinions($op_id, $isFeedback)
    {
        ...
        if ($isFeedback) {
            $opcomm = self::getCommentFeedbackById(array($op_id));
        } else {
            $opcomm = self::getCommentOpinionById(array($op_id));
        }
    ...
    public static function printAddForm($sid, $uid, $from)
    {
        ...
        $can_post = 1;
        if ($sid) {
            $can_post = self::CheckUserCanPost($sid, $uid);
        }
        if ($from == 'norisk') {
            $can_post = 0;
        }
...
<div>
     ...
     * @return string
     */
    public static function view_op_header_item($type, $user, $sort = 0, $period = 0, $is_active = false)
    {
        switch ($type) {
        ...
        $tmp = self::GetCounts($user->uid, array($type));
     ...
     */
    public static function view_op_nav_bar($user, $sort)
    {
        global $filter_string, $author_filter_string, $period, $author, $filterCounts;
        $opinions = self::GetCounts($user->uid, array('total'));
     ...
     */
    public static function converOpinion2Advice($opinion)
    {
        if ($opinion['rating'] != 1) {
            return false;
        }
     ...
     * @return boolean
     */
    public static function setConvertOpinion($opinion_id, $convert = true)
    {
        global $DB;
        if ((int) $opinion_id <= 0) {
            return false;
