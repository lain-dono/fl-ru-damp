...
class sms_services
{
     ...
     */
    public function checkTariff($serviceCode, $phone, $countryCode = 'RU', &$error = NULL)
    {
        ...
        if (!($service = self::$services[$serviceCode])) {
            ...
            if (!self::$tariffs[$phone] || !isset($service[$phone])) {
                $error = 2;
            } else {
                if (!($tariff = self::$tariffs[$phone]['country'][$countryCode])) {
                    $error = 3;
                }
            }
        }
     ...
     */
    public function checkEvtId($evtId, &$op_id)
    {
        global $DB;
        $r = self::DUP_OP_FULL;
	 ...
	 */
	public function saveEvtId($operation, $profit = 0, $currency_str = 'RUB', $evtId = NULL)
	{
	    global $DB;
	    if ($currency_str != 'RUB') {
	        $now = time();
 ...
 *
	 ...
	 */
	public function getCurrencyForDate($date = false)
	{
	    if (!$date) {
	        $date = date('d/m/Y');
	    }
 ...
 *
	 ...
	 */
	public function recalc_sms_operation()
	{
	    global $DB;
	    ...
	    foreach ($currency as $code => $val) {
	        $insert .= "INSERT INTO temp_currency VALUES('{$code}', '".($val['kurs'] / $val['units'])."');";
	    }
