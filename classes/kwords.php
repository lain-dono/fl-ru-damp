...
class kwords
{
     ...
     * @param bool         $return Вернут ИД записанных значений или нет
     *
     * @return bool
     */
    public function add($key, $return = false)
    {
        global $DB;
     ...
     *
     * @param int $N Общее количество вызовов ключевого слова (популярность)
     *
     * @return array
     */
    public function load($N = 7, &$kw_info = array())
    {
        global $DB;
        ...
        $kw_info = $ret;
        foreach ($ret as $k => $v) {
            $ar[] = "'".htmlspecialchars(addslashes($v['name']))."'";
        }
     ...
     * @param int   $prof ИД Профессии к которой добавляли ключевые слова
     */
    public function addUserKeys($uid, $ids, $prof)
    {
     ...
     * @param int $uid ИД Пользователя
     */
    public function getUserKeys($uid, $prof)
    {
        global $DB;
        ...
        foreach ($ret as $k => $val) {
            $result[$val['id']] = addslashes($val['name']);
        }
     ...
     * @param int    $moduser_id UID изменяющего пользователя (админа). если null - то берется $uid
     * @param string $keys       строка с ключевыми словами через запятую
     *
     * @return bool true - успех, false - провал
     */
    public function moderUserKeys($uid = 0, $prof = 0, $old_ids = array(), $new_ids = array(), $moduser_id = null, $keys = '')
    {
     ...
     */
    public function isModerUserKeys($uid = 0, $prof = 0)
    {
     ...
     * @param bool   $is_key Возвращает не пустая ли строка 
     *
     * @return array
     */
    public function getKeys($key, &$is_key = false)
    {
     ...
     *
     * @param int $uid  ИД Пользователя
     * @param int $prof Ид Профессии
     */
    public function delUserKeys($uid, $prof)
    {
        global $DB;
     ...
     */
    public function getJSValue($load_keys)
    {
