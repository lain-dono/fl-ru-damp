...
class lenta
{
     ...
     */
    public function SaveUserSettings($has_lenta, $user_id, $my_team, $all_profs, $communes, $prof_groups, $blog_groups)
    {
        global $DB;
     ...
     */
    public function GetUserLenta($user_id)
    {
        global $DB; 
        $ret = NULL;
     ...
     */
    public function GetLentaThemesCount($communes = NULL)
    {
        $sql = "
     ...
     */
    public function GetLentaItems($user_id, $my_team_checked = 0, $all_profs_checked = 0, $prof_groups = NULL, $communes = NULL, $offset = 0, $limit = 'ALL', &$count = -1, $blog_groups = NULL)
    {
        global $DB;
...
*/  
     ...
     */
    public function GetFavorites($user_id, $sort = 'date')
    {
        global $DB; 
        switch ($sort) {
            case 'date': 
     ...
     */
    public function getFav($user_id, $msg_id, $type = 'CM')
    {
        global $DB; 
        switch ($type) {
            case 'CM':
     ...
     * @return integer 1 в случае успеха, 0 в случае неудачи
     */
    public function AddFav($user_id, $commune_message_id, $portfolio_id, $blog_id, $undo = FALSE, $priority, $title = false)
    {
        global $DB;
        if (!(
        ...
        if (!$commune_message_id) {
            $commune_message_id = NULL;
        }
        if (!$blog_id) {
            $blog_id = NULL;
        }
        ...
        if (!$undo) {
            ...
            if ($row['lenta_user_id'] > 0) {
                ...
                if (!$DB->query($sql, $priority, $user_id)) {
                    return 0;
                }
            } else {
                $sql = 'INSERT INTO lenta_fav (lenta_user_id, portfolio_id, commune_message_id, priority, title, blog_id) VALUES (?, ?, ?, ?, ?, ?)';
                if (!$DB->query($sql, $user_id, $portfolio_id, $commune_message_id, $priority, $stitle, $blog_id)) {
                    return 0;
                }
            }
        } else {
     ...
     * @param integer $blog_id            идентификатор топика блога.
     */
    public function DelFav($user_id, $commune_message_id, $portfolio_id, $blog_id)
    {
        global $DB;
