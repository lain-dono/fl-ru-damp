...
class static_compress
{
 ...
 private $mtime;
     ...
     */
    public function static_compress($enabled = COMPRESS_STATIC, $options = array())
    {
     ...
     */
    public function add($fname, $utf8 = false)
    {
        ...
        } elseif (strstr($fname, '.js') !== false) {
            $this->_add($utf8 ? self::TYPE_JS_UTF8 : self::TYPE_JS, $fname);
        } elseif (strstr($fname, '.css') !== false) {
            if ($this->isMSIE && COMPRESS_STATIC) {
                if (!$this->_addWorker) {
                    ...
                    if ($this->_cssSize && $this->_cssSize + $fsize > $maxsize) {
                        $this->_addWorker = new self($this->enabled);
                    }
                    $this->_cssSize += $fsize;
                }
                if ($this->_addWorker) {
     ...
     * @return bool
     */
    public function isAdded($fname)
    {
        return isset(self::$_allAddedFiles[$fname]);
    }
     ...
     * @param bool   $seed_expired флаг устаревшего $seed (например, запрашивается уже удаленный файл).
     *
     * @return int|bool $type   тип файла или FALSE.
     */
     ...
     */
    public function setBatchesInfo($batches)
    {
        $this->_batches = null;
     ...
     *
     * @param int $batch_id ид. пакета.
     *
     * @return int версия.
     */
    public function getBatchVersion($batch_id)
    {
     ...
     *
     * @param int $batch_id      ид. пакета.
     * @param int $batch_version версия.
     *
     * @return bool успешно?
     */
    public function setBatchVersion($batch_id, $batch_version)
    {
     ...
     *
     * @param int $version версия (timestamp), если NULL, то текущее время.
     */
    public function updateBatchesVersion($version = null)
    {
        $log = $this->_log;
        $ret = 0;
     ...
     * @return bool ок?
     */
    public function _islock($batch_id, $type)
    {
        ...
        foreach ($this->types as $type => $name) {
            ...
            if ($expired = $batch_version < $this->_batches['version']) {
                ...
                if (!$batch_locked && !($batch_locked = $this->memBuff->get($lock_key))) {
                    ...
                    } elseif ($batch_locked = !$this->memBuff->set($lock_key, 1, self::MEM_LOCK_LIFE)) {  // какая-то фигня с add(), но зато блокирует другие add().
     ...
     */
    public function sendUncomressed($onlytype = -1, $version = null)
    {
        if (!$version) {
     ...
     */
    public function createBatchBySeed($seed, $light = false)
    {
     ...
     */
    public function compressJsFiles($files, $root = null, $light = false, $charset = 'windows-1251')
    {
        $out = '';
        if ($light) {
    ...
    public function compressCssContent(&$contents)
    {
     ...
     */
    public function printTags($file, $type, $version = null)
    {
     ...
     */
    public function printTagsCss($file, $charset)
    {
     ...
     * @param string $file путь к файлу
     */
    public function printTagsJs($file, $charset)
    {
     ...
     * @param string $file путь к файлу
     */
    public function printTagsPHP($file, $charset)
    {
        print "<script type=\"text/javascript\" src=\"{$file}\" charset=\"{$charset}\"></script>\n";
    }
     ...
     * @param bool   $unique     Выдать сразу уникальный контент (удаляются повторяющиеся классы) 
     *
     ...
     *
     * @param bool $unique для collectBem(): Выдать сразу уникальный контент (удаляются повторяющиеся классы) 
     *
     * @return int возвращает кол-во получившихся файлов.
     */
    public function createBemBatchFiles($unique = true)
    {
     ...
     */
    public function bemFilePath($num = 0, $abs = true)
    {
        $sfx = $num ? '-'.$num : '';
     ...
     * @param int    $ret_mode какую составляющую вернуть: -1:все в массиве, 0:ид. пакета (md5), 1:версия пакета, 2:расширение
     *
     * @return mixed
     */
    public function parseFileName($filename, $ret_mode = -1)
    {
    ...
    public function createFileName($batch_id, $version, $ext)
    {
