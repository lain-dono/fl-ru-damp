...
*
	...
	*/
	public function Update($fid, $eddition = '')
	{
	    ...
	    foreach ($class_vars as $name => $value) {
	        if (isset($this->$name) && $name != 'pr_key') {
	            if ($this->$name == 'null') {
	                ...
	                $fields[] = $name."= '".str_replace("'", '&#039;', $this->$name)."'";
	            }
	        }
	    }
	    $fld = implode(', ', $fields);
 ...
 *
	 ...
	 */
	public function GetField($uid, &$error, $fieldname)
	{
	    $current = get_class($this);
 ...
 *
	 ...
	 */
	public function GetRow($id = '', $addit = '', $order = '')
	{
	    $current = get_class($this);
	    if ($id) {
	        $addit = $this->pr_key."='$id'".$addit;
	    }
	    if ($order) {
 ...
 *
	 ...
	 */
	public function GetAll($orderby = '', $filter = '')
	{
	    $current = get_class($this);
 ...
 *
	 ...
	 */
	public function Add(&$error, $return_id = 0)
	{
	    $current = get_class($this);
	    $class_vars = get_class_vars(get_class($this));
	    $fields = array();
 ...
 *
	 ...
	 */
	public function BindRequest($arr, $force = false)
	{
	    $class_vars = get_class_vars(get_class($this));
	    foreach ($class_vars as $name => $value) {
 ...
 *
	 ...
	 */
	public function check_required($reqvs)
	{
