...
class CFile
{
    ...
    /**
     * @var bool исключить ли резервные сервера (для каких-то временных данных, чтоб не нагружать лишний раз).
     */
     ...
     */
    public function __construct($file_arr = 0, $table = null)
    {
        if ($table) {
            $this->table = $table;
        }
    ...
    {
        if ($this->tmp_name && !$this->unlinkOff) {
    ...
    public function GetInfo($file)
    {
        if (!($row = $GLOBALS['CFileCache']->get($file))) {
            $rows = self::selectFilesByFullName($this->table, $file);
            $row = $rows[0];
        }
        $this->initByRow($row);
    }
     ...
     *
     * @param int $id - id файла в таблице file
     */
    ...
    public static function selectFilesBySrc($t_name, $values, $order_by = null, $add_where = null, $limit = null)
    {
        if (is_array($values)) {
    ...
    public static function selectFilesById($t_name, $values, $order_by = null, $add_where = null, $limit = null)
    {
        if (is_array($values)) {
            foreach ($values as $k => $v) {
                $values[$k] = intval($v);
    ...
    public static function selectFilesByFullName($t_name, $values, $order_by = null, $add_where = null, $limit = null)
    {
     ...
     */
    public function cyrillicExtension($ext)
    {
     ...
     */
    public function secure_tmpname($dir = null, $postfix = '.temp', $prefix = 'f_')
    {
        // validate arguments
     ...
     * @param bool   $virusScan    Если TRUE, то проверять на вирусы
     * @param string $destFileName Если не пусто, то сохранить файл с этим именем
     *
     * @return string Название закаченного файла
     */
    public function MoveUploadedFile($dir, $virusScan = true, $destFileName = '')
    {
        if (@$this->error[0]) {
            return;
        }
     ...
     *
     * @return bool true - если запись прошла удачно, false - ошибка
     */
     ...
     *
     * @return bool true - все прошло удачно, false - не получилось загрузить
     */
     ...
     *
     * @param int $type Тип файла (1 = GIF, 2 = JPG, 3 = PNG, 4 = SWF, 5 = PSD, 6 = BMP, 7 = TIFF(orden de bytes intel), 8 = TIFF(orden de bytes motorola), 9 = JPC, 10 = JP2, 11 = JPX, 12 = JB2, 13 = SWC, 14 = IFF, 15 = WBMP, 16 = XBM)
     */
    public function validExtensionFile($type)
    {
        ...
        switch ($type) {
            case 1:
                if ($exp != 'gif') {
                    $this->name = preg_replace('/\.[^.]+$/', '.gif', $this->name);
                }
                break;
            case 2:
                if ($exp != 'jpg') {
                    $this->name = preg_replace('/\.[^.]+$/', '.jpg', $this->name);
                }
                break;
            case 3:
                if ($exp != 'png') {
                    $this->name = preg_replace('/\.[^.]+$/', '.png', $this->name);
                }
                break;
            case 4:
            case 13:
                if ($exp != 'swf') {
                    $this->name = preg_replace('/\.[^.]+$/', '.swf', $this->name);
                }
                break;
            case 6:
                if ($exp != 'bmp') {
                    $this->name = preg_replace('/\.[^.]+$/', '.bmp', $this->name);
                }
                break;
     ...
     */
    public function updateFileParams($params, $nomod = false)
    {
        global $DB;
     ...
     *
     * @return int|bool Возвращает id файла в случае успеха или false, если файла не существует.
     */
    public function DBImport($from)
    {
        if (!file_exists(ABS_PATH.'upload/'.$from)) {
            return false;
        }
     ...
     * @param bool   $new_name Задать новое имя файлу или нет(true - да, false - нет)
     *
     * @return bool true - если создание прошло супешно, false - не успешно
     */
     ...
     */
    public function Delete($id, $dir = '', $fname = '')
    {
        if ($id = intval($id)) {
            $where = 'id = '.$id;
            ...
            if (!($rows = $GLOBALS['CFileCache']->get($dir.$fname))) {
                $rows = self::selectFilesByFullName($this->table, $dir.$fname);
            }
            if (isset($rows[0])) {
                $count_links = intval($rows[0]['count_links']);
            } else {
     ...
     *
     * @return bool Возвращает true, если переименование прошло успешно
     */
     ...
     *
     * @return bool Возвращает true, если создание прошло успешно
     */
     ...
     * @param bool   $dont_check_put true: не проверять, если webdav сидит на nginx и включен create_full_put_path, просто вернуть true.
     *
     * @return bool Возвращает true, если директория существует
     */
    public function CheckPath($path, $dont_check_put = false)
    {
        if (!$path) {
            $path = $this->path;
        }
     ...
     *
     * @param string $path  путь до директории (относительно корня, без первого слеша) Например: users/te/temp/
     * @param bool   $check использовать ли проверку, чтобы случайно не потереть вообще все (разрешает удалять только в пределах директории users/??/)
     *
     * @return bool true, если удалено. false - если не удалено.
     */
    public function DeleteDir($path = '', $check = true)
    {
        if (!$path) {
            $path = $this->path;
        }
     ...
     *
     * @return bool true, если успешно. false - если не успешно.
     */
    public function MoveDir($new_login, $old_login)
    {
        $udir = 'users/';
     ...
     * @param bool   $allow_less    Разрешить или нет размер картинок меньше указанного
     *
     * @return bool false, если не сработало
     */
    public function img_to_small($destanation, $tn_image_size, $allow_less = false)
    {
        $src = $this->tmp_name;
     ...
     * @param int      $ovr_x Ширина начального файла
     * @param int      $ovr_y Высота начального файла
     * @param int      $ovr_w Ширина конечного файла (если false не учитывается)
     * @param int      $ovr_h Высота конечного файла (если fakse не учитывается)
     */
    public function imageComposeAlpha(&$src, &$ovr, $ovr_x, $ovr_y, $ovr_w = false, $ovr_h = false)
    {
        if ($ovr_w && $ovr_h) {
     ...
     */
    public function imageResizeAlpha(&$src, $w, $h)
    {
     ...
     *
     * @return bool|string false - если не удалось получить mime-тип, или вощзвращает mime-тип файла
     */
    public function getContentType($fname = '')
    {
        if (!$fname) {
            $fname = $this->path.$this->name;
        }
        if (!$fname) {
            return false;
        }
     ...
     * 
     * @param bool $delete Если TRUE, то удалит зараженный файл
     *
     * @return int Код проверки (см. self::$virus) или FALSE в случае ошибки
     */
     ...
     *
     * @return bool treu - удачно, false - не удачно
     */
     ...
     * @param int    $maxLength максимальная длина имени файла с расширением и точкой
     * 
