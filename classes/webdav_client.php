...
class webdav_client
{
  ...
  */
    ...
    public $_path = '/';
    public $_user;
    public $_protocol = 'HTTP/1.0';
    public $_pass;
    public $_socket_timeout = 5;
    public $_errno;
    public $_errstr;
    public $_user_agent = 'php class webdav_client $Revision: 1.8 $';
    public $_crlf = "\r\n";
    public $_req;
    public $_resp_status;
    public $_parser;
    ...
    public $_ls = array();
    public $_ls_ref;
    public $_ls_ref_cdata;

    public $_delete = array();
    public $_delete_ref;
    public $_delete_ref_cdata;

    public $_lock = array();
    public $_lock_ref;
    public $_lock_rec_cdata;

    public $_null = null;
    public $_header = '';
    public $_body = '';
    public $_connection_closed = false;
    public $_maxheaderlenth = 1000;

    public $is_reserved = false;
    public $prefix = null;
    public $prefix_local = null;
     ...
     */
    public function copy_file($src_path, $dst_path, $overwrite)
    {
     ...
     * @return int status code (look at rfc 2518). false on error.
     */
    public function copy_coll($src_path, $dst_path, $overwrite)
    {
        $this->_path = $this->_translate_uri($src_path);
        $this->_header_unset();
        $this->_create_basic_request('COPY');
 ...
 */
     ...
     * @return unknown
     */
    public function is_file_light($path)
    {
        if (trim($path) == '') {
        ...
        $this->_get_respond();
        $response = $this->_process_respond();
        if (strcmp($response['status']['status-code'], '404') == 0) {
            return false;
        } else {
            return true;
        }
     ...
     */
    public function _propfind_startElement($parser, $name, $attrs)
    {
     ...
     */
    public function _propfind_cData($parser, $cdata)
    {
        if (trim($cdata) != '') {
     ...
     * @param resource parser, string name, string attrs)
     */
    public function _delete_startElement($parser, $name, $attrs)
    {
        // lower XML Names... maybe break a RFC, don't know ...
        $propname = strtolower($name);
        $this->_xmltree[$parser] .= $propname.'_';
     ...
     * @param resource parser, string cdata
     */
    public function _delete_cData($parser, $cdata)
    {
        if (trim($cdata) != '') {
     ...
     * @param resource parser, string name, string attrs
     */
    public function _lock_startElement($parser, $name, $attrs)
    {
        // lower XML Names... maybe break a RFC, don't know ...
        $propname = strtolower($name);
        $this->_xmltree[$parser] .= $propname.'_';
     ...
     * @param resource parser, string cdata
     */
    public function _lock_cData($parser, $cdata)
    {
        if (trim($cdata) != '') {
     ...
     * @param string string
     */
    public function _header_add($string)
    {
        $this->_req[] = $string;
    }
     ...
     * @param string method
     */
    public function _create_basic_request($method)
    {
        $request = '';
     ...
     */
    public function _process_respond()
    {
        $lines = explode("\r\n", $this->_header);
        $header_done = false;
     ...
     */
    public function _utf_decode_path($path)
    {
        $fullpath = $path;
     ...
     */
    public function _error_log($err_string)
    {
        if ($this->_debug) {
