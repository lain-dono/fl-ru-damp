...
class blogs
{
     ...
     * @var array
     */
    public static $nav_my = array('my_all', 'my_posts', 'my_comments');
     ...
     * @var array
     */
    public static $nav_favs = array('favs_list', 'favs_std');
     ...
     * @var array
     */
    public static $priority_name = array('Низкая', 'Средняя', 'Высокая', 'Высшая');
     ...
     * @var array
     */
    public static $priority_img = array('bsg.png', 'bsgr.png', 'bsy.png', 'bsr.png');
     ...
     * @var type
     */
    public static $copiny_group = array(10,17,57);
  ...
  /**
     ...
     */
    public function GetThemes(&$error, $mod = 1, $usecache = true)
    {
        global $DB;
    ...
    public function getRandomThemes($size = 3, $mod = 1)
    {
        global $DB;
     ...
     */
    public function GetGroup($gr_num, &$gr_name, &$num_treads, $page, &$error, $fid = 0, $mod = 1, $having_message = 0, &$read_only, $ord = 'new', $is_ban = true, $sort_order = '')
    {
        global $DB;
        $cachedelay = 300; 
                        ...
                        if (!$get_ontops) {
                            ...
                            if (!$fid) {
                                for ($year, $ycnt = 0; $year >= 2008; --$year) {
                                    $ylcnt = self::getThemesCount($year, $gr_num);
                                    $ycnt += $ylcnt;
                                    if ($ycnt > $offset) {
                                        break;
                                    }
     ...
     * @return int
     */
    public function getThemesCount($year, $gr_num = 0)
    {
        global $DB;
     ...
     */
    public function GetBannedThreads(&$nums, &$error, $page, $sort, $search = '', $admin = 0)
    {
        global $DB;
        $limit = $GLOBALS['blogspp'];
        $offset = $limit * ($page - 1);
     ...
     */
    public function NumsBlockedThreads()
    {
        global $DB;
     ...
     * @return array [[информация о сообщении]]
     */
    public function GetMsgs($fid, $page, &$num_treads, &$error, $mod = 1)
    {
        global $DB;
        $limit = $GLOBALS['blogspp'];
        ...
        //приватные записи
        if (hasPermissions('blogs') || $fid == get_uid(false)) {
            $where_private = '';
        } else {
            $where_private = ' AND (is_private IS NOT true) ';
        }
     ...
     */
    public function GetThread($thread_id, &$error, $mod = 1, $fid = 0)
    {
        global $DB;
     ...
     */
    public function SearchFirstChild($id)
    {
        $ret = -1;
        $i = 0;
     ...
     */
    public function SearchLastChildId($id)
    {
        $count = 0;
        $tmpid = $id;
        foreach ($this->thread as $ikey => $node) {
     ...
     * @return boolean true
     */
    public function GetThreeId($id, &$threearr, $cc = 0)
    {
        $counter = 0;
        $from = 0;
        if ($cc) {
     ...
     */
    public function Add($fid, $reply, $thread, $msg, $name, $attach, $ip, &$error, $small, $yt_link = '')
    {
        global $DB;
        $sql =
     ...
     */
    public function Edit($fid, $edit_id, $msg, $name, $attach, $ip, &$error, $mod = 1, $tags = '', &$group = null, $base = null, $deleteattach = false, $olduserlogin = false, $yt_link = '', $close_comments = 'f', $is_private = 'f', $ontop = null, $small = null, $poll_question = null, $poll_answers = null, $poll_answers_has = null, $poll_multiple = null, $modified_reason = '')
    {
        global $DB;
     ...
     */
    public function NewThread($fid, $group, $base, $name, $msg, $attach, $ip, $mod, $small = 0, $tags = '', $yt_link = '', $close_comments = 'f', $is_private = 'f', $ontop = 'f', $poll_question = '', $poll_answers = '', $poll_multiple = false)
    {
        global $DB;
        if ($group == 7) {
            $max_image_size = array('width' => 400, 'height' => 600, 'less' => 0);
     ...
     */
    public function insertIntoModeration($msg_id = 0, $stop_words_cnt = 0, $sort_order = 0)
    {
        /*if ( $msg_id ) {
            require_once( $_SERVER['DOCUMENT_ROOT'] . '/classes/user_content.php' );
 ...
 *
	 * @return string Сообщение об ошибке
	 */
    public function MarkDeleteBlog($fid, $id, &$group, &$base, &$thread_id, &$page, &$msg, $mod = 1, $reason = '')
    {
        global $DB;
        $fid = intval($fid);
        $id = intval($id);
 ...
 *
	 * @return string Сообщение об ошибке
	 */
    public function RestoreMsg($fid, $id, &$group, &$base, &$thread_id, &$page, &$msg, $mod = 1)
    {
        global $DB;
        ...
        if (!($fid == $deluser_id || hasPermissions('blogs'))) {
            return 'Вы не можете восстановить сообщение';
        }
	 ...
	 */
    public function MarkDeleteMsg($fid, $edit_id, $ip, &$error, $mod = 1, $reason = '')
    {
        global $DB;
 ...
 *
	 * @return integer $thread_id ИД удаленного сообщения
	 */
    public function RestoreDeleteMsg($fid, $edit_id, $ip, &$error, $mod = 1)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function GetGroupName($id, $base = 0, $mod = 1)
    {
        global $DB;
        if ($base == 3) {
     ...
     */
    public function getGroupId($link)
    {
	 ...
	 */
    public function SetRead($thread_id, $fid, $status = 0)
    {
        global $DB;
 ...
 *
	 * @return string Сообщение об ошибке
	 */
    public function setMail($thread_id, $fid, $mail = 'f')
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function GetMsgInfo($msg_id, &$error, &$perm)
    {
        global $DB;
        $sql = '
			SELECT 
     ...
     */
    public function GetComments4Sending($message_ids, $connect = NULL)
    {
        global $DB;
        if (!$message_ids) {
            return NULL;
 ...
 *
	 ...
	 */
    public function GetThreadMsgInfo($thread_id, &$error, &$perm)
    {
        global $DB;
        $sql = '
          SELECT *
 ...
 *
	 * @return string Сообщение об ошибке
	 */
    public function LayoutChange($pos, $hide, $read_only)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function ChangeFav($thread_id, $priority = 0, $uid, $action = 'add')
    {
        global $DB;
 ...
 *
	 * @return integer 1
	 */
    public function UpdateFav($thread_id, $uid, $priority = 0, $title = '')
    {
        global $DB;
        $thread_id = intval($thread_id);
        $uid = intval($uid);
 ...
 *
	 ...
	 */
    public function GetFavorites($uid, $refresh_order = '', $gr_num = 0)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function GetFavoriteByThreadID($uid, $thread_id)
    {
        global $DB;
        $uid = intval($uid);
        $thread_id = intval($thread_id);
 ...
 *
	 ...
	 */
    public function GetAttachCount($msg_id)
    {
     ...
     */
    public function Blocked($thread_id, $reason, $reason_id = null, $uid = 0, $is_stream = true)
    {
        global $DB;
     ...
     * @return string Сообщение об ошибке
     */
    public function UnBlocked($thread_id)
    {
        global $DB;
     ...
     */
    public function getCorporateBlog($page = 1, $count = 10)
    {
        global $DB;
        --$page;
        $sql_page = $page * $count;
     ...
     */
    public function GetAttach($msg_id, $deleteattach = array())
    {
     ...
     */
    public function ShowMoreAttaches($num)
    {
        $num = intval($num - 1);
        if ($num) {
            if ($num == 1) {
     ...
     */
	public function Poll_Voted($uid, $thread_id)
	{
	    global $DB;
     ...
     */
	public function Poll_Vote($uid, $answers, &$error)
	{
	    global $DB;
	    $error = '';
	    $poll = $DB->row('
     ...
     */
	public function Poll_Answers($thread_id)
	{
	    global $DB;
     ...
     */
	public function Poll_Close($thread_id)
	{
	    global $DB;
     ...
     * @return boolean
     */
	public function Poll_Closed($thread_id)
	{
	    global $DB;
     ...
     */
	public function Poll_Remove($thread_id, &$msgtext)
	{
	    global $DB;
	    $msgtext = FALSE;
 ...
 *
	 * @return array
	 */
	public function Poll_GetPostAnswers($edit_msg)
	{
	    $answers = array();
	    ...
	    if ($edit_msg['poll']) {
	        for ($i = 0; $i < count($edit_msg['poll']); ++$i) {
	            ...
	            if ($ok) {
	                $answers[] = array('id' => $edit_msg['poll'][$i]['id'], 'answer' => (($has[$edit_msg['poll'][$i]['id']]) ? str_replace(array('"', "'", '\\'), array('&quot;', '&#039;', '&#92;'), stripslashes($has[$edit_msg['poll'][$i]['id']])) : $edit_msg['poll'][$i]['answer']));
	            }
	        }
	    }
	    if ($_POST['answers']) {
	        foreach ($_POST['answers'] as $answer) {
	            $answers[] = array('id' => 0, 'answer' => str_replace(array('"', "'", '\\'), array('&quot;', '&#039;', '&#92;'), stripslashes($answer)));
	        }
	    }
	    if (!$answers) {
     ...
     */
    public function createTreeBlog($blog, $pkey = 'reply_to')
    {
        $ch = $thread = array();
        // Идентифицируем узлы.
        foreach ($blog->thread as $b) {
            $thread[$b['id']] = $b;
        }
     ...
     */
    public function viewTreeNodes(&$n, $otag = '<ul class="bg-ul">', $ctag = '</ul>')
    {
        global $blog;
        if (!$n['children']) {
            return;
     ...
     */
    public function viewTreeNode(&$n)
    {
        ...
        if (!$n['deleted'] && (!$n['is_banned'] || hasPermissions('blogs'))) {
            ...
            if ($uid == $n['fromuser_id'] || $buser_id == $uid) {
                echo '<a href="/alert.php?alert=comment&id='.$n['id'].'&tr='.intval($_GET['tr']).'">Удалить</a> / ';
            }
            if ($uid == $n['fromuser_id']) {
                echo '<a href="'.$parseURL['path'].'?action=edit&id='.$n['id'].'&editcnt">Редактировать</a> / ';
            }
     ...
     */
    public function isBlogSubscribe($theme, $uid)
    {
        global $DB;
     ...
     * @return array
     */
    public function getUsersSubscribe($themes_ids, $connect = NULL, $get_moderator_info = false)
    {
        global $DB;
        if (!$themes_ids) {
            return NULL;
     ...
     * @return array
     */
    public function getGroupAndPos($thread_id, $fid = 0, $ord = '')
    {
        switch ($ord) {
            case 'my_all':
     ...
     */
    public function getAttachedFiles($msg_id)
    {
        $fList = array();
     ...
     */
    public function addAttachedFiles($files, $msg_id, $login = NULL, $from_draft = false)
    {
        global $DB;
        ...
        $max_image_size = array('width' => self::MAX_IMAGE_WIDTH, 'height' => self::MAX_IMAGE_HEIGHT, 'less' => 0);
        if ($files) {
            foreach ($files as $file) {
                switch ($file['status']) {
                    case 4:
     ...
     * */
    public function isTopicDeleted($topicId)
    {
        $id = (int) $topicId;
        global $DB;
