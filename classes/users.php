...
class users
{
    ...
    */
     ...
     * @return string
     */
    public function hashPasswd($passwd, $version = 0)
    {
        if ($version == 0) {
     ...
     * @return string
     */
    public function cookieHashPasswd($uid, &$pwd_data = NULL)
    {
        $pwd_data = self::GetUserSoltCookie($uid);
     ...
     */
    public function DeleteUser($id, $passwd, $error, $login = '', $force = 0)
    {
        global $DB;
        if ($force && $login) {
            $sql = "DELETE FROM users_counters WHERE user_id = '$id';DELETE FROM users WHERE (uid=$id AND login='$login') RETURNING login";
        } else {
            $sql = "DELETE FROM users WHERE (uid='$id' AND passwd='$passwd') RETURNING login";
        }
        $dlogin = $DB->val($sql);
        $error .= $DB->error;
        if ($error) {
            die('Ошибка выполнения: '.$error);
        }
        if ($login == '') {
            $login = $dlogin;
        }
    ...
    */
     ...
     */
    public function increaseLoginsCnt($uid)
    {
        global $DB;
    ...
    */
    public function SaveLoginIPLog($uid, $ip)
    {
        global $DB;
        $int_ip = ip2long($ip);
        ...
        if (!$res) {
            ...
            $DB->query($sql, $uid, (self::MAX_LOGIN_IP_LOG - 1));
    ...
    */
    public function SaveChangeEmailLog($uid, $email)
    {
        global $DB;
        ...
        if (!$log) {
            ...
            $DB->query($sql, $uid, $uid, (self::MAX_CHANGE_EMAIL_LOG - 1));
     ...
     * @param type $uid
     */
    public function initChangeEmailLog($uid)
    {
        global $DB;
     ...
     * @return array
     */
    public function getLastIps($sUid = '', $nCount = 10)
    {
     ...
     * @return array
     */
    public function getLastEmails($sUid = '', $nCount = 10)
    {
    ...
    */
    public function GetUserSoltCookie($uid)
    {
        global $DB;
    ...
    */
    public function CheckUserAllowIP($ip, $uid)
    {
        global $DB;
        $ret = TRUE;
        $longip = ip2long($ip);
     ...
     */
    public function GetUser($login, $anybase = true, $email = false)
    {
        if ($email) {
     ...
     * @return string сообщение об ошибке
     */
    public function GetUserByLoginPasswd($login, $passwd)
    {
     ...
     * @return string сообщение об ошибке
     */
    public function UpdateMain($fid, $name, $surname, $email, $oldpwd, $pname, $try_pwd = 0)
    {
        global $DB;
        $this->uname = $name;
        ...
        if ($try_pwd) {
            ...
            $res = $DB->query($sql, $fid, self::hashPasswd(stripslashes($oldpwd)));
        } else {
            $error = $this->Update($fid, $res, "AND passwd = '".self::hashPasswd(stripslashes($oldpwd))."'");
        }
        $error = parse_db_error($error);
        if ($error) {
            return ($error);
     ...
     * @return string сообщение об ошибке
     */
    public function UpdatePwd($fid, $oldpwd, $pwd, $try_pwd = 0)
    {
        global $DB;
        $this->passwd = $pwd;
        if ($try_pwd) {
     ...
     */
    public function GetField($uid, &$error, $fieldname, $nocache = true)
    {
        global $DB;
        static $user;
        if (!$uid) {
     ...
     */
    public function IsPro($uid, &$error)
    {
        global $DB;
     ...
     */
    public function GetName($uid, &$error)
    {
        global $DB;
        $current = get_class($this);
     ...
     */
    public function GetUid($error, $login = '')
    {
        global $DB;
        if (!$login && $this) {
            $login = $this->login;
     ...
     * @return mixed данные пользователя или ошибка в случае неуспеха
     */
    public function GetRole($login, &$error)
    {
        global $DB;
     ...
     */
    public function UpdateFoto($fid, $foto, $del)
    {
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function GetUsers($where = NULL, $order_by = NULL)
    {
        global $DB;
        $current = get_class($this);
     ...
     */
    public function GetAll(&$size, $addit = '', $order = 'uid', $frl_pp = FRL_PP, $offset = 0, $orderby = 'general')
    {
        global $DB;
        $size = 0;
        $current = get_class($this);
        ...
        $ssum = '';
        if ($current == 'users') {
            $sql = "SELECT COUNT(*) FROM users $addit";
        } else {
            $sql = "SELECT COUNT(*) FROM freelancer $addit";
        }
        ...
        if (!$error && $size >= $offset) {
            if ($current == 'users') {
                $sql = "SELECT uname, usurname, login, photo, is_banned, email, last_ip, email, reg_ip, role FROM users $addit ORDER BY $order LIMIT $frl_pp OFFSET $offset";
            } else {
     ...
     * @return string текст ошибки в случае неуспеха
     */
    public function UpdateSubscr($fid, $newmsgs, $vacan, $comments, $opin, $prj_comments, $commune_subscr, $commune_top_subscr, $adm_subscr, $contest_subscr, $team, $defilecomments, $articlescomments, $shop, $paid_advice, $payment)
    {
     ...
     * @return array
     */
    public function UpdateSubscr2($uid, $subscr)
    {
     ...
     */
    public function UpdateCommuneSubscr($user_id, $communeSubscr)
    {
     ...
     */
    public function updateSettings($sUid = 0, $aSettings = array())
    {
        // мнемоники битов
        $aBits = array(
     ...
     */
    public function isRemindByPhoneOnly($sParam = '', $bIsEmail = '')
    {
     ...
     * @return mixed список пользователей или текст ошибки в случае неуспеха
     */
    public function GetAllEx(&$size, &$info, &$fpinfo, $addit = '', $order = 'uid', $frl_pp = FRL_PP, $offset = 0)
    {
        global $DB;
        $size = 0;
    ...
    */
     ...
     */
    public function setUserBan($uid, $ban_where, $reason, $reason_id, $date_to = '', $no_send = 0, $self_deleted = false)
    {
        if (!$no_send) {
            require_once $_SERVER['DOCUMENT_ROOT'].'/classes/smail.php';
            $mail = new smail();
     ...
     * @return string текст ошибки
     */
    public function Banreason($login, $reason)
    {
        if ($reason && $login) {
     ...
     */
    public function Warn($login, $reason, $reasonId = null, $link, $userContent)
    {
        global $DB;
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
        $uid = $this->GetUid($error, $login);
     ...
     * @return string текст ошибки
     */
    public function UnWarn($warn_id)
    {
        global $DB;
     ...
     */
    public function GetBannedUsers(&$nums, &$error, $page, $sort, $show_type = 0, $search = '', $admin = 0)
    {
        global $DB;
        $limit = USERS_ON_PAGE;
        $offset = $limit * ($page - 1);
     ...
     * @return string текст ошибки
     */
    public function ResetTodayHits()
    {
        global $DB;
    ...
    public function getCountUsersAll()
    {
        global $DB;
     ...
     * @return array список пользователей
     */
    public function FindByIp($ip, &$count, $limit = 30, $offset = 0)
    {
        global $DB;
        if (!ip2long($ip)) {
            return;
...
//        }
     ...
     */
    public function ChangeMail($uid, $email)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал
     */
    public function NullRating($uid, $can_change = false)
    {
        $bRet = false;
     ...
     */
    public function ChModer($uid, $can_change = 0)
    {
        global $DB;
        if ($can_change = 0) {
            return 0;
     ...
     * @return integer 0 в любом случае
     */
    public function ChRedact($uid, $can_change = 0)
    {
        global $DB;
        if ($can_change = 0) {
            return 0;
     ...
     * @return bool true - успех, false - провал
     */
    public function SetActiveByUid($uid)
    {
        global $DB;
     ...
     */
    public function GetPopByVoter($voter_id)
    {
        global $DB;
     ...
     * @return string HTML
     */
    public function PrintPopBtn($voter_id, $voter_login)
    {
        $p_href = '';
        $m_href = '';
        $p_click = '';
     ...
     * @return string HTML
     */
    public function PrintPopBtnNew($voter_id, $voter_login)
    {
        $p_href = '';
        $m_href = '';
        $p_click = '';
        ...
        ?>
<script type="text/javascript">var lockPop=0;</script>
    <a class="b-button b-button_vote_down <?= !$m_click ? 'b-button_disabled' : '';
        ?>" <?=$m_href.$m_click?>></a>
    ...
    <a class="b-button b-button_vote_up <?= !$p_click ? 'b-button_disabled' : '';
        ?>" <?=$p_href.$p_click?>></a>
    <?php
      return ob_get_clean();
    }
     ...
     */
    public function PopVote($voter_id, $vote)
    {
        global $DB;
        if ($vote != 1 && $vote != -1) {
            return 0;
     ...
     */
    public function GetUidByFUID($fuid)
    {
        global $DB;
     ...
     * @see подробные комментарии в /rss/commune.php
     */
    public function GetFUIDByUid($uid)
    {
        global $DB;
     ...
     * @see подробные комментарии в /rss/commune.php
     */
    public function SetUserFUID($uid)
    {
        global $DB;
        if ($fuid = self::GetFUIDByUid($uid)) {
            return $fuid;
        }
    ...
    public function CheckEmail($email)
    {
        ...
        if ($email == strtolower($this->email)) {
            return 0;
        } else {
     ...
     */
    public function CheckEmailArray($emails)
    {
        global $DB;
     ...
     */
    public function setPROAutoProlong($pro_auto_prolong, $uid)
    {
        global $DB;
    ...
    * @return   integer                     1 в случае успеха, 0 в случае неуспеха
    */
    public function UpdateSafetyInformation($uid, $ip_addresses, $bind_ip)
    {
        global $DB;
        $ret = 1;
    ...
    */
    public function GetSafetyIP($uid)
    {
        global $DB;
        $ip = array();
        $sql = 'SELECT b_ip,e_ip FROM users_safety WHERE uid=?i';
    ...
    */
    public function CheckSafetyIP($ip_addresses)
    {
        $flag_error_ip = 0;
        ...
        if (!empty($ip_addresses)) {
            ...
            while (list($key, $ip) = each($t_ip_addresses)) {
                ...
                if (strstr($ip, ',')) {
                    $t_ips = preg_split('/,/', $ip);
                    while (list($k, $v) = each($t_ips)) {
                        array_push($ip_addresses, $v);
                    }
                    continue;
                }
    ...
    */
    public function CheckSafetyPhone($phone)
    {
        $t_phone = $phone;
    ...
    */
    public function FindUsersBySafetyPhone($phone)
    {
        global $DB;
    ...
    * @return   string  сообщение об ошибке
    */
    public function ChangeSafetyIP($uid, $ip)
    {
        global $DB;
        $res = '';
        $ip_addresses = preg_replace('/,[ ]{0,}/', "\n", $ip);
        $c_ip = self::CheckSafetyIP($ip_addresses);
     ...
     * @return resource результат запроса.
     */
    public function GetSuspiciousUsers($offset = 0, $limit = 'ALL')
    {
     ...
     * @return bool true - успех, false - провал.
     */
    public function resetAllSuspiciousUsers()
    {
        global $DB;
        set_time_limit(0);

        $words_login = self::GetSuspiciousWordsLogin();
        $words_name = self::GetSuspiciousWordsName();
        $sql_login = '';
        $sql_name = '';
     ...
     * @return bool true - успех, false - провал.
     */
    public function approveSuspiciousUser($sUid = 0)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал.
     */
    public function banSuspiciousUser($sUid = 0)
    {
        global $DB;
     ...
     */
    public function isSuspiciousUser($sUid = 0, $sLogin = '', $sOldLogin = '', $sName = '', $sOldName = '', $sSurname = '', $sOldSurname = '')
    {
        $bSuspicious = $bLoginSusp = $bNameSusp = $bSurnameSusp = false;

        if ($sLogin) {
            $aWordsLogin = self::GetSuspiciousWordsLogin();
            $bLoginSusp = self::_isWordSuspicious($sLogin, $aWordsLogin);
        }

        if ($sName || $sSurname) {
            $aWordsName = self::GetSuspiciousWordsName();

            if ($sName) {
                $bNameSusp = self::_isWordSuspicious($sName, $aWordsName);
            }

            if ($sSurname) {
                $bSurnameSusp = self::_isWordSuspicious($sSurname, $aWordsName);
            }
        }
     ...
     */
    public function _isWordSuspicious($sWord = '', $aWords = array())
    {
        $bSuspicious = false;
 ...
 *
	 ...
	 */
	public function getSuspectActivationData($sUid = '')
	{
	    global $DB;
     ...
     * @return int
     */
    public function GetCountSuspiciousUsers()
    {
        global $DB;
 ...
 *
	 * @return array
     */
    public function GetSuspiciousIPs(&$nCount, $nLimit = 0, $nOffset = 0)
    {
        $nCount = 0;
        $aReturn = array();
        global $DB;
     ...
     */
    public function GetSuspiciousWordsLogin()
    {
     ...
     * @return array Список подозрительных слов
     */
    public function GetSuspiciousWordsName()
    {
     ...
     * @return string Сообщение об ошибке
     */
    public function setSuspeciousWordsName($words = '', $type = 1)
    {
        global $DB;
        $type = intval($type);
     ...
     * @return <type>
     */
    public function FixPassword($sql, $login)
    {
        global $DB;
        ...
        if ($res = $DB->row($sql, strtolower($login), self::hashPasswd(stripslashes($pwd2)))) {
            $DB->update('users', array('passwd' => self::hashPasswd(stripslashes($pwd))),
                        'uid = ?i', $res['uid']);
        }
     ...
     */
    public function isSafetyPhoneShow($uid = 0)
    {
        global $DB;
     ...
     * @return bool true - успех, false - провал
     */
    public function updateMainSettings($uid = 0, $q_button = 0, $p_block = 0, $e_links = 0)
    {
        $aBits = array(
     ...
     * @return bool true - успех, false - провал
     */
    public function setDirectExternalLinks($uid = 0, $new = 0)
    {
     ...
     * @return bool true - успех, false - провал
     */
    public function setSbrSlash($uid = 0, $new = 0)
    {
     ...
     * @return bool true - успех, false - провал
     */
    public function setPromoBlockShow($uid, $new)
    {
     ...
     * @return bool true - успех, false - провал
     */
    public function setSafetyPhoneHide($uid = 0)
    {
     ...
     * @return bool true - успех, false - провал
     */
    public function setQuestionButtonShow($uid = 0, $new)
    {
     ...
     * @return bool true - успех, false - провал
     */
    public function updateSafetyPhone($uid = 0, $only_phone = '')
    {
        global $DB;
        $bRet = true;
        $sql = 'UPDATE users SET safety_only_phone=? WHERE uid=?i';
        if (!$DB->query($sql, $only_phone, $uid)) {
            $bRet = false;
        }
     ...
     * @return bool true - успех, false - провал
     */
    public function setPromoBlockShowCookie($uid, $new)
    {
        $obj = null;
     ...
     * @return type
     */
    public function setWmPaymaster($uid, $value)
    {
        global $DB;
     ...
     */
    public function getLastVisit($uid)
    {
     ...
     * @return string
     */
    public function getOnlineStatus4Profile(&$message = null)
    {
        global $session;
        if (!$this->uid) {
            return;
        ...
        if (!$session->is_active) {
            $lt = max((int) strtotime($this->last_time), self::getLastVisit($this->uid));
            $fmt = 'ynjGi';
            if (time() - $lt > 24 * 3600) {
                $fmt = 'ynjG';
     ...
     * @return array
     * */
    public static function GetUsersInfoByIds($ids, $table = 'users')
    {
        global $DB;
        if (is_array($ids)) {
            $ids = implode(',', $ids);
     ...
     * @return array
     * */
    public static function GetUsersBySubstringInFinInfo($substring, $limit, &$count)
    {
     ...
     * @return array
     * */
    public static function getUserShortInfoFinInfo($uid)
    {
        global $DB;
     ...
     * @return array
     * */
     public static function GetContactsBySubstring($substring, $userType, $limit, &$count, $in, $is_delete = false)
     {
         $s = $substring;
         ...
         $a = preg_split("#\s+#", $s);
         if (count($a) > 1) {
             $filter = " OR (u.uname LIKE('%{$a[0]}%') AND u.usurname LIKE('%{$a[1]}%')) 
      ...
      * @return array
      * */ 
     public static function GetMoreContactsByRole($userType, $limit, &$count, $in, $is_delete = false)
     {
         $deleted = !$is_delete ? " AND u.is_banned = B'0' AND u.self_deleted = false " : ' ';  
         $userTypeFilter = self::getUserTypeFilter($userType);
         $cmd = "SELECT u.uid, u.uname, u.usurname, u.login, u.photo, file.path, u.role
    ...
    /**
      ...
      * @return string 
      * */ 
    private static function getUserTypeFilter($userType)
    {
        $userTypeFilter = '';
        switch ($userType) {
    	        case 1:
     ...
     * */
    public static function GetSbrPartners($uid, $userRole, $limit, &$size)
    {
   ...
   /**
     ...
     */
    public function isChangeOnModeration($sUid = 0, $sField = '')
    {
     ...
     */
    public function GetUserInfoByUnsubscribeKey($ukey)
    {
        global $DB;
     ...
     */
    public function GetUnsubscribeKey($login)
    {
        global $DB;
        $row = $DB->row("
        ...
        $val = $row['key'];
        if (!$val) {
            return self::writeUnsubscribeKey($row['uid']);
        }
     ...
     */
    public function writeUnsubscribeKey($uid, $force = false)
    {
     ...
     * @param string $uid ID пользователя
     */
    public static function userWasInOldYear($uid)
    {
        global $DB;
        $uid = (int) $uid;
     ...
     * @param string $uid ID пользователя
     */
    public static function GetUserLangs($uid)
    {
        global $DB;
        $uid = (int) $uid;
