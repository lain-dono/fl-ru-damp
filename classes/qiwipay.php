...
class qiwipay
{
     ...
     */
    public function validate($request, $account_id)
    {
        $error = NULL;
...
//                    break;
     ...
     */
    public function checkBillsStatus(&$error = NULL)
    {
        $offset = 0;
        $limit = 300;
        $completed_cnt = 0;
        ...
        if (!($res = $this->DB->query($sql)) || !pg_num_rows($res)) {
            return 0;
        }
     ...
     */
    public function updateBillStatus(&$error, $id, $status)
    {
     ...
     * @return boolean успех?
     */
    public function completeBill(&$error, $bill, $sum)
    {
        $account = new account();
     ...
     * @return boolean успех?
     */
    public function deleteBill(&$error, $id)
    {
     ...
     */
    public function _checkResultError($result)
    {
        libxml_disable_entity_loader();
        if ($result) {
