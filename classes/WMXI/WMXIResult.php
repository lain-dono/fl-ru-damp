...
class WMXIResult
{
    ...
    {
        libxml_disable_entity_loader();
        if (!defined('WMXI_LOCALE')) {
            define('WMXI_LOCALE', 'en_US');
        };
    ...
    public function ErrorCode()
    {
        ...
        # cURL error code
        if (isset($obj->errno)) {
            return intval($obj->errno);
        }

        # WMXI error code
        if (isset($obj->retval)) {
            return intval($obj->retval);
        }
        if (isset($obj['retval'])) {
            return intval($obj['retval']);
        }
    ...
    public function ErrorText($code = false)
    {
        ...
        $message = '';
        if (isset($obj->error)) {
            $message = strval($obj->error);
        }
        if (isset($obj->retdesc)) {
            $message = strval($obj->retdesc);
        }
        ...
        foreach ($this->i18n as $k => $v) {
            ...
            if (strval($v['code']) == strval($code)) {
                $result[$scope] = $value;
            }
        }

        if (isset($result[$this->scope])) {
            return $result[$this->scope].$message;
        }
        if (isset($result['*'])) {
