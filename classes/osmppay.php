 ...
 *
	 ...
	 */
	public function prepare(&$err_code, $login, $operation_id, $ammount)
	{
	    ...
	    if (!preg_match('/^[a-zA-Z0-9]+[-a-zA-Z0-9_]{2,}$/', $login)) {
	        ...
	        return 'Неверный логин на сайте!';
	    }
	    $user = new users();
	    $uid = $user->GetUid($error, $login);
	    if (!$uid) {
	        $err_code = 5;
	        $error = 'Неверный логин на сайте!';
 ...
 *
	 * @return string Сообщение об ошибке
	 */
	public function checkdeposit(&$op_id, &$err_code, &$ammount, $login, $operation_id, $op_date)
	{
	    if (floatval($ammount) <= 0) {
	        $err_code = 241;
