...
class links
{
     ...
     *
     * @param int    $dst_id   ИД Объекта
     * @param int    $dst_type Тип ссылки
     * @param string $title    название ссылки
     * @param string $url      Ссылка
     *
     * @return string Сообщение об ошибке
     */
    public function set_link($dst_id, $dst_type, $title, $url)
    {
        global $DB;
     ...
     * @param bool  $is_found     прошла ли обработку или вернулся исходный адрес?
     * @param int   $max_link_len на каком символе обрезать хвост текста ссылки и заменять его на '...'
     *
     * @return string
     */
    public function save_find($matches, &$is_found, $max_link_len = 25)
    {
        global $host;
        $is_found = false;
        $URI = $matches[0];
        ...
        } // Разбираем передаваемые данные из ссылки
        ...
        if ($result) {
            ...
            switch (true) {
                case ($type == 'blogs'): // Обработка ссылок на блоги
                    ...
                    $link = self::get_title($where, $URI, self::BLOG_TYPE);
                    break;
                case ($type == 'users' && $exp[3] != 'viewproj.php') : // Обработка ссылок на пользователя
                    ...
                    $link = self::get_title($where, $URI, self::USER_TYPE);
                    break;
                case ($type == 'users' && $exp[3] == 'viewproj.php'): // Обработка ссылок на портфолио
                    $where = 'id = '.intval($urlvars['prjid']);
                    $link = self::get_title($where, $URI, self::PORTFOLIO_TYPE);
                    break;
                case ($type == 'commune' && !isset($post)): // Обработка ссылок на сообщество
                    $where = 'id = '.intval($id);
                    $link = self::get_title($where, $URI, self::COMMUNE_TYPE);
                    break;
                case ($type == 'commune' && isset($post)): // Обработка ссылок на сообщения в сообществах
                    $e = explode('.', $post);
                    $where = count($e) == 2 ? 'id = '.intval($e[1]) : 'id = '.intval($post);
                    $link = self::get_title($where, $URI, self::COMMUNE_TYPE_MESSAGE);
                    break;
                case ($type === 'projects'):
                    $e = explode('/', $url['path']);
                    $where = 'id = '.intval($e[2]);
                    $link = self::get_title($where, $URI, self::PROJECT_TYPE);
                    break;
                case ($type === 'articles'):
                    $e = explode('/', $url['path']);
                    $where = 'id = '.intval($e[2]);
                    $link = self::get_title($where, $URI, self::ARTICLES_TYPE);
                    break;
                case ($type === 'interview'):
                    $e = explode('/', $url['path']);
                    $where = 'id = '.intval($e[2]);
                    $link = self::get_title($where, $URI, self::INTERVIEW_TYPE);
                    break;
            }
        }
     ...
     * @return string
     */
    public function get_title($where, $URI, $type)
    {
        switch ($type) {
            case self::BLOG_TYPE:
            ...
            case self::COMMUNE_TYPE :
                ...
                break;
            case self::COMMUNE_TYPE_MESSAGE:
     ...
     * @return string
     */
    public function getHrefLink($title, $URI)
    {
        $attrTitle = $title;
        if (!$title) {
            $attrTitle = $URI;
