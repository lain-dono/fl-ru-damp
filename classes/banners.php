 ...
 *
	 ...
	 */
	public function AddCompany($name, $adr, $phone, $cont, $email, $notes)
	{
 ...
 *
	 ...
	 */
	public function GetCompanies(&$error)
	{
 ...
 *
	 ...
	 */
	public function GetCompany($id, &$error)
	{
 ...
 *
	 ...
	 */
	public function EditCompany($id, $name, $adr, $phone, $cont, $email, $notes)
	{
	...
	public function NewBanner(
	    ...
	    if ($l_id) {
	        ...
	        if (count($specs_array)) {
	            $this->UpdateSpecs($l_id, $specs_array);
	        }
	        if ($citys) {
     ...
     */
	public function addCity($l_id, $inp)
	{
	 ...
	 * @param array $specs_array список специализаций
	 */
	public function UpdateSpecs($banner_id, $specs_array)
	{
 ...
 *
	 * @return array
	 */
	public function GetSpecs($banner_id)
	{
	    $out = array();
	    if (!$banner_id) {
	        return $out;
 ...
 *
	 * @return string $error Сообщение об ошибке
	 */
	public function EditLocation($banner_id, $pages)
	{
 ...
 *
	 ...
	 */
	public function GetBanners($ord, $from_date, $to_date, &$error, $only_active = FALSE, $filters = array(), $offset = 0)
	{
 ...
 *
	 ...
	 */
	public function GetBanner($id, $user_id, &$error)
	{
	    $id = intval($id);
 ...
 *
	 ...
	 */
	public function GetPages($id, &$error)
	{
 ...
 *
	 ...
	 * 									  имя файла баннра, считать уников или нет, просмотров, кликов, RF]
	 */
	public function GetBannersByClient($cid, $user_id, &$error)
	{
	    $cid = intval($cid);
	    $user_id = intval($user_id);
 ...
 *
	 ...
	 */
	public function GetBannerSettings($id, &$error)
	{
	    $ret = $this->DB->query(
 ...
 *
	 ...
	 */
	public function ViewBanner($page, $banner_type)
	{
 ...
 *
	 ...
	 */
	public function ClickBanner($banner_id)
	{
 ...
 *
	 ...
	 */
	public function GC($banner_type)
	{
     ...
     */
    public function GetBannerUNStat($bid, $filters = null, $order = null, $offset = null)
    {
        $ord = '';
        $where_date1 = '';
        $where_date2 = '';
 ...
 *
	 ...
	 */
	public function checkAccess($uid, $ban_id)
	{
	    if ($uid) {
 ...
 *
	 * @return array
	 */
	public function GetBannersByDate($from, $to, $static = false)
	{
	    return $this->DB->rows('
			SELECT
 ...
 *
	 * @return array
	 */
	public function GetCitys($country)
	{
     ...
     * @return array
     */
    public function GetBannersCount($ord, $from_date, $to_date, &$error, $only_active = FALSE, $filters = array())
    {
        //        if ($from_date && $to_date)
//            $where = "WHERE (c_date <= '" . pg_escape_string($to_date) . "' AND c_date >= '" . pg_escape_string($from_date) . "')";
//        if ($only_active) {
 ...
 *
     ...
     */
    public function getCachedBanners($banner_type, $page)
    {
        $GLOBALS[memBuff::SERVERS_VARKEY] = self::MEM_SERVER_KEY;
        ...
        return $this->DB->cache(self::MEM_LIFE, self::MEM_GROUP_KEY)->col(
