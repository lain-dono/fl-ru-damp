...
class log
{
     ...
     */
    public function __construct($logname, $mode = 'a', $line_prefix = '')
    {
     ...
     * @param bool    $use Включить альтернативную запись или нет
     */
    public function addAlternativeMethodSave($obj, $use = null)
    {
        $obj->setLogName($this->_logname);
        $this->_alternative_methods[$obj->__toString()] = $obj;
        if ($use !== null) {
            $this->setUseAlternativeMethod($use);
        }
    }
     ...
     * @param bool $use
     */
    public function setUseAlternativeMethod($use)
    {
     ...
     */
    public function setAlternativeWrite($str, $method_alias)
    {
     ...
     *
     * @param bool $root вернет с абсолютным путем.
     *
     * @return string
     */
    public function getLogname($root = false)
    {
        if ($this->_logname) {
     ...
     * @return mixed
     */
    public function getTotalTime($fmt = '%H:%M:%S', $msecs = 0)
    {
    ...
    public function writedump($param, $title = '')
    {
        ob_start();
        var_dump($param);
        $out = ob_get_clean();
