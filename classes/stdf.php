 ...
 */
    ...
    function DBMyConnect()
    {
       ...
       // print_r($GLOBALS);
        if (!mysql_select_db($GLOBALS['dbmyname'], $link)) {
            return false;
        }
 ...
 *
    ...
    function textWrap_callback($matches, $max_word_size = NULL)
    {
        ...
        if (($len = strlen($txt)) <= $cur_ns_limit) {    // текущий кусок текста не выходит за пределы масимального размера слова.
            ...
            }       // а если есть, то остаток считаем от самого правого пробела.
            return $tags.$txt;
        }
    ...
    function reformat2()
    {
        ...
        return call_user_func_array('reformat', $args);
    }
   ...
   */
    ...
    function viewattach($ulogin, $filename, $dir, &$file, $maxh = 1000, $maxw = 450, $maxpw = 307200, $show_ico = 0, $is_tn = 0, $align = 'center', $show_blank_preview = false, $show_download_ico = 1, $alt = '', $antivirus_ico = TRUE, $limitSize = FALSE, $title = null, $wmode = 'window')
    {
        ...
        if ($ext == 'flv' || $ext == 'rar' || $ext == 'ogg' || $ext == '3gp' || $ext == 'wav') {
            $name_file = $ext == '3gp' ? $ext.'_32.png' : $ico.'_32.png';
        } else {
            $name_file = "ico_$ico.gif";
        }
...
//        return ($show_download_ico? "<img src=\"/images/ico_dowload.gif\" alt=\"загрузить\" title=\"загрузить\" width=\"14\" height=\"18\" /> ": "") . "<a href=\"".WDCPREFIX."/users/$ulogin/$dir/$filename\" target=\"_blank\" class=\"blue\">Загрузить</a> ($ext; ".$fsize.")";
    ...
    function viewattachLeft($ulogin, $filename, $dir, &$file, $maxh = 1000, $maxw = 450, $maxpw = 307200, $show_ico = 0, $is_tn = 0, $show_download_ico = 1, $file_num = 0, $alt = '', $title = false)
    {
        ...
        $lfname = $filename;
        if ($is_tn && in_array($ext, $GLOBALS['graf_array'])) {
            $lfname = $fname = $filename;
            $filename = 'sm_'.$filename;
        }
        if (substr($dir, -1) == '/') {
        ...
        if ($ext != 'swf' && in_array($ext, $GLOBALS['graf_array']) && $maxh != 0 && $maxpw != 0 && $maxpw != 0 && !$virusFlag) {
            ...
            if (($width <= $maxw && $height <= $maxh || $maxw == -1 && $maxh == -1) && $cfile->size <= $maxpw) {
                $file = 0;
                if ($type == 4 || $type == 13) {
                    return "<div class=\"b-page__desktop b-page__ipad\" style=\"text-align:center\">
                    ...
                    if ($is_tn) {
                        ...
                        $str = "<div  style=\"text-align:center\"><img src=\"{$path}{$filename}\" alt=\"$alt\" title=\"".($title ? $title : $alt)."\" width=\"$width\" height=\"$height\" /></div>";
                    }
 ...
 *
   ...
   */
    ...
    {
        if ($colorized) {
            $val = '<span style="">'.(floor($val)).'.'.(($val * 100) % 100).'</span>';
        }
        switch ($ind) {
    ...
    function view_avatar($user, $file, $size = 1, $animated = 1, $cls = 'b-pic')
    {
        if ($animated == 0) {
            ...
            if ($size) {
                if ($file) {
                    return '<img src="'.WDCPREFIX."/users/$user/foto/$file\" alt=\"$user\" width=\"50\" height=\"50\" class=\"{$cls}\" border=\"0\"/>";
                } else {
                    return '<img src="'.WDCPREFIX."/images/user-default-small.png\" alt=\"$user\" width=\"50\" height=\"50\" class=\"no_foto {$cls}\"  border=\"0\"/>";
                }
            } else {
                if ($file) {
                    return '<img src="'.WDCPREFIX."/users/$user/foto/$file\" alt=\"$user\" width=\"100\" height=\"100\" class=\"{$cls}\" border=\"0\"/>";
                } else {
                    return '<img src="'.WDCPREFIX."/images/no_foto_b.png\" alt=\"$user\" width=\"100\" height=\"100\" class=\"no_foto {$cls}\" border=\"0\"/>";
                }
            }
        }
        ...
        if ($size) {
            if ($file) {
                return '<img src="'.WDCPREFIX."/users/$user/foto/sm_$file\" alt=\"$user\" width=\"50\" height=\"50\" class=\"{$cls}\" border=\"0\"/>";
            } else {
                return '<img src="'.WDCPREFIX."/images/user-default-small.png\" alt=\"$user\" width=\"50\" height=\"50\" class=\"no_foto {$cls}\" border=\"0\"/>";
            }
        } else {
 ...
 *
    ...
    {
        if ($file) {
            return '<img src="'.WDCPREFIX."/users/$user/foto/sm_$file\" width=\"25\" height=\"25\" alt=\"\" class=\"lpl-avatar\" />";
        } else {
            return '<img src="/images/user-default-small.png" width="25" height="25" alt="" />';
        }
    }
 ...
 *
    ...
    function ago_pub($from_date, $format = 'ynjGi', $to_date = 0, $is_project = false)
    {
        ...
        if (($val = (int) date('y', $date_diff) - 70) && strpos($format, 'y') !== false) {
            ...
            if ($mod2 == 1 && ($mod1 < 10 || $mod1 > 20)) {
                ...
                if (($mod1 >= 10 && $mod1 <= 20) || $mod2 > 4 || $mod2 == 0) {
                    $out[] = "$val лет";
                } else {
                    $out[] = "$val года";
                }
        ...
        if (($val = date('n', $date_diff) - 1) && strpos($format, 'n') !== false) {
            ...
            if ($mod1 == 1 && ($val < 10 || $val > 20)) {
                ...
                if (($val >= 10 && $val <= 20) || $mod1 > 4 || $mod1 == 0) {
                    $out[] = "$val месяцев";
                } else {
                    $out[] = "$val месяца";
                }
    ...
    function ago_project_created($from_date)
    {
        ...
        if ($from_date > strtotime('-1day')) {
            ...
            if ($from_date > strtotime('-6month')) { // меннее полугода назад
 ...
 *
    ...
    function is_moder($role = '')
    {
        if ($role == '' && $_SESSION['role']) {
            return substr($_SESSION['role'], 1, 1);
        }
        if ($role) {
            return substr($role, 1, 1);
        } else {
            return 0;
        }
    }
    ...
    {
        if ($role == '' && $_SESSION['role']) {
            return substr($_SESSION['role'], 2, 1);
        }
        if ($role) {
            return substr($role, 2, 1);
        } else {
            return 0;
        }
    }
 ...
 *
    ...
    function is_mamadmin()
    {
        ...
        if (is_admin() || $_SESSION['login'] == 'sigitov') {
            return 1;
        }
 ...
 *
    ...
    {
        if ($role == '' && $_SESSION['role']) {
            return substr($_SESSION['role'], 0, 1);
        }
        if ($role) {
    ...
    function getGeo()
    {
        ...
        if (in_array($geo_record['city'], $moscow)) {
            return array('10', '100');
        }
        if (in_array($geo_record['city'], $spb)) {
            return array('10', '010');
        }
        if ($geo_record['country_code'] == 'RU') {
            return array('10', '001');
        }
    ...
    function highlight($text, $word, $fs = false)
    {
        ...
        if (preg_match_all("/\<a([^\>]*)\>([\w\s\W\S]+)\<\/a>/im", $text, $res)) {
            for ($i = 0; $i < count($res[0]); ++$i) {
                if (empty($res[2][$i]) || empty($word) || !strpos($res[2][$i], $word)) {
                    continue;
                }
    ...
    function view_time($time_value, $time_type)
    {
        if (!isset($time_value) || is_null($time_value) || ($time_value == 0) || ($time_value == '') || ($time_type < 0)  || ($time_type > 5)) {
            ...
            switch ($time_type) {
                ...
                default :
                case 0:
                    if (($mod1 == 1) && ($time_value < 10 || $time_value > 20)) {
                        $add = 'час';
                    } elseif (($mod1 < 5) && ($mod1 > 1) && ($time_value < 10 || $time_value > 20)) {
                        $add = 'часа';
                    } else {
                        $add = 'часов';
                    }
   ...
   */
    function view_range_time($time_from_value, $time_to_value, $time_type)
    {
        ...
        || ($time_type < 0)  || ($time_type > 3)) {
            ...
            if (!$time_to_value) {
                $mod = $time_from_value % 10;
            } else {
                $mod = $time_to_value % 10;
            }
  ...
  *
  ...
  function __paramInit($type = 'int', $get_name = NULL, $post_name = NULL, $empty_val = NULL, $maxlen = NULL, $strip_tags = FALSE, $value = FALSE)
  {
    ...
    if ($get_name && isset($_GET[$get_name])) {
      ...
      case 'page': $value = intvalPgSql($value);
...
return ($value <= 0) ? 1 : $value;
      ...
      case 'float'  : setlocale(LC_ALL, 'en_US.UTF-8');

return floatval($value);
      case 'money'  : setlocale(LC_ALL, 'en_US.UTF-8');
      ...
      <tr>
        <?php
          if ($pages > 1) {
              ...
              for ($i; $i <= $maxpages; ++$i) {
                  if ($i != $page) {
                      ?><td><?=call_user_func($output_func, $i, $arg)?></td><?php

                  } else {
                      ?><td class='box'><?=$i?></td><?php

                  }
              }
              if ($pages > 25 && $maxpages < $pages - 1) {
                  ?><td>...</td><td><?=call_user_func($output_func, $pages - 1, $arg)?></td><td><?=call_user_func($output_func, $pages, $arg)?></td><?php
   ...
   */
   function show_video($id, $url)
   {
       ...
       if (stristr($url, 'youtube.com') !== FALSE || stristr($url, 'youtu.be') !== FALSE) {
           ...
           if (stristr($url, 'rutube.ru') !== FALSE) {
    ...
    */
    function video_validate($url)
    {
        ...
        if (preg_match("/^((?:http|https):\/\/youtu\.be\/([-_A-Za-z0-9]+))/i", $url, $o)) {
            ...
            if (preg_match("/^((?:http|https):\/\/(?:ru\.|www\.)?youtube\.com\/watch\?).*(v=[-_A-Za-z0-9]+)/i", $url, $o)) {
                return $o[1].$o[2];
            } else {
                if (preg_match("/^((?:http|https)?:\/\/(?:www\.)?rutube\.ru\/video\/?[-_A-Za-z0-9]+\/{0,1})/i", $url, $o)) {
                    return preg_replace(array("/^http:\/\/www\./", "/^https:\/\/www\./", "/^http:\/\//"), 'https://', $o[1]);
                } else {
                    if (preg_match("/^((?:http|https)?:\/\/(?:www\.|video\.)?rutube\.ru\/(?:tracks\/)?[-_A-Za-z0-9]+(?:\.html)?)/i", $url, $o)) {
                        return preg_replace(array("/^http:\/\/www\./", "/^https:\/\/www\./", "/^http:\/\//"), 'https://', $o[1]);
                    } else {
                        if (preg_match("/^((?:http|https):\/\/(?:www\.)?vimeo\.com\/[0-9]+)/i", $url, $o)) {
 ...
 *
    ...
    function new_paginator($page, $pages, $count = PAGINATOR_PAGES_COUNT, $href = false, $link_type = 'href')
    {
        ...
        if ($page < $pages) {
            $html .= sprintf($href, '<li class="b-pager__next"><a class="b-pager__link" '.$link_type.'=', $page + 1, ' id="PrevLink"></a></li>');
        } 
        if ($page > 1) {
            $html .= sprintf($href, '<li class="b-pager__back"><a class="b-pager__link" '.$link_type.'=', $page - 1, ' id="NextLink"></a></li>');
        } //$page-1
        $html .= '</ul>';
 ...
 *
	...
	{
	    foreach ($arr as $k => $v) {
	        $result[strtolower(trim($v))] = strtolower(trim($v));
	    }
...
function getTermination($number, $variants)
{
    ...
    if ($mod1 == 1 && ($number < 10 || $number > 20)) {
        ...
        if (($number >= 10 && $number <= 20) || $mod1 > 4 || $mod1 == 0) {
            return $variants[2];
        } else {
            return $variants[1];
        }
...
{
    if (!$str = preg_replace('/[^0-9]+/i', '', $string)) {
...
function repair_html($input)
{
    ...
    function getBrowser($parse = false)
    {
        ...
        if (stristr($_SERVER['HTTP_USER_AGENT'], 'Firefox')) {
            $out .= 'Firefox ';
            if (preg_match("/(firefox|version)\/([0-9]+\.[0-9]+)/i", $_SERVER['HTTP_USER_AGENT'], $res)) {
                $out .= 'v.'.$res[2];
            }
        } elseif (stristr($_SERVER['HTTP_USER_AGENT'], 'Chrome')) {
            $out .= 'Coogle Chrome ';
            if (preg_match("/(chrome|version)\/([0-9]+\.[0-9]+)/i", $_SERVER['HTTP_USER_AGENT'], $res)) {
                $out .=  'v.'.$res[2];
            }
        } elseif (stristr($_SERVER['HTTP_USER_AGENT'], 'Safari')) {
            $out .= 'Safari ';
            if (preg_match("/(safari|version)\/([0-9]+\.[0-9]+)/i", $_SERVER['HTTP_USER_AGENT'], $res)) {
                $out .=  'v.'.$res[2];
            }
        } elseif (stristr($_SERVER['HTTP_USER_AGENT'], 'Opera')) {
            $out .= 'Opera ';
            if (preg_match("/(opera|version)\/([0-9]+\.[0-9]+)/i", $_SERVER['HTTP_USER_AGENT'], $res)) {
                $out .=  'v.'.$res[2];
            }
...
function isCurrentPage($page = '/', $retTrue = TRUE, $retFalse = FALSE, $notParams = array(), $notPage = array())
{
    ...
    if (!$site) {
        ...
        if ($path === '/') {
            ...
            $site = explode('/', preg_replace('|/*(.+?)/*$|', '\\1', $path));
        }
    ...
    if (!empty($notPage)) {
        $ret = (count(array_intersect_assoc($site, $notPage)) != count($notPage)) && $ret;
    }
