 ...
 *
	 ...
	 */
	public function prepare($wmzr, $billing_no, $ammount, $operation_type, $operation_id)
	{
	    if (!in_array($wmzr, $this->wmzr)) {
	        $error = 'Неверный кошелек!';
	    }
 ...
 *
	 * @return string Сообщение об ошибке
	 */
	public function checkdeposit($wmzr, $ammount, $payment_num,
			...
			$hash, $mode, $billing_no, $operation_type, $operation_id)
	{
	    ...
	    switch ($operation_type) {
			...
			default:		//Перевод денег на личный счет
				...
				if (substr($wmzr, 0, 1) == 'Z') {
				    $amm = $ammount * $this->exchZ;
				}
		}

	    if (substr($wmzr, 0, 1) == 'R') {
	        if ($wmzr == 'R109922555324') {
	            $ps = 10;
	        } else {
	            $ps = 2;
	        }
