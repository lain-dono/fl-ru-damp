...
class mailer
{
    public static $LINKS_HINT = array(
    ...
    public static $TYPE_REGULAR = array(
        1 => 'Без повторений',
        2 => 'Еженедельно',
        3 => 'Ежемесячно',
        4 => 'Ежегодно',
    ...
    public static $SUB_TYPE_REGULAR = array(
    ...
    {
        global $DB;
        if (!$this->checkDBTableFilter($name)) {
            return;
        }
     ...
     *
     * @return int ID Созданного фильтра 
     */
     ...
     *
     * @return bool true - название корректно, false - некорректно
     */
    public function checkDBTableFilter($name)
    {
        if ($name != 'mailer_filter_employer' && $name != 'mailer_filter_freelancer') {
            return false;
        }
     ...
     * @param type $filename
     */
    public function parseExtraList($filename, $offset = 0)
    {
        $list = array();
     ...
     * @param int   $count  Возвращает количество рассылок с учетом фильтра
     *
    ...
    public function checkRangeDate($from, $to, $error_name)
    {
     ...
     * @return int ID Созданного фильтра 
     */
    public function initPostEmpFilter($post)
    {
        ...
        if ($post['efinance'] == 1) {
            ...
            if ($post['efinance_money']) {
                $EFinance->setFilter('integer', 'money', $post['efinance_money'] / 30);
            }
            ...
            if ($post['efinance_method_deposit']) {
                $EFinance->setFilter('bit', 'method_deposit', array((int) $post['efinance_method_deposit'][0], (int) $post['efinance_method_deposit'][1], (int) $post['efinance_method_deposit'][2], (int) $post['efinance_method_deposit'][3]));
            }

            if ($EFinance->is_update) {
                $filter_emp['finance'] = (string) $EFinance;
            }
        }
        ...
        if ($post['ebuying'] == 1) {
            ...
            $EBuying = new UBuying();
            if ($post['ebuying_buying']) {
                $EBuying->setFilter('bit', 'buying', array((int) $post['ebuying_buying'][0], (int) $post['ebuying_buying'][1]));
            }
        ...
        if ($post['eproject'] == 1) {
            ...
            if ($post['eproject_executor']) {
                $EProject->setFilter('bit', 'executor', array((int) $post['eproject_executor'][0], (int) $post['eproject_executor'][1], (int) $post['eproject_executor'][2]));
            }
     ...
     * @return int ID Созданного фильтра 
     */
    public function initPostFrlFilter($post)
    {
        ...
        if ($post['ffinance'] == 1) {
            ...
            if ($post['ffinance_money']) {
                $FFinance->setFilter('integer', 'money', $post['ffinance_money'] / 30);
            }
            ...
            if ($post['ffinance_method_deposit']) {
                $FFinance->setFilter('bit', 'method_deposit', $ffinance_method);
            }

            if ($FFinance->is_update) {
                $filter_frl['finance'] = (string) $FFinance;
            }
        }
        ...
        if ($post['fbuying'] == 1) {
            ...
            $FBuying = new UBuying();
            if ($post['fbuying_buying']) {
                $FBuying->setFilter('bit', 'buying', array((int) $post['fbuying_buying'][0], (int) $post['fbuying_buying'][1]));
            }
            if (!empty($ebuying_period)) {
        ...
        if ($post['fproject'] == 1) {
            ...
            if (!empty($fproject_count)) {
                $FProject->setFilter('range_integer', 'count', $post['fproject_count']);
            }
            if ($post['fproject_type']) {
                $FProject->setFilter('bit', 'type_project', array((int) $post['fproject_type'][0], (int) $post['fproject_type'][1], (int) $post['fproject_type'][2]));
            }

            if ($FProject->is_update) {
                $filter_frl['project'] = (string) $FProject;
            }
        }
        ...
        if ($post['fblog'] == 1) {
            ...
            if (!empty($fblog_post)) {
                $FBlog->setFilter('range_integer', 'post', $post['fblog_post']);
            }

            if ($FBlog->is_update) {
                $filter_frl['blogs'] = (string) $FBlog;
            }
        }
    ...
    public function getSQLFilterFreelancer($filter, $fields = 'uid')
    {
        ...
        if (isset($filter['filter_frl'])) {
            ...
            if ($filter['fproject']) {
                ...
                if ($sql) {
                    $where = self::setMainWhereSQL($sql, 'AND');
                }//implode(" AND ", $s);
                if ($union_sql) {
                    $sub_where = self::setMainWhereSQL($union_sql, 'AND');
                }
                if ($sub_where && $main_where) {
                    $sub_where .= " AND ( $main_where )";
                } elseif ($main_where) {
    ...
    public function getSQLFilterEmployer($filter)
    {
        ...
        if (isset($filter['filter_emp'])) {
            ...
            if ($filter['eproject']) {
                ...
                $cr = 2;
                $cex = array(2,3,4,1);
                if (($cost_from = (float) $eproject['budget'][0]) < 0) {
                    $cost_from = 0;
                }
                if (($cost_to = (float) $eproject['budget'][1]) < 0) {
                    $cost_to = 0;
                }
    ...
    public function getMailerSend()
    {
        ...
        foreach ($result as $k => $message) {
            ...
            if (!$message['filter_file']) { //Нет файла: отрабатываем по фильтрам
                ...
                } elseif ($message['filter_frl'] > 0 && $message['filter_emp'] > 0) {
                    $sql = $sql_emp_recipient.' UNION '.$sql_frl_recipient;
                } elseif ($message['filter_frl'] > 0) {
                    $sql = $sql_frl_recipient;
                } elseif ($message['filter_emp'] > 0) {
     ...
     * @global type $DB
     *
     * @param int $mailer_id ИД рассылки
     */
    ...
    public function updateStatusSending($force = false)
    {
        ...
        foreach ($mailer as $message) {
            ...
            } elseif ($message['spamid'] > 0 && $spam_send == 1) {
                $this->update(array('status_sending' => ($message['type_regular'] <= 1 ? 1 : 0),
                                    'status_message' => 1, ), $message['id']);
            } elseif ($message['msgid'] > 0 && $msg_send == 1) {
                $this->update(array('status_sending' => ($message['type_regular'] <= 1 ? 1 : 0),
                                    'status_message' => 1, ), $message['id']);
            } elseif ($force) {
                $this->update(array('status_sending' => ($message['type_regular'] <= 1 ? 1 : 0),
    ...
    public function getDateSubscr($mailer)
    {
        ...
        } elseif ($mailer['type_send_regular'] > 1) { // Регулярно рассылаемые - следующая дата рассылки
            return strtotime($mailer['date_sending']);
        } elseif ($mailer['status_message'] == 1) { // Разосланные - настоящая дата рассылки
    ...
    public function getColorMailer($mailer)
    {
        ...
        } elseif ($mailer['status_message'] == 1) { // Все разосланные синие
            return '';
        } elseif (strtotime($mailer['date_sending']) <= strtotime('+1week') || $mailer['status_message'] != 1) { // Все неразосланные красные
...
class subfilter
{
    ...
    {
        if (!array_key_exists($name, $this->filter) && $this->block_added == true) {
            return false;
        }
    ...
    {
        if (!array_key_exists($name, $this->filter[$this->multi_name][0]) && $this->block_added == true) {
            return false;
        }
        if ($this->is_update === false) {
            $this->is_update = true;
