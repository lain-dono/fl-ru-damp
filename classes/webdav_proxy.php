...
class webdav_proxy
{
     ...
     * @return webdav_proxy
     */
    public static function getInst($wdcs_prms = null)
    {
        if (!self::$_inst) {
            self::$_inst = new self($wdcs_prms);
        }
     ...
     *
     * @param bool $exclude_reserved_wdc исключить ли резервные сервера (для каких-то временных данных, чтоб не нагружать лишний раз).
     *
     * @return bool успешно?
     */
    public function put_file($path, $filename, $exclude_reserved_wdc = false)
    {
     ...
     * @return bool успешно?
     */
    public function put($path, $content, $exclude_reserved_wdc = false)
    {
     ...
     * @return bool успешно?
     */
    public function delete($path, $exclude_reserved_wdc = false)
    {
        if (!$this->safePath($path)) {
            return false;
        }
     ...
     *
     * @return bool true:существует на всех серверах, false:не существует хотя бы на одном или ошибка.
     */
    public function check_file($path, $dont_check_put = false)
    {
     ...
     *
     * @param int    $wdid ид. DAV-соединения.
     * @param string $path путь до файла (без первого слеша).
     *
     * @return array
     */
    public function get_info($wdid, $path)
    {
        $info = null;
     ...
     */
    public function get_file($src_path, $localpath)
    {
        foreach ($this->_wdcs as $wdc) {
            if ($ok = $this->_connect($wdc)) {
