...
class masssending
{
     ...
     *
     * @param int                                                                                                                                    $uid    uid пользователя совершаемого рассылку
    ...
    public function Calculate($uid, array $params, $commit = 0)
    {
        ...
        if (!empty($params['sbr'])) {
            ...
            if (!empty($params['discharge3'])) {
                $discharge = 3;
            } elseif (!empty($params['discharge2'])) {
                $discharge = 2;
            } elseif (!empty($params['discharge1'])) {
                $discharge = 1;
            } else {
                $discharge = 0;
            }
    ...
    public function CalculateFromSearch($uid, array $param, $commit = 0)
    {
        ...
        if ($filter['prof'][1] && is_array($filter['prof'][1])) {
            ...
            $searchString .= implode(' ', $a_professions);
        }
     ...
     *
     * @return bool|array массив с расчетами или FALSE если произошла ошибка
     */
    public function Add($uid, array $params)
    {
        ...
        if ($params['sbr']) {
            if ($params['discharge3']) {
                $rank = 3;
            } elseif ($params['discharge2']) {
                $rank = 2;
            } elseif ($params['discharge1']) {
                $rank = 1;
            }
        }
     ...
     *
     * @return int 1 - все сработало как надо, 0 - ошибка
     */
...
--- ';
     ...
     *
     * @param int $tariff_id Если NULL, то вернет текущий тариф.
     *
     * @return array
     */
     ...
     *
     * @param int     $pro    Цена для рассылки ПРО
     ...
     * Выборка рассылки.
     *
     * @param int    $id         ИД рассылки или NULL -- взять все подходящие.
     * @param int    $order_mode режим сортировки (фильтрации) 
     * @param int    $offset     Позиция выборки
    ...
    public function Get($id = null, $order_mode = self::OM_NEW, $offset = 0, $limit = 'ALL')
    {
        ...
        } elseif ($order_mode == self::OM_OLD) {
            $where = 'WHERE ps.decided_time IS NOT NULL';
        } elseif ($order_mode & self::OM_ACCEPTED) {
            $where = 'WHERE ps.is_accepted = true';
        } elseif ($order_mode & self::OM_DENIED) {
            $where = 'WHERE ps.is_accepted = false';
        }

        if ($id) {
            $where = ($where ? ' AND' : 'WHERE')." ps.id = {$id}";
        }
        ...
        if ($res = $DB->squery($sql)) {
            ...
            if ($ls) {
                ...
                while ($row = pg_fetch_assoc($res)) {
                    if (empty($ls[ $row['mid'] ]['files'])) {
                        $ls[ $row['mid'] ]['files'] = array();
                    }
     ...
     *
     * @param int $order_mode Режим фильтрации
     *
     * @return int количество, либо 0
     */
     ...
     *
     * @param int $masssendID ID рассылки
     * @param int $accOpID    ID операции
     */
    public function UpdateAcOpID($masssendID, $accOpID)
    {
        global $DB;
        $data = array(
