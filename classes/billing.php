...
//        $this->last_history = $this->_db->rows($sql, $sbr_op_codes, $this->user['uid'], $this->user['uid'], billing::RESERVE_STATUS);
    ...
    public static function expiredTime($date)
    {
        ...
        if ($expired['is_day_expired']) {
            ...
            if ($expired['day_expired'] <= 0) {
                $expired['day_expired'] = 1;
            }
        }
     ...
     * 
     * @param int  $id    ИД записи для удаления
     * @param bool $event
     *
     * @return bool
     */
        ...
        if ($order['parent_table'] == 'draft_projects') {
            switch ($order['option']) {
                ...
                case 'office':
                    ...
                    $this->_db->query($sql, $order['parent_id'], $this->user['uid']);
                    $where = $this->_db->parse(' AND parent_table = ? AND parent_id = ? AND status = ? ', 'draft_projects', $order['parent_id'], self::STATUS_NEW);
                    $remove = $this->findOrders($where);
                    $ids = array_map(create_function('$array', 'return $array["id"];'), $remove);
                    $this->update($ids, array('status' => self::STATUS_DELETE, 'delete_time' => 'NOW()'));
     ...
     * @return bool
     */
    public function clearOrders($status = self::STATUS_NEW, $add_where = '')
    {
     ...
     *  
     * @param int    $reserve_id ИД резерва (@see table.bill_reserve)
     * @param string $status     Статусы услуг для обновления
     *
     * @return bool
     */
    public function updateOrderListStatus($reserve_id, $status = self::STATUS_NEW)
    {
     ...
     * 
     * @param int   $ammount     Стоимость всех заказанных услуг
     * @param bool  $is_personal Будет ли задействована оплата из личного счета
     ...
     * @return type
     */
    public function getReserveOperationsByStatus($status = self::RESERVE_STATUS)
    {
     ...
     * @return bool
     */
    public function startReserved($id, $status = self::RESERVE_STATUS)
    {
        $where = $this->_db->parse((is_array($id) ? 'id IN (?l)' : 'id = ?i').' AND status = ?', $id, $status);

        return $this->_db->update('bill_reserve', array('status' => self::RESERVE_PROCESS_STATUS), $where);
    }
     ...
     * @return bool
     */
    public function stopReserved($id, $status = self::RESERVE_PROCESS_STATUS)
    {
        $where = $this->_db->parse((is_array($id) ? 'id IN (?l)' : 'id = ?i').' AND status = ?', $id, $status);

        return $this->_db->update('bill_reserve', array('status' => self::RESERVE_STATUS), $where);
    }

    /**
     * Возвращает текущий статус списка.
     * 
     * @param int $id ИД списков
     *
     * @return string
     */
     ...
     * 
     * @param int $reserve_id Ид списка услуг
     *
     * @return type
     */
     ...
     * 
     * @param int $reserve_id ИД списка который оплачиваем
     *
     * @return bool
     */
     ...
     * @param bool   $auto    Автопродление (включено/выключено)
     * @param int    $id      Ид определенной услуги
     *
     * @return \billing
     */
     ...
     * @return int
     */
    public function getTotalAmmountOrders($status = self::STATUS_NEW)
    {
    ...
    public function cancelAndRemoveOrders($orders)
    {
        if (!empty($orders)) {
            ...
            if (!empty($reserve)) {
                $this->setReserveStatus($reserve, self::RESERVE_CANCEL_STATUS);
            }
     ...
     * @return array
     */
    public function getOrders($status = self::STATUS_NEW)
    {
        $sql = '
            SELECT * 
            FROM bill_queue 
     ...
     * @return array
     */
    public function getOrder($order_id, $status = self::STATUS_RESERVE)
    {
        $sql = '
            SELECT * 
            FROM bill_queue 
     ...
     * @return array
     */
    public function getCountListServices($status = self::STATUS_NEW)
    {
     ...
     * @param $uid
     */
    public static function isAnyAutoPayed($uid)
    {
        global $DB;
     ...
     * @return mixed
     */
    public static function getAllAutoPayed($uid)
    {
        global $DB;
     ...
     * @return array
     */
    public static function getOpcodesByAutopayed($service = 'all')
    {
        if ($service == 'pro') {
            $result = array(
     ...
     */
    public function cancelAllNewAndReserved()
    {
    ...
    public function getDescrByOpCode($opCode)
    {
        $_descr = false;
     ...
     * @return type
     */
    public static function getOpCodeByDiscount($opCode)
    {
        $saleOpCode = $opCode;
     ...
     * @return int
     */
    public function getDiscountOpCode($opCode)
    {
        $saleOpCode = $opCode;
    ...
    public function buyOrder($order_id, $op_code = null, $paymentSysParams = array())
    {
        ...
        if ($op_code > 0 &&
            !in_array($op_code, self::$op_code_transfer_money)) {
            return false;
        }
     ...
     * @param array $option  - опции заказа
     *
     * @return bool - удалось ли оплатить с ЛС
     */
