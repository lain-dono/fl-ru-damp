 ...
 *
	 ...
	 */
	public function Add($uid, $questions)
	{
	    $data = array();
 ...
 *
	 ...
	 */
	public function Update($id, $questions)
	{
	    $data = array();
	    foreach ($questions as $ikey => $value) {
	        if ($value != '') {
 ...
 *
	 * @return integer
	 */
	public function EditPhotos($id, $attach, $photo)
	{
	    if (!$id) {
	        return 0;
	    }
 ...
 *
	...
	public function Deletefoto($id)
	{
	    ...
	    if ($file) {
	        ...
	        if ($small == 2) {
	            $cfile->Delete(0, 'users/'.substr($_SESSION['login'], 0, 2).'/'.$_SESSION['login'].'/upload/', 'sm_'.$file);
	        }
	    }
 ...
 *
	...
	public function DelInterv($id)
	{
	    ...
	    for ($i = 0; $i < 8; ++$i) {
	        if ($row['file'.$i]) {
	            ...
	            if ($row['pt'.$i] == 2) {
	                $cfile->Delete(0, 'users/'.substr($_SESSION['login'], 0, 2).'/'.$_SESSION['login'].'/upload/', 'sm_'.$row['file'.$i]);
	            }
	        }
	    }
     ...
     * @return array
     */
    public function getInterview($uid, $id = null, $order = 'date', $year = null, $filter = 0)
    {
        global $DB;
        $users_inner_sql = '';
        switch ($filter) {
     ...
     * @return boolean
     */
    public function setInterviewLVT($user_id, $interview)
    {
        if (!$user_id) {
            return false;
        }
     ...
     */
    public function addInterview($uid, $text, $files = array(), $is_jury = 'f')
    {
     ...
     * @param array  $files файлы прикрепленные к интервью
     */
    public function updateInterview($id, $uid, $text, $files = array(), $is_jury)
    {
     ...
     */
    public function delInterview($id)
    {
        global $DB;
     ...
     * @return array
     */
    public function getInterviewFiles($interview_id)
    {
        global $DB;
     ...
     * @return array
     */
    public function getNavigation($interview, $order = 'date')
    {
        switch ($order) {
//            case 'views':
//                $field = "view_cnt";
