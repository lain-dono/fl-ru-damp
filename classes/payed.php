...
class payed
{
    public static $date_action_test_pro = array(20130210, 20130221);
 ...
 *
	 ...
	 */
	 public function GetProPrice($get_all = FALSE, $op_code = 48)
	 {
	     $base = 10;
    ...
    public static function get_opcode_action_test_pro()
    {
 ...
 *
	 ...
	 */
	public function SetOrderedTarif($fid, $transaction_id, $time, $comments = 'Все разделы', $tarif = 48, $promo_code = 0, &$error = null)
	{
	    global $DB;
	    ...
	    if ($tarif == 47 || $tarif == 114) {
	        ...
	        $time = '1 day';
	    } elseif (in_array($tarif, array(48, 163, 164))) {
	        $time = '1 month';
	    } elseif ($tarif == 49) {
	        $time = '3 month';
	    } elseif ($tarif == 50) {
 ...
 *
	 ...
	 */
	public function GiftOrderedTarif(&$bill_id, &$gift_id, $gid, $fid, $transaction_id, $time, $comments = 'Аккаунт PRO  в подарок', $tarif = 52)
	{
	    global $DB;
	    require_once $_SERVER['DOCUMENT_ROOT'].'/classes/account.php';
	    $account = new account();
     ...
     * @return array Данные выборки
     */
    public function GetProTestUsers()
    {
        global $DB;
        $sql = "
 ...
 *
	 ...
	 */
	public function SetPayed($id, $ammount, $date, $to_date)
	{
	    global $DB;
 ...
 *
     ...
     */
	public function CheckPro($login)
	{
	    global $DB;
     ...
     */
    public function checkProByUid($uid)
    {
        global $DB;
     ...
     * @return type
     */
    public function ProLastById($uid, $_notIn = array())
    {
        global $DB;
        $sql = "SELECT 
     ...
     * @param type $uid
     */
    public function getProfiDaysFromPro($uid)
    {
        global $DB;
 ...
 *
     ...
     * @return type
     */
    public function disableActivePro($uid, $_tarifNotIn = array())
    {
        global $DB;
	 ...
	 * @return integer
	 */
	public function UpdateProUsers()
	{
	    global $DB;
 ...
 *
	 * @return boolean
	 */
	public function checkNewPro($uid)
	{
	    global $DB;
	    $sql = "
	      UPDATE freelancer f
	 ...
	 * @return integer
	 */
    public function AlertPROEnding()
    {
        global $DB;
    	/**
                ...
                foreach ($users as $user) {
                    ...
                    if ($queueID) {
                        ...
                        if ($complete) {
                            ...
                            if ($user['attempt'] == 1) { // Первая попытка
...
//                $mail->sendAutoPROEnding(( $role == 'freelancer' ? 'FRL' : 'EMP' ), $users);
     ...
     */
    public function checkAutoProTest($login, $attempt = 1)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/billing.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/bar_notify.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/smail.php';
...
*
	...
	*/
	public function IsUserWasPro($uid)
	{
	    global $DB;
	    $uid = intval($uid);
     ...
     * @return type
     */
    public static function isWasPro($uid)
    {
        global $DB;
 ...
 *
	 ...
	 */
	public function AdminAddPRO($fid, $transaction_id, $time, $comments = 'Аккаунт PRO. Возмещение платного сервиса', $tarif = 63)
	{
	    global $DB;
 ...
 *
	 * @return unknown
	 */
	public function getUpRating($fid, $sum, $transaction_id, $time, $comments = 'Поднятие рейтинга за FM', $tarif = 75, $ammount = 1, $commit = 1)
	{
	    require_once ABS_PATH.'/classes/account.php';
	    $account = new account();
     ...
     * @param integer $uid ИД пользователя
     */
    public function getLastFreeze($uid)
    {
        global $DB;
        $uid = intval($uid);
     ...
     * @return boolean
     */
    public function freezePro($uid, $from_date, $to_date)
    {
        global $DB;
        $uid = intval($uid);
     ...
     * @return boolean
     */
    public function freezeProCancel($uid, $freeze_id)
    {
        global $DB;
        $uid = intval($uid);
     ...
     * @return boolean
     */
    public function freezeProStop($uid, $freeze_id)
    {
        global $DB;
        $uid = intval($uid);
        $freeze_id = intval($freeze_id);
     ...
     * @return boolean
     */
    public function freezeProDeactivate($uid)
    {
        global $DB;
        $uid = intval($uid);
     ...
     * @return boolean
     */
    public function freezeUpdateProUsers()
    {
        global $DB;
     ...
     * @return boolean
     */
    public function isProFreezed($uid)
    {
        global $DB;
        $uid = intval($uid);
     ...
     * @param integer $uid ИД пользователя
     */
    public function getFreezedDaysCnt($uid)
    {
        global $DB;
        $uid = intval($uid);
     ...
     * @return type
     */
    public static function getAvailablePayedList($is_emp = false)
    {
     ...
     * @return int
     */
    public static function getPayedPROFIList()
    {
        $payed = array();
     ...
     * @return array
     */
    public static function getPayedPROList($role = 'frl')
    {
        if ($role == 'frl') {
            $payed = array(
                /*
    ...
    public static function updateUserSession()
    {
        ...
        $pro_last = self::ProLast($_SESSION['login']);
