...
class projects_filters
{
     ...
     * @return string
     */
    public function Save($user_id, $cost_from, $cost_to, $currency, $wo_cost, $categories, $country, $city, $keywords, $my_specs, &$rerror, &$error, $force = 0, $page = 0, $nogeo = false,
    ...
    {
        $uid = get_uid(false);
        list($cost_from, $cost_to) = self::preCosts($cost_from, $cost_to);
     ...
     */
    public static function preCosts($cost_from, $cost_to)
    {
     ...
     * @return array
     */
    public function Add($user_id, $cost_from, $cost_to, $currency, $wo_cost, $categories, $country, $city, $keywords, $my_specs, $page = 0, $only_sbr, 
     ...
     * @return string
     */
    public function Update($filter_id, $cost_from, $cost_to, $currency, $wo_cost, $categories, $country, $city, $keywords, $my_specs, $page = 0, $nogeo = false, $only_sbr, 
     ...
     * @return array
     */
    public function GetFilter($user_id, &$error, $page = 0)
    {
        if ($user_id > 0) {
            global $DB;
     ...
     * @return string
     */
    public function DeleteFilter($user_id, $page = 0)
    {
        if ($user_id > 0) {
     ...
     * @return string
     */
    public function ActivateFilter($user_id, $page = 0)
    {
        global $DB;
        if ($user_id) {
     ...
     */
    public function IsFilter($user_id, $page = 0)
    {
        if ($user_id > 0) {
            global $DB;
     ...
     */
    public function ShowClosedProjects($kind, $page, $filter)
    {
        $closed_name = '';
     ...
     */
    public function initClosedProjects()
    {
        $uid = $_SESSION['uid'];
        if (!$uid) {
            return;
        ...
        if (is_array($_SESSION['ph'])) {
            foreach ($_SESSION['ph'] as $key => $name) {
                if (!trim($name)) {
                    unset($_SESSION['ph'][$key]);
                }
            }
        }
    }
...
class projects_filters_pda extends projects_filters
{
    ...
    public function IsFilter($user_id)
    {
        global $DB;
        if ($user_id > 0) {
