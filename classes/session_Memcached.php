 ...
 *
    ...
    */
   public function getActivityByLogin($login)
   {
       if (!$login) {
           return 0;
       }
     ...
     */
    public function nullActivityByLogin($login)
    {
        $bRet = false;
   ...
   */
   public function UpdateProEndingDate($login)
   {
       if (!$login) {
           return;
       }
   ...
   * @param string $login   логин пользователя
   */
   public function UpdateVerification($login)
   {
       if (!$login) {
           return;
       }
    ...
    * @return type 
    */
   public function UpdateAccountSum($login)
   {
       if (!$login) {
           return;
       }
    ...
    */
   public function view_online_status($login, $full = false, $nbsp = '&nbsp;', &$activity = NULL)
   {
       ...
       if ($this->last_ref && (time() - $last_ref_unixtime <= 30 * 60)) {
           ...
           $this->is_active = true;
           if (intval($this->ago) == 0) {
               $this->ago = 'менее минуты';
           }
    ...
    * @return string			HTML-код значка активности
    */
    public function view_online_status_new($login, $full = false, $nbsp = '&nbsp;', &$activity = NULL)
    {
        if ($login) {
