...
//       self::TYPE_REGIONS   => array('priority'=> 0.5),
    ...
    public function __construct($type)
    {
        ...
        $this->sitemap_lngth = strlen(self::SITEMAP_HEADER.self::SITEMAP_FOOTER);
        $this->siteindex_lngth = strlen(self::SITEMAP_HEADER.self::SITEMAP_FOOTER);
     ...
     *
     * @param string $type тип. карты.
     * @param bool   $send отправлять ли в гугул индекс.
     */
    public static function create($type, $send = true)
    {
        $smap = new self($type);
        $smap->_create($send);
    }
     ...
     * @param bool   $send отправлять ли в гугул индекс.
     */
    public static function update($type, $send = true)
    {
        $smap = new self($type);
        $smap->_update($send);
    }
     ...
     * @param bool $send отправлять ли в гугул индекс.
     */
    public function _create($send = true)
    {
     ...
     */
    public function createFilesSiteindex()
    {
        if (count($this->index_items) < 1) {
            $idx = $this->createFilesSitemap();
        } else {
            $idx = $this->index_items;
        }
     ...
     * @return string
     */
    public function getSiteindexXml($idx)
    {
     ...
     * @return string
     */
    public function getSitemapXml($url)
    {
        $xml = self::SITEMAP_HEADER;
        if (count($url) > 0) {
            $xml .= implode('', $url);
        }
        $xml .= self::SITEMAP_FOOTER;
     ...
     * @return array
     */
    public function createFilesSitemap($urls = false)
    {
        if (!$urls) {
        ...
        foreach ($urls as $k => $value) {
            ...
            $loc = self::PUBLIC_HOST.$this->folder_name.$fname;
            $index = '<sitemap>';
            $index .= "<loc>{$loc}</loc>";
     ...
     *  
     * @param int $x   Стартовая позиция
     * @param int $key Номер файла
     * @param int $end Условие конца работы создания
     *
     * @return int Возвращает следующую стартовую позицию
     */
    public function createOneFilesSitemap($x = 0, &$key = 0, $end = false)
    {
        if ($this->end) {
            return false;
        }
        $url = array();
        $m = ((self::MAX_SIZE_COUNT / ($this->type == 'userpages' ? 5 : 1)) / $this->limit_one_operation - 1);
        if ($m < 0) {
            $m = 1;
        }
        $this->end = $end;
     ...
     * @return array
     */
    public function getSitemapUrls($key = 0)
    {
        global $DB;
        switch ($this->type) {
            case 'freelancers':
        ...
        foreach ($result as $val) {
            switch ($this->type) {
                case 'projects':
                    $loc = self::PUBLIC_HOST.getFriendlyURL('project', $val['p_id']);
                    break;
                case 'blogs':
                    $loc = self::PUBLIC_HOST.getFriendlyURL('blog', $val['b_id']);
                    break;
                case 'commune':
                    $loc = self::PUBLIC_HOST.getFriendlyURL('commune', $val['m_id']);
                    break;
                case 'articles':
                    $loc = self::PUBLIC_HOST.getFriendlyURL('article', $val['a_id']);
                    break;
                case 'interview':
                    $loc = self::PUBLIC_HOST.getFriendlyURL('interview', $val['i_id']);
                    break;
                case 'regions':
                    $loc = self::PUBLIC_HOST.'/freelancers/'.($val['link'] ? $val['link'].'/' : '').$val['translit_country_name'].'/'.($val['translit_city_name'] ? $val['translit_city_name'].'/' : '');
                    break;
                case 'tservices':
                    $loc = self::PUBLIC_HOST.tservices_helper::card_link($val['t_id'], $val['t_name']);
                    break;
                default:
                    $loc = self::PUBLIC_HOST.$val['loc'];
                    break;
            }
            ...
            if (count($ret[$key]) >= (self::MAX_SIZE_COUNT / ($this->type == 'userpages' ? 5 : 1))) {
                $strlen = $this->sitemap_lngth;
                $key = $key + 1;
            }
     ...
     *
     * @param int $count    Количество ссылок в файле
     * @param int $end_date Последняя дата
     * @param int $n        Количество файлов    
     */
    ...
    public function update_file($data)
    {
        ...
        if ($new_count >= self::MAX_SIZE_COUNT / ($this->type == 'userpages' ? 5 : 1)) {
            $new_file_count = $new_count - self::MAX_SIZE_COUNT / ($this->type == 'userpages' ? 5 : 1);
            ...
            $old_xml = implode('', $old_file_data).self::SITEMAP_FOOTER;
            ...
            $this->update_index_file(self::PUBLIC_HOST.$this->folder_name.sprintf($this->fname_sitemap, $this->type, $data['n']), true);
            $this->update_index_file(self::PUBLIC_HOST.$this->folder_name.sprintf($this->fname_sitemap, $this->type, '_new'));
            ...
            */
            ...
            $xml = implode('', $result[0]).self::SITEMAP_FOOTER;

            $filename = $this->folder.sprintf($this->fname_sitemap, $this->type, '_new');
            $this->save_new_file($filename, $xml);

            $this->update_index_file(self::PUBLIC_HOST.$this->folder_name.sprintf($this->fname_sitemap, $this->type, '_new'), true);
     ...
     */
    public function save_new_file($filename, $xml)
    {
        $filesize = filesize($filename);
        $fp = fopen($filename, 'r+');
        fseek($fp, $filesize - strlen(self::SITEMAP_FOOTER));
        fwrite($fp, $xml);
        fclose($fp);
    }
     ...
     * @param bool   $delete Удалять или нет последний индекс (для перезаписи)  
     */
    public function update_index_file($loc, $delete = false)
    {
        libxml_disable_entity_loader();

        $xml = file_get_contents($this->folder.$this->index_name);
        $xml = self::SITEINDEX_HEADER.$xml;
        $xml = $xml.self::SITEINDEX_FOOTER;

        $sxe = new SimpleXMLElement($xml, null, false);
        $cnt = count($sxe->sitemap);
        if ($delete) {
            unset($sxe->sitemap[$cnt - 1]);
        }
        ...
        $xml = $sxe->asXML();
        $xml = str_replace(self::SITEINDEX_HEADER, '', $xml);
        $xml = str_replace(self::SITEINDEX_FOOTER, '', $xml);
