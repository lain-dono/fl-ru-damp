 ...
 *
	 ...
	 */
	public function CreateGroup($title, $position)
	{
    ...
    */
    public function ReorderTeam($order)
    {
        global $DB;
        $DB->clear();
        $p = preg_split("/\|/", $order);
 ...
 *
	 * @return boolean всегда true
	 */
	public function EditGroup($groupid, $title, $position)
	{
	    //на всякий случай перед операцией перемещения обновим позиции всех групп
		$res = $GLOBALS['DB']->query('SELECT id, position FROM team_groups ORDER BY position, id');
	    $i = 1;
 ...
 *
	 * @return boolean всегда true
	 */
	public function MoveGroup($moveto, $groupid)
	{
 ...
 *
	 * @return boolean всегда true
	 */
	public function MoveUser($moveto, $groupid, $userid)
	{
	 ...
	 */
	public function AddUser($name, $login, $occupation, $userpic, $groupid, $position, $info)
	{
	 ...
	 * @return ineger               ID добавленного пользователя
	 */
	public function EditUser($id, $name, $login, $occupation, $userpic, $groupid, $position, $info)
	{
