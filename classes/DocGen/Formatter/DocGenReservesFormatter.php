...
- Осуществления возврата Заказчику части Зарезервированной суммы в размере {$emp_price} в порядке, предусмотренном п. 6.14, раздела 7 Договора.';
    ...
    */
    ...
    public function text2top1($options)
    {
        ...
        if ($arbitrage_price == 0) {
            $text = self::TEXT2_1_TOP_1;
        } elseif ($arbitrage_price == $price) {
            $text = self::TEXT2_2_TOP_1;
        } else {
    ...
    public function text2top2($options)
    {
        ...
        if ($arbitrage_price == 0) {
            $text = self::TEXT2_1_TOP_2;
        } elseif ($arbitrage_price == $price) {
            $text = self::TEXT2_2_TOP_2;
        } else {
    ...
    public function text2mid($options)
    {
        ...
        if ($arbitrage_price == 0) {
            $text = self::TEXT2_1_MID;
        } elseif ($arbitrage_price == $price) {
            $text = self::TEXT2_2_MID;
        } else {
    ...
    public function text2bot($options)
    {
        ...
        if ($arbitrage_price == 0) {
            $text = self::TEXT2_1_BOT;
        } elseif ($arbitrage_price == $price) {
            $text = self::TEXT2_2_BOT;
        } else {
    ...
    public function text5($options)
    {
        ...
        if ($arbitrage_price === null) {
            $text = self::TEXT5_3_TML;
        } elseif ($arbitrage_price == 0) {
            $text = self::TEXT5_2_TML;
        } elseif ($arbitrage_price == $price) {
            $text = self::TEXT5_0_TML;
        } else {
            $text = self::TEXT5_1_TML;
        }
    ...
    public function text3($options)
    {
        ...
        if ($arbitrage_price === null) {
            $text = sprintf(self::TEXT3_0_TML,
                $this->pricelong($pricePay),
                $ndfl_txt);
        } elseif ($arbitrage_price == $price) {
            $text = sprintf(self::TEXT3_1_TML,
                $this->pricelong($pricePay),
                $ndfl_txt);
        } else {
            $text = sprintf(self::TEXT3_2_TML,
