...
class wizard_billing
{
     ...
     * @return bool
     */
    public function paymentOptions($operations)
    {
     ...
     * @return bool
     */
    public function billingOperation($option, $transaction_id)
    {
        global $DB;
        $ok = false;
        $account = new account();
     ...
     * @return type
     */
    public function updateColorProject()
    {
        global $DB;
     ...
     * @return type
     */
    public function clearBlockedOperations($op_code, $option = false)
    {
        global $DB;
        if ($option) {
     ...
     * @return type
     */
    public function deleteDraftAccountOperation($id)
    {
        global $DB;
        if (is_array($id)) {
            $where = 'id IN (?l)';
     ...
     * @global type $DB
     *
     * @param array|int $operations Ид операций, может быть массив ИД
     * @param int       $uid
     *
     * @return array
     */
     ...
     * @global type $DB
     *
     * @param int|array $id Ид операции или массив ИД операций
     *
     * @return array
     */
     ...
     *
     * @param int $parent = null : родитель платной опции (например id проекта)
     */
    public function clearPayedOptions($parent = null)
    {
        global $DB;
        if ($parent) {
     ...
     * @return type
     */
    public function showProjectsFeedbacks()
    {
        global $DB;
