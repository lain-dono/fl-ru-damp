...
class step_wizard_registration extends step_wizard
{
    ...
    public function registration($type_wizard = self::TYPE_WIZARD_EMP)
    {
        ...
        if ($action == 'registration' && $this->status == 0) {
            ...
            if ($type_wizard == self::TYPE_WIZARD_EMP) {
    ...
    public function completeData($type_role = 1)
    {
        ...
        if ($type_role == self::TYPE_WIZARD_EMP) {
            $user = new employer();
     ...
     * @return array
     */
    public function transferFiles($files, $table, $dir, $newName = true)
    {
