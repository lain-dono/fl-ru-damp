...
class step_freelancer extends step_wizard_registration
{
    ...
    public function actionProjects()
    {
        ...
        if ($project['id'] > 0) {
            ...
            $obj_project->page_size = self::DEF_PROJECTS_PER_PAGE;
     ...
     *
     * @return int Возвращает ИД ответа 
     */
    ...
    {
        return $this->_db->val('SELECT SUM(option) FROM wizard_billing WHERE op_code = ? AND wiz_uid = ?', self::OFFERS_OP_CODE, $this->getWizardUserID());
    }
     ...
     * 
     * @param int|string $uid     Ид пользователя (если int - то это ИД зарегистрированного пользователя, если string - Ид пользователя мастера)
     * @param string     $limit   Лимит выборки обычных ответов
     * @param bool       $not_pro Берем проекты где могут отвечать только ПРО или нет (если true - не берем)
     *
    ...
    public function actionProcessingPortfolio()
    {
        ...
        if (($data['exp'] < 0) || ($data['exp'] > self::MAX_YEAR_VALUE)) {
            $error['exp'] = 'Недопустимое значение. Опыт работы должен быть в пределе от 0 до '.(self::MAX_YEAR_VALUE).'.';
        }
    ...
    public function actionCompletedWizard()
    {
        ...
        foreach ($payed as $pay) {
            if (in_array($pay['op_code'], self::getOperationCodePRO())) {
                $is_pro = true;
                $op_id = $pay['id'];
            } else {
