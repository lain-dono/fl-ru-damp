...
class user_search extends admin_parent
{
     ...
     * @return array
     */
    public function getUserByUid($sUid = 0)
    {
     ...
     * @return array
     */
    public function searchUsers(&$count, $filter, $page = 1)
    {
        $this->filter = $filter;
     ...
     * @return array
     */
    public function getUsers(&$count, $page = 1, $order = 'general', $direction = 0, $unlimited = false)
    {
        $this->aSQL = array();
...
//echo $GLOBALS['DB']->parse( $sQuery );//die;
     ...
     */
    public function _setUsersFrom($bCount = false)
    {
     ...
     */
    public function _getUsersInnerSql($sTable = '', $bCount = false)
    {
        $sSql = '';
     ...
     * @return string внутренний запрос
     */
    public function _getIpInnerSql($sTable = '', $bCount = false)
    {
        $sSql = '';
     ...
     */
    public function _setUsersWhere()
    {
        ...
        if ($this->isFilter('status')) {
            if ($this->filter['status'] == '1') {
                $this->aSQL['where'][] = "u.is_banned = B'1' AND NOT u.self_deleted = TRUE";
            }
            if ($this->filter['status'] == '2') {
                $this->aSQL['where'][] = 'u.active = false';
            }
            if ($this->filter['status'] == '3') {
                $this->aSQL['where'][] = 'u.warn > 0';
            }
            if ($this->filter['status'] == '4') {
                $this->aSQL['where'][] = 'u.self_deleted = TRUE';
            }
            if ($this->filter['status'] == '5') {
                $this->aSQL['where'][] = "u.is_banned = B'0'";
            }
        }
    }
     ...
     * @return string
     */
    public function _getWhereOrFields($aThree = array(), $aZero = array(), $aFields = array(), $sSearch = '', $bExact = false)
    {
     ...
     * @return string
     */
    public function _getIpWhere($fromIp = '', $toIp = '')
    {
        $sReturn = '';
     ...
     * @return string
     */
    public function _getIpQuery($sTable, $fromIp = '', $toIp = '')
    {
        $sReturn = '';
     ...
     * @param bool $unlimited опционально. установить в true если нужно получить все записи (без постраничного вывода)
     */
    public function _setUsersLimit($offset = 0, $unlimited = false)
    {
