...
class manager
{
     ...
     */
    public function addMsg($text, $phone, $email, $fio, $uid, $files = false)
    {
     ...
     * @return bool
     */
    public function OrderCall($uid, $fio, $phone, $time_to_call, $email)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/smail.php';
...
//                "icq" => "553045731",
     ...
     *
     * @param int    $type  Тип заявки (1- отвеченные, 2 - не отвеченные, 3 - оконченные, 4 - неоконченные)
     * @param string $field Поле для сортировки
     * @param string $d     Тип сортировки (ASC, DESC)
     *
     * @return array Данные по выборке
     */
    public function getMsg($type = 0, $field = 'post_date', $d = 'DESC')
    {
        global $DB;
     ...
     */
    public function getTotalTopMenu()
    {
        global $DB;
     ...
     */
    public function getFieldSort($sort)
    {
        switch ($sort) {
