...
class registration
{
    ...
    {
        global $DB;
        if (empty($_SESSION['email']) && $_SESSION['suspect'] == true) {
            return false;
        }
    ...
    public function actionRegistration($is_preset = false)
    {
        ...
        if ($this->is_validate) {
            ...
            if ($this->role == self::ROLE_FREELANCER) {
                ...
                $newuser->role = 0;
            } elseif ($this->role == self::ROLE_EMPLOYER) {
                require_once $_SERVER['DOCUMENT_ROOT'].'/classes/employer.php';
     ...
     * @param users $user
     */
    public function checkGrayIp($user)
    {
        ...
        switch ($name) {
            ...
            case 'password':
                if ($value == '') {
                    ...
                    $this->errno[$name] = 3;
                } elseif (strlen(preg_replace("#[a-zA-Z\d\!\@\#\$\%\^\&\*\(\)\_\+\-\=\;\,\.\/\?\[\]\{\}]#", '', $value)) != 0) {
                    $this->error[$name] = 'Поле заполнено некорректно';
                    $this->errno[$name] = 4;
                }
                break;
    ...
    public function listenerAccess($request)
    {
        ...
        if (is_array($request) && isset($request['action'])) {
            ...
            if (isset($this->access_action_page[$uri])) {
                ...
                if (!is_emp() && isset($this->access_action_page[$uri]['confirm_frl'])) {
                    $confirm = $this->access_action_page[$uri]['confirm_frl'];
                }
    ...
    public function fillData($data = array(), $only_generate = false)
    {
        ...
        if (empty($data) || !isset($data['email']) || !strlen($data['email'])) {
            return false;
        }
            ...
            if (empty($value)) {
                switch ($key) {
                    case 'login':
                        ...
                        if (!strlen($value)) {
                            $value = substr(md5(uniqid(mt_rand(), true)), 12, 8);
                        }
                        break;
