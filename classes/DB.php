...
class DB
{
     ...
     * @var array
     */
    protected static $_stby_log = array();
    ...
    public function __call($name, $args)
    {
        if (in_array($name, array('mquery', 'mrows', 'mrow', 'mcol', 'mval'))) {
            $this->_sqlType = self::SQLTYPE_MOD;
    ...
    public function __destruct()
    {
        ...
        if (!self::$objects) {
            ...
            if (self::$_stby_log) { // можно убрать, отладочное.
     ...
     * @param int    $traceLevel уровень вложенности для debug_backtrace()
     */
    protected function err($error, $sql = '', $traceLevel = false, &$pgcode = -1)
    {
        ...
        if ($pgcode !== -1) {
            if (strpos($this->error, 'read-only transaction') !== false) {
                $pgcode = self::PG_ERR_READ_ONLY;
            } elseif (strpos($this->error, 'conflict with recovery') !== false) {
                $pgcode = self::PG_ERR_RECOVERY_CONFLICT;
            }
            // ...
        }
                ...
                if ($this->keys) {
                    ...
                    foreach ($this->keys as $v) {
                        if (empty($this->res[$v])) {
                            $keys[$v] = $version;
                            $exec = true;
                        } elseif (!$rebuild && $this->res[$v] > $this->res[$hash]['ver']) {
                            $exec = true;
                        }
                    }
                }
            ...
            if ($exec) {
                for ($i = 0, $only_master = false; $i < 2; ++$i) {
                    ...
                    if (!$this->res) {
                        ...
                        if (!$i && ($pgcode == self::PG_ERR_READ_ONLY || $pgcode == self::PG_ERR_RECOVERY_CONFLICT)) {
                            $only_master = true;
                            continue;
                        }
                    }
                    if (isset($this->_stby_log_row)) {
                        $pfx = strpos($this->realAlias, self::STBY_SUFFIX) ? 'standby_' : 'master_';
    ...
    public function row($sql = '')
    {
        ...
        } elseif ($mode > 0 && isset($this->res[0])) {
            $r = &$this->res[0];
        } else {
            $r = &$this->res;
        }
    ...
    public function col($sql = '')
    {
        ...
        } elseif ($mode > 0 && is_array($this->res)) {
            $c = count($this->res);
            for ($i = 0; $i < $c; ++$i) {
     ...
     *
     * @param bool $force коммит будет произведен, даже если существует внешняя транзакция (с более поздней фиксацией)
     *
     ...
     * Откат транзакции.
     *
     * @param bool $force откат будет произведен, даже если существует внешняя транзакция (с более поздней фиксацией)
     *
     * @return resource ресурс выполненного запроса или FALSE в случае неудачи
     */
     ...
     * Только для последующего запроса.
     *
     * @param int    $ttl     Время жизни в секундах результата в memcache
     ...
     * Только для последующего запроса.
     *
     * @param int $ttl Время жизни в секундах результата в memcached
     *
     * @return DB Сам объект для дальнейшего использования в цепочке
     */
    ...
    public function array_to_php($str)
    {
        ...
        $tmp = '';
        $flg = false;
        if (!preg_match("/^\s*?(?:\{|\[|ARRAY\[)\s*(.*)(?:\}|\])\s*$/i", $str, $o)) {
            return false;
        }
        $o[1] = rtrim($o[1]);
        if ($o[1] == '') {
     ...
     * 
     * @return bool TRUE - соединение установлено, FALSE - не установлено
     */
    protected function _connMemcached()
    {
        ...
        if (class_exists('Memcached')) {
            ...
            } elseif (count($GLOBALS['memcachedServers']) > 1) {
    ...
    public function setCheckAutoSlashes($mode = true)
    {
    ...
    protected function _parse($p)
    {
        ...
        switch ($p[0]) {
            ...
            case '?ts':
                if (is_string($value) || is_numeric($value)) {
                    return "'".self::inputto_tsquery($value)."'";
                } else {
                    $this->perr = 'Placeholder '.($this->pcnt - 1).' should be string or numeric';
     ...
     * @return bool
     */
    public function hasAutoSlashes($str)
    {
     ...
     */
    public static function setStandby($alias, $opts = array())
    {
        $old_opts = self::$_standbyOpts[$alias];
        self::$_standbyOpts[$alias] = $opts;
     ...
     *
     * @return bool true, если read-only
     */
    public function checkSqlReadonly()
    {
        return !preg_match(
     ...
     * @return bool можно?
     */
    public function checkStandby($alias = null)
    {
        $alias = $alias ? $alias : $this->alias;
        if (isset(self::$_standbyOpts[$alias])) { // !!! && !$this->_transaction
            $opts = self::$_standbyOpts[$alias];
            $any_mask = (int) $opts[self::STBY_OPTS_ANY_MASK];
            $all_mask = (int) $opts[self::STBY_OPTS_ALL_MASK];
            $capacity = (isset($opts[self::STBY_OPTS_CAPACITY]) ? $opts[self::STBY_OPTS_CAPACITY] : 1);
            $mask = 0
                  | self::STBY_CACHED  * ($this->mode == 1)
                  | self::STBY_NOACT   * (!defined('IS_USER_ACTION') && isset($_SESSION['last_user_action']) + max($opts[self::STBY_OPTS_NOACT_LAG], 0) <= $_SERVER['REQUEST_TIME'])
                  | self::STBY_NOAUTH  * (!isset($_SESSION['uid']))
                  | self::STBY_NOSHELL * (!isset($_SERVER['SHELL']) || isset($_SERVER['REQUEST_URI']))
                  ;

            if ($opts[self::STBY_OPTS_LOGSTAT]) {
                $key = date('Y-m-d').'='.$mask.'='.$any_mask.','.$all_mask.','.$capacity.','.(int) $opts[self::STBY_OPTS_NOACT_LAG];
                self::$_stby_log[$key] = (array) self::$_stby_log[$key];
                $this->_stby_log_row = &self::$_stby_log[$key];
            }

            if ($this->_sqlType != self::SQLTYPE_MOD && !$this->_transaction) { // !!! эти условия потом перетащить в начало функции.
    ...
    public static function londiste($type, $master_alias = 'master')
    {
     ...
     * @return string
     */
    public static function inputto_tsquery($str, $as = '\\s', $os = ',', $ns = '', $pg_escape = true)
    {
