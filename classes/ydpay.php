 ...
 *
	 ...
	 */
	public function prepare($shopid, $billing_no, $ammount, $operation_type, $operation_id)
	{
	    if (!in_array($shopid, $this->shopid)) {
	        $error = 'Неверный магазин!';
	    }
	    if (!$this->is_dep_exists($billing_no)) {
 ...
 *
	 * @return string Сообщение об ошибке
	 */
	public function checkdeposit($shopid, $ammount, $orderIsPaid,
        $orderNumber, $billing_no, $hash, $fromcode, $paymentDateTime, $operation_type, $operation_id)
	{
	    ...
	    if ($operation_type == sbr::OP_RESERVE) { // убрать проверку после тестирования.
            $shopid = self::SHOP_SBR_RESERVE;
	    }

	    switch ($shopid) {
            case self::SHOP_SBR_RESERVE : // Резерв денег по СбР (новая)
                $op_code = sbr::OP_RESERVE;
                $amm = 0;
                $descr .= ' СбР #'.$operation_id;
                break;
            case self::SHOP_DEPOSIT : // Перевод денег на личный счет
                $op_code = 12;
