...
class pay_place
{
     ...
     *
     * @return bool false - если ошибка, true - если пользователь был добавлен
     */
    public function addUser($uid, $adHead = null, $adText = null, $adImg = null)
    {
        global $DB;
        $insert = array(
            'uid' => $uid,
     ...
     */
    public function updatePaidPlace($nId = 0, $aParams = array(), $bDelImg = false)
    {
     ...
     * @return type
     */
    public function updatePaidPlaceWithOutAuth($nId = 0, $aParams = array(), $bDelImg = false)
    {
        global $DB;
     ...
     * @return array
     */
    public function getPaidPlace($nId = 0)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/user_content.php';
     ...
     *
     * @param int $uid  Id Пользователя
     * @param int $type Тип места (см. табл. paid_places)
     *
     * @return bool true - если все прошло удачно, иначе false
     */
    public function addUserTop($uid)
    {
        global $DB;

        return !!$DB->insert('paid_places', array('uid' => $uid, 'type_place' => $this->type_place));
    }
    public function addUserTopNew($ad)
    {
        global $DB;
        $insert = array();
        $insert['uid'] = $ad['uid'];
     ...
     * @global type $DB
     */
    public function getUserPlaceNew()
    {
        self::getDoneShow();
        if (self::isDone()) {
     ...
     * @return bool true - если все прошло удачно, иначе false
     */
    public function getLastQueueUser()
    {
        global $DB;
    ...
    public function getLastQueueUserNew()
    {
        global $DB;
     ...
     *
     * @param int $count Количество выборки
     *
     * @return array Данные пользователей
     */
    public function getUserHistory($count = false)
    {
        if (!$count) {
            $count = $this->history_count;
        }
     ...
     * @return bool
     */
    public function updateShow($tShow)
    {
        global $DB;
     ...
     */
    public function getAllInfo($uids = false)
    {
        if (!$uids) {
            return false;
        }
     ...
     *
     * @param int    $bill_id  Ид Биллинга
     * @param int    $gift_id  Возвращает ид подарка
     * @param int    $tr_id    Ид транзакции
     * @param int    $gid      ИД получателя
     * @param int    $fid      ИД дарителя
     * @param string $comments Комментарий
     * @param int    $tarif    ИД услуги (op_codes)
     *
     * @return bool
     */
    public function gift(&$bill_id, &$gift_id, $tr_id, $gid, $fid, $comments, $tarif = 69)
    {
     ...
     *
     * @param int $bill_id id операции в account_operations
     ...
     * @return bool true - успех, false - провал
     */
    public function setModeration($id = 0, $fake = 0, $moder_id = 0)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/user_content.php';
     ...
     */
    public function checkModeration($id = 0)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/user_content.php';
