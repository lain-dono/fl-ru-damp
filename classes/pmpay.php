...
class pmpay extends account
{
     ...
     * @param $descr
     */
    public static function getPaymentBillNO($descr)
    {
     ...
     * 
     * @return int кол-во обработанных платежей.
     */
    public function checkInvoiced()
    {
        global $DB;
        ...
        $sql = "
          SELECT *
           ...
           WHERE is_canceled = false
             ...
             AND invoiced_time < now() - interval '".self::CHECK_INVOICED_LAG." seconds'
    ...
    public function findRefundById($refunds, $refund_id)
    {
     ...
     *
     * @param int $invoice_id ид. счета (pm_payments.id или LMI_PAYMENT_NO).
     *
     * @return bool true: все ок, можно зачилять деньги.
     */
     ...
     *
     * @param int $invoice_id  ид. счета.
     * @param int $merchant_id ид. магазина (мерчанта).
     *
     * @return array
     */
    public function getPaymentByInvoiceID($invoice_id, $merchant_id)
    {
    ...
    public function getContentSite($url)
    {
     ...
     * @return string Сообщение об ошибке
     */
    public function checkdeposit($req)
    {
    ...
    private function _setDeposit($req)
    {
        ...
        $error = $this->deposit($op_id, $req['PAYMENT_BILL_NO'], $amm, $descr, self::OP_CODE, $ammount, $op_code, $req['OPERATION_ID']);
        if ($op_id) {
