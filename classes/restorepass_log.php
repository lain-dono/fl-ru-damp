...
class restorepass_log
{
     ...
     *
     * @param int    $uid     - ID пользователя для которого меняется пароль
     * @param string $ip      - IP с которого меняется пароль
     * @param int    $service - Через что меняется пароль: 1 - Email, 2 - SMS, 3 - в личном кабинете пользователя; 4 - админка
     */
    public function SaveToLog($uid, $ip, $service, $modified_uid = null)
    {
        $ip = ip2long($ip);
     ...
     * @param int    $is_sms   - Выбирать ли если использовалось SMS для сменя логина, 1 - да
     * @param int    $is_email - Выбирать ли если использовалось E-mail для сменя логина, 1 - да
     *
     * @return array - Информация о сменах паролей в данній период времени
     */
    public function GetLogData($login, $ip, $ds, $de, $is_sms, $is_email)
    {
        if ($login) {
