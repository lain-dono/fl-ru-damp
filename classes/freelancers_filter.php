...
class freelancers_filters
{
     ...
     */
    public function __construct($uid = false, $fid = false)
    {
        if (!$uid) {
     ...
     */
    public function addFilter($exp, $age, $login, $country, $city, $in_office, $in_fav, $only_free, $is_pro, /*$only_online,*/ $sbr_is_positive, $is_preview, $sbr_not_negative, $success_sbr, $kword, $sex, $opi_is_positive, $opi_not_negative, $is_verify, $only_tu)
    {
     ...
     * @return string Сообщение об ошибке
     */
    public function updateFilter($exp, $age, $login, $country, $city, $in_office, $in_fav, $only_free, $is_pro, /*$only_online,*/ $sbr_is_positive, $is_preview, $sbr_not_negative, $success_sbr, $kword, $sex, $opi_is_positive, $opi_not_negative, $is_verify, $only_tu)
    {
        $exp_from = intval($exp[0]) < 0 ? 0 : (int) $exp[0];    
        $exp_to = intval($exp[1]) < 0 ? 0 : (int) $exp[1];  
        $age_from = intval($age[0]) < 0 ? 0 : (int) $age[0];
     ...
     */
    public function addCost($from_cost, $to_cost, $cost_type, $curr_type)
    {
        if (get_uid(false)) {
            if (count($from_cost) > 0 && $this->fid > 0) {
                ...
                for ($i = 0;$i < count($from_cost);++$i) {
                    ...
                    if ($from_cost[$i] != 0 || $to_cost[$i] != 0) {
                        $sql .= "INSERT INTO freelancers_filters_cost (filter_id, cost_from, cost_to, cost_type, type_date) VALUES('{$this->fid}', '{$from_cost[$i]}', '{$to_cost[$i]}', '{$curr_type[$i]}', '{$cost_type[$i]}'); ";
                    }
                }
     ...
     * @return string Сообщение об ошибке
     */
    public function addGroup($category)
    {
     ...
     */
    public function isActive($act = true)
    {
        if (get_uid(false)) {
            if ($this->fid > 0) {
                global $DB;
     ...
     */
    public function getRankCount($prof_id = 0)
    {
        $memBuff = new memBuff();
        if ($prof_id) {
     ...
     * @return string
    **/
    public static function getMirroredSpecsJsObject($all_mirrored_specs)
    {
        $result = array();
     ...
     * */
    public static function mirrorExistsInArray($id, $set, $mirrored_specs)
    {
        if (in_array($mirrored_specs[$id], $set)) {
            return true;
        }
        foreach ($set as $i) {
