...
class sbr_emp extends sbr
{
    ...
    public function checkProject($project_id)
    {
        ...
        if ($this->project = new_projects::getPrj($project_id)) {
            ...
            } elseif ($this->project['no_risk'] == 't') { // !!! триггер сделать + проверить работает ли флаг в старых СБР
                $this->error['project_id'] = 'Данный проект уже участвует в другой «Безопасной Сделке»';
            } // !!! тут, может, просто не привязывать к проекту...
     ...
     * @return type
     */
    public function checkTService($service_id, $user_id = 0)
    {
        global $DB;
     ...
     * @return type
     */
    public function tserviceHash($service_id)
    {
     ...
     * @return bool
     */
    public function addRefTService($stage_id)
    {
     ...
     * @return bool
     */
    public function initFromTService($service_id, $request)
    {
     ...
     */
    public function initFromRequest($request, $files = null, $old = null)
    {
        ...
        if ($old) {
            if (!$request['cost_sys']) {
                $request['cost_sys'] = array($old->cost_sys);
            }
     ...
     */
    public function getStageByIdForData($id)
    {
...
//                    $attached[$att_id]['id'] = md5($attached[$att_id]['id']);
     ...
     * @return bool успешно?
     */
    public function _new_edit($request, attachedfiles $files)
    {
     ...
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function _new_getDrafts($limit = false)
    {
     ...
     * @param int $excludeID ID черновика который не должен попасть в выдачу
     *
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function getDraftsList($limit = false, $excludeID = false)
    {
        global $DB;
     ...
     * @return bool успешно?
     */
    public function resendCanceled($sbr_id)
    {
    ...
    {
        foreach ($ids as $id) {
            $csbr = new self($this->uid, $this->login, $this->session_uid);
            $csbr->initFromId($id, true, false);
            if (!$csbr->checkSendReady()) {
                return false;
            }
        }
     ...
     * 
     * @param int|array $ids      ид. сделок.
     * @param bool      $is_draft true: в черновик, false:отправить на утверждение.
     *
     * @return bool успешно?
     */
     ...
     */
    public function showInvoiced($form_type, $account)
    {
    ...
    public function setCostSys($cost_sys)
    {
        global $DB;
     ...
     * @return bool
     */
    public function isReserveProcess()
    {
