...
class letters
{
     ...
     * @param int     $limit  Количество документов, которые нужно получить (0 - все)
     * @param ingeter $offset Смещение от начала списка
     *
     * @return array Список документов
     */
    public function getLetters($type = 0, $filter = null, $limit = '', $offset = '')
    {
        global $DB;
     ...
     */
    public function processDocs($ids)
    {
        global $DB;
     ...
     */
    public function processSendDocs($d_ids, $u_ids, $delivery_ids)
    {
        global $DB;

        $filtes['ids'] = $d_ids;
        $docs_list = self::getLetters(6, $filter);
     ...
     */
    public function calcDeliveryCost($ids)
    {
        global $DB;
        $cost = 0;
        if ($ids) {
     ...
     */
    public function updateMassStatus($ids_docs, $ids_statuses)
    {
        global $DB;
     ...
     *
     * @param int  $id         ID документа
     * @param bool $is_checked статус свойства
     */
    public function changeWithoutourdocs($id, $is_checked)
    {
        global $DB;
     ...
     */
    public function updateMassDeliveryCost($ids_docs, $cost)
    {
        global $DB;
     ...
     */
    public function updateMassDate($ids_docs, $date)
    {
        global $DB;
     ...
     */
    public function getDocumentsStatuses($ids)
    {
        global $DB;
        $statuses = array();
        if ($ids) {
            ...
            if ($qstatuses) {
                ...
                foreach ($qstatuses as $status) {
                    if ($status['user_1']) {
                        $statuses[intval($status['user_status_1'])] = $tstatuses[intval($status['user_status_1'])];
                    }
                    if ($status['user_2']) {
                        $statuses[intval($status['user_status_2'])] = $tstatuses[intval($status['user_status_2'])];
                    }
                    if ($status['user_3']) {
                        $statuses[intval($status['user_status_3'])] = $tstatuses[intval($status['user_status_3'])];
                    }
                }
            }
        }
     ...
     * @param     int  Сколько груп получить
     *
     * @return array Данные о группах
     */
    public function getGroups($word = null, $limit = 'ALL')
    {
        global $DB;
     ...
     * @param     int  Сколько документов получить
     *
     * @return array Данные о документах
     */
    public function getDocuments($word = null, $limit = 'ALL')
    {
        global $DB;
     ...
     * @param     int  Сколько компаний получить
     *
     * @return array Данные о компаниях
     */
    public function getCompanies($word = null, $limit = 'ALL')
    {
        global $DB;
     ...
     * @return array Данные о документах
     */
    public function getSearchDocuments($word = null, $limit = 'ALL')
    {
        global $DB;
     ...
     */
    public function checkCreateGroup($title)
    {
        global $DB;
    ...
    public function addDocument($data)
    {
        ...
        if (!$data['letters_doc_frm_delivery_cost']) {
            $data['letters_doc_frm_delivery_cost'] = null;
        }
        if (!$data['letters_doc_frm_comment']) {
            $data['letters_doc_frm_comment'] = null;
        }
        ...
        if (!$data['letters_doc_frm_group_db_id'] || $data['letters_doc_frm_group_db_id'] == 'null') {
            if ($data['letters_doc_frm_group'] && $data['letters_doc_frm_group'] != '' && $data['letters_doc_frm_group'] != 'null') {
                $data['letters_doc_frm_group_db_id'] = self::checkCreateGroup($data['letters_doc_frm_group']);
            }
        }
     ...
     * @param int $id Шаблон
     */
    public function delTemplateDocs($id)
    {
        global $DB;
        $sql = 'DELETE FROM letters_template_doc WHERE template_id=?i';
        $DB->query($sql, $id);
     ...
     *
     * @param int   $tpl  ID шаблона
     * @param array $data Данные шаблона
     *
     * @return int ID документа
     */
    public function addTemplateDoc($data)
    {
        global $DB;
     ...
     */
    public function addTemplate($title)
    {
        global $DB;
    ...
    public function updateDocument($id, $data)
    {
        ...
        if ($doc_data['user_status_1'] != $doc['user_status_1'] || $doc_data['user_status_2'] != $doc['user_status_2'] || $doc_data['user_status_3'] != $doc['user_status_3']) {
            self::updateDateStatusChange($id);
        }

        self::saveHistory($id, $doc_data);
     ...
     * @param int $id ID документа
     */
    public function updateDateStatusChange($id)
    {
        global $DB;
     ...
     */
    public function saveHistory($id, $new_data)
    {
        global $DB;
        $old_data = self::getDocument($id);
        $changed = array();
        if ($new_data) {
            ...
            foreach ($changed as $change_type) {
                switch ($change_type) {
                    ...
                    case 'parent':
                        $type_field = 11;
                        if ($old_data['parent']) {
                            $old_doc = self::getDocument($old_data['parent']);
                            $old_val = "ID{$old_doc['id']} {$old_doc['title']}";
                        } else {
                            $old_val = 'Нет';
                        }
                        if ($new_data['parent']) {
                            $new_doc = self::getDocument($new_data['parent']);
     ...
     */
    public function getCompaniesBySym($s)
    {
        global $DB;
     ...
     */
    public function addCompany($frm)
    {
        global $DB;
     ...
     * @param array $frm Данные о компании
     */
    public function updateCompany($frm)
    {
        global $DB;
     ...
     */
    public function getUserReqvs($uid)
    {
        ...
        if ($user && $user['form_type'] == 2 && trim($user[2]['address']) == '') {
            ...
            } elseif (trim($user[2]['address_jry']) != '') {
