...
//var_dump($this->_ratingData);
        ...
        for ($i = 0; $i < $this->_columns; ++$i) {
            ...
            if ($i == 0) { // начало
                ...
                $_rating = 0;
                if (intval($max)) {
                    $_rating = ((!$this->_ratingData[$i] ? $this->_data[0]['rating'] : $this->_ratingData[$i]['rating']) + $h_diff) * $this->_graphSize[1] / $max * $this->_zoom;
                }

                $y = $this->_graphSize[1] - $_rating;

                if (!$this->_ratingData[$i]) {
                    $y = $this->_graphSize[1] - ($h_diff * $this->_graphSize[1] / $max * $this->_zoom);
                }
//                var_dump($y, $h_diff);
            ...
            if ($this->_ratingData[$i]) {
                $_rating = 0;
                if (intval($max)) {
                    $_rating = ((!$this->_ratingData[$i] ? $last : $this->_ratingData[$i]['rating']) + $h_diff) * $this->_graphSize[1] / $max * $this->_zoom;
                }
...
//            }
    ...
    private function _setPro()
    {
        ...
        foreach ($this->_pro as $p => $pro) {
            if (date('Ym', strtotime($pro['from_time'])) > date('Ym', $this->_time)) {
                continue;
            }
    ...
    private function _setProDays()
    {
        ...
        foreach ($this->_pro as $p => $pro) {
            ...
            if (isset($this->_ratingData[$d]['tmp'])) {
                for ($i = $d; $i <= $diff + $d; ++$i) {
                    if (!isset($this->_ratingData[$i]['tmp'])) {
                        break;
                    }
                    $this->_ratingData[$i]['rating'] *= rating::PRO_FACTOR;
                }
