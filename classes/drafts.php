 ...
 *
	 ...
	 */
	public function SaveCommune($msg)
	{
	    global $DB;
	    $msg['draft_id'] = intval($msg['draft_id']);
	    $msg['id'] = intval($msg['id']);
	    if (!intval($msg['draft_post_id'])) {
	        $msg['draft_post_id'] = NULL;
	    }
	    $msg['category_id'] = intval($msg['category_id']);
	    if (!$msg['category_id']) {
	        $msg['category_id'] = NULL;
	    }
	    if ($msg['title'] === false) {
	        $msg['title'] = '';
	    }
	    if ($msg['msgtext'] === false) {
	        $msg['msgtext'] = '';
	    }
	    if ($msg['youtube_link'] === false) {
	        $msg['youtube_link'] = '';
	    }
	    if ($msg['question'] === false) {
	        $msg['question'] = '';
	    }
	    ...
	    if ($msg['close_comments'] == 1) {
	        $msg['close_comments'] = 't';
	    } else {
	        $msg['close_comments'] = 'f';
	    }
	    if ($msg['is_private'] == 1) {
	        $msg['is_private'] = 't';
	    } else {
        ...
        $_SESSION['drafts_count'] = self::getCount($msg['uid']);
 ...
 *
     * @return array ['id'] - ID черновика, ['date'] - дата сохранения черновика
	 */
	public function SaveContacts($msg)
	{
	    global $DB;
 ...
 *
     * @return array ['id'] - ID черновика, ['date'] - дата сохранения черновика
	 */
	public function SaveProject($prj, $attachedfiles_files = false)
	{
	    global $DB;
 ...
 *
     * @return array ['id'] - ID черновика, ['date'] - дата сохранения черновика
	 */
	public function SaveProjectNew($prj, $attachedfiles_files = false)
	{
	    global $DB;
	    require_once $_SERVER['DOCUMENT_ROOT'].'/classes/attachedfiles.php';
     ...
     */
    public function isDraftExists($id, $uid, $type, $prj_id = 0)
    {
        global $DB;
        switch ($type) {
            case 1:
     ...
     */
    public function getAttachedFiles($draft_id, $draft_type, $uid = null)
    {
        global $DB;
        switch ($draft_type) {
            case 1:
        ...
        if ($check) {
            ...
            if ($attaches) {
                ...
                foreach ($attaches as $attach) {
                    array_push($fAttaches, $attach['file_id']);
                }
            }
        }
     ...
     * @param integer $type Тип черновика
     */
    public function DeleteDraft($id, $uid, $type, $delete_files = false)
    {
        global $DB;
        switch ($type) {
            case 1:
        ...
        $_SESSION['drafts_count'] = self::getCount($uid);
    }
     ...
     */
    public function DeleteDraftByPrjID($prj_id)
    {
        global $DB;
     ...
     */
    public function CheckContacts($to_login, $uid)
    {
        global $DB;
     ...
     * @return integer Кол-во черновиков
     */
    public function CheckBlogs($uid)
    {
        global $DB;
     ...
     * @return integer Кол-во черновиков
     */
    public function CheckCommune($uid)
    {
        global $DB;
     ...
     */
    public function getUserDrafts($uid, $type, $limit = null)
    {
        global $DB;
        switch ($type) {
            case 1:
    ...
    */
    public function getDraft($draft_id, $uid, $type)
    {
        global $DB;
        switch ($type) {
            case 1:
