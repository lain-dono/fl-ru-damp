...
class messages
{
     ...
     * @param bool   $recipients  массив с логинами получаетелей
     ...
     *
     * @param int    $user_id        uid пользователя заказавшего рассылку
     * @param int    $masssending_id id рассылки
     ...
     */
    public function Masssending($user_id, $masssending_id, $text, $posted_time, $skip_mail = false)
    {
     ...
     * @return array
     */
    public function GetZeroMessageUsers($user_id, $message_id, $limit = 'ALL', $offset = 0, $only_subscr = true)
    {
        $DB = new DB();
     ...
     * @param int $message_id ID сообщения
     */
    public function Get($user_id, $message_id)
    {
        $DB = new DB();
     ...
     */
    public function GetMessages($to_id, $from_login, &$num_msgs_from, $offset = 1, $limit = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     * @return bool true - успех, false - провал
     */
    public function readDialog($to_id = 0, $from_id = 0)
    {
        $DB = new DB();
     ...
     * @param int    $uid2  uid (или массив с uid'ами) с кем нужна история
     * @param int    $limit количество сообщений, которые нужно получить (с конца)
     *                      только целое число. 'ALL' использовать нельзя
     * @param int    $maxid получить только сообщения с id < $maxid (0 = все)
     *                      (актуально только если $uid2 скаляр)
     *
     * @return array массив с сообщениями
     */
    public function GetHistory($uid1, $uid2, $limit = 4, $maxid = 0)
    {
     ...
     * @return bool true - успех, false - провал
     */
    public function deleteMessage($msg_id = 0, $del_uid = 0)
    {
        $DB = new DB();
     ...
     */
    public function GetContactsWithNote($uid, $search = '')
    {
        $DB = new DB();
     ...
     */
    public function GetUsersInFolders($uid)
    {
        $res = array();
     ...
     *
     * @param int $to_id   id пользователя-владельца контакта (со стороны которого рассматриваем контакт)
     * @param int $from_id id пользователя-контакта
     *
     * @return array массив папок
     */
    public function GetContactFolders($uid1, $uid2, &$error)
    {
        $DB = new DB();
        $out = array();
     ...
     * @return string текст ошибки, в случае неуспеха
     */
    public function RestoreFromUsers($from_id, $to_id)
    {
        $DB = new DB();
     ...
     * @param bool $nocache не читать из кеша, всегда делать запрос к бд
     *
     * @return int количество новых личных сообщений или NULL в случае ошибки
     */
     ...
     * @return array массив сообщений
     */
    public function getMessagesAllSinceDate($uid = 0, $time = '')
    {
        $DB = new DB();
     ...
     *
     * @param int  $uid  ид. пользователя.
     * @param bool $read нужно ли омечать прочитанными эти сообщения
     *
     * @return array массив сообщений.
     */
    public function ChatNewMessages($uid, $read = false)
    {
        $DB = new DB();
     ...
     */
    public function getMessageAuthorByUid($uid)
    {
        global $DB;
...
Надеемся на понимание, Команда Free-lance.ru.';
     ...
     * @return @see messages::Add()
     */
    public function SendMsgDelWarn($msgid, $w_msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/blogs.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
...
Надеемся на понимание, Команда Free-lance.ru.';
     ...
     * @return @see messages::Add()
     */
    public function sendCommentDeleteWarn($w_msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
Надеемся на понимание, Команда Free-lance.ru.';
     ...
     * @param type $w_msg
     */
    public function sendCommuneCommentDeleteWarn($w_msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
Команда Free-lance.ru.';
     ...
     * @return @see messages::Add()
     */
    public function SendProjectWarn($login, $prjid = 0)
    {
...
Надеемся на понимание, Команда Free-lance.ru.';
     ...
     */
    public function SendBlockedCommuneTheme($topic = array(), $reason = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/commune.php';
...
Команда Free-lance.ru.';
            ...
            self::Add(users::GetUid($err, 'admin'), $topic['user_login'], $message, '', 1);
        }
    }
     ...
     */
    public function SendDeletedCommuneTheme($topic = array(), $deleter = 'admin')
    {
        if ($topic) {
...
';
     ...
     * @return mix
     */
    public function SendBlockedTServices($service = array(), $reason)
    {
        if (!count($service)) {
            return false;
        }
...
Желаем выгодных заказов! Команда Free-lance.ru.';
        ...
        return self::Add(users::GetUid($err, 'admin'), $service['login'], $message, '', 1);
    }
     ...
     * @return @see messages::Add()
     */
    public function SendBlockedProject($project_id = 0, $reason)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $pr = new projects();
...
С уважением, команда FL.ru";
     ...
     * @return @see messages::Add()
     */
    public function SendUnBlockedProject($project_id = 0)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $pr = new projects();
     ...
     * @return @see messages::Add()
     */
    public function SendBlockedProjectOffer($offer_id = 0, $user_id = 0, $project_id = 0, $reason = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
Надеемся на понимание, Команда Free-lance.ru.";
     ...
     * @return @see messages::Add()
     */
    public function SendUserWarn($uid = 0, $reason, $link)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $f_user = new users();
...
С уважением, команда FL.ru";
     ...
     *
     * @param int    $uid          кому
     * @param int    $project_url  ИД проекта
     * @param int    $project_name Название проекта
...
С уважением, команда FL.ru";
     ...
     * @return bool true - успех, false - провал
     */
    public function messageDeletedNotification($from_uid = 0, $to_uid = 0, $msg = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
';
     ...
     * @return bool true - успех, false - провал
     */
    public function messageModifiedNotification($from_uid = 0, $to_uid = 0, $msg = '', $reason = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
';
     ...
     * @return bool true - успех, false - провал
     */
    public function profileModifiedNotification($from_uid = 0, $ucolumn = '', $utable = '', $reason = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
';
     ...
     * @return @see messages::Add()
     */
    public function portfolioModifiedNotification($portfolio = 0, $f_user = null, $reason = '')
    {
        $sFeedback = str_replace('//', '/{службу поддержки}/', $GLOBALS['host'].'/about/feedback/');
        $sName = $portfolio['name'] ? ' &laquo;'.$portfolio['name'].'&raquo;' : '';
        $link = getAbsUrl('/users/'.$f_user->login.'/viewproj.php?prjid='.$portfolio['id']);
...
Надеемся на понимание, Команда Free-lance.ru";
     ...
     * @return @see messages::Add()
     */
    public function blogModifiedNotification($rec_type = 0, $title = '', $post_time = '', $uname = '', $usurname = '', $login = '', $reason = '')
    {
        $sFeedback = str_replace('//', '/{службу поддержки}/', $GLOBALS['host'].'/about/feedback/');
...
Надеемся на понимание, Команда Free-lance.ru.";
     ...
     * @return @see messages::Add()
     */
    public function communityModifiedNotification($rec_id = 0, $rec_type = 0, $login = '', $uname = '', $usurname = '', $reason = '', $post_id = 0)
    {
        if ($rec_type == 1) {
...
Команда Free-lance.ru.";
     ...
     */
    public function communityPostModifiedNotification($comm, $post, $editor = 'site-moder')
    {
...
Команда Free-lance.ru.";
        ...
        self::Add(users::GetUid($err, 'admin'), $post['user_login'], $message, '', 1);
    }
     ...
     * @return @see messages::Add()
     */
    public function artComModifiedNotification($rec_id = 0, $login = '', $uname = '', $usurname = '', $reason = '', $art_id = 0)
    {
...
Надеемся на понимание, Команда Free-lance.ru.";
     ...
     * @return @see messages::Add()
     */
    public function projectsModifiedNotification($rec_id = 0, $rec_type = 0, $login = '', $uname = '', $usurname = '', $reason = '')
    {
        $sFeedback = str_replace('//', '/{службу поддержки}/', $GLOBALS['host'].'/about/feedback/');
...
Надеемся на понимание, Команда Free-lance.ru.";
     ...
     * @return bool true - успех, false - провал
     */
    public function prjOfferModifiedNotification($from_uid = 0, $project_id = 0, $reason = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';
...
';
     ...
     * @return @see messages::Add()
     */
    public function contestOfferModifiedNotification($rec_id = 0, $prj_id = 0, $login = '', $uname = '', $usurname = '', $reason = '')
    {
        $sFeedback = str_replace('//', '/{службу поддержки}/', $GLOBALS['host'].'/about/feedback/');
...
';
     ...
     * @return bool true - успех, false - провал
     */
    public function contestComModifiedNotification($pid = 0, $oid = 0, $cid = 0, $login = '', $uname = '', $usurname = '', $reason = '')
    {
...
';
     ...
     * @return bool true - успех, false - провал
     */
    public function sdelauModifiedNotification($msg = '', $login = '', $uname = '', $usurname = '', $reason = '')
    {
        $sRason = $reason ? "\n\nПричина: ".$reason : '';
        $sFeedback = str_replace('//', '/{службу поддержки}/', $GLOBALS['host'].'/about/feedback/');
        $message = 'Здравствуйте, '.$uname.' '.$usurname.'
...
';
     ...
     * @return bool true - успех, false - провал
     */
    public function carouselModifiedNotification($login = '', $uname = '', $usurname = '', $reason = '')
    {
        $sRason = $reason ? "\n\nПричина: ".$reason : '';
        $sFeedback = str_replace('//', '/{службу поддержки}/', $GLOBALS['host'].'/about/feedback/');
        $message = 'Здравствуйте, '.$uname.' '.$usurname.'
...
';
     ...
     */
    public function offerDeletedNotification($offer = array())
    {
        if ($offer) {
...
';
            ...
            self::Add(users::GetUid($err, 'admin'), $offer['login'], $message, '', 1);
        }
    }
     ...
     */
    public function dialogueMessageDeletedNotification($dialogue = array())
    {
        if ($dialogue) {
...
';
            ...
            self::Add(users::GetUid($err, 'admin'), $dialogue['login'], $message, '', 1);
        }
    }

    public function portfolioDeletedNotification($name = '', $surname = '', $login = '')
    {
        if ($login) {
...
Команда Free-lance.ru';
     ...
     * @param string $message Текст сообщения
     */
    public function AdminMsgFromTempl($message)
    {
        $tpl = "
Здравствуйте.
...
www.free-lance.ru
     ...
     * @return array
     */
    public function pmAutoFoldersGetYears($sUid = '')
    {
        $DBproxy = new DB();
     ...
     * @return array
     */
    public function pmAutoFolders($sUid = '', $sYear = '', $nLimit = 0, $nOffset = 0)
    {
        $DBproxy = new DB();
     ...
     * @return int
     */
    public function pmAutoFoldersCount($sUid = '', $sYear = '')
    {
        $DBproxy = new DB();
     ...
     * @return array
     */
    public function pmAutoFolderGetById($sUid = '', $sFolderId = '')
    {
        $DBproxy = new DB();
     ...
     */
    public function pmAutoFolderRename($sUid, $sFolderId, $sName = '')
    {
        $sError = '';
     ...
     */
    public function pmAutoFolderDelete($sUid, $sFolderId)
    {
        $DBproxy = new DB();
    ...
    public function updateSendLog($uid)
    {
        global $DB;
        $sql = 'SELECT EXTRACT(EPOCH FROM date) as date, count FROM messages_sendlog WHERE uid=?i';
        $log = $DB->row($sql, $uid);
