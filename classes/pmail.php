...
class pmail extends SMTP
{
     ...
     * 
     * @param int    $msgid      id личного сообщения
     * @param int    $spamid     id рассылки или NULL, если рассылку нужно создать
     ...
     * @param bool   $useVars    использование переменных
     *
     * @return int id сообщения, 0 - ошибка
     */
...
";
     ...
     */
    public function NewMessage($from_uid, $to_uid, $msg)
    {
...
", array('header' => 'default', 'footer' => 'default'), array('login' => $to->login));
     ...
     * @return int количество отправленных уведомлений.
     */
    public function NewTserviceMessage($from_uid, $to_uid, $order, $msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
", array('header' => 'default', 'footer' => 'default'), array('login' => $to->login));
     ...
     * @return int количество отправленных уведомлений.
     */
    public function CommuneNewComment($message_ids, $connect = null)
    {
...
<a href=\"{$userlink}\">{$comment['uname']}</a> <a href=\"{$userlink}\">{$comment['usurname']}</a> [<a href=\"{$userlink}\">{$comment['login']}</a>] оставил(а) коментарий в ".($comment['parent_id'] == $comment['top_id'] ? 'топике' : 'ветке топика')." {$link_topic} сообщества {$link_commune} на которое вы подписаны. $p_body";
     ...
     * @return int количество отправленных уведомлений.
     */
    public function CommuneUpdateComment($message_ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/commune.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
        $commune = new commune();
...
--------';
     ...
     */
    public function CommuneUpdateTopic($subscr, $msg_id)
    {
        $subscribers = array();
...
Модератор сообщества отредактировал ".($info['parent_id'] ? 'комментарий к Вашему посту' : 'Ваш пост')." {$link_topic} в сообществе {$link_commune}.";
     ...
     * @return int количество отправленных уведомлений.
     */
    public function CommuneDeleteComment($message_ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/commune.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
        $commune = new commune();
...
--------';
     ...
     * @param $msg_id - идентификатор темы сообщества или комментария сообщества
     */
    public function CommuneDeleteTopic($subscr, $msg_id)
    {
        $subscribers = array();
        $info = commune::getMessageInfoByMsgID($msg_id);
        $link_commune = "<a href='{$GLOBALS['host']}/commune/?id={$info['commune_id']}' target='_blank'>{$info['commune_name']}</a>";
...
Модератор сообщества удалил комметарий к Вашему посту {$link_topic} в сообществе {$link_commune}.";
     ...
     * @return int количество отправленных уведомлений
     */
    public function UserRazban($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     * @return int количество отправленных уведомлений
     */
    public function DepositMail($operation_ids, $connect = null)
    {
        return; //##0027187
        global $host;
...
По всем возникающим вопросам обращайтесь в нашу <a href="https://feedback.fl.ru/'.$this->_addUrlParams('b', '?').'">службу поддержки</a>.';
     ...
     * @return int количество отправленных уведомлений
     */
    public function AdminLogCommentsMail($message_ids, $connect = null)
    {
...
", array('header' => 'simple', 'footer' => 'simple'));
     ...
     * @return int количество отправленных уведомлений
     */
    public function AdminLogNotice($log_ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/admin_log.php';
...
<br />
     ...
     * @return int количество отправленных уведомлений.
     */
    public function BlogNewComment($message_ids, $connect = null)
    {
...
', array('header' => 'subscribe', 'footer' => 'subscribe'), array('type' => 1, 'title' => $link_title));
     ...
     * @return int количество отправленных уведомлений.
     */
    public function BlogUpdateComment($message_ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/blogs.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
        $blogs = new blogs();
...
.$this->ToHtml(input_ref(LenghtFormatEx($comment['msgtext'], 300), 1)).'
 ...
 ';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ContestChangeDates($ids, $connect = null)
    {
     ...
     * @return int количество отправленных уведомлений.
     */
    public function NewPrjOffer($ids, $connect = null)
    {
     ...
     * @return int количество отправленных уведомлений.
     */
    public function NewPrjMessageOnOffer($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects_offers.php';
        $offers = new projects_offers();
     ...
     *
     * @param int $from_id   ID пользователя кто добавляет
     * @param int $target_id ID пользователя кого добавляют
     *
     * @return int количество отправленных уведомлений
     */
    public function addTeamPeople($from_id, $target_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     *
     * @param int $from_id   ID пользователя кто удаляет
     * @param int $target_id ID пользователя кого удаляют
     *
     * @return int количество отправленных уведомлений.
     */
    public function delTeamPeople($from_id, $target_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ContestNewComment($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/contest.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function NewOpinion($ids, $connect = null)
    {
        ...
        foreach ($opinions as $opinion) {
            ...
            if (substr($opinion['f_role'], 0, 1) == '1') {
                $path = '/users/'.$opinion['t_login'].'/opinions/';
            } else {
                $path = '/users/'.$opinion['t_login'].'/opinions/?from=frl';
            }
...
Вы можете ознакомиться с <a href='{$GLOBALS['host']}{$path}{$this->_addUrlParams('b', '&')}'>новым отзывом</a> на странице вашего аккаунта.";
     ...
     * @return int количество отправленных уведомлений.
     */
    public function EditOpinion($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/opinions.php';
...
Вы можете прочитать его на странице вашего аккаунта - <a href='{$GLOBALS['host']}{$path}{$this->_addUrlParams('b', '&')}'>".$GLOBALS['host'].$path.'</a>';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ProjectsOfferRefused($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects_offers.php';

        $offers = new projects_offers();
        if (!($data = $offers->getRefusedProjectOffers($ids, $connect))) {
            return;
        }

        foreach ($data as $offer) {
            if (substr($offer['subscr'], 4, 1) != '1' || $offer['is_banned'] == '1') {
                continue;
            } // если не нужны уведомления пропускаем отсылку
     ...
     */
    public function ExecutorCandidateBanMail($ids, $connect = null)
    {
        if (empty($ids)) {
            return false;
        }
     ...
     */
    public function _ExecutorCandidateBanMail($nCnf = 0, $aRecipient = array())
    {
        if (!$nCnf || !$aRecipient) {
            return false;
        }
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ProjectsOfferSelected($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects_offers.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ProjectsExecSelected($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';

        $projects = new projects();
        if (!($data = $projects->GetExecProjects($ids, $connect))) {
            return;
        }
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ContestUserBlocked($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/contest.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ContestUserUnblocked($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/contest.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ContestAddCandidate($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/contest.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ContestWinners($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/contest.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ProjectPosted($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';

        if (!($data = projects::getProjects4Sending($ids, $connect))) {
            return;
        }
     ...
     * @return int количество отправленных уведомлений.
     */
    public function DeleteOpinion($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        ...
        foreach ($ids as $id) {
            ...
            if (substr($subscr, 3, 1) != '1' || $t_user->is_banned == '1') {
                continue;
            } // если не нужны уведомления пропускаем отсылку

            if (substr($role, 0, 1) == '1') {
                $path = '/users/'.$to_user['login'].'/opinions/';
            } else {
                $path = '/users/'.$to_user['login'].'/opinions/?from=frl';
            }
     ...
     * @return int количество отправленных уведомлений.
     */
    public function RestoreOpinion($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     * @return int количество отправленных уведомлений.
     */
    public function ArticleNewComment($ids, $connect = null)
    {
     ...
     * @return int количество отправленных уведомлений.
     */
    public function SbrNewEvents($xids, $connect = null)
    {
     ...
     */
    public function SbrOpened($events)
    {
        $ev0 = $events[0];
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrReserved($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrAgreed($events)
    {
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrRefused($events)
    {
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrChangesAgreed($events)
    {
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrChangesRefused($events)
    {
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrArbResolved($events)
    {
        $ev0 = $events[0];
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrTzChanged($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrStatusChanged($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrCanceled($events)
    {
        $ev0 = $events[0];
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrArbCanceled($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrCompleted($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
     ...
     */
    public function SbrStageCompleted($events)
    {
        $ev0 = $events[0];
        ...
        if (($stage->sbr->scheme_type == sbr::SCHEME_PDRD || $stage->sbr->scheme_type == sbr::SCHEME_PDRD2) && $ev0['own_role'] == sbr::EVROLE_FRL && $stage->status == sbr_stages::STATUS_COMPLETED) {
            ...
            $this->recipient = $ev0[$r.'uname'].' '.$ev0[$r.'usurname'].' ['.$ev0[$r.'login'].'] <'.$ev0[$r.'email'].'>';
            $this->SmtpMail('text/html');
        } elseif (($stage->sbr->scheme_type == sbr::SCHEME_PDRD || $stage->sbr->scheme_type == sbr::SCHEME_PDRD2) && $ev0['own_role'] == sbr::EVROLE_FRL && $stage->status == sbr_stages::STATUS_ARBITRAGED) {
            $sbr->getDocs();
            $r = 'f_';
            $this->subject = "Необходимо прислать подписанные документы (проект «{$ev0['sbr_name']}»)";
            foreach ($stage->sbr->docs as $hdoc) {
     ...
     * @param type $events
     */
    public function SbrDocReceived($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        $r = 'f_';
     ...
     * @param type $events
     */
    public function SbrOvertime($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        $r = 'f_';
     ...
     * @param array $events
     */
    public function SbrPauseReset($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/sbr.php';
     ...
     * @param array $events
     */
    public function SbrPauseOver($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/sbr.php';
     ...
     * @param type $events
     */
    public function SbrMoneyPaidFrl($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        $r = 'f_';
     ...
     * @param type $events
     */
    public function SbrMoneyPaidEmp($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        $r = 'f_';
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrFeedback($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrAddDoc($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        $sbr_name = sbr_meta::getNameForMail($ev0, 'sbr');
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrDelDoc($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        if (!($doc = sbr_meta::getDoc($ev0['old_val'], false))) {
            return 0;
        }
        $doc_link = " «{$doc['name']}»";
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrDocStatusChanged($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        if (!($doc = sbr_meta::getDoc($ev0['own_id'], false))) {
            return 0;
        }
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrDocAccessChanged($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        if (!($doc = sbr_meta::getDoc($ev0['own_id'], false))) {
     ...
     * @param array $events информация по событиям (если событий нескольлко, то содержит несколько элементов).
     */
    public function SbrDocReload($events)
    {
        $ev0 = $events[0];
        $url = $GLOBALS['host'].'/'.sbr::NEW_TEMPLATE_SBR.'/';
        if (!($doc = sbr_meta::getDoc($ev0['own_id'], false))) {
     ...
     * @return int количество отправленных уведомлений.
     */
    public function SbrNewComment($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/sbr.php';
        ...
        foreach ($comments as $comment) {
            ...
            if ($comment['is_admin'] == 't') {
                ...
                $rs[] = 'f_';
                $rs[] = 'e_';
            } elseif ($comment['user_id'] == $comment['e_uid']) {
                ...
                $rs[] = 'f_';
            } elseif ($comment['user_id'] == $comment['f_uid']) {
     ...
     * @param string $msg    Текст сообщения
     */
    public function SendMasssending($params, $from, $to, $msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
";
     ...
     * @param string $msg    Текст сообщения
     */
    public function SendAdminMessage($params)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/messages.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
     ...
     * @param array $events
     */
    public function newPaidAdvice($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $f_user = new users();
        $t_user = new users();
     ...
     * @param array $events
     */
    public function changePaidAdvice($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        ...
        foreach ($ids as $id) {
            ...
            } elseif ($mod_status == paid_advices::MOD_STATUS_DECLINED && $status == paid_advices::STATUS_BLOCKED) {
            ...
            } elseif ($mod_status == paid_advices::MOD_STATUS_DECLINED) {
     ...
     * 
     * @return int количество пользователей получивших рассылку
     */
    public function noActiveFreelancers()
    {
        $DB = new DB('master');
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
&nbsp;&nbsp;&nbsp;&nbsp;<a href='{$GLOBALS['host']}/?utm_source=newsletter4&utm_medium=rassilka&utm_campaign=freelancers_comeback'>Перейти на FL.ru</a>
     ...
     * @return int количество пользователей получивших рассылку
     */
    public function noActiveEmployers()
    {
        $DB = new DB('master');
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $eHost = $GLOBALS['host'];
...
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"{$eHost}/?utm_source=newsletter4&utm_medium=rassilka&utm_campaign=clients_comeback\" target=\"_blank\">Перейти на FL.ru</a></p>";
     ...
     * @return int количество пользователей получивших рассылку
     */
    public function withoutProfileFrelancers()
    {
        $DB = new DB('master');
        $this->recipient = '';
...
&nbsp;&nbsp;&nbsp;&nbsp;<a href='{$GLOBALS['host']}'>Перейти на FL.ru</a>";
     ...
     * @return int количество пользователей получивших рассылку
     */
    public function withoutProfileEmployers()
    {
        $DB = new DB('master');
        $this->recipient = '';
...
&nbsp;&nbsp;&nbsp;&nbsp;<a href='{$GLOBALS['host']}'>Будем рады видеть вас на FL.ru</a>!
     ...
     * @param array $recipients массив с uid пользователей для рассылки
     *
     * @return int id сообщения или 0 в случае ошибки
     */
     ...
     */
    public function blogDeleteNotification($mId = 0)
    {
...
';
     ...
     */
    public function contestOfferDeleteNotification($mId)
    {
        $sId = !is_array($mId) ? array($mId) : $mId;
...
';
     ...
     */
    public function contestMessageDeleteNotification($mId)
    {
        $sId = !is_array($mId) ? array($mId) : $mId;
...
';
     ...
     */
    public function freelancerOfferBlockedNotification($mId)
    {
        $sId = !is_array($mId) ? array($mId) : $mId;
...
';
     ...
     */
    public function ProjectComplainsSend($ids, $connect = null)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
