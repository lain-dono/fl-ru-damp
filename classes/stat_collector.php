...
class stat_collector
{
   ...
   * @param   int   $log_type   тип события
   *
  ...
  private function log($msg, $log_type = self::LT_NOTICE)
  {
      ...
      case self::LT_SERVER_MESSAGE :
        if ($msg) {
            $m .= 'ПОЛУЧЕНЫ СООБЩЕНИЯ ОТ СЕРВЕРА: '.$msg."\n";
        } else {
            return $e;
        }
        break;
      ...
      * @param bool $only_mem   true: сохранить только в мемкэш, иначе еще и в БД.
      *
      * @return bool   успешно?
      */
     private function _setLogTable($table, $only_mem = false)
     {
         ...
         if ($only_mem || $this->_sDB->update('stat_variables', array('value' => $table), 'name = ?', 'log_table')) {
             return $MEM->set(self::LOGTBL_MEM_KEY, $table, 3000);
         }
    ...
    {
        $MEM = new memBuff();
        if (!($table = $MEM->get(self::LOGTBL_MEM_KEY))) {
     ...
     * 
     * @return int 0:не отключена, 1:отчключена совсем, 2:включена только для сбора.
     */
     ...
     *
     * @param int    $user_id    uid пользователя, страницу которого просматривают
     * @param int    $guest_id   uid пользователя, смотрящего страницу
     * @param string $guest_ip   IP смотрящего
     * @param int    $referer_id место, с которого перешел смотрящий (смотри костанты REFID_*)
     * @param int    $by_e       1 - если просматривает работодатель, 0 - если кто-то другой
     *
     * @return string сообщение об ошибке или 0, если все прошло успешно
     */
    public function LogStat($user_id, $guest_id, $guest_ip, $referer_id, $by_e, $stamp = false)
    {
        ...
        if (self::checkStamp($stamp, true)) {
            $referer_id = 0;
        } //return false; // Проверяем пользователь не специально ли зашел на аккаунт с параметром
        if ((int) $guest_id < 0) {
            $guest_id = 0;
        } // бывают баны лезут...
     ...
     */
    public function wordsStatLog($user_id, $guest_id, $guest_ip, $is_emp, $words)
    {
...
-- !!! и индекс переделать (user_id, guest_id, guest_ip, DATE_TRUNC('hour', _time))
  ...
  public function Step3()
  {
    ...
    if ($daily_affected) {
        // может быть разгрузить все-таки функцию. Например, это конкретно в 3 часа ночи выполнять.
  ...
  public function Step6()
  {
     ...
     */
    public function wordsStatStep1()
    {
        global $DB;
     ...
     */
    public function wordsStatStep2()
    {
        global $DB;
     ...
     */
    public function wordsStatStep3()
    {
        global $DB;
  ...
  public function setStamp()
  {
      if (!isset($_SESSION['stamp'])) {
          $_SESSION['stamp'] = mt_rand(10000, 99999);
      }
  }
   ...
   * Проверяем параметр.
   *
   * @param int|bool $stamp Параметр проверки
   * @param bool $del   Удалить параметр или нет
   *
   * @return bool
   */
  public function checkStamp($stamp = false, $del = true)
  {
      $s_stamp = $_SESSION['stamp'];
      if ($del) {
          unset($_SESSION['stamp']);
      }
      if ($stamp !== false && $stamp !== $s_stamp) {
          return true;
      }
  ...
  public function __destruct()
  {
     ...
     * @return <type>
     */
    public function stat_monthly_split()
    {
        if ($this->isDisabled() == 1) {
            $this->log('Сервер временно отключен.', self::LT_HEADER);
