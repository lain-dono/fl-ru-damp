 ...
 *
	 ...
	 */
    public function AddPortf($fid, $name, $pict, $sm_pict, $link, $descr, $prof, $cost, $cost_type, $time_type, $time_value, $prev_type, &$file_error, &$preview_error, $new_position, $in_shop = 0, $is_video = 'f', $video_link = '')
    {
        global $DB; 
        $sp = 'f';
        $filename = '';
     ...
     */
    public function GenerateStaticPreview(&$work, $login)
    {
        global $DB;
 ...
 *
	 ...
	 */
    public function GetPortf($uid, $prof = 'NULL', $all = false)
    {
        global $DB;
     ...
     * @return array
     */
    public function GetPortfById($portf_id = 0)
    {
 ...
 *
	 ...
	 */
    public function GetPortfProf($fid, $prof, $onlyWithPreview = false, $skip_blocked = true)
    {
        global $DB; 
 ...
 *
	 ...
	 */
    public function GetSpecPortf($prof_id = 0, &$count, &$size, $frl_pp = PRF_PP, $offset = 0, $order = 'random', $direction = 0, $favorite = 0, $filter_apply = false, $filter = null)
    {
        global $DB; 
 ...
 *
	 ...
	 * @return array
	 */
    public function GetPrjNear($uid, $prjid, $sign = 1, $skip_blocked = true)
    {
        global $DB;
	 ...
	 * @return array
	 */
    public function GetPrj($prjid, $uid = false)
    {
        global $DB; 
        $sel_blocked = ', pb.reason as blocked_reason, pb.blocked_time, COALESCE(pb.src_id::boolean, false) as is_blocked, 
            admins.login as admin_login, admins.uname as admin_uname, admins.usurname as admin_usurname';
     ...
     */
    public function Blocked($portfolio_id = 0, $reason, $reason_id = null, $uid = 0, $from_stream = false)
    {
     ...
     * @return string Сообщение об ошибке
     */
    public function UnBlocked($portfolio_id)
    {
	 ...
	 * @return string Сообщение об ошибке
	 */
    public function EditPortf($fid, $name, $pict, $sm_pict, $link, $descr, $prof, $cost,
 ...
 *
	 ...
	 */
    public function ChangeProfOrder($uid, $prof_id, $direction)
    {
        global $DB; 
        if ($direction > 0) {
            $d = ' + 1';
 ...
 *
	 * @return integer ID
	 */
    public function ChangePos($uid, $proj_id, $direction)
    {
        global $DB; 
        if ($direction > 0) {
            $d = ' + 1';
   ...
   */
    public function ChangeTextPrev($uid, $proj_id, $ch)
    {
        global $DB;
        $error = '';
    ...
    public function OnAllPrev($uid)
    {
        ...
        if ($res) {
            foreach ($res as $v) {
                $prof_id = $v['prof_id'];
                self::ChangeGrPrev($uid, $prof_id, $projs, 't');
            }
        }
    }
	/**
 ...
 *
	 * @return string  Сообщение об ошибке
	 */
    public function ChangeGrPrev($uid, $prof_id, &$projs, $force_val = NULL)
    {
        global $DB;
        $sql = 
 ...
 *
	 ...
	 */
    public function ChangeFav($prf_id, $prof_id, $uid)
    {
        global $DB;
 ...
 *
   ...
   */
    public function GetFavorites($prof_id, $uid)
    {
        global $DB;
        if ($prof_id > 0) {
   ...
   */
    public function CountAll($uid, $prof_id = NULL, $not_count = false)
    {
        global $DB;
        if ($not_count && $prof_id == professions::CLIENTS_PROF_ID) {
            return 1;
        }
        $join_blocked = ' LEFT JOIN portfolio_blocked pb ON p.id = pb.src_id ';
