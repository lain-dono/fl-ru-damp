...
class feedback
{
     ...
     * @param int    $deskid номер тикета в helpdesk
     *
     * @return bool TRUE если отзыв не оставляли, FALSE если оставляли или отзыва не существует
     */
     ...
     * @param int    $deskid номер тикета в helpdesk
     * @param int    $e1     Баллы (от 1 до 5)за "Ожидание ответа"
     * @param int    $e2     Баллы (от 1 до 5)за "Доступное содержание"
     * @param int    $e3     Баллы (от 1 до 5)за "Общее впечатление"
     ...
     *
     * @param int    $nums    возвращает количество найдениых тикетов
     * @param array  $filter  содержит фильтр запроса (для WHERE)
    ...
    public function ShowAll(&$nums, array $filter, $sort, $pagenum)
    {
        ...
        if (!empty($filter['sdate'])) {
            $where .= " AND f.request_time >= '{$filter['sdate']} 00:00:00' ";
        }
        if (!empty($filter['edate'])) {
            $where .= " AND f.request_time <= '{$filter['edate']} 23:59:59' ";
        }
        if (!empty($filter['kind'])) {
            $where .= " AND f.dept_id = '{$filter['kind']}' ";
        }
        if (!empty($sort)) {
            if ($sort == 'date') {
