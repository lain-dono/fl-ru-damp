...
class Verification
{
     ...
     * 
     * @param int $uid uid верифицируемого пользователя
     *
     * @return bool успех
     */
...
//                }
     ...
     *
     * @return bool|int FALSE - не пробовал верифицироваться, 0 - пробовал, но еще не закончил, 1 - верифицирован
     */
    ...
    public function verify($uid)
    {
        ...
        if ($antiuid) {
            $DB->hold()->query("UPDATE users SET is_verify='t' WHERE uid=?i", $antiuid);
        }
     ...
     * 
     * @return int кол-во верифицированных пользователей
     */
     ...
     */
    public static function verifyLast($user_id)
    {
        global $DB;
     ...
     * @param bool   $is_verify Прошли верификацию или нет
     * @param bool   $role      true - Фрилансер, false - Исполнитель
     *
     * @return array
     */
    public static function getStatVerify($fromDate, $toDate, $type = 'wm', $is_verify = false, $role = null)
    {
        global $DB;
     ...
     * @param int $uid
     **/
    public static function decrementStat($uid)
    {
        $uid = intval($uid);
        global $DB;
