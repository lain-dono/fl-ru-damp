...
class validation
{
    ...
    // Vimeo
    elseif (preg_match($REGEXP_ID_VIMEO_1, $url, $matches) ||
    ...
    // ruTube
    elseif (preg_match($REGEXP_ID_RUTUBE_1, $url, $matches) ||
     ...
     *
     * @return string|bool если валидация успешна, возвращает правильно отредактированную ссылку, или FALSE
     * 
     * 
    ...
    public function video_validate($url)
    {
        ...
        if (preg_match($REGEXP_IS_YOUTU_BE, $url, $o)) {
            return $o[1];
        } elseif (preg_match($REGEXP_IS_YOUTUBE_COM, $url, $o)) {
            return $o[1].$o[2];
        } elseif (preg_match($REGEXP_IS_RUTUBE, $url, $o)) {
            return $REGEXP_REPLACE_PROTOCOL($o[1]);
        } elseif (preg_match($REGEXP_IS_RUTUBE2, $url, $o)) {
            return $REGEXP_REPLACE_PROTOCOL($o[1]);
        } elseif (preg_match($REGEXP_IS_VIMEO, $url, $o)) {
