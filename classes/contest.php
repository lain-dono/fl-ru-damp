...
class contest
{
    ...
    */
     ...
     * Конструктор.
     *
     * @param int  $pid      id проекта (для некоторых методов, его можно не указывать)
     * @param int  $uid      uid пользотеля, который работает с конкурсом
     * @param bool $is_emp   пользователь работодатель?
     * @param bool $is_owner пользователь автор этого конкурса?
     * @param bool $is_moder пользователь модератор, админ или малый админ?
     * @param bool $is_pro   у пользователя статус pro?
     */
    ...
    public function GetOffers($filter = '', $set_read = true)
    {
        ...
        if (!$watch) {
            $watch = array('status' => 'f', 'last_view' => '1970-01-01 00:00:00');
        }
        if (hasPermissions('projects') && $watch['user_id']) {
            $watch['user_id'] = $this->uid;
        }
        ...
        if ($res) {
            foreach ($res as $row) {
                $this->offers[$i] = $row;
                $in[] = $this->offers[$i]['id'];
                if ($this->offers[$i]['user_id'] == $this->uid) {
                    $this->has_offer = true;
                }  // делал ли уже пользователь предложение в этом конкурсе
                if (!$this->is_moder) {
                    $this->offers[$i]['msg_count'] = 0;
                }                 // сброс кол-ва сообещний, для правильно пересчета
                if ($this->uid && $this->offers[$i]['post_date'] > $watch['last_view'] && $this->offers[$i]['user_id'] != $this->uid) {
                    $this->offers[$i]['is_new'] = true;
                }  // видели ли это предложение?
                ...
                if ($this->offers[$i]['selected'] == 't' && $this->offers[$i]['is_deleted'] == 'f') {
                    $this->stat['candidates']++;
                }  // кол-во кандидатов (статистика)
        ...
        if (!$this->offers) {
            return true;
        }  // если нет предложений, идти дальше нет смысла
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                if (!$comments[$i]['reply_to']) {
                    ...
                    if ($this->offers[$idx]['comments'][$j]['is_new']) {
                        $this->offers[$idx]['new_comments'] = 1;
                    }
        ...
        for ($i = 0, $c = count($comments); $i < $c; ++$i) {
            if (($idx = $comments[$i]['reply_to']) && isset($commidx[$idx])) {
                ...
                if (!$this->is_moder) {
                    $this->offers[ $offidx[$comments[$i]['offer_id']] ]['msg_count']++;
                }
                ...
                if ($comments[$commidx[$idx]]['comments'][$j]['is_new']) {
                    $this->offers[ $offidx[$comments[$i]['offer_id']] ]['new_comments'] = 1;
                }
                    //if (strtotime($comments[$i]['post_date']) > $today) $this->stat['comments_today']++;
                //}
            }
     ...
     * @param bool   $comm_blocked автор запретил комментирование?
     *
    ...
    public function CreateOffer($descr, $files, $comm_blocked)
    {
        ...
        if ($row[0]) {
            return 'У Вас уже есть предложение';
        }
        ...
        if (strtotime($aData['end_date']) < time()) {
            return 'Конкурс окончен';
        }
        ...
        if ($files) {
            $files = explode('/', $files);
            if (is_array($_SESSION['contest_files'])) {
                $k = array_keys($_SESSION['contest_files']);
            } else {
                $k = array();
            }
     ...
     * Создает предложение.
     *
     * @param int    $oid          id редактируемого предложения
     * @param string $descr        описание
     * @param string $files        данные о загруженных файлах примерно следующего вида: "u34356/u37857/o983982"
     *                             префикс "u" обозначает что файл был загружен, префикс "o", что файл не именялся, т.е.
    ...
    public function ChangeOffer($oid, $descr, $files, $comm_blocked)
    {
        ...
        if (!($offer = $this->GetOfferRow($oid))) {
            return 'Нет предложения для редактирования';
        }
        if ($offer['user_id'] != $this->uid && !$this->is_moder) {
            return 'Нет предложения для редактирования';
        }
        ...
        for ($i = 0; $i < count($o); ++$i) {
            if (!in_array($o[$i], $h)) {
                $cn->Delete($o[$i]);
                if ($p[$o[$i]]['prev_id'] && ($p[$o[$i]]['prev_id'] != $o[$i])) {
                    $cn->Delete($p[$o[$i]]['prev_id']);
                }
            }
        }
     ...
     * @param int   $pos номер файла по порядку
     */
    public function addOfferFiles($oid, $s, $pos = 0)
    {
        global $DB;
        ...
        if ($s) {
            ...
            if ($data) {
                $DB->insert('projects_contest_attach', $data);
            }
            if ($data_pdate) {
                $DB->insert('projects_contest_attach', $data_pdate);
            }
        }
    }
    ...
    public function ChangeComment($cid, $comment)
    {
        global $DB;
        $cid = intval($cid);
        if ($this->is_banned && !$this->is_moder) {
            return 'Забаненые пользователи не могут редактировать комментарии';
        }
        if (!($p_comment = $this->GetComment($cid))) {
            return "Несуществующий комментарий. $cid";
        }
        if ($p_comment['user_id'] != $this->uid && !$this->is_moder) {
            return 'Несуществующий комментарий!';
        }
        $offer = $this->GetOffer($p_comment['offer_id']);
        if ($offer['closed'] == 't' && !$this->is_moder) {
            return 'Проект завершен';
        }
     ...
     * @param int $blocked возвращает TRUE, если пользователя заблокировали или FALSE если разблокировали
     *
     * @return string сообщение об ошибке или пустая строка, если все нормально
     */
    ...
    {
        global $DB;
        if (!($this->is_moder || $this->is_owner)) {
            return 'У Вас нет прав для проведения этой операции';
        }
        $uid = intval($uid);
     ...
     *
     * @param int $time дата в формате unixtime
     *
     * @return string сообщение об ошибке или пустая строка, если все нормально
     */
     ...
     *
     * @param int $pid id конкурса. можно не указывать, тогда беретса $this->pid
     *
     ...
     * @param int $uid uid пользователя
     *
     * @return int количество закончившихся конкурсов
     */
     ...
     */
    public function GetContests4Sending($prj_ids, $connect = null)
    {
        global $DB;
        if (is_array($prj_ids)) {
            $prj_ids = implode(',', array_map('intval', $prj_ids));
        }
        if (!$prj_ids) {
            return;
        }
    ...
    * @return array|mixed
    */
   public function getContestsBlockedUsers($ids, $connect = null)
   {
       global $DB;
       if (!$ids) {
           return;
     ...
     * @return array|mixed
     */
    public function getContestsUnblocked($params, $connect = null)
    {
        global $DB;
        if (!$params) {
            return;
     ...
     * @return array|mixed Если есть ответы к проектам то возвращает массив их, если нет то NULL
     */
    public function getSelectedOffers($offer_ids, $connect = null)
    {
        global $DB;
        if (!$offer_ids) {
            return;
     ...
     * @return array|mixed Если есть ответы к проектам то возвращает массив их, если нет то NULL
     */
    public function getWinnerOffers($offer_ids, $connect = null)
    {
        global $DB;
        if (!$offer_ids) {
            return;
