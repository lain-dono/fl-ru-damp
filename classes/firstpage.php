...
class firstpage
{
     ...
     * @return type
     */
    protected static function getBilling($uid)
    {
     ...
     * @return type
     */
    public static function getPlacementUpPrice($prof_group_id, $prof_id, $uid = 0)
    {
     ...
     * @return type
     */
    public static function getPlacementPrice($prof_group_id, $prof_id, $uid = 0)
    {
        $opCode = $prof_group_id ? 
 ...
 *
	 ...
	 */
	public function adddescr($text, $ordid, $user_id, $admin = 0)
	{
	    global $DB;
	    ...
	    if (!$admin && $id != $user_id) {
	        ...
	        $sql = 'UPDATE users_first_page SET descr = ? WHERE user_id = ?i AND from_date >= ?';
	    }
 ...
 *
	 ...
	 */
	public function GetDescr($ordid)
	{
	    global $DB;
   ...
   */
  public function IncWatchCount($ufp_description_id)
  {
      global $DB;
   ...
   */
  public function IncJump2UserCount($user_id, $prof_id)
  {
      global $DB;
   ...
   * @return  boolean              успех операции
   */
  public function IncJump2WorkCount($portf_id, $user_id, $prof_id)
  {
      global $DB;
   ...
   * @return  boolean                  успех операции
   */
  public function UpdateFullDescription($ufp_id, $title, $descr, $full_descr, $sels)
  {
      global $DB;
...
//        if() {
   ...
   */
  public function GetFullDescription($ufp_id)
  {
      global $DB;
      $sql =
   ...
   */
  public function GetFullDescrID($ufp_id)
  {
      global $DB;
 ...
 *
	 ...
	 */
	public function GetAll($page = -1, $profs = array(), $is_group = false)
	{
 ...
 *
	 ...
	 */
	public function getBufferSumInfo($user_id)
	{
	    global $DB;
	    $sql = 
 ...
 *
	 ...
	 */
	public function GetLastUfpID($id, &$moderator_status = null)
	{
	    global $DB;
	    if (!$id) {
	        return NULL;
 ...
 *
	 ...
	 */
	public function SetOrdered($user, $transaction_id, $pages, $tarif = null, $promo = 0, &$error)
	{
	    if (is_array($tarif)) {
     ...
     */
    public function bindDescription($nId = 0, &$sError = '')
    {
        $bRet = false;
        ...
        if ($nId) {
            ...
            if ($nDescriptionId) {
                ...
                if (empty($sError) && !$nModeratorStatus) {
                    ...
                    $aDescr = self::GetFullDescription($nId);
   ...
   */
  public function Gift(&$bill_id, &$gift_id, $gid, $fid, $transaction_id, $inv, $tarif, $comments, $ammount = 1)
  {
      global $DB; 
      $tarif = 17;
      require_once $_SERVER['DOCUMENT_ROOT'].'/classes/account.php';
   ...
   * @return   integer                     id нового заказа
   */	
	public function GiftOrdered(&$bill_id, &$gift_id, $gid, $fid, $transaction_id, $inv, $tarif, $descr, $comments)
	{
	    global $DB; 
	    require_once $_SERVER['DOCUMENT_ROOT'].'/classes/account.php';
	    $account = new account();
   ...
   * @return   integer                     id нового заказа
   */
	public function GiftOrderedCat(&$bill_id, &$gift_id, $gid, $fid, $transaction_id, $prof, $ammount, $tarif, $comments)
	{
	    if (!$ammount) {
	        return 0;
	    }
 ...
 *
	 ...
	 */
	public function GetOrders($page = 1, $id = 0)
	{
	    global $DB; 
	    if (!$id) {
 ...
 *
	 ...
	 */
	public function SetPayed($id, $ammount, $date, $to_date, $type)
	{
	    global $DB; 
 ...
 *
	 ...
	 */
	public function GetCurOrdersByUid($uid)
	{
	    global $DB; 
 ...
 *
	 ...
	 */
	public function GetOrdersByPage($page_id, $uid = null)
	{
	    global $DB; 
	    $sql = "
 ...
 *
	 * @return  boolean                      успех операции
	 */
	public function BidPlaces($user, $transaction_id, $bids)
	{
	    global $DB; 
	    if ($bids) {
 ...
 *
	 ...
	 */
	public function AdminAddFP($user, $transaction_id, $page, $time, $tarif = 64, $comments = 'Платное размещение. Возмещение платного сервиса')
	{
	    global $DB; 
 ...
 *
	...
	public function getSuccessInfo($data)
	{
	    ...
	    $poss = self::GetPositions($uid, $profs);
 ...
 *
	 * @return array
	 */
    public function getAutoPayed($user_id)
    {
        global $DB; 
     ...
     */
    public function setAutoPayed($user_id, $profession)
    {
        global $DB; 
        if (!$user_id) {
            return false;
     ...
     * @return string Сообщение об ошибке
     */
    public function delAutoPayed($user_id, $profession)
    {
        global $DB; 
        if (!$user_id) {
            return false;
     ...
     * @return array
     */
    public function getReminderAutoPayed($days = 1, $test = false)
    {
        global $DB;
...
//        }
            ...
            foreach ($users as $uid => $user) {
                ...
                if (!empty($queue)) {
                    ...
                    if ($complete) {
                        ...
                        if ($interval == 'days') { // Первая попытка
     ...
     * @return int
     */
    public function getPaidPlacesModerCounter()
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/user_content.php';
     ...
     * @return bool true - успех, false - провал
     */
    public function setModeration($fpage_id = 0, $descr_id = 0, $moder_id = 0)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/user_content.php';
     ...
     */
    public function checkModeration($fpage_id = 0)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/user_content.php';
...
function __fpPrntPosition($value)   // Вывод позиции платного объявления.
{
  ...
  <div id="mgCapsule<?=$value['id']?>">
        ...
        <tr valign="top">
          <td>
            <a href="/users/<?=$value['login']?>/?f=<?=stat_collector::REFID_PAIDSEATINGS?>&stamp=<?=$_SESSION['stamp']?>" title="<?=$value['uname']?> <?=$value['usurname']?>"
              <?php /* if ($ufp_mod | (!$_SESSION['uid'] || is_emp()))  { ?> onmouseover="mg_onOver(<?=$value['id']?>, <?=$ufp_mod?>);" onmouseout="mg_onLeave()"<? } */ ?> class="img50x50">
    ...
    ?>
            <a <?php /* if ($ufp_mod | (!$_SESSION['uid'] || is_emp()))  { ?> onmouseover="mg_onOver(<?=$value['id']?>, <?=$ufp_mod?>);" onmouseout="mg_onLeave()"<? } */ ?>
            ...
            <?php 
              if ($ufp_mod) {
                  ?><div id="fptext<?=$value['id']?>" style="overflow:hidden;"><?php
