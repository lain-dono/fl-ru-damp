...
class sbr extends sbr_meta
{
     ...
     * @var array
     */
    public static $docs_access = array (
     ...
     * @var array
     */
    public static $docs_types = array (
     ...
     * @var array
     */
    public static $ss_classes = array(
        self::STATUS_NEW => array('nr-list-new', 'Новые «Безопасные Сделки» без утверждения', 'Тех. задание не утверждено'),
        self::STATUS_CHANGED => array('nr-list-changed', 'Измененные «Безопасные Сделки» без утверждения', 'Проект в работе'),
        self::STATUS_PROCESS => array('nr-list-progress', 'В разработке', 'Проект в работе'),
        self::STATUS_CANCELED => array('nr-list-canceled', 'Отмененные проекты', 'Сделка отменена'),
        self::STATUS_REFUSED => array('nr-list-canceled', 'Отклоненные проекты', 'Сделка отклонена'),
        self::STATUS_COMPLETED => array('nr-list-completed', 'Завершенные', 'Проект завершен'),
        self::STATUS_CLOSED => array('nr-list-canceled', 'Завершенные', 'Проект закрыт'),
    );
     ...
     * @var array
     */
    public static $scheme_types = array (
    ...
    public static $name_filter = array(
     ...
     */
    public function __construct($uid, $login = NULL, $session_uid = NULL)
    {
        $this->uid = $uid;
        $this->login = $login;
    ...
    {
        return (get_class($this) == 'sbr_adm_finance');
    }
    /**
     * По типу класса определяет, кто инициализировал класс.
     *
     ...
     */
    public function isFrl()
    {
        return (get_class($this) == 'sbr_frl');
    }
     ...
     */
    public function getSchemes($get_taxes = true)
    {
        $sql = '
          SELECT *
            FROM (
     ...
     * @return boolean
     */
    public function getScheme($get_taxes = true)
    {
        $this->getUserReqvs();
     ...
     */
    public function getTaxes($scheme_id)
    {
        $sql = '
          SELECT *
     ...
     */
    public function markNotUsedTaxes($role = self::EMP)
    {
     ...
     */
    public function getTotalCost($look_arbitrage = true)
    {
        $total = 0;
        $this->setStagesEx();
     ...
     * @return float
     */
    public function getTotalTax($tax, $coeff = 1, $dvals = NULL, $round = true)
    {
        $total = 0;
        $this->setStagesEx();
        foreach ($this->stages as $s) {
     ...
     * @return float
     */
    public function getCostSysCoeff($newsys)
    {
        $cost_coeff = 1;
     ...
     * @return string
     */
    public function getContractNum($id = NULL, $scheme_type = NULL, $posted = null)
    {
    ...
    public function getContractStageNum($id = NULL, $scheme_type = NULL, $stage = 1, $posted = null)
    {
        $prefix = $this->getSbrPrefix($id, $posted);
    ...
    public function getSbrPrefix($id = null, $posted = null)
    {
        if ($posted) {
     ...
     * @return float
     */
    public function getCommNds(&$comm)
    {
        $comm = 0;
        if (!$this->scheme) {
            $this->getScheme();
        ...
        if ($this->scheme_type == self::SCHEME_AGNT || $this->scheme_type == self::SCHEME_LC) {
            $comm = $this->getTotalTax($this->scheme['taxes'][self::EMP][self::TAX_EMP_COM]);

            return 18 * $comm / 118;
        } else {
            if ($this->scheme_type == self::SCHEME_PDRD || $this->scheme_type == self::SCHEME_PDRD2) {
                return $this->getTotalTax($this->scheme['taxes'][self::EMP][self::TAX_NDS]);
            }
        }
    }
     ...
     * @return float
     */
    public function maxNoRezCost()
    {
        $default = self::MAX_COST_USD * 28;
        if (!$this->max_norez_cost) {
            $this->max_norez_cost = (int) $this->usd2rur(self::MAX_COST_USD);
        }
     ...
     */
    public function getFrlReqvs($force = false)
    {
     ...
     * @return array массив с реквизитами, индексированный: [1] -- реквизиты физ. лица, [2] -- реквизиты юр. лица, [any] -- др. поля, флаги.
     */
    public function getEmpReqvs($force = false)
    {
     ...
     */
    public function checkUserReqvs($reqvs = NULL, $form_type = NULL)
    {
        if (!$reqvs) {
     ...
     */
    public function setCheckEmpReqvs($stage_id)
    {
        ...
        if ($this->emp_reqvs['form_type'] == self::FT_PHYS && $this->emp_reqvs[self::FT_PHYS]['fio'] == '') {
            $this->emp_reqvs[self::FT_PHYS]['fio'] = 'Физическое лицо';
        }

        if ($this->emp_reqvs['form_type'] == self::FT_PHYS && ($this->cost_sys == exrates::WMR || $this->cost_sys == exrates::YM)) {
            ...
            if ($this->emp_reqvs['form_type'] == self::FT_PHYS) {
                if (trim($this->emp_reqvs[$this->emp_reqvs['form_type']]['inn']) == '') {
                    $this->emp_reqvs[$this->emp_reqvs['form_type']]['inn'] = '0000000000';
                }
            }
     ...
     * @return boolean
     */
    public function isOffIntrates($stage = NULL)
    {
     ...
     * @see intrates
     */
    public function getIntrates($stage = NULL)
    {
     ...
     */
    public function uploadFile($file, $max_size)
    {
    ...
    protected function _new_getAllCommon($sbr_id = NULL, $get_stages = true, $get_attach = false, $is_draft = false, $get_scheme = false, $only_active = false, $only_reserved = false, $filter = false, $limit = false, $offset = false)
    {
        ...
        switch ($filter) {
            case 'disable_emp':
            case 'disable':
                $where .= ' AND ( s.status IN ('.self::STATUS_NEW.', '.self::STATUS_CHANGED.', '.self::STATUS_PROCESS.') AND s.reserved_id IS NULL)';
                break;
            case 'enable':
                $where .= ' AND ( s.status IN ('.self::STATUS_PROCESS.', '.self::STATUS_CHANGED.') AND s.reserved_id IS NOT NULL)';
                break;
            case 'cancel':
                $where .= ' AND (s.status = '.self::STATUS_CANCELED.' OR s.status = '.self::STATUS_REFUSED.')';
                break;
            case 'complete':
                $where .= ' AND s.status = '.self::STATUS_COMPLETED;
                break;
        }
     ...
     * @todo префиксом _new_ будут помечатся дублирующие функции которые необходимо будет убрать при релизе
     */
    public function _new_getCurrents($filter = false, $limit = false)
    {
     ...
     */
    public function _new_getOpenSbrPartners()
    {
     ...
     */
    public function _new_isAnySbrExists()
    {
     ...
     * @return this
     */
    public function getOldSbrCompleted($get_stages = true, $get_attach = false, $get_scheme = false)
    {
        $where = $this->getWhereNewSbr();
        ...
        $sql = "SELECT s.*, date_part('day', work_time) as work_days,
              ...
              LEFT JOIN
                employer a
                  ON a.uid = emp_id
               WHERE s.{$this->uid_col} = {$this->uid} AND COALESCE(s.completed, s.completed, CURRENT_TIMESTAMP) < CURRENT_TIMESTAMP - interval '".self::MAX_DATE_LIMIT."'
                AND s.is_draft = '0' AND s.status IN (".self::STATUS_COMPLETED.', '.self::STATUS_CANCELED.', '.self::STATUS_REFUSED.', '.self::STATUS_CLOSED.") {$where}
               ORDER BY s.posted DESC";
    ...
    public function getCountCompleteSbr()
    {
        global $DB;
     ...
     * Берем СБР по его ИД.
     */
    public function getSbrForId($sbr_id = false)
    {
        if (!$sbr_id) {
            return false;
        }
     ...
     * @return integer
     */
    public function getCountCurrentsSbr($filter = false)
    {
        global $DB;
        if (!$this->uid_col) {
            return false;
        ...
        switch ($filter) {
            case 'disable':
                $where .= ' AND (s.status = '.self::STATUS_NEW.' OR s.status = '.self::STATUS_CHANGED.')';
                break;
            case 'enable':
                $where .= ' AND s.status = '.self::STATUS_PROCESS;
                break;
            case 'cancel':
                $where .= ' AND (s.status = '.self::STATUS_CANCELED.' OR s.status = '.self::STATUS_REFUSED.')';
                break;
     ...
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function getRatings($limit = 0)
    {
     ...
     */
    public function initFromStage($stage_id, $get_attach = true)
    {
        if (!$stage_id) {
            return NULL;
        }
     ...
     * @return object
     */
    public function setStage($stage_id)
    {
        $args = func_get_args();
     ...
     * @return array
     */
    public function getStages($stage_id = NULL, $get_attach = false, $get_su = false, $get_feedbacks = false, $get_info_pskb = true)
    {
        $stages = NULL;
     ...
     */
    public function getSbrIdFromStage($stage_id)
    {
        global $DB;
     ...
     */
    public function initFromId($sbr_id, $get_stages = true, $get_attach = true, $is_draft = NULL, $get_scheme = true)
    {
     ...
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function getActives($get_stages = false, $get_attach = false, $get_scheme = false)
    {
     ...
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function getActivesCount()
    {
     ...
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function getReserved($get_stages = false, $get_attach = false, $get_scheme = true, $only_active = true)
    {
     ...
     * @return array массив объектов sbr, индексированный ид. сделок.
     */
    public function getProcessings($get_stages = false, $get_attach = false, $get_scheme = false)
    {
     ...
     * @return array
     */
    public function getHistory(&$filter = NULL, $dir_col = 0, $dir = 'ASC')
    {
        $dirs = array(
     ...
     */
    public function refuseChanges($version)
    {
        if (self::$XACT_ID) {
     ...
     * @return boolean успешно?
     */
    public function agreeChanges($version)
    {
        if (self::$XACT_ID) {
     ...
     * @return array
     */
    public function getVersion($version, &$old_data)
    {
        $sql = '
     ...
     */
    public function view_scheme_info($curr_cost = NULL)
    {
        if (!$curr_cost) {
            $curr_cost = $this->cost;
        }
        ...
        if ($curr_sbr->scheme['taxes'][self::FRL]) {
            foreach ($curr_sbr->scheme['taxes'][self::FRL] as $tid => $tax) {
     ...
     */
    public function view_sign_alert()
    {
        $curr_sbr = $this;
     ...
     * @return boolean успешно?
     */
    public function setLastView($interface = 'new')
    {
     ...
     * @return array
     */
    public function getDocs($doc_id = NULL, $is_deleted = false, $only_publ = false, $stage_id = NULL, $diff = false)
    {
     ...
     * @return array
     */
    public function getLastPublishedDocByType($type, $stage_id = NULL)
    {
     ...
     */
    public function _docInitFromRequest($request, $files)
    {
        $this->post_doc = array();
        ...
        if (!$this->post_doc['name']) {
            ...
            $dn = self::$docs_types[$this->post_doc['type']][0];
            switch ($this->post_doc['type']) {
                case self::DOCS_TYPE_STATEMENT      :
                    $this->post_doc['name'] = "{$dn} № {$cnum}";
                    break;
                case self::DOCS_TYPE_TZ_PDRD        :
                    $this->post_doc['name'] = "{$dn} № {$cnum}";
                    break;
                case self::DOCS_TYPE_ACT            :
                case self::DOCS_TYPE_FACTURA        :
                case self::DOCS_TYPE_REP            :
                case self::DOCS_TYPE_COPY_ACT       :
                case self::DOCS_TYPE_COPY_FACTURA   :
                case self::DOCS_TYPE_AGENT_REP      :
                case self::DOCS_TYPE_PSKB_ACT      :
                    $this->post_doc['name'] = "{$dn} по договору № {$cnum}";
                    break;
                case self::DOCS_TYPE_COPY_CONTRACT  :
                case self::DOCS_TYPE_OFFER          :
                    ...
                    break;
                case self::DOCS_TYPE_ARB_REP        :
                case self::DOCS_TYPE_COPY_AGREEMENT :
                case self::DOCS_TYPE_FM_APPL :
                case self::DOCS_TYPE_WM_APPL :
                case self::DOCS_TYPE_YM_APPL :
     ...
     * @return boolean успешно?
     */
    public function addDocR($request, $files = NULL)
    {
     ...
     * @return boolean успешно?
     */
    public function addDoc($doc, $system)
    {
        ...
        if ($inxact || $this->_openXact(TRUE)) {
            ...
            if (!isset($sql_data['owner_role'])) {
                $sql_data['owner_role'] = (self::DOCS_ACCESS_EMP * $this->isEmp() | self::DOCS_ACCESS_FRL * $this->isFrl());
            }
     ...
     * @return int
     */
    public function regArbReportNum()
    {
     ...
     * @return int
     */
    public function regAgentRepNum()
    {
     ...
     */
    public function doc_form($doc, $stage_id = NULL, $is_edit = false)
    {
        $sbr = $this;
        ob_start();
     ...
     */
    public function _saveDocFile($content, $ext = '.pdf')
    {
        if (trim($content) == '') {
            return NULL;
        }
            ...
            $sql = '
              ...
              INNER JOIN
                sbr s
                 ...
                 AND s.status NOT IN ('.self::STATUS_CANCELED.','.self::STATUS_REFUSED.','.self::STATUS_NEW.")
     ...
     */
    public function usd2rur($usd_sum)
    {
        $rates = getCBRates();
     ...
     */
    public function getReserveSum($force = false, $ps = null, $round = true)
    {
        if (!$this->reserve_sum || $force) {
            if ($this->cost_sys) {
                ...
                $tsum = 0;
                foreach ($this->scheme['taxes'][self::EMP] as $tax) {
                    $dvals = array('A' => NULL);
                    if ($ps) {
                        $dvals['P'] = $ps;
     ...
     */
    public function getTaxByCode($codes = '', $role = null)
    {
        if (!$this->scheme) {
            $this->getScheme();
        }
        ...
        if (!is_array($codes)) {
            ...
            if ($role) {
                ...
                $taxes = array_merge($scheme['taxes'][self::EMP], $scheme['taxes'][self::FRL]);
            }
     ...
     * @return string
     */
    public function getDescrTaxes($tax_id, $percent = '13', $is_emp = null)
    {
        if ($is_emp === null) {
            $is_emp = is_emp();
        }
     ...
     * @return string
     */
    public function getSbrCalcHash($calc)
    {
     ...
     */
    public function setSbrCalc($calc, &$hash)
    {
        global $DB;
        $calc = serialize($calc);
        $hash = self::getSbrCalcHash($calc);
        if (($info = self::getSbrCalc($hash)) == false) {
     ...
     */
    public function getSbrCalc($hash)
    {
        global $DB;
     ...
     * @return boolean
     */
    public function isEmpReservedMoney()
    {
        return ($this->status == self::STATUS_PROCESS && $this->stages_version == $this->frl_stages_version && $this->version == $this->frl_version && !$this->data['reserved_id'] && $this->isEmp());
    }
