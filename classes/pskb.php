...
//    private static $_instance;
    ...
    public function getMd5Reqvs($lc, $pfx = '')
    {
        ...
        foreach ($check_params as $check) {
            if ($check == 'num') {
                $lc[$check.$pfx] = str_replace('+', '', $lc[$check.$pfx]);
            } // В таблице pskb_lc идет без плюса а в pskb_users с плюсом
     ...
     * 
     * @param int $ps Тип оплаты
     *
     * @return bool|\pskb_user
     */
    ...
    public function prepareLC($ps)
    {
        ...
        if ($this->_sbr->isEmp()) {
            $user_pfx = 'Cust';
            $data['ps_emp'] = $ps;
            $data['sum'] = $this->_sbr->getReserveSum(true, self::$exrates_map[$ps]);
        } else {
            $data['ps_frl'] = $ps;
    ...
    public function reserve()
    {
        ...
        if ($lc['lc_id'] > 0 && $lc['state'] == self::STATE_ERR) {
            ...
            if ($lc_ch->state == self::STATE_NEW) {
                ...
                self::checkStatus(array($lc['lc_id']), $in, $out);
...
//            $lc['kppPerf'],
    ...
    public function payoutConfirm($code, $stage_id)
    {
        ...
        if ($resp->state != 'trans') {
            $resp->state = 'err';
        } // Тут ожидаем от state либо trans либо err, все остальное считаем err
    ...
    public function prolongLC($id, $days)
    {
        ...
        if (preg_match('/^exp/', $lc['state']) || in_array($lc['state'], array(self::STATE_END, self::STATE_ERR))) {
            $this->_setError('Недопустимый статус аккредитива.');
    ...
    public function resendCode(sbr_stages $stage)
    {
        ...
        if ($pskb_lc->state == self::STATE_COVER && strstr($pskb_lc->stateReason, 'nosignPerf')) {
            $emp_percent = 0;
            $frl_percent = 1;
            ...
            if ($resp->state == self::STATE_PASSED) {
                $data['state'] = $resp->state;
                $data['stateReason'] = $resp->stateReason;
                $this->upLC($data, $lc['lc_id']);
            }
...
//                ORDER BY st.num";
        ...
        foreach ($data as $row) {
            ...
            switch ($lc->state) {
                ...
                case self::STATE_END:
                    ...
                    $emp_payout = true;
                    if ($frl_percent > 0) {
                        $frl_payout = !$stage->payoutAgnt($row['frl_id'], new self($stage->sbr->data['id']));
                    }
                    if ($emp_percent > 0) {
                        $emp_payout = !$stage->payoutAgnt($row['emp_id'], new self($stage->sbr->data['id']));
                    }
                    if (!$frl_payout && !$emp_payout) {
                        continue;
                    }
    ...
    protected function _addLC($sum, $numDog, $dateExecLC, $dateEndLC, $dateCoverLC,
        ...
        $sumPlanned)
    {
        $numCust = self::phone($numCust);
        $numPerf = self::phone($numPerf);
...
//        var_dump($res);
    ...
    protected function _SuperCheck($ids, $url_rejoin = false)
    {
        if (!$url_rejoin) {
            $_host = !defined('IS_LOCAL') ? str_replace('http://', 'https://', $GLOBALS['host']) : $GLOBALS['host'];
            $url_rejoin = $_host.(defined('PSKB_TEST_MODE') ? self::URL_REJOIN_DEBUG : self::URL_REJOIN);
        } else {
            $_host = !defined('IS_LOCAL') ? str_replace('http://', 'https://', $GLOBALS['host']) : $GLOBALS['host'];
            $url_rejoin = $_host.$url_rejoin;
        }

        $url_rejoin .= '?key='.self::KEY_CHECK_AUTH; // Авторизация на  нашей стороне

        // Так как у товарищей лимит на входящий запрос в 4000 символов делим идишники на части
        $parts_ids = array_chunk($ids, self::SUPERCHECK_LIMIT_ID_REQUEST);
...
//            curl_setopt($ch, CURLOPT_URL, "http://localhost/apiLCPlace/" . $method);
     ...
     * @param bool   $prepareError Обработка ошибок перед записью
     */
     ...
     * 
     * @param int $rez_type Тип резидентства пользователя @see sbr::RT_*
     *
     * @return array
     */
    ...
    public function __construct($json = null)
    {
        if ($json) {
            ...
            foreach ($json as $k => $v) {
                ...
                if ($k == 'history') {
                    foreach ($v  as $hist) {
                        $this->history[] = new self(json_encode($hist));
                    }
                } else {
