...
class projects_offers_dialogue
{
     ...
     */
    public function GetDialogueForOffer($po_id)
    {
     ...
     * @return array
     */
    public function getDialogueMessageById($dialogue_id = 0)
    {
        $sel_blocked = ', pb.reason as blocked_reason, pb.blocked_time, COALESCE(pb.src_id::boolean, false) as is_blocked, 
            admins.login as admin_login, admins.uname as admin_uname, admins.usurname as admin_usurname';
        $join_blocked = 'LEFT JOIN projects_offers_dialogue_blocked pb ON d.id = pb.src_id 
     ...
     */
    public function Blocked($dialogue_id = 0, $reason, $reason_id = null, $uid = 0, $from_stream = false)
    {
     ...
     */
    public function UnBlocked($dialogue_id)
    {
     ...
     */
    public function GetProjectIDFromDialogue($po_id)
    {
     ...
     */
    public function GetProjectFromDialogue($po_id)
    {
	 ...
	 */
	public function GetLastDialogueMessage($user_id, $po_id)
	{
	 ...
	 * @return id                        id последнего сообщения ветки комментариев
	 */
	public function GetLastDialogueMessageData($user_id, $po_id)
	{
	 ...
	 * @return string                    пустая строка если метод выполнен успешно или сообщение об ошибке
	 */
	public function markReadFrl($po_id, $user_id)
	{
     ...
     * @return string пустая строка если метод выполнен успешно или сообщение об ошибке
     */
    public function markReadMod($po_ids, $user_id)
    {
     ...
     * @return string пустая строка если метод выполнен успешно или сообщение об ошибке
     */
    public function markReadEmp($po_ids, $user_id)
    {
     ...
     * @return string пустая строка если метод выполнен успешно или сообщение об ошибке
     */	
	public function markAllReadEmp($prj_id, $user_id)
	{
     ...
     */
    public function CountMessagesForEmp($user_id, $new = false, $as_array = false)
    {
        $mem = new memBuff();
     ...
     */
	public function CountMessagesPrjForEmp($prj_id)
	{
     ...
     * @return integer количество сообщений
     */
    public function CountMessagesForFrl($user_id, $new = false, $waste = true)
    {
        $mem = new memBuff();
	 ...
	 */
	public function FindLastMessageProjectForEmp($user_id)
	{
	    $mem = new memBuff();
     ...
     * @return mixed id проекта, если есть проекты с непрочитанными сообщениями в диалоге и false, если нет.
     */
    public function FindLastMessageProjectForFrl($user_id)
    {
     ...
     * @return array
     */
    public function FindAllUnreadMessageFrl($uid)
    {
     ...
     * @return array
     */
    public function FindAllUnreadMessageEmp($uid)
    {
        if (!hasPermissions('projects')) {
            return false;
        }
     ...
     * @return string Сообщение об ошибке
     */
    public function getUnread2Read($msgs, $frl = true)
    {
     ...
     */
    public function DelDialogueMessageNotification($msg_id = 0, $deluser_id = 0)
    {
