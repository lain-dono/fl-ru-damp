...
class clients
{
     ...
     */
    public function newClient($name, $link, $logo, &$error)
    {
        global $DB;
        if ($logo) {
     ...
     * @param int    $limit Лимит показа
     *
     * @return array Данные клиенты
     */
    public function getClients($rand = 'RANDOM()', $limit = 90)
    {
        global $DB;
     ...
     * @param int $count Возвращает общее количество клиентов
     * @param int $limit Лимит показа
     *
     * @return array Данные клиентов
     */
    public function getAdminClients($page = 0, &$count, $limit = 10)
    {
        global $DB;
        --$page;
        if ($page < 0) {
