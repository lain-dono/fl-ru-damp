...
class smail extends SMTP
{
     ...
     */
    public function NewPromoCommune($name, $msg)
    {
        if (!$this->Connect()) {
...
";
     ...
     *
     * @return int количетво отправленных писем.
     */
    public function FeedbackPost($login, $email, $kind, $msg, $ucode = '', $fid = 0)
    {
        $nRet = 0;
     ...
     */
    public function CommuneJoinAction($user_id, $comm)
    {
...
Вы можете <a href=\"{$GLOBALS['host']}/commune/?id={$comm['id']}&site=Admin.members&mode=Asked{$this->_addUrlParams('b', '&')}\">отклонить или принять</a> его заявку.";
     ...
     */
    public function CommuneMemberAction($user_id, $action, $comm)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $user = new users();
        $user->GetUserByUID($user_id);
...
скачайте и установите бесплатное приложение-информер <a href="'.$GLOBALS['host'].'/promo/freetray/'.$this->_addUrlParams('f').'">Free-tray</a>. 
     ...
     * @return string возможная ошибка.
     */
    public function NewUser($login, $passwd = false, $code = false, $masterId = false, $uType = false)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        ...
        if ($code) {
            $message .= "<br/>Осталось лишь активировать аккаунт по ссылке <a href='".$GLOBALS['host']."/registration/activate.php?code=$code".($masterId ? "&m={$masterId}" : '').($uType ? "&u={$uType}" : '')."{$this->_addUrlParams('b', '&')}' target='_blank'>".$GLOBALS['host']."/registration/activate.php?code=$code".($masterId ? "&m={$masterId}" : '').($uType ? "&u={$uType}" : '').'</a>';
        }
        ...
        if (!$this->send('text/html')) {
            return 'Неудалось отправить сообщение';
        }
     ...
     */
    public function adminNewSuspectUser($sLogin = '', $sName = '', $sSurname = '')
    {
        $this->message = $this->GetHtml('',
...
----------------------------<br />');
     ...
     * @return string возможная ошибка.
     */
    public function ChangePwd($uid, $passwd)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
...
'.$GLOBALS['host'];
     ...
     * @return string возможная ошибка.
     */
    public function OrderFP($login, $order_id, $d_time, $sum)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
...
".$GLOBALS['host'];
     ...
     * @return string возможная ошибка.
     */
    public function NewGift($from_login, $to_login, $msg, $idg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
     ...
     */
    public function alphaBankMistakeSorry($to_id, $op_date)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
...
', 'simple');
     ...
     * @return string возможная ошибка.
     */
    public function ConfirmNewEmail($login, $newmail, $code)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUser($login);
...
Команда FL.ru";
     ...
     */
    public function SendBan2($uid, $reason)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $f_user = new users();
...
";
     ...
     *
     * @param int $uid UID юзера
     *
     * @return string строка об ошибке
     */
    public function DocsBack($uid)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $f_user = new users();
        $f_user->GetUserByUID($uid);
...
С уважением, команда FL.ru", array());
     ...
     *
     * @param int $frl_id UID фрилансера
     * @param int $prj_id id проекта
     *
     * @return string
     */
    public function NoRiskCancelFrl($frl_id, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
...
', 'simple');
     ...
     * @return string
     */
    public function NoRiskT3Send($login, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUser($login);
...
', 'simple');
     ...
     * @return string
     */
    public function NoRiskNewT3Send($login, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUser($login);
...
', 'simple');
     ...
     *
     * @param int $uid    UID Заказчика
     * @param int $prj_id id проекта
     *
     * @return string
     */
    public function NoRiskArbitrageEmp($uid, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/employer.php';
        $t_user = new employer();
        $t_user->GetUserByUID($uid);
...
', 'simple');
     ...
     * @return string
     */
    public function NoRiskArbitrageFrl($uid, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/freelancer.php';
        $t_user = new freelancer();
        $t_user->GetUserByUID($uid);
...
', 'simple');
     ...
     * @return string
     */
    public function NoRiskClosed($frl_id, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($frl_id);
...
', 'simple');
     ...
     * @return string
     */
    public function NoRiskArbiterClosed($uid, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($uid);
...
', 'simple');
     ...
     * @return string
     */
    public function NoRiskPaymentCommited($uid, $prj_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($uid);
...
', 'simple');
     ...
     * @return string
     */
    public function EndTopDaysPrjSendAlerts()
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/projects.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
...
Напоминаем вам, что пользователи с <a href='".$GLOBALS['host'].'/payed/'.$this->_addUrlParams('e')."'>аккаунтом PRO</a> экономят на платных услугах сайта.", array('header' => 'simple', 'footer' => 'emp_projects'));
     ...
     * @return string
     */
    public function SendBlogsBan($login, $reason = 1)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUser($login);
     ...
     * @return bool true - успех, false - провал
     */
    public function SendBlogsBan2($uid, $reason)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($uid);
     ...
     *
     * @param int $uid UID юзера
     * @param int $msg текст ответа.
     *
     * @return string возможная ошибка.
     */
    public function SendManagerAnswer($uid, $msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($uid);
     ...
     *
     * @param int $uid  users.uid юзера, оставившего отзыв.
     * @param int $uid2 users.uid юзера, оставившего комментарий на этот отзыв.
     *
     * @return string возможная ошибка.
     */
    public function SendCommentOpinions($uid, $uid2)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($uid);
        if ($t_user->is_banned == '1') {
            return;
        }
...
", 'simple');
     ...
     * @return string возможная ошибка.
     */
    public function SendCommentFeedback($uid, $uid2, $isEdit = false)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        $t_user->GetUserByUID($uid);
...
", 'simple');
     ...
     * @return string возможная ошибка
     */
    public function SendManagerWork($uid, $msg, $phone, $umail = '', $fio = '', $files = false, $email = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        if ($uid) {
        ...
        if (!$this->SmtpMail('text/html', $attaches)) {
            $error = 'Невозможно отправить сообщение';
        }
     ...
     * @return string возможная ошибка
     */
    public function SendManagerOrderCall($uid, $fio, $phone, $time_to_call, $client_email = '', $email = '')
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $t_user = new users();
        if ($uid) {
     ...
     * @return string возможная ошибка
     */
    public function PROEnding($user_id, $to_date)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/payed.php';
        $user = new users();
     ...
     * @return string возможная ошибка
     */
    public function PROAutoProlongError($user_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/payed.php';
        $user = new users();
...
Вы можете отказаться от данной функции на странице <a href='{$GLOBALS['host']}/$page/{$this->_addUrlParams('b', '&')}#pro_autoprolong'>{$GLOBALS['host']}/$page/#pro_autoprolong</a>", 'simple');
     ...
     * @return string возможная ошибка
     */
    public function PROAutoProlongOk($user_id)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/payed.php';
        $user = new users();
     ...
     */
    public function CorporativeBlogNewComment($comment, $user, $p_user, $link = null)
    {
...
");
     ...
     * @return string возможная ошибка
     */
    public function ContestReminder()
    {
...
", array('header' => 'simple', 'footer' => 'default'), array('login' => $user['login']));
     ...
     * @return string
     */
    public function DocSend(users $user, CFile $file, $type = 'sf')
    {
        if (!$user->uid) {
            return false;
        }
     ...
     * @return string
     */
    public function LMDocSend(users $user, CFile $file, $type = 'sf')
    {
        if (!$user->uid) {
            return false;
        }
     ...
     */
    public function massSendingForFile($file_name, $subject, $message, $from = 'FL.ru <no_reply@free-lance.ru>')
    {
     ...
     */
    public function PaidSpecsEnding($user_id, $sum)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $user = new users();
        $user->GetUser($user->GetField($user_id, $ee, 'login'));
     ...
     * @return string сообщение об ошибке
     */
    public function PaidSpecsAutopayed($user_id, $sum)
    {
        return; // #0022795
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $user = new users();
     ...
     */
    public function delArticleSendReason($user_id, $title, $msg)
    {
        require_once $_SERVER['DOCUMENT_ROOT'].'/classes/users.php';
        $user = new users();
        $user->GetUserByUID($user_id);
...
'), array('header' => 'simple', 'footer' => 'simple'));
     ...
     * 
     * @param int $days За сколько дней смотреть.
     */
...
По всем возникающим вопросам вы можете обращаться в нашу {$pHttp}:/{службу поддержки}/feedback.FL.ru/.";
     ...
     * 
     * @return int id письма (0 - не отправлено)
     */
    public function sbrFeedbackEdit($fbId, $userId, $moderId, $sbr)
    {
        $moder = new users();
        $user = new users();
