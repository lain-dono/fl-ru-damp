...
class portfolio_filters
{
	 ...
	 * @param integer $force
	 */
	public function Save($user_id, $cost_from, $cost_to, $cost_type, &$rerror, &$error, $force = 0)
	{
 ...
 *
	 ...
	 */
	public function Add($user_id, $cost_from, $cost_to, $cost_type, $force = 0)
	{
	    global $session;
 ...
 *
	 * @return string сообщение об ошибке или пустая строка
	 */
	public function Update($user_id, $cost_from, $cost_to, $cost_type, $force = 0)
	{
	    $cost_from = intval(str_replace(' ', '', $cost_from) * 100) / 100;
	    $cost_to = intval(str_replace(' ', '', $cost_to) * 100) / 100;
	    $cost_type = intval($cost_type);
   ...
   * @return array фильтр
   */
	public function GetFilter($user_id, &$error)
	{
	    if ($user_id > 0) {
	        global $DB;
	        ...
	        $ret = $_SESSION['pf'];
	        if (!$ret) {
	            $ret = array('user_id' => 0,  'cost_from' => '', 'cost_to' => '', 'cost_type' => '');
	        }
	    }
   ...
   * @return string сообщение об ошибке или пустая строка
   */
	public function DeleteFilter($user_id)
	{
	    global $session;
	    if ($user_id > 0) {
 ...
 *
	 ...
	 */
	public function GetDescription($filter, $filter_apply)
	{
	    $ret = '';
	    if ($filter_apply) {
