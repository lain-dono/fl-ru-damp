...
class JWS extends JWT
{
     ...
     * @return bool
     */
    public static function isAllowedAlg($alg)
    {
...
class JWT
{
     ...
     * @return string
     */
    public static function getEncodedHeader($jwt)
    {
        // проверяем все ли части на месте
        $part = explode('.', $jwt);
        if (!is_array($part) || empty($part) || count($part) !== 3) {
     ...
     * @return string
     */
    public static function getEncodedPayload($jwt)
    {
        // проверяем все ли части на месте
        $part = explode('.', $jwt);
        if (!is_array($part) || empty($part) || count($part) !== 3) {
     ...
     * @return string
     */
    public static function getSigningInput($jwt)
    {
        // проверяем все ли части на месте
        $part = explode('.', $jwt);
        if (!is_array($part) || empty($part) || count($part) !== 3) {
