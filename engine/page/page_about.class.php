...
class page_about extends page_base
{
     ...
     */
    public function corporativeAction()
    {
            ...
            switch ($this->uri[0]) {
                ...
                case 'post':
                    ...
                    switch ($action) {
                        ...
                        case 'addcmt':
                            if (!get_uid()) {
                                header("Location: /{$this->name_page}/corporative/post/{$_POST['blogID']}/");
                            }
                        ...
                        case 'editcmt':
                            ...
                            if (!get_uid()) {
                                header("Location: /{$this->name_page}/corporative/post/{$_POST['blogID']}/?new={$idCom}");
                            }
                    ...
                    if ($comments) {
                        foreach ($comments as $k => $v) {
                            if ($v['id_modified']) {
                                $mod[$v['id_modified']] = $v['id_modified'];
                            }
                            if ($v['id_deleted']) {
                                $mod[$v['id_deleted']] = $v['id_deleted'];
                            }
                            $cid[$v['id']] = $v['id'];
                        }
                    }
                    ...
                    if ($mod) {
                        $moders = front::og('db')->select('SELECT login, uname, usurname, uid, role, is_pro, is_pro_test, boss_rate FROM users WHERE uid IN(?a)', $mod)->fetchAll();
                    }

                    if ($moders) {
                        foreach ($moders as $key => $val) {
                            $res_mod[$val['uid']] = $val;
                        }
                    ...
                    switch ($action2) {
                        ...
                        case 'edit':
                            ...
                            if ($comments[$IDEdit]['id_user'] != get_uid() && (!hasPermissions('about'))) {
                                header("Location: /{$this->name_page}/corporative/post/{$comments[$IDEdit]['id_blog']}/");
                            }
                        ...
                        case 'adminedit':
                            if (!hasPermissions('about')) {
                                header("Location: /{$this->name_page}/corporative/post/$id/");
                            }
    ...
    public function evaluateAction()
    {
        ...
        if (preg_match("/\/([A-Za-z0-9]{14,32})\/$/", front::$_req['pg'], $o)) {
            $from = 'feedback';
        } elseif (preg_match("/\/([0-9]+)_([0-9\.]+)\/$/", front::$_req['pg'], $o)) {
            $from = 'webim';
        }
     ...
     */
    public function getCorporateBlog($page = 1, $count = 10, $tags = false)
    {
        if (!$tags) {
     ...
     */
    public function sortTreeComment(&$comments)
    {
...
//            $idCom =  front::og("db")->select("SELECT id FROM corporative_blog WHERE title = ? AND msg = ? AND id_blog = ? AND id_user = ?", $msg_name, $msg, $blog, $user)->fetchOne();
     ...
     * @return unknown
     */
    public function tagsAdd($trid, $tg)
    {
        $DB = new DB('master');
    ...
    public function tagsDelete($itemid)
    {
        $DB = new DB('master');
    ...
    public function GetTags(&$count, $limit, $offset = 0)
    {
        $DB = new DB('master');
    ...
    public function getPostTags($id = false)
    {
        if (!$id) {
            return false;
        }
     ...
     *
     * @param int $id ИД редактируемого комментария
     *
     ...
     *
     * @param int $uid ID юзера который смотрит
     * @param int $bid ID блога который он смотрит
     *
     * @return array [дата, ИД]
     */
    public function getViewPost($uid, $bid)
    {
     ...
     */
    public function getViewPostsDate($bids)
    {
    ...
    public function getTagsPosts($bids)
    {
