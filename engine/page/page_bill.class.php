...
class page_bill extends page_base
{
     ...
     */
    public function alphabankAction()
    {
                ...
                if ($_POST['act']) {
                    ...
                    if (!$type) {
                        ...
                        if ($login) {
                            ...
                            if (is_emp($user->role)) {
                                $alert['login'] = 'Пользователь не является фрилансером';
                            }
                            if ($gid == get_uid()) {
                                $alert['login'] = 'Вы не можете сделать подарок самому себе';
                            }
...
//                                                                                $account ->/ view_error("Невозможно завершить транзакцию. Попробуйте повторить операцию с самого начала.");
     ...
     */
    public function svyasnoyAction()
    {
        front::og('tpl')->page = 'index';
        front::og('tpl')->uri = $this->uri;
     ...
     */
    public function elecsnetAction()
    {
        //front::og("tpl")->page = "index";
        //front::og("tpl")->uri = $this->uri;
...
//    	echo $pages." - ".$total;
     ...
     */
    public function cardsuccessAction()
    {
     ...
     *
     * @return array|bool Типы, если тип не найден false
     */
    public function getTypeBillOperation($type)
    {
        switch ($type) {
            case 1: # Зачисления
                $t = array(12);
    ...
    public function getOperation2Promo($op_code)
    {
        switch ($op_code) {
            case 15:
            case 16:
