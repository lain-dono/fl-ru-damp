...
<div class="b-freelancers-collection <?php if ($cur_prof || $prof_group_id && $prof_name) {
    ?>b-breadcrumbed-freelancers-collection<?php 
} ?>">
...
<div class="b-layout__right b-layout__right_relative b-layout__left_width_72ps b-layout__left_float_left">
			...
			<?php endif; ?>
				...
				<li class="b-menu__item b-menu__item_padbot_null b-page__desktop"><a class="b-menu__link" href="/profi/"><span class="b-icon b-icon__profi b-icon_valign_bas" data-profi-txt="Лучшие фрилансеры сайта FL.ru. Работают на сайте более 2-х лет, прошли верификацию личности и имеют не менее 98% положительных отзывов."></span></a>
                        ...
                        <?php //include($_SERVER['DOCUMENT_ROOT'].'/freelancers/filter.php') ?>
        ...
        while ($table < 2) {
            while ($iter <= $size && (($frl['is_pro'] == 't' && $table == 0) || ($frl['is_pro'] == 'f' && $table == 1))) {
                ...
                ?>
                                ...
                                <tr class="cf-line <?php if ($table == 0) {
    ?>is-pro<?php 
}
                ?>">
                                    <td class="cf-fav">
                                        <div>
                                            <a href="javascript:void(1)"><?php if (($frl['uid'] != $uid) && ($uid > 0)) {
    ?><img id="favstar_<?=$frl['uid']?>" src="/images/<?php if ($table == 0) {
    ?>ico_star_<?php if (in_array($frl['uid'], $favs) || $fav_show) {
    ?>yellow<?php 
} else {
    ?>empty<?php 
}
    ...
    ?>.gif" alt="" width="10" height="11" border="0" style="cursor:pointer" onClick="xajax_AddFav(<?=$frl['uid']?>, <?=$prof_id?>, '<?=$frl['is_pro']?>')"><?php 
} else {
    ?><img src="/images/1.gif" alt="" width="10" height="11" border="0"><?php 
                ...
                ?>
                                                <?php if ($frl['additional_spec']) {
    ?><br/>Дополнительные специализации: <?=$frl['additional_spec']?><?php 
}//if?>
                ...
                ?>
                                            ...
                                            <?php if ($frl['status_type'] != -1) {
    ?><?=freelancer::viewStatus($frl['status_type'], true)?> <?php 
}
                ?> 
                                        </div>
                                    </td>
                ...
                ?>
                                    ...
                                    <td class="price_prj" style="width:70px;"><?php if ($prof_type) {
    ?><?=view_cost2($frl['cost_1000'], '', '', true, $frl['cost_type'])?><?php 
} else {
    ?><?=view_cost2($frl['cost_from'], 'от', '', true, $frl['cost_type'])?><?php 
}
                ?></td>
...
<?// Работы в каталоге ?>
    ...
    ?>
      ...
      ?>
                    ...
                    <?php if ($page == 1 && $cur_prof['descr_text2']) {
    ?>
...
<?php 
    ...
    ?>
    <div class="b-banner b-banner_pf b-banner_margbot_20">
        <a class="b-banner__link" href="/promo/<?= sbr::NEW_TEMPLATE_SBR;
    ?>" title="Воспользуйтесь сервисом «Безопасная Сделка»"><img class="b-banner__pf" src="/css/block/b-banner/b-banner__sbr.png" alt="«Безопасная Сделка»"/></a>
    </div>
<?php 
}//if?>
...
<div class="b-layout__left b-layout__left_width_25ps b-layout__right_margleft_3ps b-layout__right_float_left">
                        <?php // if ($uid) include($_SERVER['DOCUMENT_ROOT'] . '/freelancers/tpl.filter.php'); ?>
