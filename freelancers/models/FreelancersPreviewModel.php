...
class FreelancersPreviewModel extends CModel
{
    ...
    public static $type_classes = array(
        self::TYPE_PF => 'PortfolioItem',
        self::TYPE_TU => 'TServiceItem',
    );

    public static $type_from_string = array(
     ...
     * 
     * @return \FreelancersPreviewItemIterator|bool
     */
    ...
    public function save()
    {
        ...
        $key = sprintf(self::TAG_CURRENT_USER_PREVIEW, $this->group_id, $this->prof_id);
        $cache_keys[$key] = $key;
        ...
        if ($this->data_extra_update) {
            foreach ($this->data_extra_update as $insert_data) {
                $key = sprintf(self::TAG_CURRENT_USER_PREVIEW, $insert_data['group_id'], $insert_data['prof_id']);
