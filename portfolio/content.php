...
<div class="b-layout__right b-layout__right_relative b-layout__left_width_72ps b-layout__left_float_left">
    ...
    <div class="b-menu b-menu_simple b-menu_padbot_20 b-menu_bordbot_b2">
      <ul class="b-menu__list">
          <li class="b-menu__item b-menu__item_margright_15"><a class="b-menu__link b-menu__link_fontsize_11 <?php if (!$order || $order == 'rating'): ?>b-menu__link_bold<?php endif; ?>" href="./?order=rating<?=($prof_id) ? '&prof='.$prof_id : ''?><?=($fav_show) ? '&fs='.$fav_show : ''?>">По рейтингу фрилансера</a></li>
          <li class="b-menu__item b-menu__item_margright_15"><a class="b-menu__link b-menu__link_fontsize_11 <?php if ($order == 'rnd'): ?>b-menu__link_bold<?php endif; ?>" href="./?order=rnd<?=($prof_id) ? '&prof='.$prof_id : ''?><?=($fav_show) ? '&fs='.$fav_show : ''?>">Случайно</a></li>
          <li class="b-menu__item b-menu__item_margright_15"><a class="b-menu__link b-menu__link_fontsize_11 <?php if ($order == 'prc'): ?>b-menu__link_bold<?php endif; ?>" href="./?order=prc<?=($prof_id) ? '&prof='.$prof_id : ''?><?=($fav_show) ? '&fs='.$fav_show : ''?>">По цене работы</a></li>
          <li class="b-menu__item"><a class="b-menu__link b-menu__link_fontsize_11 <?php if ($order == 'ops'): ?>b-menu__link_bold<?php endif; ?>" href="./?order=ops<?=($prof_id) ? '&prof='.$prof_id : ''?><?=($fav_show) ? '&fs='.$fav_show : ''?>">По отзывам заказчиков фрилансеру</a></li>
      </ul>
    </div>                
    <table class="b-layout__table b-layout__table_width_full" cellpadding="0" cellspacing="0">
        <?php if ($prtfs): ?>
        <?php $i = 0; foreach ($prtfs as $key => $prf): if ($i >= 3) {
     $i = 0;
 } ?>
        <?=($i == 0 ? "<tr class='b-layout__tr'>" : '');++$i;?>
            <td class="b-layout__td b-layout__td_bordbot_c3 b-layout__td_padright_20 b-layout__td_padtb_20 b-layout__td_width_33ps">
                    ...
                    <?php if ($prf['prev_type'] == 1 || ($prof_id == 0 && isset($prf['is_text']) && $prf['is_text'] == 't')): ?>
                    ...
                    <div class="b-layout__txt b-layout__txt_padtop_10">
                        ...
                        <?php $txt_cost = view_cost2($prf['cost'], '', '', true, $prf['cost_type']);  $txt_time = view_time($prf['time_value'], $prf['time_type']);?>
                        <strong class="b-layout__txt b-layout__txt_bold"><?=$txt_cost?></strong><?php if ($txt_cost != '' && $txt_time != '') {
    ?>, <?php 
} ?><?=$txt_time?>
                    </div>
            </td>
...
<div class="b-layout__left b-layout__left_width_25ps b-layout__right_margleft_3ps b-layout__right_float_left">
    ...
    <?php /* include($_SERVER['DOCUMENT_ROOT'] . '/freelancers/tpl.catmenu.new.php'); */ ?>
