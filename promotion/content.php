...
$xajax->printJavascript('/xajax/');
  ...
  if ($DEBUG) {
      if (!hasPermissions('users') && $_SESSION['login'] != 'sll' || !$login) {
          exit;
      }
  }
          ...
          if ($pro_days[$nday]
            ...
            if ($ngbs = promotion::GetNeighbours($neighbours, $u_pro_rating)) {
              ...
              $min_pro_to = $TODAY_TIME;
                ...
                if ($ngbs['B'] && ($t = strtotime($ngbs['B']['pro_to'])) < $min_pro_to) {
                    $min_pro_to = $t;
                }
                if ($ngbs['C'] && ($t = strtotime($ngbs['C']['pro_to'])) < $min_pro_to) {
                    $min_pro_to = $t;
                }
                if ($ngbs['D'] && ($t = strtotime($ngbs['D']['pro_to'])) < $min_pro_to) {
                    $min_pro_to = $t;
                }
                if ($ngbs['E'] && ($t = strtotime($ngbs['E']['pro_to'])) < $min_pro_to) {
                    $min_pro_to = $t;
                }
    ...
    for ($j;$j < 30;++$j) {
      ...
      $n30_ctm = $time + (($j + 1) * 24 * 3600);
        ...
        if ($prev27  // $prev27 существует, только если $prm_is_PRO
         && !$npcnt) {
            // если хоть один раз пришлось брать данные "сзади", то это значит, что впереди $prev27 абсолютно пуст,
...
<?php //// ШАПКА /////////////////////////////////////////////////////////////// ?>
    ...
    ?>
                  <div class="big-s">
                      <a href="/promotion/<?=($DEBUG ? "?user={$login}" : '')?>" class="blue">Аккаунт <b><?php if ($u_is_profi): ?>PROFI<?php else: ?>PRO<?php endif;
    ?></b></a>
                  </div>
                <?php 
} else {
    ?>
                  <div class="big-s">
                      Аккаунт <b><?php if ($u_is_profi): ?>PROFI<?php else: ?>PRO<?php endif;
    ?></b>
                  </div>
                <?php 
} ?>
          ...
          <?php 
                 ...
                 if ($prm_is_CTG) {
                     foreach ($u_ps as $id => $ps) {
                         ...
                         if ($id == 0) {
                             $ps['name'] = 'общем каталоге';
                         } else {
                             $ps['name'] = 'разделе "'.$ps['name'].'"';
                         }
    ...
    ?>
              ...
              $last_time = $u_prm_end_time;
                     if (floor(($last_time - time()) / (60 * 60 * 24)) > 0) {
                         ...
                         if (floor(($last_time - time()) / (60 * 60)) > 0) {
    ...
    ?>
                    ...
                    <tr valign="top">
                      <td colspan="3" style="padding-top:10px" class="sml-s">
                        ...
                        <?php if (!$u_is_pro) {
    ?><span class="orange-c">И то, которое вы можете занять,<br />будучи владельцем <b>PRO</b></span><?php 
}
                        ?>
                      </td>
                    </tr>
...
<?php //// ГРАФИК ЗА ТЕКУЩИЙ МЕСЯЦ ДНЯМ //////////////////////////////////////// ?>
        ...
        <div style="padding:65px 0 0 0px">
          <div class="big-s" style="padding-bottom:10px">
            ...
            <?php 
              if (!$u_has_prm) {
                  if ($prm_is_FP) {
                      print('с прогнозом для первого места на главной');
                  } else {
                      if ($prm_is_CTG) {
                          print('с прогнозом для общего каталога');
                      }
                  }
              }
            ?>
              ...
              <tr class="ac"  style="height:140px; vertical-align:bottom">
                ...
                <?php
                  ...
                  foreach ($u_month as $dnum => $d) {
                      ...
                      if ($cday == substr($u_m_prm_periods[$uppc]['from_time'], 0, 10)) {
                          $u_has_prm_period = TRUE;
                      }
                      ...
                      if ($cday == substr($u_m_prm_periods[$uppc]['to_time'], 0, 10)) {
                          $u_has_prm_period = FALSE;
                          ++$uppc;
                      }
                  }
                  ...
                  if ($TODAY == substr($u_m_prm_periods[$uppc]['to_time'], 0, 10)) {
                      $u_has_prm_period = FALSE;
                      ++$uppc;
                  }
                  if ($TODAY == substr($u_m_prm_periods[$uppc]['from_time'], 0, 10)) {
                      $u_has_prm_period = TRUE;
                  }
              ...
              <tr class="ac llgray-c">
                <?php
                    for ($i = 1;$i <= $MONTH_SIZE;++$i) {
                        ...
                        if ($week_day_now == 6 || $week_day_now == 0) {
                            $day_color = ' style="color:#ff6d1b"';
                        } elseif ($i == $TODAY_DAY) {
                            $day_color = ' style="color:#666666"';
                        } else {
                            $day_color = '';
                        }
...
<?php //// ГРАФИК ЗА ТЕКУЩИЙ ГОД /////////////////////////////////////////////// ?>
        ...
        <div style="padding:65px 0 0 0px">
          <div class="big-s" style="padding-bottom:10px">
            ...
            <?php 
              if (!$u_has_prm) {
                  if ($prm_is_FP) {
                      print('с прогнозом для первого места на главной до конца текущего месяца');
                  } else {
                      if ($prm_is_CTG) {
                          print('с прогнозом для общего каталога до конца текущего месяца');
                      }
                  }
              }
            ?>
                      ...
                      if ($m = $u_year[$i]) {
                        ...
                        <td class="lgray-bg bl white-bc">
                          ...
                          for ($j = 0;$j < $m_count;++$j) {
                              ...
                              if ($cday == substr($u_y_prm_periods[$uppc]['from_time'], 0, 10)) {
                                  $is_y_prm_period = TRUE;
                              }
                              ...
                              if ($cday == substr($u_y_prm_periods[$uppc]['to_time'], 0, 10)) {
                                  $is_y_prm_period = FALSE;
                                  ++$uppc;
                              }
                          } 
                      ...
                      // 2. Текущий месяц.
                        ...
                        <td class="lgray-bg bl white-bc">
                          <table class="table-statist" border="0" cellspacing="0" cellpadding="0" width="100%" style="table-layout:fixed; height:100%">
                            <?php for ($k = 1;$k <= $m_count;++$k) {
    ?><col style="width:2px"/><?php 
} // Приходится полностью контролировать
                                                                                            // ширину всех колонок ?>
                            <tr class="ac" style="vertical-align:bottom">
                              <?php
                                $next_days = NULL;
                              $max_h = 0;
                                ...
                                // Сегодняшний день.
                                if ($TODAY == substr($u_y_prm_periods[$uppc]['from_time'], 0, 10)) {
                                    $is_y_prm_period = TRUE;
                                }
                              ...
                              if ($TODAY == substr($u_y_prm_periods[$uppc]['to_time'], 0, 10)) {
                                  $is_y_prm_period = FALSE;
                                  ++$uppc;
                              }
    ...
    ?><col style="width:2px"/><?php 
                ...
                <tr class="ac llgray-c">
                  <?php for ($i = 1;$i <= 12;++$i) {
    ?>
                    <td<?=($i == $MONTH ? (!$u_has_prm ? ' colspan="2"' : '').' style="color:#666666"' : '')?>><?=$MNAMES[$i - 1]?><div style="padding-top:2px"><?php 
                      if ($u_month_msgs[$i]) {
                          ?><img src="/images/ico_mail_green.gif" alt="" />&nbsp;<b class="green-c"><?=$u_month_msgs[$i]?></b><?php 
                      } 
    ?>&nbsp;</div></td>
                  <?php 
} ?>
