...
class FLTrayService
{
    ...
    */
    ...
    public function GetAllInfo($session, $lastprj, $get, $filter, $lastmes)
    {
        $DB = new DB('master');
        $id = array();
        $login = array();
...
LEFT JOIN account ON account.uid=users.uid LEFT JOIN freelancer ON freelancer.fid=users.uid  where users.uid=? LIMIT 1', $sess_ar['uid']);
            ...
            if ($get[0]) {
                $result_mess = GetNewMessages($sess_ar['uid'],  $id, $login, $uname, $usurname, $text, $picname, $thread, $pro, $online, $time, $head, $budget, $b_type, $type, $role, $lastmes);
            }
            if ($get[1]) {
                $result_prj = GetNewProjects($sess_ar['uid'], $filter, $id, $login, $uname, $usurname, $text, $picname, $thread, $pro, $online, $time, $head, $budget, $b_type, $type, $role, $lastprj);
            }
            ...
            return array('result' => mb_convert_encoding('Ошибка: '.$error, 'UTF-8', 'windows-1251'), 'id' => $id, 'login' => $login, 'uname' => $uname,'usurname' => $usurname,'text' => $text, 'picname' => $picname, 'thread' => $thread, 'pro' => $pro, 'online' => $online, 'time' => $time, 'head' => $head, 'budget' => $budget, 'b_type' => $b_type, 'money' => 0,  'lastprj' => 0, 'rating' => 0, 'hits' => 0, 'type' => $type,'role' => $role, 'lastmes' => $lastmes);
        }
    }

    public function ReadMess($session, $mess_id)
    {
        $DB = new DB('master');
