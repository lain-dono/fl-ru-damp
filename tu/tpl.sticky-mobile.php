...
?>
    ...
    ?>
                   <a href="<?php echo tservices_helper::edit_link($user_obj->login, $data['id']);
    ?>" class="b-button b-button_flat b-button_flat_green">
                       Редактировать
                   </a>
                   <?php if ($is_adm) {
    ?>
                   <a id="__tservices_blocked" href="javascript:void(0);" onclick="banned.delReason('22_<?php echo $data['id'] ?>_0', 0, '', 0);" class="b-button b-button_flat b-button_flat_red <?php if ($data['is_blocked'] == 't') {
    ?>b-button_hide<?php 
}
    ?>">
                       Заблокировать
                   </a>
                   <a id="__tservices_unblocked" href="javascript:void(0);" onclick="banned.unBlocked('22_<?php echo $data['id'] ?>_0');" class="b-button b-button_flat b-button_flat_orange <?php if ($data['is_blocked'] == 'f') {
    ?>b-button_hide<?php 
}
    ?>">
                       Разблокировать
    ...
    ?>
                   <a <?php if (is_emp()) {
    ?>href="javascript:void(0)" onclick="TServices.showPopup();"<?php 
} else {
    ?>href="/registration/?type=empl&user_action=tu"<?php 
}
    ?> class="b-button b-button_flat <?php if (/*is_emp()*/true) {
