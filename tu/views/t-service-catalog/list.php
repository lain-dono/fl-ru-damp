 ...
 *
 *
 * @var TServiceCatalogController
 * @var array                     название выбранной категории
 * @var int                       общее количество карточек по параметрам поиска
 * @var bool                      true если искали, но ничего не нашли
 * @var array                     найденные карточки типовых услуг
 * @var int                       номер страницы
 * @var int                       количество карточек на странице
 * @var string                    базовый URL без номера текущей страницы для пейджинга
 */
...
<div class="b-layout__txt b-layout__txt_float_right b-layout__txt_padtop_5 b-layout__txt_padbot_20 b-layout__txt_float_none_iphone b-layout__txt_bold">
 ...
 <span class="i-shadow">
   ...
   <div class="b-shadow b-shadow_pad_10 b-shadow_width_140 b-shadow_top_20 b-shadow_left_-30 b-shadow_hide">
      <?php $i = 0;
    foreach ($orders as $orderKey => $orderTitle) {
        ++$i;
       ...
       <div class="b-layout__txt b-layout__txt_weight_normal<?=$padbot?>">
           <?php if ($useLink) {
    ?><a class="b-layout__link b-layout__link_bordbot_dot_0f71c8 tu-order-link"  onclick="TServices_Catalog.changeFilterOrder('<?=$orderKey?>');return false;" href="javascript:void(0)"><?php 
}
        ?>
           <?=$orderTitle?>
           <?php if ($useLink) {
...
<aside class="b-layout__side b-layout__side_content b-layout__side_padtop_10">
    ...
    ?>
		<div class="b-post b-post_padtop_60">
            ...
            if (is_emp() && is_pro()) {
                print '&utm_source=tu_catalog&utm_medium=emp&utm_content=pro&utm_campaign=btn_new_project';
            } elseif (is_emp() && !is_pro()) {
                print '&utm_source=tu_catalog&utm_medium=emp&utm_content=unpro&utm_campaign=btn_new_project';
            } elseif (!is_emp() && is_pro() && get_uid()) {
                print '&utm_source=tu_catalog&utm_medium=frl&utm_content=pro&utm_campaign=btn_new_project';
            } elseif (!is_emp() && !is_pro() && get_uid()) {
                print '&utm_source=tu_catalog&utm_medium=frl&utm_content=unpro&utm_campaign=btn_new_project';
            } elseif (!get_uid()) {
                print '&utm_source=tu_catalog&utm_medium=uauth&utm_content=unpro&utm_campaign=btn_new_project';
            }
    ...
    ?>
		...
		<article class="b-post <?php if (!$tservice['is_binded']): ?>b-post_margbot_20<?php endif;
    ?> b-layout_box" itemscope itemtype="http://schema.org/ImageObject">
			<span class="i-pic i-pic_fl">
				<a href="<?=$tservice_url?>" class="b-pic__lnk b-pic__lnk_relative">
					<div class="b-pic__bord_e6e6e6"></div>
					<?php if ($hasVideo) {
    ?><div class="b-icon b-icon__play b-icon_absolute b-icon_bot_4 b-icon_right_4"></div><?php 
}
    ?>
					<?=$thumbnail100x75?>
