...
<?php foreach ($feedbacks as $feedback) {
    ?>
    <article id="feedback-<?php echo $feedback['id'];
    ?>" class="b-post b-post_margbot_30 b-user b-user_grey">
    ...
    ?>
            <a class="b-post__anchor b-post__anchor_margleft_10" href="#feedback-<?php echo $feedback['id'];
    ?>"></a>
        </div> 
        <div class="b-icon b-icon__cat_<?php if ($feedback['rating'] >= 0) {
    ?>thumbsup b-icon_top_-3<?php 
} else {
    ?>thumbsdown<?php 
}
    ?> b-icon_float_right b-icon_margright_10"></div>
        
        <a class="b-user__link b-user__link_bold b-user__link_color_55b02e" href="/users/<?php echo $feedback['login'];
    ?>/" title="<?php if (!empty($feedback['login'])) {
    echo $feedback['uname'].' '.$feedback['usurname'];
} else {
    echo $feedback['login'];
}
    ?>">
            <img class="b-user__pic" width="50" height="50" alt="<?php if (!empty($feedback['login'])) {
    echo $feedback['uname'].' '.$feedback['usurname'];
} else {
    echo $feedback['login'];
}
    ?>" src="<?php echo tservices_helper::photo_src($feedback['photo'], $feedback['login']) ?>"> 
            <?php if (!empty($feedback['login'])) {
    ...
    ?>
        <div class="b-txt b-txt_padtop_5 b-txt_fs_11 b-txt_padleft_60">
            <?php echo $feedback['country_name'];
    ?><?php if ($feedback['city']) {
    echo ', '.$feedback['city_name'];
}
    ?>
        </div>
        <?php 
