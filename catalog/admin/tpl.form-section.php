...
<!--                        <option value="0">Не выбрано</option>-->
                    ...
                    <select name="subdomain" <?= ($seo->subdomain['id'] > 0) ? 'disabled' : ''?>>
                      ...
                      foreach ($countries as $country) {
                          ...
                          foreach ($subdomains as $key => $subdomain) {
                              if ($subdomain['country_id'] != $country['id']) {
                                  continue;
                              }
    ...
    foreach ($sections as $section) {
        if ($parent_section == $section['id']) {
            $parent_pos_num = $section['pos_num'];
        }
        ?>
                        <option value="<?= $section['id']?>" <?= ($parent_section == $section['id'] ? 'selected="selected"' : '');
        ?>><?=$section['name_section']?></option>
                        <?php 
    }
} //foreach ?>
                      </select> 
                  ...
                  <div class="form-group">
                    ...
                    <div class="form-value form-select">
                        <select name="new_position" id="new_position" <?= ($disabled_position ? 'disabled' : '')?>>
                            <?php for ($p = 1;$p <= $positions;++$p) {
    if ($form_section['pos_num'] == $p) {
        $selected = 'selected';
        $is_selected = true;
    } else {
        $selected = '';
