    ...
    ?>
        <div class="block-seo-links">
            <div class="form fs-o">
                ...
                <div class="form-in">
                    <ul>
                        <?php for ($i = 0;$i < count($dinamic_content_articles);++$i) {
    if ($i == 3) {
        $continue = true;
        break;
    }
...
?>    
    ...
    ?>
          <li class="all-employers"><span class="wrap-item"><a 
                      href="<?=seo::getFriendlyURL($subdomain, $direction['name_section_link'], '');
    ?>"><?= reformat($direction['dir_name']) ?></a></span></li>
      <?php $grnum = 0;
    if ($sections[$direction['id']]) {
        foreach ($sections[$direction['id']] as $key => $section) {
            ...
            $count_section = count($section['subsection']);
            ?>
        <li>
    ...
    if ($section_content['id'] == $subsection['id'] || $section['id'] == $catid) {
        $gr_show = $grnum;
    }
    ?>
            <?php if ($k == 0) {
    ?><script type="text/javascript">initCI('submenu<?=$section['id']?>')</script><?php 
}//if?>
            ...
            <li <?= ($section_content['id'] == $subsection['id'] ? 'class="active"' : '');
    ?>> <a href="<?=seo::getFriendlyURL($subdomains[$subsection['subdomain_id']]['subdomain'], $direction['name_section_link'], $subsection['name_section_link'], $section['name_section_link'])?>"><?= $subsection['name_section']?></a></li>
            <?php ++$k;
}// foreach?>
          <?php 
} //if?>
      ...
      <?php if ($direct_id) {
    ?>
      <li class="all-employers"><span class="wrap-item"><a href="<?=seo::getFriendlyURL($subdomain, '', '');
