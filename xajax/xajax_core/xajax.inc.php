...
class xajax
{
    ...
    */
    public function xajax($sRequestURI = null, $sLanguage = null)
    {
        $GLOBALS['xajax'] = $this;
...
//EndSkipAIO
    ...
    */
    public function configure($sName, $mValue)
    {
        ...
        } elseif ('exitAllowed' == $sName) {
            if (true === $mValue || false === $mValue) {
                $this->bExitAllowed = $mValue;
            }
        } elseif ('cleanBuffer' == $sName) {
            if (true === $mValue || false === $mValue) {
                $this->bCleanBuffer = $mValue;
            }
        } elseif ('logFile' == $sName) {
        ...
        if (!file_exists($realJsFile) || true == $bAlways) {
            ...
            if (!file_exists($srcFile)) {
                trigger_error(
                    ...
                    .dirname($realJsFile)
                    .$this->objLanguageManager->getText('CMPRSJS:RDERR:02'), E_USER_ERROR
                    );
            }
            ...
            if (!$fH) {
                trigger_error(
                    ...
                    .dirname($realJsFile)
                    .$this->objLanguageManager->getText('CMPRSJS:WTERR:02'), E_USER_ERROR
                    );
            } else {
    ...
    public function _compressSelf($sFolder = null)
    {
        ...
        if ($handle = opendir($sFolder)) {
            while (!(false === ($sName = readdir($handle)))) {
                ...
                } elseif (8 < strlen($sName) && 0 == strpos($sName, '.compressed')) {
                        ...
                        if (file_exists($sPath)) {
                            ...
                            foreach (array_keys($aFile) as $sKey) {
                                if ('//SkipDebug' == $aFile[$sKey]) {
                                    ++$nSkip;
                                } elseif ('//EndSkipDebug' == $aFile[$sKey]) {
                                    --$nSkip;
                                } elseif (0 == $nSkip) {
                                    $aParsed[] = $aFile[$sKey];
                                }
                            ...
                            if (!$fH) {
                                trigger_error(
                                    ...
                                    .$sNewPath
                                    .$this->objLanguageManager->getText('CMPRSPHP:WTERR:02'), E_USER_ERROR
                                    );
                            } else {
                                fwrite($fH, $compressedScript);
                                fclose($fH);
                            ...
                            if (file_exists($sPath)) {
                                ...
                                foreach (array_keys($aFile) as $sKey) {
                                    if ('//SkipDebug' == substr($aFile[$sKey], 0, 11)) {
                                        ++$nSkip;
                                    } elseif ('//EndSkipDebug' == substr($aFile[$sKey], 0, 14)) {
                                        --$nSkip;
                                    } elseif ('//SkipAIO' == substr($aFile[$sKey], 0, 9)) {
                                        ++$nSkip;
                                    } elseif ('//EndSkipAIO' == substr($aFile[$sKey], 0, 12)) {
                                        --$nSkip;
                                    } elseif ('<'.'?php' == substr($aFile[$sKey], 0, 5)) {
                                    } elseif ('?'.'>' == substr($aFile[$sKey], 0, 2)) {
                                    } elseif (0 == $nSkip) {
                                        $aParsed[] = $aFile[$sKey];
                                    }
                                }
        ...
        if ($bWriteFile) {
            ...
            if (!$fH) {
                trigger_error(
                    ...
                    .$sFolder
                    .$this->objLanguageManager->getText('CMPRSAIO:WTERR:02'), E_USER_ERROR
                    );
            } else {
                fwrite($fH, '<'.'?php ');
