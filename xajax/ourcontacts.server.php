...
function AddContact($frm)
{
    ...
    if (hasPermissions('ourcontacts')) {
        ...
        if ($frm['fld_add_email']) {
            array_push($emails, $frm['fld_add_email']);
        }
        for ($i = 1; $i < 5; ++$i) {
            $frm['fld_add_email_'.$i] = trim(strip_tags(stripslashes($frm['fld_add_email_'.$i])));
            if ($frm['fld_add_email_'.$i]) {
                array_push($emails, $frm['fld_add_email_'.$i]);
            }
        }
        reset($emails);
        if (!$emails) {
        ...
        for ($i = 1; $i < 5; ++$i) {
            ...
            if ($frm['fld_add_phone_'.$i]) {
                array_push($phones, $frm['fld_add_phone_'.$i]);
            }
        }    

        $skypes = array();
        $frm['fld_add_skype'] = trim(strip_tags(stripslashes($frm['fld_add_skype'])));
        if ($frm['fld_add_skype']) {
            array_push($skypes, $frm['fld_add_skype']);
        }
        for ($i = 1; $i < 5; ++$i) {
            $frm['fld_add_skype_'.$i] = trim(strip_tags(stripslashes($frm['fld_add_skype_'.$i])));
            if ($frm['fld_add_skype_'.$i]) {
                array_push($skypes, $frm['fld_add_skype_'.$i]);
            }
        }    
        ...
        if ($frm['fld_add_icq']) {
            array_push($icqs, $frm['fld_add_icq']);
        }
        for ($i = 1; $i < 5; ++$i) {
            $frm['fld_add_icq_'.$i] = trim(strip_tags(stripslashes($frm['fld_add_icq_'.$i])));
            if ($frm['fld_add_icq_'.$i]) {
                array_push($icqs, $frm['fld_add_icq_'.$i]);
            }
        }    

        $others = array();
        $frm['fld_add_other'] = trim(strip_tags(stripslashes($frm['fld_add_other'])));
        if ($frm['fld_add_other']) {
            array_push($others, $frm['fld_add_other']);
        }
        for ($i = 1; $i < 5; ++$i) {
            $frm['fld_add_other_'.$i] = trim(strip_tags(stripslashes($frm['fld_add_other_'.$i])));
            if ($frm['fld_add_other_'.$i]) {
                array_push($others, $frm['fld_add_other_'.$i]);
            }
        }    
