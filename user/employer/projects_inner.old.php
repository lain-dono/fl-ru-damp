...
if ($prjs) {
    foreach ($prjs as $ikey => $prj) {
        if ($prj['pro_only'] == 't' && $_SESSION['login'] != $user->login && !$_SESSION['pro_last'] && !hasPermissions('projects')) {
            ?><!--<tr class="qpr"><td class="qp" align="center" colspan="3"><h2>Только для про</h2></td></tr>--><?php
  ...
  <?php if (is_new_prj($prj['post_date'])) {
    ?>
	...
	<td width="30"><?php if ($prj['closed'] == 't') {
    ?>Проект закрыт<?php 
} else {
    ?><a id="prj<?=$prj['id']?>" name="prj<?=$prj['id']?>" href="/projects/?pid=<?=$prj['id']?>" class="org">Подробнее</a><?php 
}
            ?>
	</td>
	</tr>
