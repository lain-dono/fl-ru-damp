...
<?php if ($note_version == 2) {
    ?>
  <div style="padding-bottom:10px" id="notetd<?=$inc?>">
    <b>Ваша заметка:</b><br/>
    <div id="notetext<?=$inc?>"><?=$text?></div>
    <?php if ($_SESSION['uid']) {
    ?><a href="javascript:UpdateNote(<?=$inc?>,2,'<?=$textarea_w?>')" class="blue"><?php 
} else {
    ?><a href="/fbd.php" class="blue"><?php 
}
    ?>Изменить</a>
  </div>
<?php 
    ...
    ?>
  <td class="note<?=$s_role?> note-cont" id="notetd<?=$inc?>"<?php if ($rowspan) {
    ?> rowspan="<?=$rowspan?>"<?php 
}
    ?>>
