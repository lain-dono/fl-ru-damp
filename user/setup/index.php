    ...
    //www.x.ru" onclick="alert(12345)
        ...
        // more jabber
        	...
        	if ($frl->icq = change_q(substr(strip_tags(trim($_POST['icq'])), 0, 96), true)) {
        	    if (!preg_match("/^[-0-9\s]*$/", $frl->icq) && !is_email($frl->icq)) {
        	        $error_flag = 1;
        	        $alert[2] = 'Поле заполнено некорректно';
        	    }
        	}
		if ($frl->icq_1 = change_q(substr(strip_tags(trim($_POST['icq_1'])), 0, 96), true)) {
		    if (!preg_match("/^[-0-9\s]*$/", $frl->icq_1) && !is_email($frl->icq_1)) {
		        $error_flag = 1;
		        $alert[31] = 'Поле заполнено некорректно';
		    }
		}
		if ($frl->icq_2 = change_q(substr(strip_tags(trim($_POST['icq_2'])), 0, 96), true)) {
		    if (!preg_match("/^[-0-9\s]*$/", $frl->icq_2) && !is_email($frl->icq_2)) {
		        $error_flag = 1;
		        $alert[32] = 'Поле заполнено некорректно';
		    }
		}
		if ($frl->icq_3 = change_q(substr(strip_tags(trim($_POST['icq_3'])), 0, 96), true)) {
		    if (!preg_match("/^[-0-9\s]*$/", $frl->icq_3) && !is_email($frl->icq_3)) {
		        $error_flag = 1;
		        $alert[32] = 'Поле заполнено некорректно';
		    }
		}
...
//        	  if (!preg_match($icq_regexp, $frl->icq_3)) { $error_flag = 1; $alert[33] = "Поле заполнено некорректно"; }
        ...
        if ($frl->site = change_q(substr(addhttp(trim($_POST['site'])), 0, 96), true)) {
            if (!url_validate($frl->site, true)) {
                $error_flag = 1;
                $alert[11] = 'Поле заполнено некорректно';
            }
        }
        ...
        if ($frl->second_email = change_q(substr(trim($_POST['second_email']), 0, 96), true)) {
            if (!is_email($frl->second_email)) {
                $error_flag = 1;
                $alert[10] = 'Поле заполнено некорректно';
            }
        ...
        if ($frl->email_1 = change_q(substr(trim($_POST['email_1']), 0, 96), true)) {
            if (!is_email($frl->email_1)) {
                $error_flag = 1;
                $alert[21] = 'Поле заполнено некорректно';
            }
        ...
        if ($frl->email_2 = change_q(substr(trim($_POST['email_2']), 0, 96), true)) {
            if (!is_email($frl->email_2)) {
                $error_flag = 1;
                $alert[22] = 'Поле заполнено некорректно';
            }
        ...
        if ($frl->email_3 = change_q(substr(trim($_POST['email_3']), 0, 96), true)) {
            if (!is_email($frl->email_3)) {
                $error_flag = 1;
                $alert[23] = 'Поле заполнено некорректно';
            }
        ...
        if ($frl->phone_1 = change_q(substr(trim($_POST['phone_1']), 0, 24), true)) {
            if (!preg_match('/^[-+0-9)( #]*$/', $frl->phone_1)) {
                $error_flag = 1;
                $alert[61] = 'Поле заполнено некорректно';
            }
        }
        if ($frl->phone_2 = change_q(substr(trim($_POST['phone_2']), 0, 24), true)) {
            if (!preg_match('/^[-+0-9)( #]*$/', $frl->phone_2)) {
                $error_flag = 1;
                $alert[62] = 'Поле заполнено некорректно';
            }
        }
        if ($frl->phone_3 = change_q(substr(trim($_POST['phone_3']), 0, 24), true)) {
            if (!preg_match('/^[-+0-9)( #]*$/', $frl->phone_3)) {
                $error_flag = 1;
        ...
        if (strlen($resumetxt) > 4000) {
            $error_flag = 1;
            $alert[8] = 'Количество знаков превышает допустимое значение';
        } else {
		...
		if (strlen($_POST['konk']) > 4000) {
		    $error_flag = 1;
		    $alert[9] = 'Количество знаков превышает допустимое значение';
		} else {
        ...
        if (!preg_match('/^[a-zA-Z0-9_-]*$/', $frl->ljuser)) {
            $error_flag = 1;
            $alert[12] = 'Поле заполнено некорректно';
        }
...
//            $link = stripslashes(trim($_POST['link']));
	    ...
	    if ($prj_id) {
			 ...
			 * Проверка значений.
			 */
	    if ($img->error[0] && $img->name) {
	        $error_flag = 1;
	        $alert[3] = 'Слишком большой файл';
	    }

        // --

	    if (!$name || (strlen(trim(stripslashes($_POST['pname']))) > 80)) {
	        $error_flag = 1;
	        if ($is_video == 'f') {
	            $enum = 1;
	        } else {
