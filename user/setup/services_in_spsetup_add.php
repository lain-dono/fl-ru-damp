...
for ($i = 0, $a = 0, $p = 0, $j = 0, $aa = $add_cnt; $i < $all_cnt; ++$i) {
    if ($add_cnt < $free_cnt && $i < $free_cnt - $add_cnt) {
        $prof = array();
    } else {
        if ($add_cnt && $a < $add_cnt) {
            $prof = $add_specs[$a++];
            --$aa;
        } else {
            if ($paid_cnt) {
...
foreach ($specs as $i => $spec) {
    if (isset($spec['prof_id']) && isset($specs[($i - ($spec['paid_id'] ? 0 : 1))]['prof_id'])) {
        $specs[$i]['top'] = true;
    }
    if (isset($spec['prof_id']) && isset($specs[($i + ($spec['paid_id'] ? 1 : 0))]['prof_id'])) {
        $specs[$i]['bot'] = true;
    }
...
var CATEGORIES={<?php // категории/подкатегории: {ид_кат:{имя_кат:{ид_подкат:имя_подкат,ид_подкат:...}},ид_кат:...}
            ...
            <table class="tbl-eight-specs" id="specsTbl">
                ...
                <tr>
                    ...
                    <td class="es-c2"><span class="b-page__ipad b-page__iphone"><?=($i + 1)?>.</span>
                        <a href="javascript:;" onclick="moveSpec(this, -1)" style="display:<?= $prof['top'] ? '' : 'none' ?>;"><img src="/images/arrow2-top.png" alt="Вверх" /></a><?php
                        ?><img style="display:<?= $prof['top'] ? 'none' : '' ?>;" src="/images/arrow2-top-a.png" alt="Вверх" /><?php
                        ?><a href="javascript:;" onclick="moveSpec(this, +1)" style="display:<?= $prof['bot'] ? '' : 'none' ?>;"><img src="/images/arrow2-bottom.png" alt="Вниз" /></a><?php
