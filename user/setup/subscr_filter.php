    ...
    <div id="filter_body" class="b-layout__txt b-layout__txt_padbot_10">
        ...
        foreach (explode(':', $user->mailer_str) as $vl) {
            if (preg_match('/c([0-9]+)s?([0-9]*)/i', $vl, $res)) {
                ...
                ?>" />
                    <span class="b-layout__bold"><?= $cat_name;
                ?></span> &#160;
                    <?= $sub_name;
                ?> &#160;
                    <a href="javascript:void(0)" onclick="xajax_removeSubscFilter(<?= (int) $res[1];
                ?>,<?= (int) $res[2];
                ?>); unset(<?= (int) $res[1];
                ?>,<?= (int) $res[2];
                ?>); document.getElementById('filter_body').removeChild(this.parentNode);"><img class="b-layout__pic" src="/images/btn-remove2.png" alt="Удалить" /></a>
                </div>
                <?php
