...
var mirrors = [];
<?php for ($i = 0, $m = count($mirrors); $i < $m; ++$i) {
    ?>
...
<form id="frm" action="/users/<?=$user->login?>/setup/portfsetup/" method="post">
          ...
          <td class="b-layout__td b-layout__td_width_33ps b-layout__td_padbot_20 b-layout__td_width_full_ipad">
            <?php 
              ...
              foreach ($DB->rows("SELECT DISTINCT P.prof_id,M.mirror_prof  FROM portfolio P
                  ...
                  if ((int) $value['mirror_prof']) {
                      $list[] = (int) $value['mirror_prof'];
                  }
              }
              ...
              foreach ($profs as $prof) {
                  if ($lastgrname != $prof['groupname']) {
                      ...
                      ?><h3 class="b-layout__h3"><?=$prof['groupname']?></h3><?php
                  ++$j;
                      $lastgrname = $prof['groupname'];
                  }
                  ?>
                    <div class="b-check b-check_padbot_5">
                       <input class="b-check__input" type="checkbox" <?= $prof['checked'] && in_array($prof['id'], $list) ? 'disabled="disabled"' : '';
                  ?> name="prof[]" value="<?=$prof['id']?>" onchange="checkMirrors(this.value);" id="lb<?=$prof['id']?>"<?=($prof['checked'] ? 'checked="checked"' : '')?> />
    ...
    <div class="b-buttons">
	   <a class="b-button b-button_flat b-button_flat_green b-button_float_right" onclick="$('frm').submit();"><?php if ($_SESSION['text_spec']) {
    ?>Сохранить и добавить новую работу<?php 
} else {
    ?>Сохранить изменения<?php 
}//else?></a>
    </div>
