...
<form action="." method="post" id="subscr_form">
        ...
        <div class="b-layout b-layout_padtop_20">
            ...
            <ul class="settings-subscribe">
                <li><label><input type="checkbox" id="ch1" name="newmsgs" class="i-chk" value="1" <?php if ($user->subscr[0]) {
    print "checked='checked'";
} ?> /> Уведомления о новых сообщениях</label></li>
            <li><label><input type="checkbox" id="ch6" name="commune" class="i-chk" value="1" <?php if ($user->subscr[5]) {
    print "checked='checked'";
} ?> /> Уведомления о новых действиях в сообществах</label></li>
            <li><label><input type="checkbox" class="i-chk" id="ch7" name="commune_topics" value="1" <?php if ($user->subscr[6]) {
    print "checked='checked'";
} ?> onclick="this.getParent('li').getElements('ul input').set('checked', this.get('checked'))" /> Уведомления о новых темах в сообществах</label>
...
<?php foreach ($communes as $ckey => $comm) {
    if ($comm['id'] != 0) {
        ?>
                                    <li><label><input type="checkbox" class="i-chk" name="comm[]" value="<?= $comm['id'] ?>" id="chcomm<?= $comm['id'] ?>" <?php if ($user->subscr[6] && in_array($comm['id'], $c_signed)) {
    print "checked='checked'";
}
        ?> onclick="this.get('checked') && $('ch7').set('checked', 1)" /> <?= $comm['name'] ?></label></li>
<?php 
    }
}
    ?>
...
<?php 
                    ...
                    <li>
                        <label>
                            <input type="checkbox" onclick="togglePrj(this)" class="i-chk"  id="ch2" name="vacan"  value="1" <?php if ($user->mailer) {
    print "checked='checked'";
} ?> /> 
                    ...
                    <li><label><input type="checkbox" class="i-chk" id="ch9" name="contest"  value="1" <?php if ($user->subscr[8]) {
    print "checked='checked'";
} ?> /> Уведомления в конкурсах</label></li>
                    <?php if (BLOGS_CLOSED == false) {
    ?>
                    <li><label><input type="checkbox" class="i-chk" id="ch3" name="comments"  value="1" <?php if ($user->subscr[2]) {
    print "checked='checked'";
}
    ?> /> Комментарии к сообщениям/комментариям в блогах</label></li>
                    <?php 
} ?>
                    <li><label><input type="checkbox" class="i-chk" id="ch4" name="prcomments" value="1"   <?php if ($user->subscr[4]) {
    print "checked='checked'";
} ?> /> Комментарии к сообщениям/комментариям в проектах</label></li>
                    <li><label><input type="checkbox" class="i-chk" id="ch8" name="adm_subscr"  value="1" <?php if ($user->subscr[7]) {
    print "checked='checked'";
} ?> /> Новости от команды FL.ru</label></li>
                    <li><label><input type="checkbox" class="i-chk" id="ch16" name="payment" value="1" <?php if ($user->subscr[15]) {
    print " checked='checked'";
} ?> /> Уведомления личного счета</label></li>
                    <li><label><input type="checkbox" class="i-chk" id="ch10" name="team"  value="1" <?php if ($user->subscr[9]) {
    print "checked='checked'";
} ?> /> Уведомления о добавлении в избранные</label></li>
                    <li><label><input type="checkbox" class="i-chk" id="ch13" name="massending" value="1" <?php if ($user->subscr[12]) {
    print "checked='checked'";
} ?> /> Получать платные рассылки</label></li>
