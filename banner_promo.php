...
<?php if ($banner_promo_show == true): ?>
    ...
    ?>
            <div class="<?= $banner_promo_class ?>">
                <noindex><a rel="nofollow" target="_blank" onclick="javascript:writeClickStat(<?= $banner_promo_type ?>)" href="<?= $banner_link ?>" class="<?= $banner_promo_link_class ?>" <?php if (trim($banner_promo_link_style) != '') {
    ?>style="<?php print $banner_promo_link_style ?>"<?php 
}
    ?>> <?php print $banner_promo_text ?></a></noindex>
            </div>
        <?php 
} elseif (trim($banner_promo_img) != '' && trim($banner_promo_text) == '') {
    ?>
            <div class="<?= $banner_promo_class ?>">
                <noindex><a rel="nofollow" target="_blank" onclick="javascript:writeClickStat(<?= $banner_promo_type ?>)" href="<?= $banner_link ?>" class="b-menu__link-pic" <?php if (trim($banner_promo_link_style) != '') {
    ?>style="<?php print $banner_promo_link_style ?>"<?php 
}
    ?>>
                    <img <?php if (trim($banner_promo_title) != '') {
    ?>alt="<?php print $banner_promo_title ?>" <?php print $banner_promo_image_style ?> title="<?php print $banner_promo_title ?>" <?php 
}
    ...
    <?php $bhtml = clearTextForJS(ob_get_clean()); ?><script type="text/javascript">document.write('<?= $bhtml ?>');</script>
    <?php if (isset($banner_outer_class)): ?>
        </div>
    <?php endif; ?>
<?php endif; ?>
