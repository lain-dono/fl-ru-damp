    ...
    case 'post_msg' :
        ...
        if ($_SESSION['uid'] && $reply && $thread) {
            ...
            if (!$error_flag && !$f_name) {
                ...
                //$error .= $err;
                if ($last_id) {
                    header('Location: '.getFriendlyURL('blog', $thread).'?pagefrom='.($_POST['onpage'] ? $_POST['onpage'] : $_POST['pagefrom']).'&openlevel='.$last_id.'&ord='.$_GET['ord'].($PDA ? '#o'.$last_id : ''));
                    exit;
                }
    ...
    case 'change':
        ...
        if ($_SESSION['uid'] && $reply && $thread) {
            ...
            if ($msg === '' && $question === '' && empty($alert[5]) && !($filecount || $attachedfiles_info['count']) && $yt_link === '') {
                $error_flag = 1;
                $alert[1] = 'Ошибка. Сообщение не должно быть пустым!';
...
if ($PDA) {
    if (isset($_GET['newcnt'])) {
        $content = 'new_cnt.php';
    }
    if (isset($_GET['editcnt'])) {
        $content = 'edit_cnt.php';
    }
}
...
if (empty($additional_header)) {
    $additional_header = '';
      ...
      $om_clean_uri = array();
        foreach ($_GET as $key => $value) {
            if ($value && !in_array($key, array('tr', 'b', 'ord', 'openlevel', 'newurl'))) {
