...
if ($ban_where != 1) {
        ...
        if ($site == 'journal') {
            ...
            if (!empty($_POST['answers_exists']) && is_array($_POST['answers_exists'])) {
                foreach ($_POST['answers_exists'] as $key => $answer_exist) {
                    $answers_exists[$key] = str_replace(array('"', "'", '\\'), array('&quot;', '&#039;', '&#92;'), stripslashes((string) $answer_exist));
                }
            }
            ...
            if ($alert || $error || $error_flag) {
                ...
                if (isset($_POST['question'])) {
                    $_SESSION['user.journal.new_tr.result']['question'] = $question;
                }
    ...
    case 'restore':
        ...
        $o_gr = $gr;
        if ($id && $uid) {
            $error = $blog_obj->RestoreMsg($uid, $id, $gr, $base, $null, $page, $from, $mod);
        }
    ...
    case 'delete':
        ...
        $o_gr = $gr;
        if ($id && $uid) {
            $error = $blog_obj->MarkDeleteBlog($uid, $id, $gr, $base, $null, $page, $from, $mod);
        }
        if (!($page = __paramInit('int', 'page'))) {
            $page = 1;
        }
        ...
        */
        ...
        if ($site == 'journal') {
            ...
            if ($alert || $error || $error_flag) {
                ...
                if (isset($_POST['question'])) {
                    $_SESSION['user.journal.change.result']['question'] = $question;
                }
...
$ord = trim($_GET['ord']);
if ($ord != 'best' && $ord != 'my' && $ord != 'relevant' && $ord != 'favs') {
    $ord = 'new';
}
