...
class Thread
{
  ...
  KIND_AVATAR => 'avatar',
    ...
    public function GetListThreads($operatorid, $q = null, $showEmpty = true, $nLimit = 15, $nOffset = 0)
    {
    ...
    public function GetListThreadsCount($operatorid, $q = null, $showEmpty = true)
    {
        $departmentsExist = MapperFactory::getMapper('Department')->departmentsExist();
    ...
    public function GetListThreadsAdv($operatorid, $q, $start, $end, $operator, $showEmpty = true, $departmentid, $locale, $rate, $offline, $nLimit = 15, $nOffset = 0)
    {
        $departmentsExist = MapperFactory::getMapper('Department')->departmentsExist();
    ...
    public function GetListThreadsAdvCount($operatorid, $q, $start, $end, $operator, $showEmpty = true, $departmentid, $locale, $rate, $offline)
    {
        $departmentsExist = MapperFactory::getMapper('Department')->departmentsExist();
    ...
    public function CommitThread($threadid, $params)
    {
    ...
    public function PostMessage($threadid, $kind, $message, $sendername = null, $utime = null, $operatorId = null, $feedbackButon = null)
    {
        $hash = array(
      'threadid' => $threadid,
      'kind' => $kind,
    ...
    public function isHasMessagesForOperator($threadid)
    {
        return is_has_threads(self::getOperatorHasMesagesFilename($threadid));
    }

    public function isHasMessagesForVisitor($threadid)
    {
        return is_has_threads(self::getVisitorHasMesagesFilename($threadid));
    }

    public function setThreadHasMessagesForOperator($threadid)
    {
        set_has_threads(self::getOperatorHasMesagesFilename($threadid));
    }

    public function setThreadHasMessagesForVisitor($threadid)
    {
        set_has_threads(self::getVisitorHasMesagesFilename($threadid));
    }

    public function unsetThreadHasMessagesForOperator($threadid)
    {
        unset_has_threads(self::getOperatorHasMesagesFilename($threadid));
    }

    public function unsetThreadHasMessagesForVisitor($threadid)
    {
        unset_has_threads(self::getVisitorHasMesagesFilename($threadid));
    }

    public static function getOperatorHasMesagesFilename($threadid)
    {
    ...
    public static function getVisitorHasMesagesFilename($threadid)
    {
    ...
    public function GetMessages($threadid, $meth, $isvisitor, &$lastid, $forceShowingRates = false)
    {
    ...
    public function CreateThread($lang, $stateid = STATE_LOADING, $additional = array())
    {
        $hash = array(
...
//    $stateid = self::$threadStateStrings[$thread['state']];
    ...
    public function BuildVisitorXml($visitor)
    {
        $xml = array();
    ...
    public function RenameVisitor($thread, $newname)
    {
        $visitSession = MapperFactory::getMapper('VisitSession')->getById($thread['visitsessionid']);
    ...
    public function SendVisitedPageForOpenThreads($visitorid, $page, $title)
    {
        if (empty($page)) {
            return;
        }
    ...
    public function PrintThreadMessages($thread, $token, $lastid, $isvisitor, $format, $viewonly = false)
    {
    ...
    public function GetReportByAgent($start, $end, $departmentid = null, $locale = null)
    {
    ...
    public function RateOperator($thread, $rate)
    {
        $crm = MapperFactory::getMapper('Rate');
        $crate = array(
          'threadid' => $thread['threadid'],
    ...
    public function GetThreadsByVisitSessionId($visitsessionid)
    {
    ...
    public function sendFirstMessageWithVisitorInfo($thread, $params = array())
    {
      ...
      array(
        ...
        $chats = self::getInstance()->CountNonEmptyThreads($visitSession['visitorid']);
        if ($chats > 0) {
            $message .= str_replace(array('%PARAM%', '%URL%', '%VALUE%'),
        ...
        self::getInstance()->PostMessage($thread['threadid'], KIND_FOR_AGENT, $message);
    }
