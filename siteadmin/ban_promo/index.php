...
function b_promo_validate(&$bannerData, $recId = 0)
{
    ...
    $a = explode('-', $from);
    $a = array_reverse($a);
    $bannerData->from = $from = implode('-', $a);
    $a = explode('-', $to);
    $a = array_reverse($a);
    $bannerData->to = $to = implode('-', $a);
...
if (isset($_GET['edit'])) {
    ...
    if ($bpromo->info['is_role'][0] == '1') {
        $isFrlChecked = 'checked="checked"';
    }
    if ($bpromo->info['is_role'][1] == '1') {
        $isEmpChecked = 'checked="checked"';
    }
    $inner_page = 'inner_edit.php';
    if (isset($_POST['save'])) {
        // проверка на заполненность полей
        $error = b_promo_validate($bpromo, (int) $_POST['id']);
        if (__paramInit('bool', null, 'advertising')) {
            $advChecked = 'checked="checked"';
        }        
        if (__paramInit('bool', null, 'is_activity')) {
            $activeChecked = 'checked="checked"';
        }
        if (!$error) {
...
if (isset($_GET['delete'])) {
    ...
    if ($deleted) {
        header('Location: /siteadmin/ban_promo/');
    }
}
...
if (isset($_GET['new'])) {
    ...
    if (isset($_POST['new'])) {
        ...
        if (__paramInit('bool', null, 'is_pro')) {
            $isPROChecked = 'checked="checked"';
        }
        if (__paramInit('bool', null, 'is_not_pro')) {
            $isNotPROChecked = 'checked="checked"';
        }
        if (__paramInit('bool', null, 'is_frl')) {
            $isFrlChecked = 'checked="checked"';
        }
        if (__paramInit('bool', null, 'is_emp')) {
            $isEmpChecked = 'checked="checked"';
        }
        if (!$error) {
            $login_access = __paramInit('string', null, 'login_access');
