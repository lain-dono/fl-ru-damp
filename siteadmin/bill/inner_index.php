...
<?php if (!is_release() || in_array($_SESSION['login'], $GLOBALS['balanceCanChangeAdmins'])) {
    ?>
...
<div class="money-filtr">
    ...
    ?>
                                      <option value="<?= $event['id']?>" <?= ($_POST['event'] == $event['id'] ? 'selected="selected"' : '');
    ?>><?= $event['op_name']?></option>
                                      <?php 
} //foreach?>
                                    </select>
                                </td>
    ...
    ?>
                                        <option value="<?= $limit?>" <?= ($_POST['limit'] == $limit ? 'selected="selected"' : '');
    ?>><?= $limit?></option>
                                        <?php 
}//foreach?>
                                    </select>
...
<tr class="small" valign="baseline">
	...
	<?php if ($hist['op_code'] == 23 && !($hist['is_blocked'] > 0)): ?> <a href="#" id="lock_<?=$hist['id']?>" onclick="xajax_BlockOperation(<?=$hist['id']?>); return false;" style="color:red;">Заблокировать</a><?php endif;
      ?>
	<?php if ($hist['is_blocked'] > 0): ?>| <a href="#" id="lock_<?=$hist['id']?>" onclick="xajax_UnBlockOperation(<?=$hist['id']?>); return false;" style="color:red;">Разблокировать<?php endif;
      ?>
	</td>
	<td><?=($comments ? (stripslashes(reformat($comments, 35, 0, 1)).'<br/>') : '')?>
	<?php	if ($hist['op_code'] < 7 || $hist['op_code'] == 12 || $hist['op_code'] == 8 || $hist['op_code'] == 10 || $hist['op_code'] == 11 || $hist['op_code'] == 23
