...
<?php if (empty($banned)) {
    print "<center><div style='font: bold 16px Tahoma'>Нет пользователей.</div></center>";
...
<tr>
    ...
    <?php if ($user['is_banned'] || $user['ban_where'] > 0) {
    ?>
        <?php if ($user['from']) {
    ?><div><b><?=((($user['ban_where'] == 1) ? 'Заблокирован в блогах' : 'Забанен').': '.dateFormat('d.m.Y H:i', $user['from']))?></b></div><?php 
}
    ?>
    </td>
    <td>
        ...
        <?php if ($user['admin_comment']) {
    ?><div><b>Комментарий модератора:</b> <?=$user['admin_comment']?></div><?php 
}
    ?>
    </td>
    <?php 
...
$sBanTitle = (!$user['is_banned'] && !$user['ban_where']) ? 'Забанить!' : 'Разбанить';
    ?>
<div class="warnlink-<?=$user['uid']?>"><a class="admn" href="javascript:void(0);" onclick="banned.userBan(<?=$user['uid']?>, 'admin',0)"><?=$sBanTitle?></a> <?php if ($user['admin']) {
    ?><span>Заблокировал: <?=$user['admin_login']?></span><?php 
}
    ?></div>
</td>
