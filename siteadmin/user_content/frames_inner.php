...
<head>
    ...
    <?php $stc->Add('/css/nav.css'); ?>
    <?php $stc->Add('/scripts/swfobject.js'); ?>
    <?php $stc->Add('/scripts/warning.js'); ?>
    <?php $stc->Add('/scripts/mootools-new.js'); ?>
    <?php $stc->Add('/scripts/mootools-more.js'); ?>
    <?php $stc->Add('/scripts/new_site.js'); ?>   
    <?php $stc->Add('/scripts/nav.js'); ?>
    <?php $stc->Add('/scripts/navigate.js'); ?>
    <?php $stc->Add('/scripts/new.js'); ?>
    <?php $stc->Add('/scripts/ajax_blocks.js'); ?>
    <?php $stc->Add('/scripts/csrf.js'); ?>
    <?php $stc->Add('/scripts/kwords.js'); ?>
    <?php $stc->Add('/kword_js.php'); ?>
    <?php $stc->Add('/professions_js.php'); ?>
    <?php $stc->Add('/cities_js.php'); ?>
    <?php $stc->Add('/kword_search_js.php?type=projects');?>
    <?php $stc->Add('/scripts/b-combo/b-combo-dynamic-input.js'); ?>
    <?php $stc->Add('/css/block/b-textarea/b-textarea.js'); ?>
    <?php $stc->Add('/scripts/b-combo/b-combo-multidropdown.js'); ?>
    <?php $stc->Add('/scripts/b-combo/b-combo-autocomplete.js'); ?>
    <?php $stc->Add('/scripts/b-combo/b-combo-calendar.js'); ?>
    <?php $stc->Add('/scripts/b-combo/b-combo-manager.js'); ?>      
    <?php $stc->Add('/css/block/b-page/b-page.js'); ?>
    <?php $stc->Add('/css/block/b-menu/b-menu.js'); ?>
    <?php $stc->Add('/css/block/b-input-hint/b-input-hint.js'); ?>
    <?php $stc->Add('/css/block/b-ext-filter/b-ext-filter.js'); ?>
    <?php $stc->Add('/css/block/b-catalog/b-catalog.js'); ?>
    <?php $stc->Add('/scripts/b-bar.js'); ?>
    <?php $stc->Add('/css/block/b-ext-filter/b-ext-filter.js'); ?>
    <?php $stc->Add('/css/block/b-chat/b-chat.css'); ?>
    <?php $stc->Add('/scripts/b-chat2.js'); ?>        

    <?php if ($css_file) {
    foreach ((array) $css_file as $css) {
        $stc2->Add(($css[0] == '/' ? '' : '/css/').$css);
    }
} ?>
    <?php if ($js_file) {
    foreach ((array) $js_file as $js) {
        $stc2->Add(($js[0] == '/' ? '' : '/scripts/').$js);
    }
} ?>
    <?php if ($js_file_utf8) {
    foreach ((array) $js_file_utf8 as $js) {
        $stc2->Add(($js[0] == '/' ? '' : '/scripts/').$js, true);
    }
} ?>
...
*/ ?>   
<?php if ($bChooseErr) {
    ?><script type="text/javascript">alert('Захват потока не удался');</script><?php 
} ?>
