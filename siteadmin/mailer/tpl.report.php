...
<div class="b-layout">	
                ...
                <div class="b-fon b-fon_padbot_20 b-fon_width_full b-file">
                    <div class="b-fon__body b-fon__body_pad_10 b-fon__body_fontsize_13 b-fon__body_bg_f0ffdf i-button">
                        <table cellspacing="0" cellpadding="0" border="0" class="b-icon-layout__table">
                            <tbody>
                                <?php foreach ($attachedfiles_files as $file) {
    $cFile = new CFile($file['fid']);
    ?>
                                <tr class="b-icon-layout__tr">
                                    <td class="b-icon-layout__icon b-icon-layout__icon_height_25"><i class="b-icon b-icon_attach_<?= $cFile->getext();
    ?>"></i></td>
                                    <td class="b-icon-layout__files b-icon-layout__files_fontsize_13"><a href="<?= (WDCPREFIX."/{$cFile->path}/{$cFile->name}");
    ?>" class="b-icon-layout__link b-icon-layout__link_fontsize_13"><?=$cFile->original_name?></a>, <?=ConvertBtoMB($cFile->size);
    ?></td>
                                </tr>
                                <?php 
}//foreach?>
    ...
    echo (int) ($sum_rec).' '.ending((int) ($sum_rec), 'человек', 'человека', 'человек');
}?></div>
            </td>
        </tr>
    </table>
<?php if (!$message['filter_file']) {
    ...
    <?php include 'tpl.filter.emp.php';
    ?>
    ...
    <?php include 'tpl.filter.frl.php';
    ?>
<?php 
} ?>
