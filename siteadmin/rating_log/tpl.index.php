<?php if (!defined('IS_SITE_ADMIN')) {
                        ...
                        <div class="form-el">
                            ...
                            <div class="form-value">
                                <select name="filter_factor" class="sw205">
                                    ...
                                    <?php foreach ($rating->bit_factor as $id => $name) {
    ?>
                                        <option value="<?= $id ?>" <?= ($filter === $id ? 'selected="selected"' : '');
    ?>><?= $name ?></option>
                                    <?php 
} ?>
                                </select>
    ...
    ?>
        <table class="tbl-cnc">
            ...
            <tbody>
                ...
                foreach ($rlog as $row) {
                    $factors = $rating->getBitFactors($row['factor']);
                    ?>
                    <tr id="query<?= $row['log_id'] ?>">
                        <td><a href="/users/<?= $row['login']?>/"><?= "{$row['uname']} {$row['usurname']}  [{$row['login']}]";
                    ?></a></td>
    ...
    $verify_actions[ $row['login'] ] = 1;
                    ...
                    ?>
                        <td title="<?=$row['factor']?>">
                        <?php foreach ($factors as $pos => $factor) {
    ?>
