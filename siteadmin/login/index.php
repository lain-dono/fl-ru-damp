...
if ($_POST['ds']) {
    $ds = date('Y-m-d', strtotime($_POST['ds']));
}
if ($_POST['de']) {
    $de = date('Y-m-d', strtotime($_POST['de']));
}
if (!$ds) {
    $ds = date('Y-m-d', mktime(0, 0, 1, date('m'), date('d'), date('Y')));
}
if (!$de) {
    $de = date('Y-m-d', mktime(23, 59, 59, date('m'), date('d'), date('Y')));
}
...
if ($old_login && $new_login && $_SESSION['rand'] == $_POST['u_token_key']) {
    ...
    if (!preg_match('/^[a-zA-Z0-9]+[-a-zA-Z0-9_]{2,}$/', $new_login)) {
