...
if ($stype == 'ban') {
    $additsql = "is_banned='1'";
    $no_more = 1;
}
if ($stype == 'active') {
    $additsql = 'active=false';
}
if ($stype == 'warns') {
    $additsql = 'warn > 0';
    $no_more = 1;
}
if ($stype == 'emp' || $stype == 'frl' || !$stype) {
    $additsql = "(role&'$empmask')='".$$type."' AND is_banned='0'";
    $no_more = 1;
}
...
switch ($action) {
    ...
    case 'searchip':
        ...
        if (!$searchip) {
            $searchip = trim(stripslashes($_GET['searchip']));
        }
...
<?php $info_iter = 0; $fpinfo_iter = 0; if ($users) {
...
<td>
    ...
    ?><font color="red">(<?=implode(', ', $old_logs)?>)</font><?php 
}
         ?>
</td>
<td width="30">&nbsp;</td>
<td width="280">
    ...
    ?>
         ...
         ?>
			...
			<?php /* <a href="/siteadmin/users/?action=delete&amp;login=<?=$user['login']?>&amp;type=<?=$utype?>" class="blue" onclick="return warning(20);">удалить</a> */ ?>

			<?php if (hasPermissions('users') && $stype != 'active') {
    ?>
    ...
    ?> Права: 
            ...
            <?php if ($user['active'] == 'f') {
    ?> | <a href="/siteadmin/users/?action=activate&amp;login=<?=$user['login']?>&amp;type=<?=$utype?>" class="blue">активировать</a><?php 
}
    ?>
            <br>
            <?php 
    ...
    ?>
		...
		<?php if (isset($history_bill)) {
    $history_user = $history_bill[$user['uid']];
    ?>
		<tr style="border:0px">
    ...
    ?>
                        <tr>
                            <td align="center"><?=round($event['ammount'], 2);
    ?></td>
