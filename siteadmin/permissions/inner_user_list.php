...
<?php if ($users) {
    ...
    ?>
                            <br/><br/>
                            <?php if ($user_rights || $user_groups) {
    ?>[<a id="uid_del_<?=$user['uid']?>" href="?action=user_delete&uid=<?=$user['uid']?>" onClick="return addTokenToLink('uid_del_<?=$user['uid']?>', 'Вы действительно хотите удалить все права пользователя?')">удалить все права</a>] <?php 
}
    ?>[<a href="?action=user_edit&uid=<?=$user['uid']?>">редактировать</a>]
                            <br/><hr>
                        </td>
                    </tr>
                </table>
