...
<?php if ($sUid) { // просмотр истории конкретного пользователя старт
    ...
    ?>
                <li class="color-e37101"><?php if ($bHasAll || $bHasPayments) {
    ?> | <?php 
}
    ?><a onclick="xajax_getUserWarns(<?=$user['uid']?>,'admin_log_users','admuserpage');" href="javascript:void(0);">Предупреждения</a></li>
                <?php 
}
    ?>
            </ul>
    ...
    ?>
            <tr id="tr_<?=$aOne['id']?>" onclick="window.location.href='<?=$sHref?>'" <?=$sTrClass?>>
                ...
                <td class="cell-who"><?php if ($aOne['adm_login']) {
    ?><a target="_blank" href="/users/<?=$aOne['adm_login']?>">[<?=$aOne['adm_login']?>]</a><?php 
} else {
    ?>[не известно]<?php 
}
    ?></td>
                <td class="cell-date"><?php if ($aOne['act_time']) {
...
<div class="admin-lenta">
    ...
    ?>
    	<th class="cell-user"><a href="<?=$href?>" class="lnk-dot-666">Пользователь</a><?php if ($order == 'name') {
    ?><a href="<?=$href?>"><img src="<?=$sSrc?>" alt=""  /></a><?php 
}
    ?></th>
    	<?php $href = $sHref.'&sort=act&dir='.($order == 'act' ? ($direction == 'asc' ? 'desc' : 'asc') : 'asc');
    ?>
    	<th class="cell-act"><a href="<?=$href?>" class="lnk-dot-666">Действие</a><?php if ($order == 'act') {
    ?><a href="<?=$href?>"><img src="<?=$sSrc?>" alt=""  /></a><?php 
}
    ?></th>
    ...
    ?>
    	...
    	<td class="cell-blocking"><span><?php if ($aOne['act_time']) {
    ?><?=date('d.m.Y H:i', strtotime($aOne['act_time']))?><?php 
} else {
    ?>не известно<?php 
}
