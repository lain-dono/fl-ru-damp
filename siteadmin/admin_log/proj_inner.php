...
<div class="admin-lenta">
	...
	<tr>
    	...
    	<td rowspan="<?=$nRowSpan?>" class="cell-blocking">
    	   <span><?php if ($aOne['act_time']) {
    ?><?=date('d.m.Y H:i', strtotime($aOne['act_time']))?><?php 
} else {
    ?>не известно<?php 
}
    ?></span>[<?php if ($aOne['adm_login']) {
    ?><a target="_blank" href="/users/<?=$aOne['adm_login']?>"><?=$aOne['adm_login']?></a><?php 
} else {
    ?>не известно<?php 
}
    ?>]
	...
	<tr>
    	...
    	<td class="cell-body"><?=projects::getSpecsStr($aOne['object_id'], ' / ', ', ', true);
    ?></td>
   	</tr>
	<tr>
    	<td class="cell-number">Автор:</td>
    	<td class="cell-body"><a href="/users/<?=$aOne['aut_login']?>" class="cell-autor"><?=$aOne['aut_uname']?> <?=$aOne['aut_usurname']?> [<?=$aOne['aut_login']?>]</a><?php if ($aOne['warn_cnt']) {
    ?>  <a onclick="xajax_getUserWarns(<?=$aOne['user_id']?>,'admin_log_page','');" href="javascript:void(0);" class="notice">Предупреждения: <span id="warn_<?=$aOne['user_id']?>_<?=$aOne['id']?>" class="warncount-<?=$aOne['user_id']?>"><?=$aOne['warn_cnt']?></span><?php 
}
    ?></a></td>
   	</tr>
   	<tr>
    ...
    <tr id="tr_<?=$aOne['id']?>" onclick="window.location.href='<?=$sHref?>'">
        ...
        <td class="cell-who cell-top"><?php if ($aOne['adm_login']) {
    ?><a target="_blank" href="/users/<?=$aOne['adm_login']?>">[<?=$aOne['adm_login']?>]</a><?php 
} else {
    ?>[не известно]<?php 
}
    ?></td>
        <td class="cell-date cell-top"><?php if ($aOne['act_time']) {
    ?><?=date('d.m.Y H:i', strtotime($aOne['act_time']))?><?php 
} else {
    ?>не известно<?php 
}
    ...
    <tr>
        <td colspan="6" style="padding: 0px;">
            <div style="padding-left: 20px;" id="div_comments_<?=$aOne['id']?>"><?php if ($sLogId == $aOne['id'] && $comments_html) {
    echo $comments_html;
}
    ?></div>
        </td>
...
<div class="admin-lenta">
    ...
    ?>
    	<th class="cell-number"><a href="<?=$href?>" class="lnk-dot-666">Номер</a><?php if ($order == 'num') {
    ?><a href="<?=$href?>"><img src="<?=$sSrc?>" alt=""  /></a><?php 
}
    ?></th>
    	<?php $href = $sHref.'&sort=name&dir='.($order == 'name' ? ($direction == 'asc' ? 'desc' : 'asc') : 'asc');
    ?>
    	<th class="cell-body"><a href="<?=$href?>" class="lnk-dot-666">Название</a><?php if ($order == 'name') {
    ?><a href="<?=$href?>"><img src="<?=$sSrc?>" alt=""  /></a><?php 
}
    ?></th>
    	<?php $href = $sHref.'&sort=date&dir='.($order == 'date' ? ($direction == 'asc' ? 'desc' : 'asc') : 'asc');
    ?>
    	<th class="cell-blocking"><a href="<?=$href?>" class="lnk-dot-666">Дата</a><?php if ($order == 'date') {
    ?><a href="<?=$href?>"><img src="<?=$sSrc?>" alt=""  /></a><?php 
}
    ?></th>
    </tr>
	...
	<tr>
    	...
    	<td rowspan="<?=$nRowSpan?>" class="cell-blocking">
    	   <span><?php if ($aOne['act_time']) {
    ?><?=date('d.m.Y H:i:s', strtotime($aOne['act_time']))?><?php 
} else {
    ?>не известно<?php 
}
        ?></span>[<?php if ($aOne['adm_login']) {
	...
	<tr>
    	<td class="cell-number">Автор:</td>
    	<td class="cell-body"><a href="/users/<?=$aOne['aut_login']?>" class="cell-autor"><?=$aOne['aut_uname']?> <?=$aOne['aut_usurname']?> [<?=$aOne['aut_login']?>]</a><?php if ($aOne['warn_cnt']) {
    ?>  <a onclick="xajax_getUserWarns(<?=$aOne['user_id']?>,'admin_log_page','');" href="javascript:void(0);" class="notice">Предупреждения: <span id="warn_<?=$aOne['user_id']?>_<?=$aOne['id']?>" class="warncount-<?=$aOne['user_id']?>"><?=$aOne['warn_cnt']?></span></a><?php 
}
        ?></td>
   	</tr>
   	<tr>
