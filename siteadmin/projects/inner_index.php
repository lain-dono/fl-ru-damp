...
<tr valign="bottom" class="qpr">
	...
	<td class="qph">
	<a href=".?kind=1&page=<?=$page?>" style="margin-left:10px;<?php if (!$kind || $kind == 1) {
    print ' text-decoration: underline;';
}?>">Фри-ланс</a>
	<a href=".?kind=2&page=<?$page?>" style="margin-left:10px;<?php if ($kind == 2) {
    print ' text-decoration: underline;';
}?>">Конкурсы</a>
	<a href=".?kind=4&page=<?$page?>" style="margin-left:10px;<?php if ($kind == 4) {
    print ' text-decoration: underline;';
}?>">В офис</a>
	<td class="qph">&nbsp;</td>
</tr>
...
<tr class="qpr">
	...
	<td class="qp">
	<?php if ($prj['closed'] != 't') {
    ?><a name="prj<?=$prj['id']?>" id="prj<?=$prj['id']?>" href="/users/<?=$prj['login']?>/project/?prjid=<?=$prj['id']?>" class="cntb" title="<?=$prj['name']?>"><?=$prj['name']?></a><?php 
} else {
    ?><strong><?=$prj['name']?></strong><?php 
}
        ?> [<a href="mailto:<?=$prj['email']?>"><?=$prj['email']?></a>]<br>
	...
	<?php if ($prj['anon_id']) {
    ?><a href=".?kind=<?=$kind?>&action=allow&aid=<?=$prj['anon_id']?>&prjid=<?=$prj['id']?>" class="blue" title="<?=$prj['name']?>"><?=($prj['visible'] == 'f') ? 'Разрешить' : 'Запретить'?></a><br><?php 
}
        ?>
	</td>
</tr>
...
<tr><?php // Страницы
	...
	if ($pages > 1) {
	    ...
	    if ($pages > 40) {
	        ...
	        if ($maxpages - $i < 39 && $maxpages - 39 > 0) {
	            $i = $maxpages - 39;
	        }
	    }
