...
Sbr.prototype.EXCODES={<?php
$i = 0;
    foreach ($EXRATE_CODES as $exc => $exn) {
        if (!$stage->checkPayoutSys($exc, $only_reserved_sys)) {
            continue;
        }
        echo ($i++ ? ',' : '')."$exc:['{$exn[1]}'";
...
<div class="tabs-in nr-tabs-in2">
    ...
    ?>
                ...
                <?php if (($stage->status == sbr_stages::STATUS_COMPLETED || $stage->status == sbr_stages::STATUS_ARBITRAGED) && !$stage->data[$sbr->upfx.'feedback_id']) {
    ?>
                ...
                <?php if ($sbr->status == sbr::STATUS_COMPLETED && !$sbr->data[$sbr->upfx.'feedback_id']) {
    ?>
                    <div class="form-block<?=($fbc++ ? '' : ' first')?>">
                        <div class="form-el">
    ...
    ?>
                        <div class="form-block<?=($fbc++ ? '' : ' first')?>">
                            <div class="form-el">
                                ...
                                <?php if ($sbr->user_reqvs['rezdoc_status'] != sbr::RS_ACCEPTED) {
    ?>
                                ...
                                <?php
                                  ...
                                  <li><label>
                                    <input type="radio" name="credit_sys" value="<?=$ex_code?>" 
                                           ...
                                           <?php if (!$stage->checkPayoutReqvs($ex_code)) {
    ...
    ?>
                ...
                <?php foreach ($EXRATE_CODES as $ex_code => $ex_name) {
    if (!$stage->checkPayoutSys($ex_code)) {
        continue;
    }
    if ($need_payout && !$stage->checkPayoutReqvs($ex_code)) {
        $dsbl[$ex_code] = 1;
    }
} 
