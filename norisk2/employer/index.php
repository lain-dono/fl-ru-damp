...
switch ($site) {
    ...
    case 'create' :
        ...
        if ($rt_disabled = $sbr->checkChangeRT()) {
            if (!($rez_type = $sbr->user_reqvs['rez_type'])) {
                $rez_type = sbr::RT_RU;
            } // если не установлен флаг в базе, но checkChangeRT, то считаем, что он руський (т.к. до флага только резиденты были).
        }
    ...
    case 'editstage' :
        ...
        if ($version && ($stage->frl_version > $stage->version || $sbr->frl_version > $sbr->version)) {
            if ($stage->frl_version > $stage->version) {
                $sbr->data['cost'] -= $stage->cost;
                $stage->data = $stage->getVersion($stage->version, $stage->data);
                $sbr->data['cost'] += $stage->cost;
            } // !!!
            if ($sbr->frl_version   > $sbr->version) {
                $sbr->data = $sbr->getVersion($sbr->version, $sbr->data);
            }
        }
    ...
    case 'Stage' :
        ...
        if ($stage->status != sbr_stages::STATUS_INARBITRAGE && $stage->status != sbr_stages::STATUS_ARBITRAGED) {
            if (($sbr->status == sbr::STATUS_PROCESS || $sbr->status == sbr::STATUS_CHANGED) && ($stage_changed = ($stage->frl_version > $stage->version || $sbr->frl_version > $sbr->version))) {
                ...
                if ($sbr->frl_version   > $sbr->version) {
                    $sbr->data = $sbr->getVersion($sbr->version, $sbr->v_data);
                    $sbr->getScheme();
                }
                if ($stage->frl_version > $stage->version) {
                    $stage->data = $stage->getVersion($stage->version, $stage->v_data);
                }
            } else {
                if ($sbr->status == sbr::STATUS_PROCESS && $sbr->stages_version == $sbr->frl_stages_version && $sbr->version == $sbr->frl_version && !$sbr->reserved_id) { // !!! проверить. Можно ли резервировать, если фрилансер не согласился с изменениями.
