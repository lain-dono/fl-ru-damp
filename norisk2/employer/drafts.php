<div class="tabs-in nr-tabs-in2">
            ...
            <div class="form">
                ...
                <div class="form-in">
                    <table>
                        ...
                        <col width="150" />
                        <tbody>
                            <?php $i = 0; foreach ($sbr_drafts as $id => $curr_sbr) {
     ?>
                            <tr<?=(++$i == $sbr_count ? ' class="last"' : '')?>>
                                ...
                                <td>
                                  <?php if ($curr_sbr->data['frl_login']) {
    ?>
    ...
    ?>Не выбран<?php 
     ...
     ?>
                                ...
                                <td>
                                  <?php if ($curr_sbr->data['cost']) {
    ?>
                                    <?=sbr_meta::view_cost($curr_sbr->data['cost'], $curr_sbr->cost_sys)?>
                                  <?php 
} else {
    echo ($curr_sbr->data['cost'] ? 'Не определен' : 'Не задан');
}
     ?>
                                </td>
                                <td>
                                  <?php if ($curr_sbr->data['work_days']) {
    ?>
                                    <?=$curr_sbr->data['work_days'].'&nbsp;'.ending($curr_sbr->data['work_days'], 'день', 'дня', 'дней')?>
                                  <?php 
} else {
    echo ($curr_sbr->data['work_days'] ? 'Не определен' : 'Не указан');
}
     ?>
                                </td>
                            </tr>
