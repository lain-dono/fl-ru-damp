...
Sbr.prototype.CATEGORIES={<?php // категории/подкатегории: {ид_кат:{имя_кат:{ид_подкат:имя_подкат,ид_подкат:...}},ид_кат:...}
...
Sbr.prototype.STAGE_FILES_COUNT={<?php
$j = 0;
foreach ($sbr->stages as $i => $s) {
    if ($site == 'editstage' && $s->id != $stage_id) {
        continue;
    }
    echo ($j++ ? ',' : '').$i.':'.($s->attach ? count($s->attach) : 0);
}
?>};
Sbr.prototype.STAGE_DEADLINES={<?php
$j = 0;
foreach ($sbr->stages as $i => $s) {
    if ($site == 'editstage' && $s->id != $stage_id) {
        continue;
...
<div class="tabs-in">
            ...
            <div class="form-in">
                <div class="form-block first last">
                    <div class="form-el c">
                        ...
                        <?php if ($site == 'create' || $sbr->isDraft() || $sbr->data['status'] == sbr::STATUS_CANCELED || $sbr->data['status'] == sbr::STATUS_REFUSED) {
    ?>
        ...
        <?php 
          foreach ($sbr->stages as $num => $stage) {
              if ($site == 'editstage' && $stage->id != $stage_id) {
                  continue;
              }
              ?>
        <fieldset class="nr-task">
            
            <legend>Задача №<a class="nr-task-anchor" name="stage<?=($num + 1)?>" innum="<?=$num?>"><?=($num + 1)?></a> <?php // !!! переименовать якорь ?>
              <?php if ($site == 'create' || $sbr->isDraft()) { // !!! !$sbr->reserved_id || ?>
    ...
    foreach ($stage->data['attach'] as $id => $a) {
        ?>
																																		<?php if ($a['source_type'] == sbr_stages::ATTACH_SOURCE_PRJ) {
    ?>
    ...
    ?>
              ...
              ?>
                                ...
                                <?php foreach ($EXRATE_CODES as $id => $ex) {
    if ($id == exrates::FM || $id == exrates::WMZ) {
        continue;
    }
    if (($id == exrates::YM || $id == exrates::WMR) && $sbr->user_reqvs['form_type'] == sbr::FT_JURI) {
        continue;
    }
    ?>
              ...
              ?>
                        ...
                        <div class="form-el">
                            <?php if (!$sbr->data['reserved_id'] || !$stage->data['dead_time']) {
    ?>
    ...
    ?>
            ...
            <input type="hidden" name="stages[<?=$num?>][id]" value="<?=$stage->data['id']?>" />
                        ...
                        <?php if ($site != 'editstage' && ($site == 'create' || $sbr->isDraft())) { // !!! ?>
                ...
                <div class="form-block last">
                    <div class="form-el c">
                        <ul class="form-nr-scheme-ul">
                            <?php if ($sbr->scheme_type == sbr::SCHEME_OLD) {
    ?>
    ...
    ?>
                            <table style="display:none" id="sch_<?=$sch['type']?>">
                                ...
                                <?php foreach ($sch['taxes'][1] as $id => $tax) {
    $s = $e = '';
    if ($id == sbr::TAX_NDS) {
        $s = '<strong>';
        $e = '</strong>';
            ...
            <div class="form-in">
                <div class="form-block first last">
                    <div class="form-el">
                        ...
                        <?php if ($sbr->status == sbr::STATUS_CANCELED || $sbr->status == sbr::STATUS_REFUSED) {
    ?>
    ...
    ?>
            ...
            <b class="b2"></b>
            <b class="b1"></b>
        <?php if ($sbr->data['delstages']) {
    foreach ($sbr->data['delstages'] as $id => $d) {
        ;
    }
    ?>
