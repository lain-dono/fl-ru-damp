...
<div class="tabs-in nr-tabs-in">
    ...
    ?>
                                                ...
                                                <?php if (($status == sbr::STATUS_PROCESS || $status == sbr::STATUS_CHANGED) && $stage->status != sbr_stages::STATUS_INARBITRAGE && ($stage->frl_version > $stage->version || $curr_sbr->frl_version > $curr_sbr->version)) {
    ?>
    ...
    ?>
                                <form action="?id=<?=$id?>" method="post" id="currentsFrm<?=$id?>">
                                    <?php if (!$curr_sbr->reserved_id && ($curr_sbr->status == sbr::STATUS_PROCESS || $curr_sbr->status == sbr::STATUS_CHANGED)) {
    ?>
    ...
    ?>
                                    ...
                                    <?php /* if($sbr->error[$curr_sbr->data['id']]['canceled']) { ?>
                                    ...
                                    <? } */ ?>
                                    <?php if ($status == sbr::STATUS_NEW || (!$curr_sbr->reserved_id && $status != sbr::STATUS_CANCELED && $status != sbr::STATUS_REFUSED && $status != sbr::STATUS_COMPLETED)) {
    ?>
    ...
    ?>
                                    <?php if ($status == sbr::STATUS_REFUSED || $status == sbr::STATUS_CANCELED) {
    ?>
                                        <div class="nr-cancel-reason c">
                                            <strong>
