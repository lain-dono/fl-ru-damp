...
<div class="tabs-in nr-tabs-in2">
    ...
    ?>
        <?php if ($sbr->isAdmin()) {
    ?><form action="." method="post" onsubmit="if(!checked_cnt)return false;if(this['action'].value=='delete')return window.confirm('Вы действительно хотите удалить выбранные документы?');"><?php 
}//if?>
    ...
    foreach ($sbr->docs as $doc) {
        ?>
                            <tr class="<?=(++$i == 1 ? 'first' : '')?><?=($i == $docs_cnt ? ' last' : '')?><?=($sbr->post_doc['id'] == $doc['id'] ? ' tr-edit' : '')?>"<?=($sbr->post_doc['id'] == $doc['id'] ? ' id="edit_tr"' : '')?>>
                                ...
                                <td>
                                  <?php if ($doc['stage_id'] && ($stg = $sbr->getStageById($doc['stage_id']))) {
    ?>
    ...
    ?>&nbsp;<?php 
        ...
        ?>
                                ...
                                <td class="nr-d-c6">
                                  <?php if (($doc['access_role'] & sbr::DOCS_ACCESS_EMP) == sbr::DOCS_ACCESS_EMP) {
    ?><span class="nr-d-e">Работодатель</span><?php 
}
        ?>
                                  <?php if (($doc['access_role'] & sbr::DOCS_ACCESS_FRL) == sbr::DOCS_ACCESS_FRL) {
    ?><span class="nr-d-f">Исполнитель</span><?php 
}
        ?>
                                </td>
    ...
    <?php if ($sbr->isAdmin()) {
    ?>
    <div id="doc_edit_box"><?php if ($sbr->post_doc['id']) {
    echo $sbr->doc_form($sbr->post_doc, $stage_id, TRUE);
}
    ?></div>
