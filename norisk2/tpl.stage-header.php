	...
	<div class="nr-prj-status">
        <?php if ($stage->status == sbr_stages::STATUS_COMPLETED) {
    ?>
    ...
    <ul class="nr-prj-options c">
        <?php if ($sbr->data['reserved_id'] && !$sbr->isAdmin() && !$sbr->isAdminFinance() && ($stage->status & sbr_stages::STATUS_COMPLETED) != sbr_stages::STATUS_COMPLETED && $stage->status != sbr_stages::STATUS_INARBITRAGE) {
    ?>
		<li class="nr-prj-o1">
    ...
    if (($ain = $stage->status == sbr_stages::STATUS_INARBITRAGE) || $stage->status == sbr_stages::STATUS_ARBITRAGED) {
        ...
        <?php if ($sbr->isEmp() && !$stage_changed && ($stage->status & sbr_stages::STATUS_COMPLETED) != sbr_stages::STATUS_COMPLETED && $stage->status != sbr_stages::STATUS_INARBITRAGE) {
    ?>
		<li class="nr-prj-o2">
		...
		<div class="nr-tbl-inf-in">
			<table>
                ...
                <tr class="last<?=($ain ? ' nr-task-arb' : '')?>">
                    ...
                    <?php if ($sbr->isEmp() && !$stage_changed && ($stage->status & sbr_stages::STATUS_COMPLETED) != sbr_stages::STATUS_COMPLETED && $stage->status != sbr_stages::STATUS_INARBITRAGE && $sbr->data['reserved_id']) {
    ?>
                        <td class="td-expand">
                                    ...
                                    <div class="expand <?=sbr_stages::$ss_classes[$stage->status][2]?>">
                                        ...
                                        <div class="e-list" onclick="ssBoxLock=1;">
                                            <div class="e-ul" id="status_box">
                                                <ul>
                                                    <?php foreach (sbr_stages::$ss_classes as $id => $stts) {
    if ($id == sbr_stages::STATUS_INARBITRAGE || $id == $stage->status || $id == sbr_stages::STATUS_ARBITRAGED) {
        continue;
    }
    if ($id == sbr_stages::STATUS_COMPLETED && ($stage->frl_version != $stage->version || $sbr->frl_version != $sbr->version)) {
        continue;
    }
    ?>
    ...
    ?>
                    ...
                    <td class="last">
                        <?php include($fpath.'tpl.stage-user.php') ?>
                    </td>
                </tr>
			</table>
		</div>
