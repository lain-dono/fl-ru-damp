...
?>
<?php /*= $gr_id ? getCrumbs($crumbs, "commune") : '<h1 class="b-page__title">Сообщества фрилансеров</h1>' */?>
                ...
                <tr class="b-search__tr">
                    <td class="b-search__input">
                            <div class="b-input b-input_height_24">
                                <input type="text" name="search" class="b-input__text" id="b-input" value="<?= htmlspecialchars(stripslashes($_GET['search'])); ?>" placeholder="<?php if (!$_GET['search']) {
    ?>Найти сообщество, пост, комментарий<?php 
} else {
    htmlspecialchars(stripslashes($_GET['search']));
} ?>">
        ...
        <?= seo_end();?> 
            ...
            <?php $is_empty_commune = (count($communes) == 0);
            if ($om) {
                ?>
                                        ...
                                        <div class="b-shadow__body b-shadow__body_pad_15 b-shadow__body_bg_fff">
                                            <ul class="b-filter__list">
                                                <li class="b-filter__item b-filter__item_padbot_10 b-filter__item_lineheight_1 b-filter__item_msie_lineheight_15">
                                                    <a class="b-filter__link<?php if ($sub_om != commune::OM_CM_JOINED_ACCEPTED) {
    ?> b-filter__link_dot_0f71c8<?php 
} else {
    ?> b-filter__link_no<?php 
}
    ?>" onclick="window.location='<?=$href?>&sub_om=<?=commune::OM_CM_JOINED_ACCEPTED?>'">по дате вступления</a>
                                                    <span class="b-filter__marker b-filter__marker_top_4 b-filter__marker_galka<?php if ($sub_om != commune::OM_CM_JOINED_ACCEPTED) {
    ?> b-filter__marker_hide<?php 
}
    ?>"></span>
                                                </li>
                                                <li class="b-filter__item b-filter__item_padbot_10 b-filter__item_lineheight_1 b-filter__item_msie_lineheight_15">
                                                    <a class="b-filter__link<?php if ($sub_om != commune::OM_CM_JOINED_CREATED) {
    ?> b-filter__link_dot_0f71c8<?php 
} else {
    ?> b-filter__link_no<?php 
}
    ?>" onclick="window.location='<?=$href?>&sub_om=<?=commune::OM_CM_JOINED_CREATED?>'">по дате создания сообщества</a>
                                                    <span class="b-filter__marker b-filter__marker_top_4 b-filter__marker_galka<?php if ($sub_om != commune::OM_CM_JOINED_CREATED) {
    ?> b-filter__marker_hide<?php 
}
    ?>"></span>
                                                </li>
                                                <li class="b-filter__item b-filter__item_padbot_10 b-filter__item_lineheight_1 b-filter__item_msie_lineheight_15">
                                                    <a class="b-filter__link<?php if ($sub_om != commune::OM_CM_JOINED_BEST) {
    ?> b-filter__link_dot_0f71c8<?php 
} else {
    ?> b-filter__link_no<?php 
}
    ?>" onclick="window.location='<?=$href?>&sub_om=<?=commune::OM_CM_JOINED_BEST?>'">по рейтингу сообщества</a>
                                                    <span class="b-filter__marker b-filter__marker_top_4 b-filter__marker_galka<?php if ($sub_om != commune::OM_CM_JOINED_BEST) {
    ?> b-filter__marker_hide<?php 
}
    ?>"></span>
                                                </li>
                                                <li class="b-filter__item b-filter__item_padbot_10 b-filter__item_lineheight_1 b-filter__item_msie_lineheight_15">
                                                    <a class="b-filter__link<?php if ($sub_om != commune::OM_CM_JOINED_LAST) {
    ?> b-filter__link_dot_0f71c8<?php 
} else {
    ?> b-filter__link_no<?php 
}
    ?>"onclick="window.location='<?=$href?>&sub_om=<?=commune::OM_CM_JOINED_LAST?>'">по дате последней темы в сообществе</a>
                                                    <span class="b-filter__marker b-filter__marker_top_4  b-filter__marker_galka<?php if ($sub_om != commune::OM_CM_JOINED_LAST) {
    ?> b-filter__marker_hide<?php 
}
    ?>"></span>
                                                </li>
                                                <li class="b-filter__item b-filter__item_padbot_3 b-filter__item_lineheight_1 b-filter__item_msie_lineheight_15">
                                                    <a class="b-filter__link<?php if ($sub_om != commune::OM_CM_JOINED_MY) {
    ?> b-filter__link_dot_0f71c8<?php 
} else {
    ?> b-filter__link_no<?php 
}
    ?>"onclick="window.location='<?=$href?>&sub_om=<?=commune::OM_CM_JOINED_MY?>'">по моим предпочтениям</a>
                                                    <span class="b-filter__marker b-filter__marker_top_4  b-filter__marker_galka<?php if ($sub_om != commune::OM_CM_JOINED_MY) {
    ?> b-filter__marker_hide<?php 
}
    ?>"></span>
                                                </li>
        ...
        <?php //__commPrintPage( $page, $communes, $groupCommCnt, $sub_om, $search )?>
        <?php include ABS_PATH.'/commune/tpl.communes_list.php'; ?>
