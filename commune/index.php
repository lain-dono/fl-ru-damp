...
define('JS_BOTTOM', true);
  ...
  if ($user_mod & (commune::MOD_COMM_ACCEPTED | commune::MOD_COMM_AUTHOR)) {
	...
	switch ($site) {
    	...
    	case 'Admin.members' :
            ...
            if ($user_filter == 1) {
                $nPagesCnt = $adminCnt;
            } elseif ($user_filter == 2) {
                $nPagesCnt = $comm['a_count'] - $comm['w_count'] - $adminCnt;
            } else {
                $nPagesCnt = ($mode == 'Asked') ? $comm['w_count'] : $joinedCnt;
            }
        ...
        case 'Topic' :
            ...
            if ($top['title']) {
                ...
                if ($html_title == '') {
                    $html_title = substr($sQuestion, 0, 30).(strlen($sQuestion) > 30 ? '...' : '');
                }
          ...
          case 'Newtopic' :
            ...
            if (!((($comm['id'] == 5100 || $comm['id'] == 1008) && $cur_user->is_team == 't') || ($comm['id'] != 5100 && $comm['id'] != 1008))) {
                header('Location: '.getFriendlyURL('commune_commune', $comm['id']));
                exit;
            }
            $commune_output = 'tpl.topic_form.php';
            $js_file_utf8[] = '/scripts/ckedit/ckeditor.js';
  ...
  switch ($action) {
    ...
    case 'do.Edit.post' :
     ...
     if (!($user_mod & (commune::MOD_COMM_AUTHOR | commune::MOD_COMM_ADMIN)) && !hasGroupPermissions('administrator')) {
         $pos = false;
     }
...
//      }
  ...
  if (empty($additional_header)) {
      ...
      if (!empty($_GET['site'])) {
          $om_clean_uri[] = 'site='.(string) $_GET['site'];
      }
      if (!empty($_GET['post'])) {
          $om_clean_uri[] = 'post='.(int) $_GET['post'];
      }
      if (count($om_clean_uri)) {
          $additional_header .= '
...
// Рабочие функции -------------------------------------------------------------
    ...
    function tryLoadFile(&$files, $dir, $login, &$file_name, &$is_smalled, $max_img_width = 0, $max_img_height = 0, $max_size = 2097152, $is_fix_size = 0)
    {
        ...
        foreach ($files as $file) {
            ...
            if ($error) {
                return $error;
            } // Если есть ошибка дальше загружать файлы бессмысленно
        }
  ...
  function getBookmarksStyles($bmCnt, $curPos)
  {
      $arr = NULL;
      for ($i = 0; $i < $bmCnt; ++$i) {
          $arr[$i] = ($curPos == $i ? ' b-menu__item_active' : false);
      }// (!$i ? 'user_menu_l' : ($curPos == $i-1 ? 'user_menu_la' : 'user_menu')));
...
//  function array2tree(
  ...
  // левый (мысленно поверни поддерево корнем к верху), а не самый удаленный.
  {
      ...
      $xml = '<ROOT>';
      foreach ($array as $a) {
          $xml .= "<node idx='".($i++)."' id='".$a[$id_name]."' pid='".$a[$parent_name]."'/>";
      }
      $xml .= '</ROOT>';
