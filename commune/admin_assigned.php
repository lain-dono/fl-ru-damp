...
<table class="tbl-cau">
    ...
    ?>
										...
										<td><strong><?= date('d.m.Y H:i', strtotime($comm['created_time']));
    ?></strong></td>
...
<?php if (count($members)) {
    foreach ($members as $memb) {
        $is_moderator_check = $memb['is_moderator'] == 't' ? 1 : 0;
        $is_manager_check = $memb['is_manager'] == 't' ? 1 : 0;
        ?>

									<tr<?= $memb['is_banned'] == 't' ? ' class="cau-banned"' : '';
        ?> id="user_row_<?=$memb['id'];
        ?>">
										<td class="first">
    ...
    ?>">
										...
										<td><strong><?= date('d.m.Y H:i', strtotime($memb['accepted_time']));
        ?></strong></td>
										<td>
										...
										<td class="cau-lnks">
											<strong><a href="javascript:void(0)" onclick="if(confirm(this.innerHTML + ' пользователя?')) xajax_BanMember('idBan<?=$memb['id']?>', <?=$memb['id']?>)" id="idBan<?=$memb['id']?>" class="<?= ($memb['is_banned'] == 't' ? 'lnk-dot-green' : 'lnk-dot-red');
        ?>"><?=($memb['is_banned'] == 't' ? 'Разбанить' : 'Забанить')?></a>&nbsp;&nbsp;&nbsp; <a href="?id=<?=$id?>&site=Admin.members&mode=Asked&m=<?=$memb['id']?><?php if ($page > 1) {
    ?>&page=<?=$page?><?php 
}
        ?>&action=do.Kill.member" onclick="return confirm('Удалить пользователя?')" class="lnk-dot-red">Удалить</a></strong>
										</td>
									</tr>
