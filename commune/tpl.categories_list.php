...
<?php if ($categories) {
    foreach ($categories as $category) {
        ...
        ?>
        <li id="category_item<?= $category['id'];
        ?>" class="b-menu__item b-menu__item_padbot_10">
            <?php if ($for_admin || $for_commune_admin) {
    ?>
        ...
        ?>
            ...
            <?php if ($category['id'] == $curr_cat && $page == 1) {
    ?>
                <?= hyphen_words($category['name']) ?>
                <a id="category_name<?= $category['id'] ?>" style="color: #000;" class="b-menu__link" href="<?php getFriendlyUrl('commune_commune', $comm['id']).'?om='.$om.'&cat='.$category['id'] ?>"><?= hyphen_words($category['name']) ?></a>
            <?php 
} else {
    ?>
