<?php if ($stage->notification['ntype'] == 'sbr_stages.FRL_FEEDBACK' && $head_docs && $stage->sbr->scheme_type == sbr::SCHEME_PDRD2) {
    ...
    <div class="b-layout b-layout_padleft_35 b-layout_padtop_15 b-layout_padbot_15 b-layout_padright_15" id="head_docs">
        <div class="b-layout__txt b-layout__txt_padbot_10 b-layout__txt_color_a0763b">Для того, чтобы получить заработанные деньги, вам нужно скачать, распечатать, подписать <?= ending($hdoc_cnt, 'документ', 'документы', 'документы');
    ?>:</div>
            <table cellspacing="0" cellpadding="0" border="0" class="b-layout__table">
                <tbody>
                    <?php foreach ($head_docs as $hdoc) {
                    ...
                    <tr class="b-layout__tr">
                        <td class="b-layout__middle b-layout__middle_padbot_5">
                            <div class="b-layout__txt">
                                <i class="b-icon b-icon_attach_<?= getICOFile($ext);
    ?>"></i> 
                                <a href="<?= WDCPREFIX;
    ?>/<?= $hdoc['file_path'].$hdoc['file_name']?>" class="b-layout__link"><?= $hdoc['name']?></a>, <?= ConvertBtoMB($hdoc['file_size'])?>
                            </div>
                        </td>
                        <td class="b-layout__right b-layout__right_padleft_20 b-layout__right_padbot_5">
                            <div class="b-layout__txt">
                                <a href="<?= WDCPREFIX;
    ?>/<?= $hdoc['file_path'].$hdoc['file_name']?>" class="b-layout__link">Скачать</a>
                            </div>
                        </td>
                    </tr>
                    <?php 
