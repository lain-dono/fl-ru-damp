...
<h3 class="b-layout__h3">
    <?php if ($setting['caption_expand']) {
    ?>
        <a class="b-layout__link b-layout__link_bold b-layout__link_bordbot_dot_0f71c8 finance-block block-<?= $setting['abbr_block']?>" href="javascript:void(0)"><?= $tbl_caption;
    ?></a> <span class="b-layout__arrow"></span> &#160; 
...
<?php $pos = 0; foreach ($tbl as $key => $field) {
     if (!$field['name']) {
         continue;
     }
     ++$pos;
    ...
    ?>
                    ...
                    <div class="b-combo <?= $setting['combo_css']?> <?=($field['rez_required'] ? ' rez--req'.$field['rez_required'] : '')?> <?php /* ($field['rez_required'] && ($field['rez_required'] & $rez_type) ? ' form-imp' : '') */?>">
                        <?php 
                        $addit_html = '';
     switch ($field['type']) {
                            case 'small':
    ...
    ?>
                            ...
                            <?php 
} elseif ($setting['field'] == 'phone' && $setting['auth']) {//if?>
                    ...
                    <div class="b-layout__txt b-layout__txt_fontsize_11">
                        <?php if (in_array($field['pos'], array_keys($setting['subdescr'])) && $setting['subdescr'][$field['pos']] != '') {
    ?>
