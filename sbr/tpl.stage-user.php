...
<div class="b-username b-username_bold b-username_padbot_35">
    ...
    <?= $session->view_online_status($sbr->data['frl_login'], false, '&nbsp;', $activity) ?><a href="/users/<?= $sbr->data['frl_login'] ?>/" class="b-username__link b-username__link_color_f2922a" target="_blank"><?= ($sbr->data['frl_uname'].' '.$sbr->data['frl_usurname']);
    ?></a> 
    ...
    <?= $session->view_online_status($sbr->data['emp_login'], false, '&nbsp;', $activity) ?><a href="/users/<?= $sbr->data['emp_login'] ?>/" class="b-username__link b-username__link_color_6db335" target="_blank"><?= ($sbr->data['emp_uname'].' '.$sbr->data['emp_usurname']);
    ?></a> 
...
<div class="b-username b-username_bold b-username_padbot_35">
    ...
    <a href="/users/<?= $sbr->data[$sbr->apfx.'login'] ?>/" class="b-username__link <?= $sbr->isEmp() ? 'b-username__link_color_f2922a' : 'b-username__link_color_6db335'?>" target="_blank"><?= ($sbr->data[$sbr->apfx.'uname'].' '.$sbr->data[$sbr->apfx.'usurname']);
    ?></a> 
    <span class="b-username__login <?= $sbr->isEmp() ? 'b-username__login_color_f2922a' : 'b-username__login_color_6db335'?>">
        ...
        <span class="b-username__marks"><?php $apfx = $sbr->apfx;
    ?><?= view_mark_user_div($sbr->data[$apfx.'is_pro'] === 't', $apfx === 'emp_', $sbr->data[$apfx.'is_team'] === 't', '') ?><?= $sbr->data[$sbr->apfx.'is_verify'] == 't' ? view_verify() : '';
    ?>
        </span>
    </span>
</div>
