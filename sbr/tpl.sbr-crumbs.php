...
<div class="b-fon b-fon_float_right">
    <div class="b-fon__body b-fon__body_pad_10 b-fon__body_fontsize_13 b-fon__body_bg_ffeeeb">
		...
		<div class="b-layout__txt">
            <?php if (!$_SESSION['E'] && !$_SESSION['F']) {
    ?>Администратора<?php 
} else {//if?><a href="<?=($site_uri ? $site_uri.'&' : '?')?>access=A" class="b-layout__link">Администратора</a><?php 
}//else?>&#160;&#160;&#160;
            <?php if ($_SESSION['E']) {
    ?>Работодателя<?php 
} else {//if?><a href="<?=($site_uri ? $site_uri.'&' : '?')?>access=A&E=<?=$sbr->emp_login?>" class="b-layout__link">Работодателя</a><?php 
}//else?>&#160;&#160;&#160;
            <?php if ($_SESSION['F']) {
    ?>Исполнителя<?php 
} else {//if?><a href="<?=($site_uri ? $site_uri.'&' : '?')?>access=A&F=<?=$sbr->frl_login?>" class="b-layout__link">Исполнителя</a><?php 
}//else?>
        </div>
	</div>
</div>
