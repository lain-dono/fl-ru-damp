...
if ($stage->orders == 'DESC') {
    ...
    foreach ($reverse_history as $xact => $history) {
        ...
        if ($current['abbr'] == 'sbr_stages.AGREE') {
            break;
        }
    }
}
...
foreach ($stage->history as $xact => $history) {
    $current = current($history);
    if (empty(sbr_notification::$history[$current['abbr']])) {
        continue;
    } // Если нет названия в истории то не выводим.
    if ($reopen && $current['abbr'] == 'sbr.OPEN') {
        $current['abbr'] = 'sbr.REOPEN';
    }
    if ($current['abbr'] == 'sbr_stages.STARTED_WORK') {
        $started_work_view++;
    }
    if ($current['abbr'] == 'sbr_stages.EMP_MONEY_REFUNDED' && $sbr->isFrl()) {
        continue;
    }
    if ($current['abbr'] == 'sbr_stages.MONEY_PAID' && $sbr->isEmp()) {
        continue;
    }
    if ($current['abbr'] == 'sbr_stages.DOC_RECEIVED') {
        continue;
    }
...
<?php 
        ...
        <table class="b-layout__table b-layout__table_width_full" cellpadding="0" cellspacing="0" border="0" id="evn_<?= $xact?>">
            <tr class="b-layout__tr">
                <td class="b-layout__left <?php if (!$greenEvent) {
    ?>b-layout__left_padbot_30<?php 
}//if?> b-layout__left_width_72ps">
                    <div class="b-post">
                        <div class="b-post__body">
                            <div class="b-post__avatar b-post__avatar_padtop_4 b-post__avatar_margright_10 absolute">
                                <?php if ($current['abbr'] == 'sbr_stages.COMMENT') { // аватарка для комментариея
    ...
    if ($current['abbr'] == 'sbr_stages.OVERTIME') {
        ...
        ?>
                                    <?php if ($stage->status == sbr_stages::STATUS_ARBITRAGED || $stage->status == sbr_stages::STATUS_INARBITRAGE) {
    ?>
    ...
    ?>
                            ...
                            <div class="b-post__content b-post__content_margleft_65 min-height_70">
                                <?php if ($current['abbr'] == 'sbr_stages.COMMENT') {
    $pfx = $current['foronly_role'] == 1 ? 'frl_' : 'emp_';
    ?>
                                <div class="b-username b-username_relative b-username_bold b-username_padbot_10">
                                    ...
                                    <span class="b-username__login <?= $pfx == 'emp_' ? 'b-username__login_color_6db335' : 'b-username__login_color_f2922a'?>">[<a href="/users/<?= $sbr->data[$pfx.'login']?>/" class="b-username__link b-username__link_color_6db335"><?= $sbr->data[$pfx.'login']?></a>]</span><?php $data = $sbr->data;
    ?> <?= view_mark_user_div($data[$pfx.'is_pro'] === 't', $pfx === 'emp_', $data[$pfx.'is_team'] === 't', '') ?><?= $sbr->data[$pfx.'is_verify'] == 't' ? view_verify() : '';
    ?>
                                </div>
    ...
    ?>
                                <table class="b-layout__table b-layout__table_margtop_10" cellpadding="0" cellspacing="0" border="0">
                                    <?php foreach ($attach as $src) {
    ?>
                                    <tr class="b-layout__tr">
                                        <td class="b-layout__middle b-layout__middle_padbot_5"><div class="b-layout__txt"><i class="b-icon b-icon_attach_<?=getICOFile(CFile::getext($src['name']))?>"></i> <a class="b-layout__link" href="<?= WDCPREFIX;
    ?>/<?= $src['path'].$src['name'] ?>" target="_blank"><?= $src['orig_name']?></a>, <?= ConvertBtoMB($src['size'])?></div></td>
                                        <td class="b-layout__right b-layout__right_padleft_20 b-layout__right_padbot_5"><div class="b-layout__txt"><a class="b-layout__link" href="<?= WDCPREFIX;
    ?>/<?= $src['path'].$src['name'] ?>" target="_blank">Скачать</a></div></td>
                                    </tr>
                                    <?php 
}//foreach?>
                                </table>
                                ...
                                <div class="b-post__txt b-post__txt_fontsize_15"><?= reformat2($current['msg'], 40);
    ?></div>
                                <?php if ($current['src_id'] > 0) {
    $attach = $stage->getMsgAttach($current['own_id']);
    ?>
    ...
    ?>
                                ...
                                <?php if ($current['abbr'] == 'sbr_stages.REFUSE' && $current['new_val'] != '') {
    ?>
                                    <div class="b-post__txt b-post__txt_padbot_5 b-post__txt_fontsize_15"><?= reformat($current['new_val'], 45);
    ?></div>
                                <?php 
}//if?>
                                <?php include $_SERVER['DOCUMENT_ROOT'].'/sbr/tpl.stage-history-event.php';
    ?>
                            </div>
                        </div>
                    </div>
    ...
    ?>">
                        <a href="javascript:void(0)" onclick="if(confirm('Вы действительно хотите завершить данное событие?')) xajax_aCompleteEvent('<?=$current['xact_id'];
