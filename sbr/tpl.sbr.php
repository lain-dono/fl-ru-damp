...
foreach ($curr_sbr->stages as $num => $stage) {
    ...
    if ($stage->data['status'] == sbr::STATUS_COMPLETED) {
        $completed++;
    }
    include $_SERVER['DOCUMENT_ROOT'].'/sbr/tpl.stage-list.php';
}//foreach
?>
<?php if ($completed > 0 && $curr_sbr->data['status'] != sbr::STATUS_COMPLETED) {
    ?>
...
<?php include $_SERVER['DOCUMENT_ROOT']."/sbr/{$fpath}/tpl.alert.php";?>
