        ...
        if ($action == 'create' && $rt_checked) {
            ...
            if ($sbr->_new_create($_POST, $attachedfiles)) {
                ...
                foreach ($sbr->stages as $k => $stage) {
                    if (!is_array($stage->data['attached']) && !count($stage->data['attached'])) {
                        continue;
                    }
...
//                }
        ...
        if ($stage->status != sbr_stages::STATUS_INARBITRAGE && $stage->status != sbr_stages::STATUS_ARBITRAGED) {
            ...
            if ($stage_changed_for_frl) {
                ...
                if ($sbr->frl_version   < $sbr->version) {
                    $sbr->v_data = $sbr->getVersion($sbr->frl_version, $sbr->data);
                }
                if ($stage->frl_version < $stage->version) {
                    $stage->v_data = $stage->getVersion($stage->frl_version, $stage->data);
                }
            }
