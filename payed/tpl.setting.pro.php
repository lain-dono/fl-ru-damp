...
<?php if ($pro_last) {
    ?>
  <?php if ((strtotime($pro_last) - time()) / 86400 > 1 || $_SESSION['is_freezed'] || $freeze_set || $freezed_now) {
    ?>
                            ...
                            <div class="b-shadow__body b-shadow__body_bg_fff b-shadow__body_pad_15">
                                                <div class="b-shadow__txt" style="line-height:100%;">
													<span class="b-layout__txt b-layout__txt_fontsize_11">
                                                        ...
                                                        <?php if (!$freeze_disabled && $last_freeze['freezed_cnt'] < 4) {
    $cnt = (4 - $last_freeze['freezed_cnt']);
    ?>
                            <?php
                            $cnt = 7;
    ...
    ?>
              ...
              <div id="freeze_disable" class="b-layout__txt <?= ($freeze_disabled ? '' : 'b-layout__txt_hide');
    ?> b-layout__txt_fontsize_11">
                  Заморозка доступна 4 раза в год (4 периода по 7 дней). Вы уже использовали эту функцию.
              </div>
              <div class="b-layout__txt <?= (($freeze_set || $freezed_now) ? '' : 'b-layout__txt_hide');
    ?>" id="freeze_on">
    ...
    ?>
                            Разморозить
                        <?php 
} elseif ($_SESSION['freeze_from']) {//if?>
                            Отменить
                        <?php 
} else { //else if?>
                            Заморозить
              ...
              <div class="b-layout__txt <?= (!($freeze_set || $freezed_now) ? '' : 'b-layout__txt_hide');
    ?> <?= $freeze_disabled ? 'b-layout__txt_hide' : ''?>" id="freeze_enable">
                  ...
                  <div class="g-hidden">
                        ...
                        <div id="freez_calendar" class="b-combo b-combo_inline-block b-combo_zindex_2">
                            <div class="b-combo__input b-combo__input_calendar b-combo__input_width_170 b-combo__input_arrow-date_yes <?= $dateFrozenMinLimit;
    ?> <?= $dateFrozenMaxLimit ?>">
                                <input type="text" value="<?= date('d.m.Y', strtotime('+ 1 day'));
    ?>" size="80" name="freez_date" id="freez_date" class="b-combo__input-text">
                        ...
                        <span class="b-layout__txt b-layout__txt_padtop_5 b-layout__txt_padbot_10 b-layout__txt_inline-block freeze_info">&nbsp;на
                            <span class="b-layout__text-selected freeze_type" id="ftype1">7</span><?php if (floor((28 - $last_freeze['freezed_days']) / 7) >= 2 && !$_SESSION['is_freezed']) {
    ?>, 
                            <span class="b-layout__text-noselected b-post__label freeze_type" id="ftype2">14</span><?php 
}
    ?><?php if (floor((28 - $last_freeze['freezed_days']) / 7) >= 3 && !$_SESSION['is_freezed']) {
    ?>, 
                            <span class="b-layout__text-noselected b-post__label freeze_type" id="ftype3">21</span><?php 
}
    ?><?php if (floor((28 - $last_freeze['freezed_days']) / 7) == 4 && !$_SESSION['is_freezed']) {
    ?>, 
                            <span class="b-layout__text-noselected b-post__label freeze_type" id="ftype4">28</span><?php 
}
    ?>
                            <!--
                            <?php if (ceil($last_freeze['freezed_days'] / 7) < 1 || ceil($last_freeze['freezed_days'] / 7) == 2 && !$_SESSION['is_freezed']) {
    ?>
