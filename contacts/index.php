...
if (!$chat_with) {
    $chat_with = trim($_POST['msg_to']);
}

if (!preg_match('/^[a-zA-Z0-9_]+[-a-zA-Z0-9_]{1,}$/', $chat_with)) {
    $chat_with = '';
}
...
if ($chat_with) {
    if ($chat_with != $_SESSION['login']) {
        ...
        if ($user->login == '') {
            //|| ($user->is_banned && !hasPermissions('users')))
