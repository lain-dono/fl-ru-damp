...
<?php
    ...
    switch ($status) {
        case projects_status::STATUS_NEW:
            if (!$project['exec_id']) {
                if ((!$is_emp && $offer_status == projects_status::STATUS_DECLINE) ||
                   ($is_emp && $kind == 9 && $offer_status == projects_status::STATUS_CANCEL)) {
                    $icon = 'tu/ico_po_refuse.png';
                } elseif ((!$is_emp && $offer_status == projects_status::STATUS_CANCEL) ||
                   ($is_emp && $kind == 9 && $offer_status == projects_status::STATUS_DECLINE)) {
                    $icon = 'tu/ico_po_canceled.png';
                }
            }
            break;
        ...
        case projects_status::STATUS_EMPCLOSE:
            $icon = 'tu/ico_po_executor.png';
            if ($is_emp && $is_emp_feedback) {
                $icon = ($emp_is_good) ? 'good.png' : 'bad.png';
            } elseif (!$is_emp && $is_frl_feedback) {
                $icon = ($frl_is_good) ? 'good.png' : 'bad.png';
            }
            break;
    }
?>
...
<?php
    ...
    ?>Ваш статус в проекте изменен с "Исполнитель" на "Кандидат".<?php 
}
                ?>
            </div>  
<?php 
...
<?php
    ...
    ?>
            <div class="b-layout__txt b-layout__txt_color_<?=$emp_color?> b-layout__txt_padbot_5">
                <?php echo $fullname ?> завершил сотрудничество и оставил вам <?php if ($emp_is_good) {
    ?>положительный отзыв.<?php 
} else {
    ?>отрицательный отзыв.<?php 
}
    ?>
            </div>
            <div class="b-layout__txt b-layout__txt_margbot_10 b-layout__txt_fontsize_11 b-layout__txt_italic b-layout__txt_padleft_15 b-layout__txt_margleft_15 b-layout_bordleft_<?=$emp_color?> b-layout__txt_color_<?=$emp_color?>">
...
<?php 
    ...
    ?>
            <div class="b-layout__txt b-layout__txt_color_<?=$frl_color?> b-layout__txt_padbot_5">
                <?php echo $fullname ?> завершил сотрудничество и оставил вам <?php if ($frl_is_good) {
    ?>положительный отзыв.<?php 
} else {
    ?>отрицательный отзыв.<?php 
}
