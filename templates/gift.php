<?php foreach ($gifts as $i => $gift) {
    ...
    <div class="b-fon b-fon_width_full b-fon_padbot_10 last-gift-block<?= $i > 0 ? ' b-fon_hide' : '' ?>" id="last_gift<?= $gift['id'] ?>">
        <div class="b-fon__body b-fon__body_pad_10 b-fon__body_fontsize_13 b-fon__body_bg_f0ffdf">
            <div class="b-fon__txt b-fon__txt_center b-username">
                <?php if ($gift['op_code'] == 23) { // перевод ?>
    ...
    ?>
                    <?php if (in_array($gift['op_code'], array(16, 52, 91, 92, 95, 99, 96, 100))) {
    ?>
                    ...
                    <?php if ($gift['login'] != 'admin') {
    ?>
                        <a class="b-fon__link b-fon__link_fontsize_11" href="/bill/gift/">Ответный подарок</a> &nbsp;&nbsp;<?php if (in_array($gift['op_code'], array(16, 52, 91, 92, 95, 99, 96, 100))) {
    ?></span><?php 
}//if?>
                    <?php 
}
