...
<?php 
    ...
    <?php if ($kind != 8) {
    include $_SERVER['DOCUMENT_ROOT'].'/projects/tpl.filter.new.php';
} ?> 
    ...
    <div class="b-page__filter fornavmenu" >
        <div class="b-menu b-menu_line b-menu_relative b-menu_padbot_25">
                <div class="b-menu__filter">
                    ...
                    ?>
                    <?php if ($_SESSION['top_payed'] && $kind != 8) {
    ?>
                    <span class="b-layout__txt b-layout__txt_color_323232 b-layout__txt_valign_top b-layout__txt_float_left"><?= $_SESSION['top_payed']?> <?= ending($_SESSION['top_payed'], $prjWord_1, $prjWord_2, $prjWord_5)?>&#160;&#160; <?php if ($_SESSION['hidetopprjlenta_more'] == 1 && $hidetopprjlenta == 1) {
    ?><a class="b-menu__link b-menu__link_bordbot_dot_0f71c8" id="hide_top_project_lnk2" cmd="hide" onclick="this.hide(); xajax_HideTopProjects(this.get('cmd')); return false;">Скрыть все</a><?php 
}
    ?>&#160;&#160; <a class="b-menu__link b-menu__link_bordbot_dot_0f71c8" id="hide_top_project_lnk" cmd="<?= $hidetopprjlenta == 1 ? 'show' : 'hide' ?>" onclick="xajax_HideTopProjects(this.get('cmd')); return false;"><?= $hidetopprjlenta == 1 ? 'Показать' : 'Скрыть' ?> все</a></span>
                    <?php 
} ?>
                </div>
...
<div id="specialis" class="b-layout b-layout_clear_both b-layout_overflow_hidden <?php if (!$_SESSION['uid'] && $page <= 1) {
    ?> b-layout_padbot_50 b-layout_top_100<?php 
} else {
    ?> b-layout_padtop_30<?php 
} ?>">
...
<?php if ($_GET['quickprj_ok'] == 1) {
    ?>
    <script type="text/javascript">yaCounter6051055.reachGoal('project_bill_win');</script>
    <?php require_once $_SERVER['DOCUMENT_ROOT'].'/templates/quick_buy_prj.php';
    ?>
<?php 
} ?>