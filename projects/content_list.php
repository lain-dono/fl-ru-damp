...
?>
    <h1 class="b-page__title" id="prj_name_<?=$project['id']?>"><?=$sBox1?><?=reformat($sTitle, 30, 0, 1); ?></h1>
      ...
      <?php if ($tu_cnts['total'] > 0) {
    ?><li><a href="/tu-orders/">Все заказы</a> <span><?=$tu_cnts['total']?></span></li><?php 
} ?>
          ...
          <?php 
             if ($prj['pro_only'] != 't' || $is_pro) {
                 ...
                 if ($attach = projects::GetAllAttach($prj['id'])) {
                     foreach ($attach as $a) {
                         if ($a['name']) {
                             ?><div class="flw_offer_attach"><a href="<?=WDCPREFIX.'/'.$a['path'].$a['name']?>" target="_blank">Загрузить</a> (<?=$a['ftype']?>; <?=ConvertBtoMB($a['size'])?>)</div><?php
    ...
    ?>
            ...
            <?php endif;
             ...
             <?php if ($prj['pro_only'] == 't' || $prj['verify_only'] == 't' || $prj['prefer_sbr'] == 't') {
    ?>
             <div class="b-layout__txt b-layout__txt_fontsize_11">
                 <span style="background-color:#fff7ee">
					 <?php if ($prj['pro_only'] == 't' && $prj['verify_only'] != 't') {
    ?>
    ...
    ?>
                      <?php if ($prj['prefer_sbr'] == 't') {
    ?>Предпочитаю работать через БС <a class="b-layout__link b-layout__link_lineheight_1" href="/promo/bezopasnaya-sdelka/"><span class="b-icon b-icon__shield"></span></a><?php 
}
    ?>
                 </span>
             </div>
    ...
    ?>
<script><?php if ($pages <= $page || !$prj_count) {
    ?>function dprj(){}<?php 
} else {
         ?>var prjc=<?=$prj_count?>;function dprj(){if(!(--prjc))window.location.href='?p=list&fld=<?=$folder?>&page=<?=$page?>';}<?php 
     } ?></script>
