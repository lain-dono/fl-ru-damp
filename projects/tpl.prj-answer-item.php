	...
	<?php if (($value['user_id'] == get_uid()) && ($real_offers_count > 0) && $user_offer_exist) {
    ?><h2  class="b-layout__title">Ваш ответ по проекту</h2><?php 
} ?>
                     ...
                     <tr class="b-layout__tr">
                       ...
                       <td class="b-layout__td b-layout__td_padright_20 b-layout__td_ipad">
                           <div class="b-layout__txt b-layout__txt_fontsize_11 b-layout__txt_padbot_5">
                              ...
                              <span style="line-height:1; vertical-align:top;"><?= (view_mark_user($value)); /*!!!is_team!!!*/?> <?=($value['completed_cnt'] > 0 ? '<a class="b-layout__link" href="/promo/bezopasnaya-sdelka/" title="Пользователь работал через Безопасную Сделку" target="_blank"><span class="b-icon b-icon__shield "></span></a>' : '') ?></span> &#160; <?php if ($value['is_banned']) {
    ?><span class="b-layout__txt b-layout__txt_fontsize_11 b-layout__txt_color_c10600 b-layout__txt_bold" >Пользователь&nbsp;забанен.</span><?php 
} ?>
                           </div>
                           <?php if ($value['spec_name'] != '') {
    ?><div class="b-layout__txt b-layout__txt_fontsize_11 b-layout__txt_padbot_5 b-layout__txt_bold">Специализация: <?=$value['spec_name']?></div><?php 
}?>
    ...
    ?>
                                        ...
                                        <?php 
} elseif ($value['refused'] == 't' && $value['status'] == projects_status::STATUS_CANCEL) {
    ?>
                          			    ...
                          			    <?php 
} elseif ($value['status'] == projects_status::STATUS_DECLINE) {
    ?>
                              			<div class="po_refused">
    ...
    ?>
                        ...
                        <?php if ($is_user_offer) {
    ?>
                            <?php if (isset($value['dialogue']) && is_array($value['dialogue']) && (count($value['dialogue']) > 0) && $value['user_id'] == $_SESSION['uid'] && $value['is_blocked'] != 't') {
    $i = 0;
    ...
    ?>
                                ...
                                <div class="po_comments_<?php if ($value['frl_new_msg_count'] > 0) {
    ?>new_<?php 
}
    ?>hide" id="po_comments_<?=$value['id']?>">
    ...
    ?>
                                    ...
                                    <?php 
} elseif (count($value['dialogue']) > 1) { //if?>
                                     	<span style="float: right;"><a href="javascript:void(null)" onclick="dialogue_toggle(<?=$value['id']?>);markRead('<?=$value['id']?>');" class="internal" id="toggle_dialogue_<?=$value['id']?>">Развернуть всю переписку</a> <?=count($value['dialogue'])?></span>
                                    <?php 
}
    ?>
    ...
    if ($comment['is_blocked'] != 't' || $comment['login'] == $_SESSION['login'] || hasPermissions('projects')) {
        ?>
                                		<div style="margin-bottom:8px;font-size:100%;">
                                            <?php if (!is_emp($comment['role']) || $show_info) {
    ?>
    ...
    ?>
                                            <span style="float: right;" id="dialogue-button-<?= $comment['id'] ?>">
                                                <a class="admn" href="javascript:void(0);" onclick="banned.<?=($comment['is_blocked'] == 't' ? 'unblockedDialogue' : 'blockedDialogue')?>(<?=$comment['id']?>)"><?= $comment['is_blocked'] == 'f' ? 'Заблокировать' : 'Разблокировать';
    ?></a>
                                            </span>
                                            <?php 
}
                                                ...
                                                <div class='b-fon b-fon_clear_both b-fon_bg_ff6d2d b-fon_padtop_10 b-fon_padbot_10'>
                                                    ...
                                                    <div class="b-fon__body b-fon__body_pad_5_10 b-fon__body_fontsize_13">
                                                        ...
                                                        <div class="b-fon__txt b-fon__txt_margleft_20">
                                                                ...
                                                                <div class='b-fon__txt'><?php if (hasPermissions('projects')) {
    ?><?= ($comment['admin_login'] ? "Заблокировал: <a class='b-fon__link' href='/users/{$comment['admin_login']}'>{$comment['admin_uname']} {$comment['admin_usurname']} [{$comment['admin_login']}]</a><br />" : '') ?><?php 
}
    ?>
    ...
    ?>
              	        <div class="prj-admin-btn c" style=" padding-bottom:0; padding-top:0; float:right">
                            <ul>
                                <li id="project-button-<?=$value['id']?>"><a class="admn" href="javascript:void(0);" onclick="banned.<?=($value['is_blocked'] == 't' ? 'unblockedProjectOffer' : 'blockedProjectOffer')?>(<?=$value['id']?>,<?=$value['user_id']?>,<?= $project['id']?>)"><?= $value['is_blocked'] == 'f' ? 'Заблокировать' : 'Разблокировать';
    ?></a></li>
                                <li><?php if ($value['warn'] < 3 && !$value['is_banned'] && !$value['ban_where']) {
    ?>
                            <span class='warnlink-<?= $value['user_id']?>'><a style='color: red;' href='javascript: void(0);' onclick='banned.warnUser(<?= $value['user_id']?>, 0, "projects", "p<?= $project['id']?>", 0); return false;'>Сделать предупреждение (<span class='warncount-<?= $value['user_id']?>'><?= ($value['warn'] ? $value['warn'] : 0);
    ?></span>)</a></span> | 
                            <?php 
} else {
                            ...
                            <div class='b-fon b-fon_clear_both b-fon_bg_ff6d2d b-fon_padtop_10 b-fon_padbot_10'>
                                ...
                                <div class="b-fon__body b-fon__body_pad_5_10 b-fon__body_fontsize_13">
                                    ...
                                    <div class="b-fon__txt b-fon__txt_margleft_20">
                                            ...
                                            <div class='b-fon__txt'><?php if (hasPermissions('projects')) {
    ?><?= ($value['admin_login'] ? "Заблокировал: <a class='b-fon__link' href='/users/{$value['admin_login']}'>{$value['admin_uname']} {$value['admin_usurname']} [{$value['admin_login']}]</a><br />" : '') ?><?php 
}
    ?>
    ...
    ?>
            		    ...
            		    <?php if ($uid && $value['user_id'] == $uid && $value['refused'] != 't' && $project['closed'] != 't' && $value['frl_refused'] != 't' && $project['exec_id'] != $uid && $value['is_blocked'] != 't') {
    $ref_view = true;
    ?>
    ...
    ?>
                        ...
                        <?php if ($error_is_color) {
    print('<br/><br/>'.view_error($error_is_color));
}
    ?> 
                        <?php 
} //if?>
	...
	<?php if (($value['user_id'] == get_uid()) && ($real_offers_count > 0)) {
    ?>
		<h2  class="b-layout__title">Ответов от фрилансеров - <?= $real_offers_count ?></h2>
		<?php if ($notHiddenOffersCount != $real_offers_count) {
	...
	<?php /*} elseif ($user_offer_exist) { //else?>
    ...
    ?>
    	...
    	<?php $count_hidden_offers = $real_offers_count;
    ?>
    <?php 
} ?>
    <?php endif; ?>