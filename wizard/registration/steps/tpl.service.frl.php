...
<div class="b-layout__txt b-layout__txt_padbot_10 b-layout_hide services-blog">
    Пользователи общаются на различные темы, как профессиональные, так и общечеловеческие: <?php foreach ($themes_blogs as $key => $theme) {
    ?> <a class="b-layout__link" href="/blogs/<?=$theme['link']?>/" target="_blank"><?= ($theme['t_name'])?></a><?= ($key + 1 != count($themes_blogs)) ? ',' : ''?><?php 
}//foreach?> и другие.
</div>
<div class="b-layout__txt b-layout__txt_padbot_10 b-layout_hide services-blog">
    Кроме блогов есть сообщества — это группы пользователей по интересам. Есть множество интересных сообществ в которых вы найдете единомышленников: <?php foreach ($themes_commune as $key => $ctheme) {
    ?> <a class="b-layout__link" href="/commune/?id=<?=$ctheme['id']?>/" target="_blank"><?= ($ctheme['name'])?></a><?= ($key + 1 != count($themes_commune)) ? ',' : ''?><?php 
}//foreach?> и многие другие.
</div>