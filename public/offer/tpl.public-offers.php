...
<?php include_once $_SERVER['DOCUMENT_ROOT'].'/filter_specs.php';?>
<div class="lancer-public">
    ...
    ?>
                            ...
                            <select name="subcategories" <?if($offer['category_id']==0 && !$_POST['subcategories']):?>disabled<?endif;?> class="subcat">
                                <option value="0" <?php if ($ccat['subcategory_id'] == 0) {
    echo 'selected';
} ?>>Все специализации</option>
                                <?php for ($i = 0; $i < sizeof($categories_specs); ++$i) {
    ?>
                                <option value="<?=$categories_specs[$i]['id']?>"<?php if ($categories_specs[$i]['id'] == $offer['subcategory_id'] || $categories_specs[$i]['id'] == $_POST['subcategories']) {
    echo(' selected');
}
    ?>><?=$categories_specs[$i]['profname']?></option>
                                <?php 
} //for ?>
                            </select>
            ...
            <div class="form-block">
                <div class="form-el">
                    ...
                    <div class="attention" id="error_no_money" style="display:none">
                        ...
                        <p id="error_buy">В данный момент на счету <?= $_SESSION['ac_sum'];
    ?> FM <a href="#">Пополнить счет</a></p>
                    </div>
                </div>
            </div>
            <?php 
